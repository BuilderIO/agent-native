...
- **Additive-only schema changes** — production Netlify builds run `drizzle-kit push --force`, which auto-approves destructive statements. Until we switch to generated migrations, schema edits MUST be additive only: add new columns/tables, never rename or drop. If a column or table is wrong, add the replacement, backfill, leave the old one in place, and file a follow-up to drop it once all deploys are off it. Same rule for column type changes — introduce a new column, dual-write, migrate readers, then retire. Never rely on `--force` to do the right thing; it will silently destroy data.
