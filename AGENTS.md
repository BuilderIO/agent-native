...
editing them, and use `corepack pnpm ship:push -m "<specific change>"` only for
one complete, coherent update. The helper refuses autogenerated publish
subjects. Batch known CI and review fixes before publishing; a new head reruns
checks. Rebase or merge `origin/main` only to resolve a GitHub-reported
`CONFLICTING` PR; for shared branches, prefer a normal merge. Read
