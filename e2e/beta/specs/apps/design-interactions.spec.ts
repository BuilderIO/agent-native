...
async function heldScreenDiagnostics(page: Page) {
  return page
    .locator(`${PREVIEW}[data-screen-iframe-id]`)
    .evaluateAll((iframes) =>
      iframes.map((iframe) => {
        ...
        const badge = frameDocument?.querySelector<HTMLElement>(
          "[data-agent-native-transform-badge]",
        );
        const selection = frameDocument?.querySelector<HTMLElement>(
          '[data-agent-native-edit-overlay="selection"]',
        );
        const roots = frameDocument
          ? Array.from(
        ...
        return {
          ...
          badge: badge
            ...
            : null,
          roots: roots.map((root) => root.dataset.agentNativeNodeId),
          selection: selection
            ? {
                display: getComputedStyle(selection).display,
                rect: selection.getBoundingClientRect().toJSON(),
              }
            : null,
        };
      }),
  ...
  test("report path: Option-dragging a root frame preserves source and selects the copy", async ({
    ...
    try {
      ...
      try {
        ...
        console.log(
          "[beta-e2e] held Option state after threshold",
          JSON.stringify(await heldScreenDiagnostics(page)),
        );
        await page.mouse.move(
        ...
        console.log(
          "[beta-e2e] held Option state after extended move",
          JSON.stringify(await heldScreenDiagnostics(page)),
        );
      } finally {
