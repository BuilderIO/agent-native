...
- The generation's `$ai_latency` is now model time (run duration minus tool time) instead of the whole run, so tool duration is no longer counted both in the generation and in its sibling tool spans. Concurrent tools count their overlapping window once, and tools that `captureLlmSpans` or the per-run span cap keeps out of PostHog are not subtracted at all — nothing else would carry their time.
