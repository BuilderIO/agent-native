...
When a real Chrome browser is available, call the Design page's own
`open-visual-edit` WebMCP tool directly in the signed-in Design tab. It uses
that tab's session, so no separate MCP connector, OAuth flow, or synthetic CLI
user is needed, and its browser-visible result omits bridge credentials. Use
the connected Design MCP path below for headless hosts or older Design builds.
