...
export default createProviderApiCatalogAction(getCrmProviderApiRuntime(), {
  ...
  schema: z.object({
    provider: ProviderSchema.optional().describe(
      "Optional provider id to inspect. CRM currently exposes HubSpot only.",
    ),
  }),
  http: { method: "GET" },
  guidance:
    "CRM actions are convenience workflows, not a HubSpot capability limit. For an exact HubSpot endpoint, inspect its catalog and docs before provider-api-request. For broad work, stage a bounded paginated result, reduce it with query-staged-dataset or a data program, and report the scope, row count, pagination status, truncation, and gaps.",
});
