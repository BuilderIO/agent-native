...
export function OverallTrendChart({ months }: OverallTrendChartProps) {
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-sm text-destructive">
            Error loading trend data: {data?.error || String(error)}
          </div>
        </CardContent>
      </Card>
    );
  ...
  const recentAvg =
    recentWeeks.reduce((sum, r) => sum + r.conversion_rate_pct, 0) /
    recentWeeks.length;
  const baselineAvg =
    baselineWeeks.reduce((sum, r) => sum + r.conversion_rate_pct, 0) /
    ...
    <Card>
      <CardHeader>
        ...
        <p className="text-xs text-muted-foreground mt-1">
          Weekly conversion rate over the last {months} months with
      ...
      <CardContent>
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          <div className="rounded-lg border border-border bg-muted/50 p-4">
            ...
            <div
              className={`text-3xl font-bold mt-2 flex items-center gap-2 ${changeVsBaseline < 0 ? "text-destructive" : "text-green-600"}`}
            >
              {changeVsBaseline < 0 ? (
              ...
              <span>
                {changeVsBaseline > 0 ? "+" : ""}
                {changeVsBaseline.toFixed(2)}%
              </span>
              <span className="text-base text-muted-foreground font-normal">
                ({pctChangeVsBaseline > 0 ? "+" : ""}
                {pctChangeVsBaseline.toFixed(1)}%)
              </span>
            </div>
          </div>
        </div>
            ...
            {rows.slice(-8).map(
              (row, idx) =>
                row.wow_change_pct !== null && (
                  ...
                  >
                    {formatDate(row.week)}: {row.wow_change_pct > 0 ? "+" : ""}
                    {row.wow_change_pct.toFixed(2)}%
                  </div>
                ),
            )}
          </div>
