...
import { chartQuery, qlsQuery, pipelineQuery, ssArrQuery, pagePerformanceQuery } from "../queries";
...
export function PagePerformanceTab() {
  ...
  const utmMediumOpts = useFilterOptions("utm_medium", "pageviews", f.dateStart, f.dateEnd);
  const utmSourceOpts = useFilterOptions("utm_source", "pageviews", f.dateStart, f.dateEnd);
  const utmCampaignOpts = useFilterOptions("utm_campaign", "pageviews", f.dateStart, f.dateEnd);
  ...
  const filters = useMemo(() => ({
    ...
    f.dateStart, f.dateEnd, f.pageType, f.channel, f.referrer, f.baseUrl, f.subPageType,
    f.utmMedium, f.utmSource, f.utmTerm, f.utmCampaign, f.utmContent,
    f.author, f.type, f.subType, f.purpose, f.persona, f.pubDateStart, f.urlFilter,
  ]);

  const chartSql = useMemo(() => chartQuery(cadence, viewBy, filters, true), [cadence, viewBy, filters]);
  const qlsSql = useMemo(() => qlsQuery(cadence, viewBy, filters, true), [cadence, viewBy, filters]);
  const pipeSql = useMemo(() => pipelineQuery(cadence, viewBy, filters, true), [cadence, viewBy, filters]);
  const arrSql = useMemo(() => ssArrQuery(cadence, viewBy, filters, true), [cadence, viewBy, filters]);
  ...
  const tableSql = useMemo(() => pagePerformanceQuery(tableFilters, true, tableSort), [tableFilters, tableSort]);
  ...
  const activeFilterCount = [f.channel, f.referrer, f.author, f.type, f.subType, f.purpose, f.persona, f.utmMedium, f.utmSource, f.utmCampaign].filter(a => a.length > 0).length
    + (f.urlFilter ? 1 : 0);
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-lg border border-border p-3 space-y-2">
        <div className="flex flex-wrap gap-3 items-end">
          <DateRangeInput label="First Pageview Date" startDate={f.dateStart} endDate={f.dateEnd} onStartChange={(v) => setF("dateStart", v)} onEndChange={(v) => setF("dateEnd", v)} />
        ...
        {filtersExpanded && (
          <div className="pt-2 border-t border-border/50 space-y-3">
            ...
            <div className="flex flex-wrap gap-3 items-end">
              ...
              <MultiSelect label="Type" options={typeOpts.options} value={f.type} onChange={(v) => setF("type", v)} isLoading={typeOpts.isLoading} />
              <MultiSelect label="Sub Type" options={subTypeOpts.options} value={f.subType} onChange={(v) => setF("subType", v)} isLoading={subTypeOpts.isLoading} />
              <MultiSelect label="Purpose" options={purposeOpts.options} value={f.purpose} onChange={(v) => setF("purpose", v)} isLoading={purposeOpts.isLoading} />
              <MultiSelect label="Persona" options={personaOpts.options} value={f.persona} onChange={(v) => setF("persona", v)} isLoading={personaOpts.isLoading} />
              <SingleDateInput label="Pub Date (on or after)" value={f.pubDateStart} onChange={(v) => setF("pubDateStart", v)} />
            </div>

            {/* UTM filters */}
            <div className="flex flex-wrap gap-3 items-end">
              <MultiSelect label="UTM Medium" options={utmMediumOpts.options} value={f.utmMedium} onChange={(v) => setF("utmMedium", v)} isLoading={utmMediumOpts.isLoading} />
              <MultiSelect label="UTM Source" options={utmSourceOpts.options} value={f.utmSource} onChange={(v) => setF("utmSource", v)} isLoading={utmSourceOpts.isLoading} />
              <MultiSelect label="UTM Campaign" options={utmCampaignOpts.options} value={f.utmCampaign} onChange={(v) => setF("utmCampaign", v)} isLoading={utmCampaignOpts.isLoading} />
              <TextFilter label="URL Contains" value={f.urlFilter} onChange={(v) => setF("urlFilter", v)} placeholder="e.g. figma-to-angular" />
            </div>
          </div>
        )}
      </div>
