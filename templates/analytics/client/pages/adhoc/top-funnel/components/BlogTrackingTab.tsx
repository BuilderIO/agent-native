...
export function BlogTrackingTab() {
  ...
  const sql = useMemo(
    () => blogTrackingQuery(f.pageType, f.minDate),
    [f.pageType, f.minDate],
  );
    ...
    <div className="space-y-4">
      <div className="rounded-lg border border-border p-3 space-y-3">
        ...
        <div className="flex flex-wrap gap-3 items-end">
          ...
          <SingleDateInput
            label="Min First Pageview (on or after)"
            value={f.minDate}
            onChange={(v) => setF("minDate", v)}
          />
        </div>
      </div>
                  ...
                  {rows.map((row, i) => (
                    ...
                    >
                      ...
                      <td className="py-1.5 px-2 text-right tabular-nums">
                        {formatNumber(Number(row.visitor_count ?? 0))}
                      </td>
                      <td className="py-1.5 px-2 whitespace-nowrap">
                        {String(row.min_first_pageview_d ?? "-")}
                      </td>
                      <td className="py-1.5 px-2 whitespace-nowrap">
                        {String(row.type ?? "-")}
                      </td>
