...
const COLORS = ['#10b981', '#3b82f6', '#8b5cf6', '#f59e0b', '#ef4444', '#991b1b'];

export function TimeToCompleteChart({ dateStart, dateEnd }: TimeToCompleteChartProps) {
    ...
    <Card>
      ...
      <CardContent>
          ...
          <BarChart data={chartData}>
            ...
            <Tooltip
              ...
              formatter={(value: any, name: string) => {
                if (name === 'user_count') return [value.toLocaleString(), 'Users'];
                return [value, name];
              }}
            />
            ...
            <tbody>
              {chartData.map((row, idx) => (
                <tr key={idx} className="border-b border-border/50">
                  ...
                  <td className="text-right py-2 font-medium">{row.user_count.toLocaleString()}</td>
                  <td className="text-right py-2 text-muted-foreground">{row.percentage}%</td>
                  <td className="text-right py-2 text-muted-foreground">{row.avg_minutes}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="mt-4 pt-3 border-t border-border text-xs text-muted-foreground">
          <strong>{totalUsers.toLocaleString()}</strong> users completed onboarding in this period
        </div>
      </CardContent>
    </Card>
  );
