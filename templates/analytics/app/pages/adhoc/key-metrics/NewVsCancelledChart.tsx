...
export function NewVsCancelledChart({
  rows,
  isLoading,
  error,
  onEditSql,
}: NewVsCancelledChartProps) {
  const data = useMemo(() => {
    // Transform from long format (date, event_type, count) to wide format (date, cancelled, new_sub)
    const byDate = new Map<
      string,
      { period: string; cancelled: number; new_sub: number }
    >();
        ...
        {kpiStats && (
          <div className="flex gap-4 mt-3 text-xs">
            <div>
              ...
              <span
                className={`ml-1.5 ${kpiStats.cancelledChange >= 0 ? "text-green-500" : "text-red-500"}`}
              >
                {kpiStats.cancelledChange >= 0 ? "↑" : "↓"}{" "}
                {Math.abs(kpiStats.cancelledChange).toFixed(1)}%
              </span>
            ...
            <div>
              ...
              <span
                className={`ml-1.5 ${kpiStats.newSubChange >= 0 ? "text-green-500" : "text-red-500"}`}
              >
                {kpiStats.newSubChange >= 0 ? "↑" : "↓"}{" "}
                {Math.abs(kpiStats.newSubChange).toFixed(1)}%
              </span>
              <span className="text-muted-foreground text-[10px] ml-1">
                from {formatDate(kpiStats.baselineDate)}
              </span>
