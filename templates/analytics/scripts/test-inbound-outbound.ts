...
async function main() {
  ...
  console.error("1. Finds qualifying forms (Sales, demo, Component Indexing, Unlock Ent Trial)");
  console.error("2. Joins deals → contacts → forms");
  console.error("3. Filters where form_fill_date < deal.createdate");
  console.error("4. Classifies as Inbound if ANY qualifying form exists, else Outbound");
  console.error("");
