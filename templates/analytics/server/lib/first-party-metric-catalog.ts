...
const FIRST_PARTY_DASHBOARD_TIME_ZONE = "America/Los_Angeles";
...
function dashboardTimeRangeFilter(dateExpr = LOCAL_EVENT_DATE_SQL): string {
  return `('{{timeRange}}' IN ('', 'all') OR ('{{timeRange}}' = '7d' AND ${dateExpr} >= ${localDaysAgoSql(7)}) OR ('{{timeRange}}' = '30d' AND ${dateExpr} >= ${localDaysAgoSql(30)}) OR ('{{timeRange}}' = '90d' AND ${dateExpr} >= ${localDaysAgoSql(90)}) OR ('{{timeRange}}' = '180d' AND ${dateExpr} >= ${localDaysAgoSql(180)}) OR ('{{timeRange}}' = '365d' AND ${dateExpr} >= ${localDaysAgoSql(365)}))`;
}
...
const ENTRIES: FirstPartyMetric[] = [
  ...
  },
  {
    ...
    windowed: false,
    buildSql: fixed(
      "SELECT to_char(timestamp::timestamptz AT TIME ZONE 'America/Los_Angeles', 'YYYY-MM-DD') AS date, COUNT(*) AS count FROM analytics_events WHERE event_name = 'session status' GROUP BY to_char(timestamp::timestamptz AT TIME ZONE 'America/Los_Angeles', 'YYYY-MM-DD') ORDER BY date",
    ),
  ...
  },
  {
    ...
    windowed: false,
    buildSql: fixed(
      `SELECT to_char(timestamp::timestamptz AT TIME ZONE 'America/Los_Angeles', 'YYYY-MM-DD') AS date, COALESCE(NULLIF(template, ''), 'unknown') AS template, COUNT(*) AS count FROM analytics_events WHERE event_name = 'click template' AND ${DASHBOARD_TIME_RANGE_FILTER} AND ${DASHBOARD_EMAIL_FILTER} GROUP BY to_char(timestamp::timestamptz AT TIME ZONE 'America/Los_Angeles', 'YYYY-MM-DD'), COALESCE(NULLIF(template, ''), 'unknown') ORDER BY date, template`,
    ),
    config: {
      xKey: "date",
      yKey: "count",
  ...
  },
  {
    ...
    windowed: false,
    buildSql: fixed(
      `SELECT to_char(timestamp::timestamptz AT TIME ZONE 'America/Los_Angeles', 'YYYY-MM-DD') AS date, COALESCE(NULLIF(template, ''), 'unknown') AS template, COUNT(*) AS count FROM analytics_events WHERE event_name = 'click try demo' AND ${DASHBOARD_TIME_RANGE_FILTER} AND ${DASHBOARD_EMAIL_FILTER} GROUP BY to_char(timestamp::timestamptz AT TIME ZONE 'America/Los_Angeles', 'YYYY-MM-DD'), COALESCE(NULLIF(template, ''), 'unknown') ORDER BY date, template`,
    ),
    config: {
      xKey: "date",
      yKey: "count",
  ...
  },
  {
    ...
    windowed: false,
    buildSql: fixed(
      `SELECT to_char(timestamp::timestamptz AT TIME ZONE 'America/Los_Angeles', 'YYYY-MM-DD') AS date, COALESCE(NULLIF(template, ''), 'unknown') AS template, COUNT(*) AS count FROM analytics_events WHERE event_name = 'copy cli command' AND ${DASHBOARD_TIME_RANGE_FILTER} AND ${DASHBOARD_EMAIL_FILTER} GROUP BY to_char(timestamp::timestamptz AT TIME ZONE 'America/Los_Angeles', 'YYYY-MM-DD'), COALESCE(NULLIF(template, ''), 'unknown') ORDER BY date, template`,
    ),
    config: {
      xKey: "date",
      yKey: "count",
  ...
  // --- Activity / pageviews ------------------------------------------------
  {
    ...
    buildSql: fixed(
      `SELECT to_char(timestamp::timestamptz AT TIME ZONE 'America/Los_Angeles', 'YYYY-MM-DD') AS date, COALESCE(NULLIF(template, ''), NULLIF(app, ''), 'unknown') AS template, COUNT(*) AS count FROM analytics_events WHERE event_name = 'pageview' AND ${DASHBOARD_TIME_RANGE_FILTER} AND ${DASHBOARD_EMAIL_FILTER} GROUP BY to_char(timestamp::timestamptz AT TIME ZONE 'America/Los_Angeles', 'YYYY-MM-DD'), COALESCE(NULLIF(template, ''), NULLIF(app, ''), 'unknown') ORDER BY date, template`,
    ),
    config: {
      xKey: "date",
      yKey: "count",
  ...
  },
  {
    ...
    windowed: false,
    buildSql: fixed(
      "WITH user_days AS (SELECT COALESCE(NULLIF(user_id, ''), NULLIF(anonymous_id, '')) AS user_key, COUNT(DISTINCT to_char(timestamp::timestamptz AT TIME ZONE 'America/Los_Angeles', 'YYYY-MM-DD')) AS active_days FROM analytics_events WHERE COALESCE(NULLIF(user_id, ''), NULLIF(anonymous_id, '')) IS NOT NULL AND ('{{timeRange}}' IN ('', 'all') OR ('{{timeRange}}' = '7d' AND to_char(timestamp::timestamptz AT TIME ZONE 'America/Los_Angeles', 'YYYY-MM-DD') >= to_char((now() AT TIME ZONE 'America/Los_Angeles') - INTERVAL '7 days', 'YYYY-MM-DD')) OR ('{{timeRange}}' = '30d' AND to_char(timestamp::timestamptz AT TIME ZONE 'America/Los_Angeles', 'YYYY-MM-DD') >= to_char((now() AT TIME ZONE 'America/Los_Angeles') - INTERVAL '30 days', 'YYYY-MM-DD')) OR ('{{timeRange}}' = '90d' AND to_char(timestamp::timestamptz AT TIME ZONE 'America/Los_Angeles', 'YYYY-MM-DD') >= to_char((now() AT TIME ZONE 'America/Los_Angeles') - INTERVAL '90 days', 'YYYY-MM-DD')) OR ('{{timeRange}}' = '180d' AND to_char(timestamp::timestamptz AT TIME ZONE 'America/Los_Angeles', 'YYYY-MM-DD') >= to_char((now() AT TIME ZONE 'America/Los_Angeles') - INTERVAL '180 days', 'YYYY-MM-DD')) OR ('{{timeRange}}' = '365d' AND to_char(timestamp::timestamptz AT TIME ZONE 'America/Los_Angeles', 'YYYY-MM-DD') >= to_char((now() AT TIME ZONE 'America/Los_Angeles') - INTERVAL '365 days', 'YYYY-MM-DD'))) AND ('{{emailFilter}}' IN ('', 'all') OR ('{{emailFilter}}' = 'exclude_builder' AND lower(coalesce(user_id, '')) NOT LIKE '%@builder.io') OR ('{{emailFilter}}' = 'only_builder' AND lower(coalesce(user_id, '')) LIKE '%@builder.io')) GROUP BY COALESCE(NULLIF(user_id, ''), NULLIF(anonymous_id, ''))) SELECT COUNT(*) AS count FROM user_days WHERE active_days >= 2",
    ),
    config: {
      yKey: "count",
      yFormatter: "number",
  ...
  },
  {
    ...
    windowed: false,
    buildSql: fixed(
      "WITH base AS (SELECT COALESCE(NULLIF(user_id, ''), NULLIF(anonymous_id, '')) AS user_key, to_char(timestamp::timestamptz AT TIME ZONE 'America/Los_Angeles', 'YYYY-MM-DD') AS event_date, user_id FROM analytics_events WHERE COALESCE(NULLIF(user_id, ''), NULLIF(anonymous_id, '')) IS NOT NULL AND ('{{emailFilter}}' IN ('', 'all') OR ('{{emailFilter}}' = 'exclude_builder' AND lower(coalesce(user_id, '')) NOT LIKE '%@builder.io') OR ('{{emailFilter}}' = 'only_builder' AND lower(coalesce(user_id, '')) LIKE '%@builder.io'))), first_seen AS (SELECT user_key, MIN(event_date) AS cohort_date FROM base GROUP BY user_key), cohorts AS (SELECT user_key, cohort_date FROM first_seen WHERE cohort_date <= to_char((now() AT TIME ZONE 'America/Los_Angeles') - INTERVAL '7 days', 'YYYY-MM-DD') AND ('{{timeRange}}' IN ('', 'all') OR ('{{timeRange}}' = '7d' AND cohort_date >= to_char((now() AT TIME ZONE 'America/Los_Angeles') - INTERVAL '7 days', 'YYYY-MM-DD')) OR ('{{timeRange}}' = '30d' AND cohort_date >= to_char((now() AT TIME ZONE 'America/Los_Angeles') - INTERVAL '30 days', 'YYYY-MM-DD')) OR ('{{timeRange}}' = '90d' AND cohort_date >= to_char((now() AT TIME ZONE 'America/Los_Angeles') - INTERVAL '90 days', 'YYYY-MM-DD')) OR ('{{timeRange}}' = '180d' AND cohort_date >= to_char((now() AT TIME ZONE 'America/Los_Angeles') - INTERVAL '180 days', 'YYYY-MM-DD')) OR ('{{timeRange}}' = '365d' AND cohort_date >= to_char((now() AT TIME ZONE 'America/Los_Angeles') - INTERVAL '365 days', 'YYYY-MM-DD')))), retention AS (SELECT c.cohort_date AS date, '1d retention' AS period, COALESCE(COUNT(DISTINCT c.user_key) FILTER (WHERE b.event_date = to_char(c.cohort_date::date + INTERVAL '1 day', 'YYYY-MM-DD'))::float / NULLIF(COUNT(DISTINCT c.user_key), 0), 0) AS rate FROM cohorts c LEFT JOIN base b ON b.user_key = c.user_key GROUP BY c.cohort_date UNION ALL SELECT c.cohort_date AS date, '7d retention' AS period, COALESCE(COUNT(DISTINCT c.user_key) FILTER (WHERE b.event_date = to_char(c.cohort_date::date + INTERVAL '7 days', 'YYYY-MM-DD'))::float / NULLIF(COUNT(DISTINCT c.user_key), 0), 0) AS rate FROM cohorts c LEFT JOIN base b ON b.user_key = c.user_key GROUP BY c.cohort_date) SELECT date, period, rate FROM retention ORDER BY date, period",
    ),
    config: {
      xKey: "date",
      yKey: "rate",
