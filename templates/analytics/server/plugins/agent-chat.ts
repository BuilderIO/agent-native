...
export default createAgentChatPlugin({
  ...
  extraContext: async (event) => {
    ...
    const sourceGuidance =
      ...
      "For deal or account deep dives that ask for HubSpot and Gong context, use the fast targeted path: call `hubspot-deals` with `query` for the named company/deal, then enrich with `hubspot-records` for company, contact, or ticket records as needed, and call `gong-calls` with `company` plus `includeTranscripts=true` before making qualitative claims about calls. Do not answer a requested Gong deep dive from call metadata alone. " +
