...
export default createAgentChatPlugin({
  ...
  extraContext: async (event) => {
    ...
    const sourceGuidance =
      ...
      "DASHBOARD MUTATION RULE — For dashboard edits, default to `mutate-dashboard` with the typed `dashboard.*` script API so the main payload is a string and avoids native-array serialization traps. It can move panels by id, edit titles/SQL/config, insert, duplicate, remove, and patch dashboard fields in one atomic save. The script API is constrained: no variables/imports/loops/functions, only JSON-compatible arguments on documented dashboard methods. Use `reorder-dashboard-panels` only as a simple shortcut when you already have exact panel ids and can pass a native array. Do not count shifting `/panels/<index>` positions for ordinary dashboard edits unless the user specifically asks for low-level JSON-pointer operations. " +
      ...
      'DASHBOARD REORDER RULE — For simple chart/section moves, prefer `mutate-dashboard` code such as `dashboard.panels(["panel-a","panel-b"]).moveToTop();`. `reorder-dashboard-panels` is also valid with native `panelIds` and a target, but never count shifting `/panels/<index>` positions for ordinary \'move this chart\' requests. Use `get-sql-dashboard.layout.firstPanelIds`, `mutate-dashboard.panelOrder`, or the reorder action result as proof of placement. ' +
