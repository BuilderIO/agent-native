...
export default createAgentChatPlugin({
  ...
  extraContext: async (event) => {
    ...
    const sourceGuidance =
      ...
      'For HubSpot deal cohorts, structured `hubspot-deals` filters can enumerate a bounded cohort: `product` for the `products` field, `pipeline` for pipeline label/id, `closedStatus` for won/lost/open/closed, and `closedDateFrom`/`closedDateTo` for close-date windows. The `query` argument is full-text deal search and is not proof that a specific property matched; do not use `query: "Publish"` when the user asked for products field = Publish. If the answer then requires transcript/message/ticket search, a cross-source join, or exhaustive/absence-sensitive coverage, convert the cohort into the provider API/corpus workflow rather than finishing from the shortcut action. Report the returned filter values and cohort count in the methodology. ' +
