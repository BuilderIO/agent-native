...
export default createAgentChatPlugin({
  ...
  extraContext: async () => {
    ...
    const sourceGuidance =
      ...
      "EXECUTION CONTINUITY — An explicit request to build, create, save, adapt, edit, patch, or fix a dashboard, an existing extension, or a one-off Custom Block authorizes all non-destructive in-app steps required to finish it in the same turn. After querying or scaffolding, continue through extension-data seeding/refresh, dashboard save/embed, and navigation. Do not ask 'want me to proceed?' or stop at an empty shell. Ask one clarification only when metric scope or grain materially changes the result, and pause for destructive changes or external side effects such as sending email or outreach. " +
      "APPROVED MUTATION CONTINUITY — If this turn includes an explicitly approved dashboard or extension action with concrete input, execute that exact action and input immediately. Treat the supplied dashboard or extension id as authoritative: do not reinterpret an existing-dashboard or existing-extension edit as a template clone, ask for a template name, or substitute an inspection step. Re-reading source you have already read, or restating a diagnosis the user has already accepted, is not progress — once the user has approved a change, the next tool call is the write. A dashboard mutation is complete only when the action result proves `saved: true` and the requested change is reflected by `changed: true`, refreshed/changed panel ids, or an equivalent non-empty proof field; an extension edit is complete only when `update-extension` returns `ok: true`; a natural-language acknowledgement, a restated plan, or a no-op with skipped panels is not success. " +
