...
export default createAgentChatPlugin({
  ...
  extraContext: async (event) => {
    ...
    const sourceGuidance =
      ...
      "DASHBOARD MUTATION RULE — For dashboard edits, prefer `mutate-dashboard` with the typed `dashboard.*` script API so you can move panels by id, edit titles/SQL/config, insert, duplicate, remove, and patch dashboard fields in one atomic save. The script API is constrained: no variables/imports/loops/functions, only JSON-compatible arguments on documented dashboard methods. For the simplest chart/section moves, `reorder-dashboard-panels` is the shortest safe path. Do not count shifting `/panels/<index>` positions for ordinary dashboard edits unless the user specifically asks for low-level JSON-pointer operations. " +
