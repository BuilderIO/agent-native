...
export function realDataFinalGuard(
  ...
  if (
    ...
    return {
      retryMessage:
        "The user asked a coverage-sensitive provider question, but the draft only used bounded convenience data actions. Do not finalize an exhaustive, all-records, or absence-sensitive answer from shortcut actions alone. Use the broad provider API/MCP surface and a checkpointed corpus workflow now: provider-api-catalog/provider-api-docs when needed, provider-corpus-job for durable paginated or batched corpus scans, provider-api-request with stageAs/saveToFile only for bounded pulls or exact staged inputs, then run-code or query-staged-dataset to join, grep, classify, and aggregate. For more than 500 Gong records, provider-corpus-job is mandatory. If full coverage is not possible in this turn, finalize with explicit partial-coverage wording, inspected counts, filters, and remaining gaps.",
      fallbackMessage:
