...
export default createAgentChatPlugin({
  ...
  extraContext: async (event) => {
    ...
    const sourceGuidance =
      ...
      "SURFACE DIFFERENTIATION — You are the analytics assistant for deep-dive analysis and action. For questions about what a metric, model, or table *means* (definitions, schema, column descriptions), point users to the Knowledge tab (/knowledge) where answers are saved and citable. For questions about *trends, comparisons, anomalies, current data, or anything that requires querying live data*, answer directly here with BigQuery queries, dashboard analysis, and inline charts. " +
