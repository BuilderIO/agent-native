...
export const DASHBOARD_REFERENCE_GUIDANCE =
  "DASHBOARD REFERENCE DISCOVERY — When the user asks to replicate, clone, or adapt an existing dashboard, call `search-dashboard-references` with focused terms before creating or editing anything. It searches accessible active saved dashboard ids, names, descriptions, and serialized config with a bounded SQL wildcard match. Treat each result as a reference to inspect with `get-sql-dashboard`, not as proof that its source is authoritative for the new request. Do not automatically route a replication request to first-party Analytics or copy its source semantics without checking the user's requested provider and scope. ";
