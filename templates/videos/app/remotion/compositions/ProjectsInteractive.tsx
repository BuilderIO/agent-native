...
export const ProjectsInteractive =
  createInteractiveComposition<ProjectsInteractiveProps>({
    fallbackTracks: FALLBACK_TRACKS,

    render: (
      { cursorHistory, tracks: originalTracks, registerForCursor },
      props,
    ) => {
      const {
      ...
      const placeholderHideTrack = findTrack(
        tracks,
        "placeholder-hide",
        FALLBACK_TRACKS[2],
      );
      ...
      const typedTextHideTrack = findTrack(
        tracks,
        "typed-text-hide",
        FALLBACK_TRACKS[4],
      );
      const dropdownShowTrack = findTrack(
        tracks,
        "dropdown-show",
        FALLBACK_TRACKS[5],
      );
      const dropdownHideTrack = findTrack(
        tracks,
        "dropdown-hide",
        FALLBACK_TRACKS[6],
      );
      ...
      const isTyping =
        frame >= typingTrack.startFrame && frame <= typingTrack.endFrame;
      const typingCursorBlink = Math.floor(frame / 15) % 2 === 0;
      const shouldShowTypedText =
        frame >= typingTrack.startFrame &&
        frame < typedTextHideTrack.startFrame;

      // Input is focused from when placeholder hides until text is cleared
      const promptIsFocused =
        frame >= placeholderHideTrack.startFrame &&
        frame < typedTextHideTrack.startFrame;
      ...
      const contentWidth =
        width - outerPadding * 2 - sidebarWidth - screenPadding * 2;
      ...
      const actionButtonsStartX =
        promptX + (promptWidth - actionButtonsWidth) / 2;
      ...
      const template2Button = useInteractiveComponent({
        ...
        compositionId: "projects-interactive",
        zone: {
          x: actionButtonsStartX + 182 + 8 + 59 + 8,
          y: actionButtonsY,
          width: 59,
          height: 49,
        },
      ...
      const moreButton = useInteractiveComponent({
        ...
        compositionId: "projects-interactive",
        zone: {
          x: actionButtonsStartX + 182 + 8 + 59 + 8 + 59 + 8,
          y: actionButtonsY,
          width: 102,
          height: 49,
        },
      ...
      const showDropdown =
        frame >= dropdownShowTrack.startFrame &&
        frame < dropdownHideTrack.startFrame;
        ...
        <AbsoluteFill style={{ backgroundColor: "#0a0a0a" }}>
          ...
          >
            <ProjectsLayout
              ...
              promptValue={
                shouldShowTypedText ? (isTyping ? visiblePrompt : prompt) : ""
              }
              ...
              githubProviderIsHovered={
                showDropdown && githubItem.hover.isHovering
              }
              azureProviderIsHovered={
                showDropdown && azureItem.hover.isHovering
              }
              gitlabProviderIsHovered={
                showDropdown && gitlabItem.hover.isHovering
              }
              bitbucketProviderIsHovered={
                showDropdown && bitbucketItem.hover.isHovering
              }
            />
          </div>
        </AbsoluteFill>
