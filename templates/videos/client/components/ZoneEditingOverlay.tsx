...
export const ZoneEditingOverlay: React.FC<ZoneEditingOverlayProps> = ({
  ...
  useEffect(() => {
    localStorage.setItem(STORAGE_KEY_PREFIX + compositionId, JSON.stringify(zones));
  }, [zones, compositionId]);
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent, zoneKey: string, mode: DragMode) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const handleMouseUp = useCallback(() => {
    if (selectedZone && zones[selectedZone]) {
      ...
      console.log(`"${selectedZone}": { x: ${Math.round(zone.x)}, y: ${Math.round(zone.y)}, width: ${Math.round(zone.width)}, height: ${Math.round(zone.height)} },`);
    }
      ...
      {Object.entries(zones).map(([label, zone]) => {
        ...
        return (
          ...
          >
            ...
            >
              {label}
              <div style={{ position: "absolute", top: 2, right: 2, fontSize: 9, opacity: 0.9 }}>
                {Math.round(zone.x)}, {Math.round(zone.y)}
              </div>
              <div style={{ position: "absolute", bottom: 2, right: 2, fontSize: 9, opacity: 0.9 }}>
