...
export const StandardView: React.FC<StandardViewProps> = (props) => {
  ...
  const alexCursorX = getPropValue(alexCursorProgress, alexCursorTrack, "x", 20, 57);
  const alexCursorY = getPropValue(alexCursorProgress, alexCursorTrack, "y", 60, 32);

  // Alex cursor click animation (manual calculation for down-up motion)
  const alexCursorScale = globalFrame < 419 ? 1 :
    globalFrame < 421 ? 1 - ((globalFrame - 419) / 2) * 0.15 : // Scale down to 0.85
    globalFrame < 424 ? 0.85 + ((globalFrame - 421) / 3) * 0.15 : // Scale back up to 1
    1;
  ...
  const dataOutline = getPropValue(dataClickProgress, dataClickTrack, "outline", 0, 1);
  ...
  const chartTransformProgress = trackProgress(globalFrame, fps, chartTransformTrack);
  const chartProgress = getPropValue(chartTransformProgress, chartTransformTrack, "progress", 0, 1);

  // Cursor fade-out at frame 500
  const cursorFadeStart = 500;
  const cursorFadeEnd = 515;
  const cursorOpacity = globalFrame < cursorFadeStart ? 1 :
    globalFrame < cursorFadeEnd ? 1 - ((globalFrame - cursorFadeStart) / (cursorFadeEnd - cursorFadeStart)) :
    0;
  ...
  const getCursorMovement = (baseX: number, baseY: number, offsetX: number, offsetY: number, phase: number) => {
  ...
  const typingProgress = getPropValue(typingTrackProgress, typingTrack, "progress", 0, 1);
  ...
  const menuOpacity = getPropValue(menuTrackProgress, menuTrack, "opacity", 0, 1);
  ...
  const selectionTrackProgress = trackProgress(globalFrame, fps, selectionTrack);
  const menuSelectionProgress = getPropValue(selectionTrackProgress, selectionTrack, "progress", 0, 1);
  ...
  const sendMessageProgress = getPropValue(sendTrackProgress, sendTrack, "progress", 0, 1);
  ...
  const mentionHintTrackProgress = trackProgress(globalFrame, fps, mentionHintTrack);
  const mentionHintOpacity = getPropValue(mentionHintTrackProgress, mentionHintTrack, "opacity", 0, 1);
  const mentionHintY = getPropValue(mentionHintTrackProgress, mentionHintTrack, "y", 80, 0);
  const mentionHintScale = getPropValue(mentionHintTrackProgress, mentionHintTrack, "scale", 0.9, 1);
  ...
  const reviewPROpacity = getPropValue(reviewPRTrackProgress, reviewPRTrack, "opacity", 0, 1);
  const reviewPRY = getPropValue(reviewPRTrackProgress, reviewPRTrack, "y", -20, 0);
  const reviewPRScale = getPropValue(reviewPRTrackProgress, reviewPRTrack, "scale", 0.95, 1);
  ...
  const reviewTypingTrackProgress = trackProgress(globalFrame, fps, reviewTypingTrack);
  const reviewTypingProgress = getPropValue(reviewTypingTrackProgress, reviewTypingTrack, "progress", 0, 1);

  // Typing animation for "Get your changes reviewed"
  const reviewTypingText = "Get your changes reviewed";
  const reviewCharsToShow = Math.floor(reviewTypingProgress * reviewTypingText.length);
  ...
  const fleetTypingTrackProgress = trackProgress(globalFrame, fps, fleetTypingTrack);
  const fleetTypingProgress = getPropValue(fleetTypingTrackProgress, fleetTypingTrack, "progress", 0, 1);

  // Typing animation for "Manage a fleet of agents"
  const fleetTypingText = "Manage a fleet of agents";
  const fleetCharsToShow = Math.floor(fleetTypingProgress * fleetTypingText.length);
  ...
  const splitScreen1to2Progress = trackProgress(globalFrame, fps, splitScreenTrack1);
  const split1to2 = getPropValue(splitScreen1to2Progress, splitScreenTrack1, "progress", 0, 1);
  ...
  const splitScreen2to3Progress = trackProgress(globalFrame, fps, splitScreenTrack2);
  const split2to3 = getPropValue(splitScreen2to3Progress, splitScreenTrack2, "progress", 0, 1);
  ...
  const screen1TypingProgress = trackProgress(globalFrame, fps, screen1TypingTrack);
  const screen1Typing = getPropValue(screen1TypingProgress, screen1TypingTrack, "progress", 0, 1);
  ...
  const screen2TypingProgress = trackProgress(globalFrame, fps, screen2TypingTrack);
  const screen2Typing = getPropValue(screen2TypingProgress, screen2TypingTrack, "progress", 0, 1);
  ...
  const screen3TypingProgress = trackProgress(globalFrame, fps, screen3TypingTrack);
  const screen3Typing = getPropValue(screen3TypingProgress, screen3TypingTrack, "progress", 0, 1);
  ...
  const collapseAmount = getPropValue(collapseProgress, collapseTrack, "progress", 0, 1);
  ...
  const worksTyping = getPropValue(worksTypingProgress, worksTypingTrack, "progress", 0, 1);
  ...
  const phoneOpacity = getPropValue(phoneSlideProgress, phoneSlideTrack, "opacity", 0, 1);
  ...
  const phone2Rotate = getPropValue(phone2Progress, phone2Track, "rotate", 0, -15);
  const phone2Opacity = getPropValue(phone2Progress, phone2Track, "opacity", 0, 1);
  ...
  const phone3Rotate = getPropValue(phone3Progress, phone3Track, "rotate", 0, -30);
  const phone3Opacity = getPropValue(phone3Progress, phone3Track, "opacity", 0, 1);
  ...
  const screen1Width = split1to2 < 1
    ? 100 - (50 * split1to2)  // 100% → 50%
    : 50 - (16.67 * split2to3); // 50% → 33.33%

  const screen2Width = split2to3 < 1
    ? 50 - (16.67 * split2to3)  // 50% → 33.33%
    : 33.33;
  ...
  const screenHeightPercent = collapseAmount < 1
          ...
          >
            {collapseAmount < 0.5 ? (
              ...
              >
                ...
                chatHistory={[
                  { type: "user", message: "Set up user authentication with JWT tokens" },
                  {
                    type: "agent",
                    message: "I'll create the auth system with secure token handling.",
                    toolCards: [
                      {
                  ...
                  {
                    type: "agent",
                    message: "Auth system is ready with JWT tokens, refresh logic, and protected routes."
                  },
                ]}
            ...
            >
              {collapseAmount < 0.5 ? (
                ...
                >
                  ...
                  chatHistory={[
                    { type: "user", message: "I need to integrate Stripe payments" },
                    {
                      type: "agent",
                    ...
                    {
                      type: "agent",
                      message: "Payment system deployed! Checkout works with all major cards, webhooks handle subscription events, and receipts are auto-generated."
                    },
                  ]}
            ...
            >
              {collapseAmount < 0.5 ? (
                ...
                >
                  ...
                  chatHistory={[
                    { type: "user", message: "Can you visualize our sales data?" },
                    {
                      type: "agent",
                      message: "I'll build an interactive dashboard for your data.",
                      toolCards: [
                        {
                    ...
                    {
                      type: "agent",
                      message: "Dashboard complete with PDF export! Charts show real-time revenue trends, growth metrics, and export includes all filtered data."
                    },
                  ]}
          ...
          >
            ...
            >
              ...
              <div
                style={{
                  ...
                  borderRadius: 12,
                  padding: 20,
                  border: (dataOutline > 0.5 && shouldHighlightChart) ? "3px solid #22C55E" : "1px solid rgba(255, 255, 255, 0.1)",
              ...
              {chartProgress < 0.2 ? (
                // Data Table
                <div>
                  <div style={{ fontSize: 14, color: "#ffffff", marginBottom: 12, fontWeight: 600 }}>
                    Sales Data
                  </div>
                  {/* Table Header */}
                  <div
                  ...
                  {/* Table Rows */}
                  {[
                    { label: "Q1", value: 45, growth: "+12%", target: "50k" },
                    { label: "Q2", value: 72, growth: "+18%", target: "70k" },
                    { label: "Q3", value: 58, growth: "+8%", target: "60k" },
                    { label: "Q4", value: 89, growth: "+24%", target: "85k" },
                    { label: "Jan", value: 34, growth: "+5%", target: "40k" },
                    { label: "Feb", value: 52, growth: "+15%", target: "50k" },
                    { label: "Mar", value: 61, growth: "+22%", target: "60k" },
                    { label: "Apr", value: 48, growth: "+9%", target: "55k" }
                  ].map((item, i) => (
                    <div
                      key={i}
                      style={{
                    ...
                    >
                      ...
                      <span style={{ fontWeight: 600 }}>${item.value}k</span>
                      <span style={{ color: "#22C55E" }}>{item.growth}</span>
                      <span style={{ color: "#999999" }}>{item.target}</span>
                    </div>
                  ))}
                </div>
              ) : (
                ...
                <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
                  <div style={{ fontSize: 16, color: "#ffffff", marginBottom: 16, fontWeight: 600 }}>
                    Sales Data
                  </div>
                      ...
                      >
                        <div
                          style={{
                            width: "100%",
                            maxWidth: 50,
                            height: (item.value / 100) * 280 * Math.min(1, (chartProgress - 0.2) * 2),
                            backgroundColor: item.color,
                            borderRadius: "6px 6px 0 0",
                            transition: "height 0.3s ease",
                          }}
                        />
                        <div style={{ fontSize: 11, color: "#999999" }}>{item.label}</div>
            ...
            >
              <svg width="27" height="36" viewBox="0 0 27 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M0 36V0L26.5263 21.9935H9.47369L0 36Z" fill="#F7DF1E"/>
              </svg>
              <div
                style={{
            ...
            >
              <svg width="27" height="36" viewBox="0 0 27 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M0 36V0L26.5263 21.9935H9.47369L0 36Z" fill="#A855F7"/>
              </svg>
              <div
                style={{
            ...
            >
              <svg width="27" height="36" viewBox="0 0 27 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M0 36V0L26.5263 21.9935H9.47369L0 36Z" fill="#22C55E"/>
              </svg>
              <div
                style={{
            ...
            >
              <svg width="27" height="36" viewBox="0 0 27 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M0 36V0L26.5263 21.9935H9.47369L0 36Z" fill="#F97316"/>
              </svg>
              <div
                style={{
      ...
      {showWorksAnywhere && (
        ...
        >
          ...
          >
            ...
            >
              {worksTyping >= 0.5 ? "Works" : "Works".slice(0, Math.floor(worksTyping * 2 * 5))}
            ...
            >
              {worksTyping >= 0.5 ? "anywhere".slice(0, Math.floor((worksTyping - 0.5) * 2 * 8)) : ""}
            ...
            >
              ...
              >
                ...
                <img
                  ...
                  alt=""
                  style={{ width: 32, aspectRatio: 1, objectFit: "contain", cursor: "pointer" }}
                />
                <img
