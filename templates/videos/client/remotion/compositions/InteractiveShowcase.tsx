...
export const InteractiveShowcase =
  createInteractiveComposition<InteractiveShowcaseProps>({
    fallbackTracks: FALLBACK_TRACKS,
    render: (context, props) => {
      const { title = "Video Studio", subtitle = "Interactive Showcase" } =
        props;
          ...
          >
            ...
            >
              ...
              <FeatureCard
                ...
                fps={fps}
                zone={{
                  x: gridStartX + cardWidth + gridGap,
                  y: gridStartY,
                  width: cardWidth,
                  height: cardHeight,
                }}
              ...
              <FeatureCard
                ...
                fps={fps}
                zone={{
                  x: gridStartX + (cardWidth + gridGap) * 2,
                  y: gridStartY,
                  width: cardWidth,
                  height: cardHeight,
                }}
              ...
              <FeatureCard
                ...
                zone={{
                  x: gridStartX,
                  y: gridStartY + cardHeight + gridGap,
                  width: cardWidth,
                  height: cardHeight,
                }}
                cursorHistory={context.cursorHistory}
...
function FeatureCard({
  ...
  const animTranslateX =
    (interactive.animatedProperties?.translateX as number) ?? 0;
  const animTranslateY =
    (interactive.animatedProperties?.translateY as number) ?? 0;
  const animLift = (interactive.animatedProperties?.lift as number) ?? 0; // Alias for moving UP
  const animRotate =
    (interactive.animatedProperties?.rotate as number) ??
    (interactive.animatedProperties?.rotateZ as number) ??
    0;
  ...
  const animBrightness =
    (interactive.animatedProperties?.brightness as number) ?? 1;
  const animOpacity = (interactive.animatedProperties?.opacity as number) ?? 1;
  const animBgColor = interactive.animatedProperties?.backgroundColor as
    | string
    | undefined;
  ...
  const backgroundColor =
    isInteracting && animBgColor ? animBgColor : "rgba(17, 24, 39, 0.7)";
