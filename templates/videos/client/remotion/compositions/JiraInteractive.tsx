...
const FALLBACK_TRACKS: AnimationTrack[] = [
  {
    ...
    easing: "linear",
    animatedProps: [
      { property: "x", from: "200", to: "960", unit: "px", keyframes: [
      ...
      { property: "y", from: "200", to: "540", unit: "px", keyframes: [
...
export const JiraInteractive = createInteractiveComposition<JiraInteractiveProps>({
  ...
  render: ({ cursorHistory, tracks, registerForCursor }, props) => {
    ...
    const dropdownTrack = findTrack(tracks, "assignee-dropdown", FALLBACK_TRACKS[1]);
    const typingTrack = findTrack(tracks, "assignee-typing", FALLBACK_TRACKS[2]);
    const selectTrack = findTrack(tracks, "assignee-select", FALLBACK_TRACKS[3]);
    const firstCommentTrack = findTrack(tracks, "first-comment", FALLBACK_TRACKS[4]);
    const secondCommentTrack = findTrack(tracks, "second-comment", FALLBACK_TRACKS[5]);

    // Calculate animation states
    const dropdownP = trackProgress(frame, fps, dropdownTrack);
    const dropdownOpacity = getPropValue(dropdownP, dropdownTrack, "opacity", 0, 1);
    
    const typingP = trackProgress(frame, fps, typingTrack);
    const typingChars = Math.floor(getPropValue(typingP, typingTrack, "chars", 0, 4));
    ...
    const firstCommentSpring = firstCommentP > 0 ? iMessageSpring(firstCommentP) : 0;
    const secondCommentSpring = secondCommentP > 0 ? iMessageSpring(secondCommentP) : 0;

    // Determine assignee state based on timeline
    let assigneeState: "unassigned" | "dropdown" | "typing" | "assigned" = "unassigned";
    let typedText = "";
