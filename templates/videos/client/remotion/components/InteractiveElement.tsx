...
import { getCursorTypeForElement, type InteractiveElementType } from "../utils/interactiveElements";
...
export const InteractiveElement: React.FC<InteractiveElementProps> = ({
  ...
  const hover = useHoverAnimationSmooth(
    cursorHistory,
    { ...zone, cursorType: effectiveCursorType }
  );
 ...
 */
export function useInteractiveElement(config: Omit<InteractiveElementProps, "children" | "label">) {
  const effectiveCursorType = config.cursorType ?? getCursorTypeForElement(config.type);
  
  const hover = useHoverAnimationSmooth(
    config.cursorHistory,
    { ...config.zone, cursorType: effectiveCursorType }
  );
