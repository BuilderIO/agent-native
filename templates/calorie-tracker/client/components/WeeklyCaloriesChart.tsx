...
import { startOfWeek, endOfWeek, format, parseISO, isToday, isBefore, startOfDay } from "date-fns";
...
function aggregateWeekly(
  ...
  for (const day of history) {
    ...
    // Only count fully completed days (before today) for projection
  ...
  return Array.from(weekMap.entries())
    ...
    .map(([key, week]) => {
      ...
      if (isCurrentWeek && week.completedDays > 0) {
        ...
        const remainingDays = 7 - week.daysTracked;
        projectedCalories = Math.round(avgPerDay * remainingDays);
      }
      ...
      <ResponsiveContainer width="100%" height={300}>
        ...
        >
          ...
          <Bar
            ...
            stackId="weekly"
            maxBarSize={48}
            radius={weeklyData.some(e => e.projectedCalories > 0) ? [0, 0, 0, 0] : [6, 6, 0, 0]}
          >
          ...
          <Bar dataKey="projectedCalories" stackId="weekly" radius={[6, 6, 0, 0]} maxBarSize={48}>
            {weeklyData.map((entry, index) => {
