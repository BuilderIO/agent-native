...
export const nativeAssetCreativeContextAdapter: NativeResourceCaptureAdapter = {
  ...
  async capture(reference) {
    ...
    return { artifactKey: `assets:asset:${asset.id}`, source: { name: "Assets", kind: "native-app", externalRef: asset.id }, items: [{ externalId: `native:assets:asset:${asset.id}`, kind: asset.mediaType, title: asset.title ?? "Untitled asset", canonicalUrl: `/asset/${asset.id}`, mimeType: asset.mimeType, content: safeDescription.slice(0, 12_000), summary: safeDescription.slice(0, 500) || `Immutable ${asset.mediaType} asset.`, contentHash, sourceModifiedAt: asset.updatedAt, sourceVersion: contentHash, metadata: { preview: { type: "asset", mediaType: asset.mediaType, width: asset.width, height: asset.height, durationSeconds: asset.durationSeconds } } }], privateMetadata: { clone: { handle, appId: "assets", resourceType: "asset", resourceId: asset.id, contentHash, sourceVersion: contentHash, updatedAt: asset.updatedAt, libraryId: asset.libraryId } } };
  },
};
