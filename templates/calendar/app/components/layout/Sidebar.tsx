...
/**
 * Label for a calendar shared with the user.
 *
 * People get their address, which is both the calendar id and the only stable
 * identifier — Google's `summary` for a shared personal calendar is whatever
 * display name that account happens to carry, so a list of them mixes
 * addresses and full names. Group and room calendars keep their name: their
 * id is an opaque `<random>@group.calendar.google.com` that tells nobody
 * anything.
 */
...
/**
 * A row in the "Other Calendars" list. `google` and `person` represent the
 * same real-world calendar/relationship keyed by email, and both can be set
 * when a person shares their Google calendar and is also pinned as an
 * overlay peer.
 */
...
function GoogleCalendarsSections({ onClose }: { onClose: () => void }) {
  ...
  function renderCalendarRow(calendar: (typeof ownedCalendars)[number]) {
...
export function Sidebar({
  ...
  // A person can both share their real Google calendar and be pinned as an
  // overlay peer for booking availability — merge those into one row instead
  // of listing the same email twice.
  const otherCalendarItems = useMemo(() => {
    ...
    for (const person of overlayPeople) {
      ...
      items.set(mapKey, {
        key: mapKey,
        label: existing?.label ?? person.email,
  ...
  // A merged row is only visible when neither of its underlying sources is
  // hidden — a person and their shared Google calendar were toggled
  // independently before the two rows merged, so an old hide on either side
  // must still suppress events, not just the row still shown as one of them
  // being visible would silently leak.
  ...
  function toggleOtherItemVisibility(item: OtherCalendarItem) {
    ...
    // `toggleHiddenCalendar` flips the person's current state; it must only
    // fire when that flip actually lands on `nextVisible`. Otherwise a
    // person/Google mismatch (independently toggled before the two rows
    // merged) can flip the person source the wrong way while Google is set
    // explicitly, leaving the merged row hidden after a "Show" click.
    if (item.person) {
  ...
  function setOtherItemColor(item: OtherCalendarItem, color: string) {
    ...
    if (item.google) updateGoogleCalendarColor(item.google.canonicalKey, color);
  }
