...
export default function registerRecordingFailureBackfillJob(): void {
  ...
  if (!enabled) {
    if (process.env.DEBUG && !skippingLogged) {
      console.log(
        "[recording-failure-backfill] Skipping historical failure-code backfill (set RUN_BACKGROUND_JOBS=1 to enable in dev).",
      );
      skippingLogged = true;
    }
    return;
  ...
  console.log(
    `[recording-failure-backfill] Recurring historical failure-code mapping every ${SWEEP_INTERVAL_MS / 1000}s.`,
  );
}
