...
async function retypeBooleanColumnsOnPostgres(): Promise<void> {
  ...
  const alters: Array<[string, string, boolean]> = [
    ...
    ["clips_annotations", "resolved", false],
  ...
  [
    ...
    },
    {
      ...
      sql: [
        ...
        `CREATE INDEX IF NOT EXISTS clips_annotations_recording_idx ON clips_annotations (recording_id, start_ms)`,
    ...
    // ---------------------------------------------------------------------------
    // Upstream migrations merged 2026-07 — renumbered after this fork's 45–49
    // (video projects, editor assets, annotations, edit versions, raw
    // transcript columns), which were already applied to existing databases.
    // ---------------------------------------------------------------------------
    {
      version: 50,
