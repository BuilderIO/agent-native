...
describe("list-recordings shared view", () => {
  ...
  it("returns accessible clips owned by someone else", async () => {
    ...
    // The fork excludes meeting recordings with a raw sql template instead of
    // a notInArray(<select builder>) — the CLI action runner resolves
    // drizzle-orm as a second module instance and serializing a
    // foreign-instance builder overflows the stack. No meetings query-builder
    // call happens on this path.
    expect(mockMeetingWhere).not.toHaveBeenCalled();
