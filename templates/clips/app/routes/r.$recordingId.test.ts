...
describe("direct recording route shell cue", () => {
  it("keeps the main header return control explicitly labeled as the library", () => {
    ...
    expect(header).toContain('aria-label={t("recordingPage.backToLibrary")}');
    expect(header).toContain('t("recordingPage.backToLibrary")');
    expect(header).toContain('className="hidden sm:inline"');
    expect(header).toContain("IconArrowLeft");
  });
});
