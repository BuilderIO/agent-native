const messages = {
  common: {
    cancel: "取消",
    create: "创建",
    save: "保存",
    saving: "正在保存…",
    saveChanges: "保存更改",
    connected: "已连接",
    notConnected: "未连接",
    disconnect: "断开连接",
    disconnecting: "正在断开连接...",
  },
  root: {
    commandActions: "操作",
    commandSearch: "搜索",
    commandAppearance: "外观",
    toggleTheme: "切换主题",
    extensionSignedInTitle: "已登录",
    extensionSignedInDescription: "再次打开 Clips 扩展即可开始录制。",
    gotIt: "知道了",
  },
  builderCredits: {
    pausedTitle: "Builder.io 额度已暂停",
    transcriptionDescription:
      "升级 Builder.io 即可恢复备份转录。现有的原生转录仍会显示。",
    cleanupDescription:
      "升级 Builder.io 即可恢复转录清理、摘要和 AI 标题。原生转录已保留。",
    settingsDescription:
      "升级 Builder.io 即可恢复备份转录、转录清理、摘要和 AI 标题。现有的原生转录仍会显示。",
    titleDescription: "AI 标题生成已暂停。你的转录仍可使用。",
    upgrade: "升级 Builder.io",
    retryAfterUpgrade: "升级后重试",
    openAiSetup: "AI 设置",
    featureBackupTranscription: "备份转录",
    featureCleanup: "转录清理",
    featureSummaries: "摘要",
    featureTitles: "AI 标题",
  },
  recorder: {
    cameraBlurTitle: "虚化背景",
    cameraBlurDescription: "让你保持清晰，同时虚化身后的内容",
    cameraBlurToggle: "虚化摄像头背景",
    cameraBlurIntensityLabel: "强度",
    cameraBlurIntensityAria: "背景虚化强度",
  },
  navigation: {
    brand: "剪辑",
    library: "资料库",
    sharedWithMe: "与我共享",
    spaces: "空间",
    meetings: "会议",
    dictate: "听写",
    videoProjects: "视频项目",
    archive: "归档",
    trash: "废纸篓",
    settings: "设置",
    notifications: "通知",
    insights: "洞察",
    space: "空间",
    folder: "文件夹",
    extensions: "扩展",
    newRecording: "新建录制",
    folders: "文件夹",
    newFolder: "新建文件夹",
    noSpaces: "暂无空间",
    desktopCta: "获取桌面应用",
    desktopTitle: "获取 Clips 桌面应用。",
    desktopBody: "从菜单栏录制，使用全局快捷键，并自动更新。",
    expandSidebar: "展开侧边栏",
    collapseSidebar: "收起侧边栏",
    agentEmptyState: "我可以怎样帮助你处理录制？",
    agentSuggestionSummary: "总结我最近的录制",
    agentSuggestionPricing: "查找我提到价格的位置",
    agentSuggestionFiller: "从这个剪辑中移除口头禅",
    createFolderError: "创建失败",
    folderCreated: "文件夹已创建",
    folderNamePlaceholder: "文件夹名称",
  },
  empty: {
    library: {
      title: "你的资料库还是空的",
      body: "录制第一个屏幕视频后，它会出现在这里，随时可以分享。",
      cta: "录制第一个 Clip",
    },
    shared: {
      title: "暂时没有与你共享的剪辑",
      body: "团队成员与你共享的剪辑会显示在这里。",
    },
    folder: {
      title: "这个文件夹是空的",
      body: "拖入录制，或点击录制在此文件夹中开始新内容。",
      cta: "在这里录制",
    },
    space: {
      title: "这个空间还没有录制",
      body: "将录制分享到空间，或录制新内容，团队会在这里看到。",
      cta: "为这个空间录制",
    },
    archive: {
      title: "没有归档内容",
      body: "归档的录制会从资料库隐藏但仍安全保留。你可以随时恢复它们。",
    },
    trash: {
      title: "废纸篓是空的",
      body: "删除的录制会在这里保留 30 天，然后永久移除。",
    },
    search: {
      title: "没有匹配结果",
      body: "尝试其他搜索词，或检查你的筛选条件。",
    },
  },
  trashRoute: {
    title: "废纸篓",
    selected: "已选择 {{count}} 项",
    deselectAll: "取消全选",
    selectAll: "全选",
    restore: "恢复",
    deleteForever: "永久删除",
    deleteForeverTitle: "永久删除？",
    bulkDeleteDescription: "{{count}} 个录制将被永久移除。此操作无法撤销。",
    singleDeleteDescription: "此录制将被永久移除。此操作无法撤销。",
    restored: "已恢复",
    restoreFailed: "恢复失败",
    permanentlyDeleted: "已永久删除",
    deleteFailed: "删除失败",
    clipsRestored: "已恢复 {{count}} 个剪辑",
    clipsRestoreFailed: "{{count}} 个剪辑无法恢复",
    clipsPermanentlyDeleted: "已永久删除 {{count}} 个剪辑",
    clipsDeleteFailed: "{{count}} 个剪辑无法删除",
  },
  recordingRoute: {
    pageTitle: "剪辑录制 · Clips",
  },
  meetingDetailRoute: {
    pageTitle: "会议 · Clips",
  },
  recordingPage: {
    synthesizeEditPlan: "根据我的标记生成剪辑计划",
    untitledClip: "无标题剪辑",
    recordingNotFound: "找不到录制",
    noAccess: "您可能无权访问此剪辑。",
    backToLibrary: "返回资料库",
    storageStillDisconnected: "存储仍未连接",
    finishBuilderOrS3: "完成 Builder.io 弹出窗口或配置 S3 存储，然后重试。",
    loomImportResumed: "Loom 导入已恢复",
    clipUploadResumed: "剪辑上传已恢复",
    couldNotRetryLoom: "无法重试 Loom 导入",
    couldNotResumeUpload: "无法恢复上传",
    tryAgainMoment: "稍后再试一次。",
    aiRequestFailed: "AI请求失败",
    titleUpdated: "标题已更新",
    transcriptNotReady: "成绩单尚未准备好",
    tryAfterTranscription: "转录完成后重试。",
    titleGenerationQueued: "标题生成已排队",
    descriptionQueued: "说明请求已排队",
    chapterQueued: "章节请求已排队",
    fillerQueued: "填充词删除已排队",
    silenceQueued: "静音移除已排队",
    workflowQueued: "工作流请求已排队",
    pageTitle: "剪辑录制·Clips",
    loomMissingUrl: "此 Loom 录音缺少其源 URL。",
    finalizeFailed: "完成失败 ({{status}})",
    uploadTooLarge:
      "Clips 尝试压缩此桌面录音，但它仍然太大而无法上传。原件保存在本地，可以从 Clips 菜单重试。",
    desktopSavedUploadFailed:
      "桌面录音机完成并保存了本地副本，但Clips无法上传。您可以从 Clips 菜单重试，而无需再次录制。",
    retryLibrary: "您可以从库中重试。",
    processingStuck:
      "30 秒后处理尚未完成（状态={{status}}）。剪辑可能尚未完成上传 - 检查服务器日志中的块/最终消息。",
    uploadingAssembling: "上传和组合视频——通常只需要几秒钟。",
    connectStorageImportLoom: "连接存储导入这个Loom。",
    connectStorageFinishClip: "连接存储以完成此剪辑的保存。",
    uploadPausedSaved: "上传已暂停；剪辑保存在本地。",
    savedLocallyHint:
      "本地文件保存在菜单栏的 Clips 应用中：点击托盘图标，使用已保存剪辑上的文件夹按钮打开它，或点击重试重新上传。",
    savingWentWrong: "保存此剪辑时出现问题。",
    finishingClip: "正在完成您的剪辑...",
    loomSourcePreserved:
      "Loom 源链接被保留。连接存储：Builder.io（免费套餐存储 + AI）或 S3 兼容存储，Clips 将重试保存自己的副本。",
    clipDataPreserved:
      "Your clip data is still preserved.连接Builder.io或S3存储，Clips将自动上传。",
    details: "细节",
    importingLoom: "正在导入 Loom...",
    uploadingSavedClip: "正在上传保存的剪辑...",
    storageConnectedSavingLoom: "存储已连接。 Clips 现在正在保存自己的副本。",
    storageConnectedFinishing: "存储已连接。 Clips 正在完成上传。",
    connectStorageImportLoomTitle: "连接存储导入Loom",
    connectStorageFinishSaving: "连接存储完成保存",
    chooseStorageRetryLoom:
      "选择 Clips 存储视频的位置。连接后，Clips 将重试此 Loom 导入。",
    chooseStorageUpload:
      "选择 Clips 存储视频的位置。连接后，保存的剪辑将自动上传。",
    storageConnectedImporting: "存储已连接。正在导入 Loom...",
    storageConnectedUploading: "存储已连接。正在上传此剪辑...",
    retryImport: "重试导入",
    retryUpload: "重试上传",
    checkAgain: "再次检查",
    back: "后退",
    done: "完成",
    edit: "编辑",
    aiTools: "人工智能工具",
    enhanceRecording: "增强此录音",
    includeFullVideo: "包含完整视频",
    includeFullVideoDescription:
      "开启后，AI 工具会观看录制内容（仅限 Gemini）以获取屏幕上下文，而不仅仅是音频文字记录。也适用于默认标题和描述生成。需要通过 Builder 或 GEMINI_API_KEY 使用 Gemini 模型。",
    includeFullVideoOn: "AI 工具将使用完整视频（Gemini）",
    includeFullVideoOff: "AI 工具将只使用文字记录",
    regenerateTitle: "重新生成标题",
    regenerateDescription: "重新生成描述",
    autoChapters: "汽车章节",
    removeFillerWords: "删除填充词",
    removeSilences: "消除静音 (>1.2s)",
    generatePrSummary: "生成公关摘要",
    generateSop: "生成SOP",
    generateSopTooltip:
      "SOP 表示标准操作程序：从此剪辑生成的可重复使用的分步操作手册。",
    generateTicket: "生成票证",
    generateEmail: "生成电子邮件",
    share: "分享",
    fromMeeting: "来自会议：",
    untitled: "无题",
    agent: "代理",
    activity: "活动",
    transcript: "转录",
    insights: "洞察",
    settings: "设置",
    askAboutClip: "询问这个剪辑...",
    summarizeClip: "总结一下这个剪辑",
    generateChapters: "从成绩单生成章节",
    findActionItems: "查找行动项目和后续行动",
    draftRecap: "起草可分享的回顾",
    findKeyMoments: "找到关键时刻",
    listFollowUpActions: "列出后续行动",
    draftQuestions: "向作者提出的草稿问题",
    retryFailed: "重试失败：{{message}}",
    networkError: "网络错误",
    ownerInsights: "业主见解",
    ownerInsightsDescription:
      "此剪辑的编辑者可以看到视图、完成情况和查看者详细信息。",
  },
  sharePage: {
    agentReadableContext: "代理可读的剪辑上下文",
    agentInstructions:
      "获取脚本的 agentContextUrl 和 JPEG 帧 URLs。获取帧 URLs 以查看屏幕，而不仅仅是阅读文字记录。",
    untitledClip: "无标题剪辑",
    incorrectPassword: "密码错误",
    passwordProtected: "该剪辑受密码保护",
    linkExpired: "链接已过期",
    linkExpiredMessage: "创建者为此共享链接设置了过期时间。",
    clipUnavailable: "剪辑不可用",
    clipUnavailableMessage:
      "该录音不公开，或者链接无效。如果这是您的剪辑，请登录以检查访问权限。",
    signIn: "登录",
    somethingWentWrong: "出了点问题",
    pleaseTryAgain: "请再试一次。",
    connectStorageFinish: "连接存储来完成此剪辑。",
    needsAttention: "该剪辑需要注意才能完成。",
    savingWentWrong: "保存此剪辑时出现问题。",
    finishingClip: "完成这个剪辑...",
    loomPreservedManage:
      "Loom 源链接被保留。连接存储：Builder.io（免费套餐存储 + AI）或 S3 兼容存储，然后重试导入。",
    videoPreservedManage:
      "视频被保留。连接Builder.io或S3存储，Clips将完成上传。",
    creatorNeedsStorage:
      "创建者需要先连接存储才能完成此剪辑：Builder.io（免费套餐存储 + AI）或 S3 兼容存储。",
    signInStorage:
      "如果这是您的剪辑，请在此处登录以连接存储：Builder.io（免费套餐存储 + AI）或 S3 兼容存储，并完成上传。",
    uploadNotCompleteSession:
      "上传尚未完成。打开此剪辑的仪表板或要求创建者检查存储空间。",
    uploadNotCompleteSignIn:
      "上传尚未完成。如果这是您的剪辑，请登录以打开所有者控件并检查存储空间。",
    creatorMayRetry: "创建者可能需要重试。",
    uploadingAssembling: "上传并组装视频。该页面将自动更新。",
    details: "细节",
    connectStorageFinishSaving: "连接存储完成保存",
    chooseStorageCheck: "选择 Clips 存储视频的位置。连接后，该页面会再次检查。",
    storageConnectedChecking: "存储已连接。检查这个剪辑...",
    signInToFinish: "登录以完成",
    signInIfYours: "如果这是您的，请登录",
    openDashboard: "打开仪表盘",
    checkAgain: "再次检查",
    backToHome: "回到家",
    generatingTitle: "生成标题",
    tryClips: "试试Clips",
    clipOptions: "剪辑选项",
    downloading: "正在下载...",
    downloadMp4: "下载MP4",
    share: "分享",
    agent: "代理人",
    comments: "评论",
    transcript: "转录",
    insights: "洞察",
    downloadForMac: "下载 Mac",
    downloadForWindows: "下载 Windows",
    downloadForLinux: "下载 Linux",
    downloadDesktopApp: "下载桌面应用程序",
    agentNativeClips: "Agent-Native Clips",
    agentNativeClipsIntro: "是一款免费的",
    openSource: "开源",
    agentFriendly: "智能体友好",
    loomAlternative: "Loom 替代品",
    signUp: "注册",
    ownerInsights: "所有者洞察",
    ownerInsightsDescription:
      "观看次数、完成率和观看者详情对该剪辑的编辑者可见。",
  },
  meetingDetail: {
    untitledMeeting: "无标题会议",
    unassigned: "未分配",
    them: "他们",
    me: "我",
    regeneratingNotes: "正在重新生成摘要",
    meetingRemoved: "会议已删除",
    couldNotRemoveMeeting: "无法删除会议",
    couldNotLoadMeeting: "无法加载该会议。",
    transcriptCopied: "复制成绩单",
    couldNotCopyTranscript: "无法复制成绩单",
    allMeetings: "所有会议",
    live: "居住",
    generatingNotesInline: "正在生成摘要...",
    regenerateNotes: "重新生成摘要",
    share: "分享",
    meetingOptions: "会议选项",
    removeMeeting: "删除会议",
    removeThisMeeting: "删除这次会议吗？",
    removeDescription:
      "这将从 Clips 中删除会议。它不会删除任何链接的文字记录或更改您的 Google Calendar。",
    cancel: "取消",
    removing: "正在删除...",
    remove: "消除",
    desktopHint:
      "要开始记录，请从菜单栏打开 Clips Desktop 并选择 Start Meeting Notes，或在提醒出现时点击 Start notes。Clips 会捕获麦克风和系统音频，并在这里写入文字记录。",
    getDesktopApp: "获取桌面应用程序",
    generateNotesFailed: "无法生成摘要。再试一次。",
    attendee_one: "{{count}} 与会者",
    attendee_other: "{{count}} 与会者",
    joinCall: "加入通话",
    myNotes: "我的笔记",
    aiNotes: "人工智能笔记",
    summary: "摘要",
    actionItems: "行动项目",
    working: "在职的…",
    noActionItems: "还没有行动项目。从记录生成笔记后，它们会出现在此处。",
    transcript: "成绩单",
    segments: "{{count}} 段",
    copyTranscript: "复制成绩单",
    copyFullTranscript: "复制完整成绩单",
    timeRemaining_one: "还剩 {{count}} 分钟",
    timeRemaining_other: "还剩 {{count}} 分钟",
    endMeeting: "结束会议",
    endThisMeeting: "结束这次会议吗？",
    endMeetingDescription:
      "这将停止本次会议的录制和转录。您仍然可以根据目前捕获的内容生成笔记。",
    couldNotEndMeeting: "无法结束会议",
  },
  transcriptPanel: {
    transcribing: "正在抄写…",
    pendingDescription:
      "捕获语音后，实时文字记录就会出现。清理工作在后台继续进行。",
    noSpeechDetected: "未检测到语音",
    noSpeechDescription:
      "我们在这段录音中没有听到任何讲话。如果这是故意的，那么一切都准备好了。如果没有，请检查您的麦克风和语音权限，然后重试转录。",
    retry: "重试",
    transcriptUnavailable: "成绩单不可用：{{reason}}",
    transcriptUnavailableTitle: "转录不可用",
    providerNeedsAttention: "提供商需要处理",
    providerNeedsAttentionDescription:
      "当前提供商已暂停此请求。切换到 Builder.io 或更新密钥，然后重试。",
    enableTranscriptionTitle: "启用转录",
    enableTranscriptionDescription: "为此 Clip 启用字幕、转录搜索和摘要。",
    searchPlaceholder: "搜索成绩单",
    copyTranscript: "复制成绩单",
    downloadSrt: "下载.srt",
    regenerate: "重新生成转录文本",
    cleanupRunning: "在后台清理成绩单。",
    noMatches: "没有匹配项。",
    noTranscript: "还没有文字记录。",
    noTranscriptCaptured: "没有捕获任何文字记录。",
    backupFailed:
      "本地未捕获任何语音，备份转录未完成。重试或检查麦克风和语音权限。",
    backupNotSetup: "本地未捕获任何语音，且未设置备份转录。",
    cleanupKept: "清理工作无法完成。保留了本机转录本。",
    cleanupBuilderFailed:
      "即使 Builder.io 已连接，清理也无法完成。保留了本机转录本。",
    cleanupPaused:
      "清理已暂停。请在“设置”中连接 AI：Builder.io（免费额度）或你自己的 LLM 密钥。",
    builderNoResponse: "没有收到Builder的回复。允许弹出窗口并重试。",
    saveFailed: "保存失败（{{status}}）",
    savedRetrying: "已保存。正在重试转录...",
    getGroqKey: "获取 Groq 密钥",
    groqKeysStart: "Groq 键开头为",
    waiting: "等待…",
    nativeSpeechPrimary: "原生语音仍然是主要的转录来源。",
  },
  thumbnailPicker: {
    failedCapture: "捕捉帧失败",
    failedGif: "无法生成 GIF — 尝试更短的范围",
    nothingToApply: "还没有什么可以申请的",
    updated: "缩略图已更新",
    failedUpdate: "更新缩略图失败",
    thumbnail: "缩略图",
    upload: "上传",
    frame: "画面帧",
    animatedGif: "动态 GIF",
    uploadImage: "上传图片",
    uploadedPreview: "已上传预览",
    currentThumbnail: "当前缩略图",
    frameAt: "帧位于 {{time}}",
    captureFrame: "捕捉帧",
    preview: "预览",
    capturedFrame: "捕获的帧",
    capturePreview: "捕获一帧进行预览",
    start: "开始： {{time}}",
    duration: "持续时间：{{time}}",
    generateGif: "生成GIF",
    animatedPreview: "动画缩略图预览",
    generateGifPreview: "生成GIF来预览",
    cancel: "取消",
    saveThumbnail: "保存缩略图",
  },
  shareDialog: {
    publicDescription: "知道链接的任何人都可以查看 - 登录后发表评论或做出反应",
    shareRecording: "分享录音",
    shareTitle: "分享“{{title}}”",
    link: "链接",
    invite: "邀请",
    embed: "嵌入",
    shareLink: "分享链接",
    shareWithAgents: "与代理商分享",
    copyAgentPrompt: "复制代理提示",
    agentPrompt:
      "获取这个 Clips 代理上下文 URL：{{agentContextUrl}}。使用 transcript.segments 读取语音上下文，获取 recommendedFrames 或帧 API URL 来查看屏幕，并在 browserDiagnostics 存在时检查经过脱敏的控制台日志和 fetch/XHR 请求元数据。",
    agentTokenDescription:
      "这个临时代理 URL 可让代理读取剪辑，而无需将其设为公开。它会在两小时后过期。",
    agentLinkUnavailable: "无法创建代理链接。",
    retryAgentLink: "重试",
    gifPreview: "GIF 预览",
    openPlayer: "开放玩家",
    downloadMp4: "下载MP4",
    embedsNeedPublic: "嵌入需要公共剪辑",
    embedPublicDescription:
      "该剪辑当前为 {{visibility}}。嵌入式 iframe 会匿名加载，因此剪辑必须公开以供观看者观看。",
    makingPublic: "公开…",
    makePublic: "公开",
    askOwnerPublic: "请楼主公开一下。",
    responsive: "反应灵敏 (16:9)",
    fixedSize: "固定尺寸",
    embedIframeTitle: "Clips 视频",
    width: "宽度",
    height: "高度",
    autoplay: "自动播放",
    startAt: "开始时间（秒）",
    embedCode: "嵌入代码",
    sharePlainTitle: "分享 {{title}}",
  },
  shareUi: {
    owner: "所有者： {{email}}",
    generalAccess: "一般访问",
    restrictedLinkDescription: "此链接仅适用于已经具有访问权限的人员。",
    makingPublic: "公开…",
    makePublicAndCopy: "公开并复制",
    copy: "复制",
    addPeopleByEmail: "通过电子邮件添加人员",
    notifyPeople: "通知人们",
    peopleWithAccess: "有访问权限的人",
    ownerRole: "所有者",
    remove: "消除",
    noAccessYet: "目前还没有人可以访问。",
    visibility: {
      private: {
        label: "私人的",
        description: "只有有权访问的人才能查看",
      },
      org: {
        label: "组织",
        description: "您组织中的任何人都可以查看",
      },
      public: {
        label: "民众",
        description: "知道链接的人都可以查看",
      },
    },
    roles: {
      viewer: "观众",
      editor: "编辑",
      admin: "行政",
    },
  },
  quickAsk: {
    title: "询问本次会议",
    toggleHint: "切换 · Esc 关闭",
    quickPrompts: "快速提示",
    whatDidIMiss: "我错过了什么？",
    whatDidIMissPrompt: "我错过了什么？",
    summarizeLastFive: "总结一下最后5分钟",
    summarizeLastFivePrompt: "用 3-5 个要点总结本次会议的最后 5 分钟。",
    actionItemsForMe: "我的行动项目",
    actionItemsForMePrompt:
      "列出本次会议中分配给我的行动项目。使用复选框项目符号。",
    history: "历史",
    emptyDescription: "询问任何事情。代理拥有完整的记录和会议背景。",
    placeholder: "询问任何事情...",
    send: "发送",
    sentToChat: "已发送至聊天 - 请参阅代理侧边栏以获取回复。",
    suggestQuestions: "为我推荐可以提出的问题",
    suggestQuestionsPrompt:
      "根据本次会议目前为止讨论的内容，建议我接下来可以提出的几个好问题。",
    makeMeSoundSmart: "让我听起来更聪明",
    makeMeSoundSmartPrompt:
      "根据本次会议目前为止的内容，给我一个现在就可以补充的犀利、有见地的评论或问题。",
  },
  brandingEditor: {
    title: "品牌推广",
    uploadImageFile: "请上传图像文件。",
    logoUploaded: "徽标已上传",
    uploadFailed: "上传失败",
    brandingUpdated: "品牌更新",
    saveFailed: "保存失败",
    organizationName: "组织名称",
    brandColor: "品牌颜色",
    brandColorPicker: "品牌颜色选择器",
    useColor: "使用 {{color}}",
    logo: "标识",
    logoPreview: "标志预览",
    dropReplace: "删除一个新图像来替换",
    dropHere: "在此处放置 PNG / JPG / SVG",
    uploading: "正在上传...",
    chooseFile: "选择文件",
    remove: "消除",
    preview: "预览",
    organizationFallback: "组织",
    saving: "保存…",
    save: "节省",
  },
  downloadRoute: {
    pageTitle: "下载 Clips Desktop",
    description:
      "从系统托盘录制屏幕。适用于 macOS、Windows 和 Linux 的自动更新桌面应用程序。",
    macSublabel: "通用（Apple Silicon + Intel）",
    windowsSublabel: "64 位 MSI 安装程序",
    downloadFor: "下载 {{platform}}",
    alsoFor: "也可用于 {{platform}}",
    backToLibrary: "返回资料库",
    clipsDesktop: "Clips 桌面版",
    heroDescription:
      "用于屏幕、相机和屏幕+相机的菜单栏记录器。一键启动，可拖动相机气泡，停止时即时分享链接。",
    versionReleased: "版本 {{version}} — 发布 {{date}}",
    version: "版本{{version}}",
    manifestError: "无法加载版本清单 - 从版本页面选择安装程序。",
    loadingRelease: "正在加载最新版本...",
    chromeTitle: "Chrome 浏览器日志扩展",
    chromeDescription:
      "当您需要录制以及经过编辑的控制台并从启动的选项卡获取/XHR 诊断时，请使用该扩展。桌面应用程序仍然是最流畅的日常录音机。",
    installChrome: "安装 Chrome 扩展",
    chromePending: "Chrome Web Store URL 待定",
  },
  inviteRoute: {
    missingToken: "缺少邀请令牌。",
    notFound: "未找到邀请或邀请已过期。",
    loadFailed: "无法加载邀请",
    joined: "已加入{{team}}",
    theTeam: "团队",
    acceptFailed: "无法接受邀请",
    declined: "邀请被拒绝",
    declineFailed: "拒绝失败",
    signInRequired: "需要登录",
    signInDescription: "您需要先登录才能接受邀请。",
    signIn: "登录",
    invited: "你已被邀请",
    loading: "正在加载…",
    role: "角色：",
    invitedToJoin: "{{inviter}} 邀请您加入 Clips 上的 {{team}}。",
    emailMismatch:
      "注意：此邀请已发送至 {{inviteEmail}}。您以 {{sessionEmail}} 身份登录。接受仍然有效，但加入的将是您的帐户电子邮件。",
    decline: "拒绝",
    joining: "正在加入……",
    acceptInvite: "接受邀请",
    declineInvite: "拒绝此邀请？",
    declineDescription: "邀请将被删除。管理员可以随时向您发送新的。",
    cancel: "取消",
    pageTitle: "加入团队 · Clips",
  },
  settings: {
    openAgentSettings: "打开代理设置",
    agentDescription:
      "打开代理侧边栏设置，管理模型、API 密钥、自动化、语音和其他代理控制项。",
    agentTitle: "代理设置",
    title: "设置",
    pageTitle: "设置 · Clips",
    intro: "此 Clips 工作区的偏好设置和已连接服务。",
    languageTitle: "语言",
    languageDescription:
      "选择此账号的界面语言。Clips 会在不同设备上记住你的选择。",
    languageLabel: "界面语言",
    whatsNew: "最新变化",
    changelogEmpty: "暂无更新。",
    viewAllUpdates: "查看所有更新",
    expand: "展开",
    collapse: "收起",
    changelogMarkdown: `# 更新日志

Clips 中所有面向用户的重要更改都会记录在这里。你可以随时从命令菜单（Cmd+K -> “最新变化”）或设置中打开。

## 2026-06-23

### 新增

- 现在可以直接在 Clips 中查看最新变化。更新日志已加入命令菜单（Cmd+K）和设置。`,
    profile: "个人资料",
    email: "邮箱",
    displayName: "显示名称",
    displayNamePlaceholder: "你的姓名",
    playback: "播放",
    defaultPlaybackSpeed: "默认播放速度",
    playbackDescription: "打开录制时会自动应用。",
    transcript: "转录",
    transcriptCleanup: "AI 转写清理",
    transcriptCleanupDescription:
      "默认关闭。开启后，转写完成会由 AI 重写文本以改善标点和可读性。专业术语偶尔可能被改写 — 原始转写始终保留，不会丢失任何内容。",
    backupTranscription: "云端备用转写",
    backupTranscriptionDescription:
      "默认开启。当未捕获到本地转写时，录制的音频会发送到已连接的云服务（Builder.io 额度或 Groq）进行转写。关闭后音频严格保留在本设备上 — 此类剪辑将保持无转写状态。",
    notifications: "通知",
    emailNotifications: "邮件通知",
    emailNotificationsDescription:
      "当有人评论、回应或与你分享录制时，收到邮件通知。",
    saved: "设置已保存",
    saveFailed: "保存失败",
    builderConnectedToast: "Builder.io 已连接",
    videoStorage: "视频存储",
    videoStorageDescription:
      "Builder.io 是 Clips 上传的主要存储路径。需要自带存储桶时，也可以使用 S3。",
    checkingBuilder: "正在检查 Builder.io",
    builderConnected: "Builder.io 已连接",
    connectBuilder: "使用 Builder.io（免费）",
    builderConnectedFor: "正在为 {{orgName}} 使用 Builder.io。",
    builderConnectedGeneric: "新的剪辑会使用已连接的 Builder.io 提供方。",
    builderIncludes: "包含对象存储、上传，以及新剪辑的托管转录。",
    s3Title: "S3 兼容存储",
    secondary: "备用",
    active: "当前启用",
    s3BuilderConnectedDescription:
      "仅当此工作区应上传到你自己的存储桶而不是 Builder.io 时使用。",
    s3CurrentProvider: "当前正在使用 {{providerName}}。",
    s3OwnBucketDescription:
      "如果不想使用 Builder.io 存储，可以使用自己的存储桶。",
    configureS3: "配置 S3",
    hideS3: "隐藏 S3",
    saveStorage: "保存存储设置",
    storageSaved: "存储设置已保存",
    storageRequired: "Endpoint、bucket、access key 和 secret 为必填项。",
    s3EndpointLabel: "端点 URL",
    s3BucketLabel: "存储桶",
    s3AccessKeyLabel: "访问密钥 ID",
    s3SecretAccessKeyLabel: "秘密访问密钥",
    s3RegionLabel: "区域",
    s3PublicBaseUrlLabel: "公共基础 URL",
    s3UrlInvalid: "必须是有效的 URL（例如 https://s3.us-east-1.amazonaws.com）",
    s3BucketInvalid: "存储桶名称必须为 3–63 个小写字母、数字或连字符",
    s3RegionInvalid: '必须是有效的区域（例如 us-east-1）或 "auto"',
    apiSetup: "AI 设置",
    apiSetupDescription: "使用 Builder.io 免费额度或你自己的 LLM 密钥连接 AI。",
    builderEasySetup: "Builder.io 免费额度",
    builderAiAvailable: "Clips 可使用包含的 AI 额度和托管转录。",
    builderAiDescription:
      "优先使用 Builder.io，可获得内置 AI 额度、对象存储、上传和托管转录。",
    providerKeyTitle: "使用自己的提供方密钥",
    providerKeyDescription:
      "添加 Anthropic、OpenAI、Gemini、Groq 或 OpenRouter 密钥，用于按提供方计费的使用。",
    providerKeysSet: "已设置 {{count}} 个",
    checkingProviderKeys: "正在检查提供方密钥…",
    keySet: "已设置",
    keyCleared: "存储凭证已清除",
    clearAllS3: "清除凭证",
    replaceKey: "替换密钥…",
    pasteProviderKey: "请先粘贴提供方密钥。",
    apiKeySaved: "API 密钥已保存",
    apiKeyFailed: "保存密钥失败",
    slackTitle: "Agent-Native Clips 为 Slack",
    slackDescription:
      "分享公开剪辑，将链接粘贴到 Slack 后即可内联播放，观看者无需额外步骤。每个工作区连接一次即可。",
    checkingSlack: "正在检查 Slack",
    slackConnected_other: "已连接 {{count}} 个工作区",
    slackOauthNeeded: "需要 OAuth 凭据",
    slackPreviewDescription: "公开 Clips 链接可以显示为可播放的 Slack 预览。",
    connectSlack: "连接 Slack",
    slackClientMissing:
      "连接工作区前，请为此部署设置 SLACK_CLIENT_ID 和 SLACK_CLIENT_SECRET。",
    slackSigningMissing:
      "请设置 SLACK_SIGNING_SECRET，以便验证 Slack 事件回调。",
    connectedBy: "由 {{email}} 连接",
    disconnectSlackLabel: "断开 {{team}}",
    slackConnectedToast: "Slack 已连接",
    slackCheckedToast: "Slack 连接已检查",
    slackDisconnectedToast: "Slack 已断开连接",
    slackConnectFailed: "连接 Slack 失败",
    slackDisconnectFailed: "断开 Slack 失败",
    disconnectSlackTitle: "断开 Slack？",
    disconnectSlackDescription:
      "Clips 将删除 {{team}} 的已存储机器人令牌，并停止发送可播放的 Slack 预览。",
    thisWorkspace: "此工作区",
    slackConnected: "Slack 已连接",
  },
  insightsHub: {
    title: "洞察",
    period: "时间范围",
    last7Days: "最近 7 天",
    last14Days: "最近 14 天",
    last30Days: "最近 30 天",
    last90Days: "最近 90 天",
    exportCsv: "导出 CSV",
    description: "过去 {{days}} 天内整个组织的互动情况。",
    views: "观看次数",
    reactions: "回应",
    comments: "评论",
    recordings: "录制",
    engagementTrend: "互动趋势",
    topVideos: "热门视频",
    topCreators: "热门创作者",
  },
  folderTree: {
    noFolders: "还没有文件夹",
    rename: "重命名",
    newSubfolder: "新建子文件夹",
    delete: "删除",
    renameFolder: "重命名文件夹",
    renamed: "文件夹已重命名",
    renameFailed: "重命名失败",
    createInside: "在“{{name}}”内创建文件夹。",
    folderName: "文件夹名称",
    created: "文件夹已创建",
    createFailed: "创建失败",
    deleteFolder: "删除文件夹？",
    deleteDescription:
      "其中的录制会移动到父级范围（资料库根目录或父文件夹）。嵌套子文件夹会被移除。此操作无法撤销。",
    deleted: "文件夹已删除",
    deleteFailed: "删除失败",
  },
  organizationSettings: {
    pageTitle: "组织设置 · Clips",
    title: "组织 · 设置",
    namedTitle: "{{name}} · 设置",
    noOrganization: "还没有组织。请从组织切换器创建一个以开始使用。",
    description: "组织管理：品牌、成员、邀请。",
    adminsOnlyBranding: "只有管理员可以编辑品牌。",
    members: "成员",
    pendingInvites: "待处理邀请",
    noPendingInvites: "没有待处理邀请。",
    email: "电子邮件",
    role: "角色",
    sent: "已发送",
  },
  recordingInsights: {
    loading: "正在加载洞察…",
    noData: "还没有数据。",
    views: "观看次数",
    uniqueViewers: "独立观看者",
    completion: "完成率",
    ctaConversion: "CTA 转化",
    dropOff: "流失",
    viewersCount: "{{count}} 位观看者",
    watching: "正在观看",
    recentViewers: "最近观看者",
    noViewers: "还没有观看者。",
    anonymous: "匿名",
    anon: "匿名",
    moreViewers: "+{{count}} 更多",
    viewedBy: "观看者",
    someone: "有人",
    noViewsYet: "暂无观看记录。",
  },
  libraryGrid: {
    spaceRoot: "空间根目录",
    libraryRoot: "资料库根目录",
    clipsMoved: "已移动 {{count}} 个剪辑",
    moveFailed: "移动剪辑失败",
    titleRequired: "标题不能为空",
    clipRenamed: "剪辑已重命名",
    renameFailed: "重命名剪辑失败",
    renameClip: "重命名剪辑",
    clipTitle: "剪辑标题",
    movedToTrash: "已移至废纸篓",
    restoredFromArchive: "已从归档恢复",
    archived: "已归档",
    clipsArchived: "已归档 {{count}} 个剪辑",
    clipsArchiveFailed: "{{count}} 个剪辑无法归档",
    clipsMovedToTrash: "已将 {{count}} 个剪辑移至废纸篓",
    clipsTrashFailed: "{{count}} 个剪辑无法移至废纸篓",
    loadFailedTitle: "无法加载你的录制",
    loadFailedBody: "加载此列表时出了点问题。你的录制是安全的 —— 请重试。",
    retry: "重试",
  },
  notificationsRoute: {
    pageTitle: "通知 · Clips",
    title: "通知",
    description: "过去 30 天内你的录制上的评论、回应、提及和分享。",
    replySent: "回复已发送",
    sendFailed: "发送失败",
    all: "全部",
    comments: "评论",
    reactions: "回应",
    mentions: "提及",
    shares: "分享",
    loading: "正在加载…",
    replyTo: "回复 {{email}}，位于",
    replyPlaceholder: "写回复…",
  },
  stitchManager: {
    defaultTitle: "拼接录制",
    title: "拼接录制",
    pickAtLeastTwo: "请选择至少 2 个录制进行拼接",
    videoUrlMissing: "一个或多个录制还没有可用的视频 URL",
    connectStorage:
      "拼接录制前请连接存储：Builder.io（免费套餐存储 + AI）或 S3 兼容存储。",
    created: "拼接录制已创建",
    failed: "拼接录制失败",
    noOtherRecordings: "没有其他可用录制。",
    combineOrder: "合并顺序",
    emptyQueue: "点击左侧录制将其添加到此处。拖动可重新排序。",
    titleLabel: "标题",
    combining:
      "正在用 ffmpeg.wasm 合并 — {{progress}}%。大型拼接可能需要一分钟。",
    combine: "合并",
  },
  inviteDialog: {
    title: "邀请成员",
    description:
      "粘贴一个或多个电子邮件，用逗号或换行分隔。邀请将在 7 天后过期。",
    enterEmail: "请输入至少一个电子邮件地址。",
    invalidEmails: "无效电子邮件：{{emails}}",
    emailFailed: "{{email}}：{{message}}",
    failed: "失败",
    sent: "已发送 {{count}} 个邀请。",
    sentWithFailures: "已发送 {{sent}} 个邀请，{{failed}} 个失败。",
    emails: "电子邮件",
    memberRole: "成员 — 录制、编辑、分享",
    adminRole: "管理员 — 完全控制",
    sending: "正在发送…",
    sendInvites: "发送邀请",
  },
  membersList: {
    memberRole: "成员",
    adminRole: "管理员",
    roleUpdated: "已将 {{email}} 的角色更新为 {{role}}",
    updateFailed: "更新角色失败",
    removed: "已从组织中移除 {{email}}",
    removeFailed: "移除成员失败",
    noMembers: "还没有成员。",
    member: "成员",
    joined: "加入时间",
    you: "（你）",
    removeMemberLabel: "移除 {{email}}",
    removeTitle: "移除成员？",
    removeDescription:
      "{{email}} 将失去对此组织的访问权限。你随时可以重新邀请他们。",
    remove: "移除",
  },
  slackShareHint: {
    playsInline: "可在 Slack 中内联播放",
    connectedDescription: "将此链接粘贴到任何已连接工作区即可内联播放。",
    makeInline: "让它在 Slack 中内联播放",
    connectDescription: "连接一个工作区，让此链接展开为视频。",
    connect: "连接",
  },
  commentsPanel: {
    disabled: "此录制已禁用评论。",
    beFirst: "成为第一个评论的人",
    leaveNotePanel: "在此面板顶部留下备注。",
    leaveNoteTimestamp: "在当前时间戳留下备注。",
    leaveComment: "留下评论...",
    signInToComment: "登录以发表评论。",
    signIn: "登录",
    replyingTo: "正在回复",
    commentAt: "评论于",
    writeReply: "写回复...",
    commentButton: "评论",
    composerPlaceholder: "添加评论…",
    mentionSomeone: "提及某人",
    addEmoji: "添加表情",
  },
  shareMeeting: {
    pageTitle: "会议笔记 · Clips",
    description: "通过 Clips 分享的 AI 会议笔记。",
    unavailable: "此会议是私密的或不再可用。",
    tryClips: "试用 Clips",
    attendees: "{{count}} 位参会者",
    noAiNotes: "此会议尚未生成摘要。",
    summary: "摘要",
    keyPoints: "要点",
    actionItems: "行动项",
  },
  deleteRecordingMenu: {
    movedToTrash: "剪辑已移至废纸篓",
    deleteFailed: "删除剪辑失败",
    clipOptions: "剪辑选项",
    delete: "删除",
    moveTitle: "将此剪辑移至废纸篓？",
    moveDescription:
      "这会从你的资料库中移除此剪辑。你以后可以从废纸篓恢复，或永久删除。",
    deleting: "正在删除...",
    moveToTrash: "移至废纸篓",
  },
  recordingToolbar: {
    controls: "录制控制",
    resumeRecording: "继续录制",
    pauseRecording: "暂停录制",
    resumeShortcut: "继续 (⌥⇧P)",
    pauseShortcut: "暂停 (⌥⇧P)",
    stop: "停止录制",
    elapsed: "已用时间",
    cancel: "取消录制",
    cancelShortcut: "取消 (⌥⇧C)",
    addMarker: "添加时间戳标记",
    markerShortcut: "标记 (⌥⇧M) · 备注 ⌥⇧E · B-roll ⌥⇧B · 重录 ⌥⇧N",
  },
  annotationsStrip: {
    comment: "评论",
    resolvedChip: "已解决",
    moreComments: "还有 {{count}} 条评论",
    addKindAt: "在 {{time}} 添加{{kind}}",
    changeType: "更改类型",
    addMarkerAt: "在 {{time}} 添加标记",
    jumpTo: "跳转到 {{time}}",
    resolve: "标记为已解决",
    reopen: "重新打开",
    title: "标记（{{count}}）",
    editorNote: "剪辑备注",
    bRoll: "B-roll",
    retake: "重录",
    marker: "标记",
    wholeVideo: "整段视频",
    delete: "删除标记",
  },
  countdownOverlay: {
    startsIn: "录制将在 {{count}} 后开始",
    cancel: "取消录制",
    skip: "跳过倒计时并立即开始录制",
    hint: "Esc 取消 · Enter 立即开始",
  },
  videoPlayer: {
    loomVideo: "Loom 视频",
    noVideo: "没有可用视频",
    thanks: "感谢观看",
    playClip: "播放剪辑",
    unsupportedFormat:
      "此浏览器无法播放该视频。请尝试在 Chrome、Edge 或 Firefox 中打开链接。",
  },
  searchBar: {
    placeholder: "搜索录制…",
    clear: "清除搜索",
    searching: "正在搜索…",
    noMatchesFor: "没有匹配项：",
    titleTranscript: "标题 + 转录",
    titleComment: "标题 + 评论",
    transcript: "转录",
    comment: "评论",
    titleOrDescription: "标题或描述",
  },
  organizationSwitcher: {
    noOrganization: "没有组织",
    organizationCount: "{{count}} 个组织",
    organizations: "组织",
    noOrganizations: "还没有组织",
    switchFailed: "切换组织失败",
    newOrganization: "新建组织",
    createOrganization: "创建组织",
    organizationName: "组织名称",
    created: "已创建“{{name}}”",
    createFailed: "创建组织失败",
    creating: "正在创建…",
  },
  playerControls: {
    playbackSpeed: "播放速度",
  },
  autoRecordPrompt: {
    notesStarted: "笔记已开始。",
    cancelWithSeconds: "取消（{{count}} 秒）",
    startNotesQuestion: "会议时间到了，要开始记录吗？",
    autoStartsIn: "{{count}} 秒后自动开始",
    startNotes: "开始记录",
    dismiss: "关闭",
  },
  meetingCanvas: {
    noAiNotes: "还没有 AI 笔记。请从已完成的转录生成笔记。",
    noNotes: "没有笔记。",
    yourNotes: "你的笔记…",
    clickToEdit: "点击编辑",
  },
  meetingCard: {
    live: "直播中",
    transcript: "转录",
    aiNotesReady: "AI 笔记已就绪",
    noSummary: "还没有摘要",
    transcriptSource: "转录来源",
    now: "现在",
    started: "已开始",
    inMinutes: "{{count}} 分钟后",
    inHours: "{{count}} 小时后",
    join: "加入",
    openNotes: "打开笔记",
    open: "打开笔记",
    startFromDesktopNow: "从桌面提醒或菜单栏开始实时笔记。",
    startFromDesktopLater: "到时间时，Clips Desktop 会显示 Start notes。",
  },
  transcriptBubbles: {
    listening: "正在聆听…",
    noTranscript: "还没有转录。",
    liveTranscriptDescription: "开始记录后，实时转录会显示在这里。",
    me: "我",
    them: "对方",
    searchTranscript: "搜索转录",
    searchPlaceholder: "搜索转录…",
    searchMatchCount: "第 {{current}} 项，共 {{total}} 项",
    searchNoMatches: "没有匹配项",
    searchPrevMatch: "上一个匹配项",
    searchNextMatch: "下一个匹配项",
    searchClose: "关闭搜索",
  },
  editorLayout: {
    trimmed: "已修剪",
    trimFailed: "修剪失败",
    cut: "已剪切",
    cutFailed: "剪切失败",
    split: "已拆分",
    splitFailed: "拆分失败",
    loadingRecording: "正在加载录制…",
    recordingNotFound: "找不到录制",
    noVideoYet: "还没有可用视频。",
  },
  transcriptEditor: {
    segmentStart: "分段开始",
    ignore: "忽略",
    restore: "恢复",
    addMarker: "添加标记",
    createSection: "创建片段区间",
    copy: "复制",
    cut: "剪切",
    delete: "删除",
    transcript: "转录",
    selectionRange: "· 已选择 {{start}} → {{end}}",
    selectTextToTrim: "· 选择文本进行修剪",
    noTranscript: "还没有转录。",
  },
  createSpaceDialog: {
    newSpace: "新建空间",
    description: "在当前组织中创建空间。",
    name: "名称",
    color: "颜色",
    useColor: "使用颜色 {{color}}",
    spaceCreated: "空间已创建",
    createFailed: "无法创建空间",
  },
  signInPrompt: {
    title: "登录以{{intent}}",
    description:
      "创建账号或登录即可在此剪辑上{{intent}}。完成后我们会把你带回这里。",
    notNow: "暂不",
    signIn: "登录",
  },
  embedRoute: {
    passwordRequired: "需要密码",
    unavailable: "剪辑不可用。",
  },
  cameraVisualizer: {
    bubble: "气泡",
    live: "实时",
    waiting: "等待中",
    opening: "正在打开",
    stop: "停止",
    openingEllipsis: "正在打开...",
    test: "测试",
    selectedPreview: "已选摄像头预览",
    preview: "摄像头预览",
    setBubbleSize: "设置摄像头气泡大小 {{size}}",
  },
  storageSetup: {
    builderTimeout: "5 分钟内未收到 Builder 响应。请检查弹出窗口并重试。",
    builderConnected: "Builder.io 已连接",
    waitingForBuilder: "正在等待 Builder...",
    connectBuilder: "使用 Builder.io（免费）",
    free: "免费",
    configureS3: "配置 S3 兼容存储",
    whyPrompt: "为什么会看到这个？",
    whyDescription:
      "Clips 是 100% 免费且开源的应用，所以你需要连接一种方式来存储剪辑。使用 Builder.io 可获得免费套餐存储和 AI，或使用 S3 兼容对象存储和你自己的 LLM 密钥。",
  },
  captureInstall: {
    title: "选择你的录制工具",
    description: "需要浏览器日志时使用 Chrome，日常录制最流畅则使用桌面应用。",
    chromeTitle: "Chrome 扩展",
    chromeDescription:
      "当你需要来自浏览器标签页的脱敏控制台和网络诊断信息时最合适。",
    chromePendingDescription:
      "浏览器日志选项已准备就绪，正在等待 Chrome Web Store 网址。",
    desktopTitle: "桌面应用",
    desktopDescription: "全局快捷键、菜单栏录制、会议和重复录制的最顺畅选择。",
  },
  editableTitle: {
    untitled: "未命名剪辑",
    renameFailed: "重命名剪辑失败",
    emptyTitle: "标题不能为空",
    generatingTitle: "正在生成标题",
    placeholder: "剪辑标题",
    editLabel: "编辑剪辑标题",
  },
  chapters: {
    title: "章节",
    addHere: "在此添加",
    empty: "还没有章节。",
    remove: "移除章节",
    saveFailed: "保存章节失败",
    duplicateAtPoint: "此位置已存在章节",
    defaultTitle: "章节 {{count}}",
    seekTo: "跳转到 {{time}}",
  },
  editVersions: {
    history: "编辑历史",
    historyTitle: "编辑版本",
    historyDescription:
      "此录制的所有已提议、已接受、已拒绝和已替换的剪辑版本。",
    historyEmpty: "暂无编辑版本",
    preview: "预览剪辑",
    previewing: "正在预览：{{title}}",
    previewFailed: "无法加载预览",
    exitPreview: "退出预览",
    restore: "恢复",
    restored: "版本已恢复 — 之前的编辑已归档",
    restoreFailed: "恢复失败",
    statusProposed: "已提议",
    statusAccepted: "已接受",
    statusRejected: "已拒绝",
    statusSuperseded: "已替换",
    review: "审阅",
    accept: "接受版本",
    reject: "拒绝",
    accepted: "已应用版本，此前的剪辑已归档",
    rejected: "已拒绝版本",
    reviewFailed: "审阅失败",
    pendingFrom: "{{author}} 提议的剪辑等待您审阅",
    editor: "编辑",
  },
  editorToolbar: {
    selectionSegment: "分段",
    selectionRange: "选区",
    clearSelection: "清除选择",
    split: "拆分",
    cutSegment: "剪切分段",
    removeSplit: "移除拆分点",
    actions: "操作",
    splitAtTime: "在 {{time}} 拆分",
    cutSelection: "剪掉所选范围",
    cutStartToTime: "剪掉开头 → {{time}}",
    cutTimeToEnd: "剪掉 {{time}} → 结尾",
    undoTooltip: "撤销 (Cmd/Ctrl+Z)",
    playPauseTooltip: "播放 / 暂停 (Space)",
    sourceDuration: "（原始 {{duration}}）",
    previewSpeed: "预览速度",
    previewSpeedTooltip: "修剪时的预览速度",
    cutSelectedRange: "剪切所选范围",
    edit: "编辑",
    playheadEdits: "播放头编辑",
    splitAtPlayhead: "在播放头处分割",
    cutBeforePlayhead: "剪切播放头之前的内容",
    cutAfterPlayhead: "剪切播放头之后的内容",
    panels: "面板",
    hideChapters: "隐藏章节",
    showChapters: "显示章节",
    thumbnail: "缩略图",
    stitchClips: "拼接剪辑",
    zoom: "缩放",
    zoomOut: "缩小",
    fitToWidth: "适应宽度",
    zoomIn: "放大",
    clearAllEdits: "清除所有编辑",
    loadingFfmpeg: "正在加载 ffmpeg…",
    exportMp4: "导出 MP4",
    longExportTitle: "此次导出时间较长",
    longExportDescription:
      "编辑后的视频时长为 {{duration}}。ffmpeg.wasm 在你的浏览器中运行，导出过长的视频时可能会内存不足。你可以继续尝试，或改为下载原始文件。",
    downloadOriginal: "下载原始文件",
    exportAnyway: "仍然导出",
    clearAllEditsTitle: "清除所有编辑？",
    clearAllEditsDescription:
      "这会移除此录制的所有修剪、模糊和自定义缩略图。原始视频不会被修改。",
    clearEdits: "清除编辑",
    nothingToUndo: "没有可撤销的操作",
    undoFailed: "撤销失败",
    editsCleared: "已清除编辑",
    clearFailed: "清除失败",
    selectRangeFirst: "请先在波形或转录中选择一个范围",
    selectionCut: "已剪切所选内容",
    trimFailed: "修剪失败",
    movePlayheadPastIntro: "将播放头移到要剪掉的开头之后",
    startCut: "剪掉开头",
    movePlayheadBeforeEnding: "将播放头移到要剪掉的结尾之前",
    endCut: "剪掉结尾",
    splitAdded: "已添加分割",
    splitFailed: "分割失败",
    videoNotReady: "视频尚未就绪",
    exportedMp4: "已导出 MP4",
    exportFailed:
      "导出失败 — ffmpeg.wasm 不一定能处理较长的视频。请尝试缩短编辑范围或使用原始文件。",
  },
  preRecord: {
    modeScreenCamera: "屏幕 + 摄像头",
    modeScreenOnly: "仅屏幕",
    modeCameraOnly: "仅摄像头",
    surfaceWindow: "窗口",
    surfaceWindowDescription: "最适合幻灯片或单个应用",
    surfaceBrowser: "浏览器标签页",
    surfaceBrowserDescription: "选择一个已打开的标签页",
    surfaceScreen: "屏幕",
    surfaceScreenDescription: "捕获所有内容",
    microphoneSelectionUnsupported: "此浏览器不支持选择麦克风设备。",
    enumerateFailed: "无法枚举设备",
    noMicrophone: "无麦克风",
    defaultMicrophone: "默认麦克风",
    shortMicLabel: "麦克风 {{id}}",
    defaultCamera: "默认摄像头",
    shortCameraLabel: "摄像头 {{id}}",
    noAudio: "无音频",
    noCamera: "无摄像头",
    loomImportFailed: "无法导入该 Loom。",
    fixMicrophoneAccess: "请先修复麦克风访问权限，或在录制前关闭音频。",
    fixCameraAccess: "请先修复摄像头访问权限，或在录制前切换到仅屏幕模式。",
    captureSource: "捕获来源",
    selectedSurface: "已选择 {{surface}}",
    change: "更改",
    audioAndCamera: "音频和摄像头",
    audio: "音频",
    check: "检测",
    choose: "选择",
    defaultMicPlaceholder: "默认麦克风",
    openingMicrophone: "正在打开麦克风...",
    chooseMicrophone: "选择麦克风...",
    includeAudioAria: "在此录制中包含音频",
    tryClipsDesktop: "试试 Clips 桌面版。",
    cameraOff: "摄像头已关闭",
    includeCameraAria: "在此录制中包含摄像头",
    startRecording: "开始录制",
    uploadVideo: "上传视频",
    importLoom: "导入 Loom",
    importing: "正在导入...",
    import: "导入",
  },
  playerSettings: {
    title: "设置",
    privacy: "隐私",
    visibility: "可见性",
    visibilityPrivate: "私密",
    visibilityOrg: "组织",
    visibilityPublic: "公开",
    passwordProtection: "密码保护",
    passwordSetPlaceholder: "已设置密码 — 输入以替换，留空并保存即可清除",
    noPasswordPlaceholder: "无密码",
    expiry: "过期时间",
    viewerOptions: "观看者选项",
    comments: "评论",
    reactions: "回应",
    allowDownloads: "允许下载",
    animatedThumbnail: "动态缩略图",
    defaultPlaybackSpeed: "默认播放速度",
    callToAction: "行动号召",
    defaultCtaLabel: "了解更多",
    addCta: "+ 添加",
    noCtas: "暂无行动号召按钮。添加一个在视频播放中或结束后显示的按钮。",
    buttonLabelPlaceholder: "按钮文字",
    placementThroughout: "全程显示",
    placementEnd: "结尾显示",
    delete: "删除",
  },
  recordRoute: {
    pageTitle: "新建录制 — Clips",
    uploadFailed: "上传失败",
    uploadFailedAtChunk: "上传在第 {{chunk}}/{{total}} 块时失败：{{message}}",
    couldNotStartRecording: "无法开始录制",
    couldNotStartRecorder: "无法启动录制器",
    openSettings: "打开设置",
    openedRecorderInNewTab: "已在新标签页中打开录制器",
    videoReadyToUpload: "视频已准备好上传",
    videoUploaded: "视频已上传",
    videoTooLarge: "视频过大",
    storageNeededToFinishLoomImport: "需要存储才能完成 Loom 导入",
    loomImported: "Loom 已导入",
    couldNotImportLoom: "无法导入该 Loom。",
    recordingReadyToUpload: "录制已准备好上传",
    recordingSaved: "录制已保存",
    noLocalRecordingData: "没有可下载的本地录制数据。",
    recordingDownloadStarted: "录制下载已开始",
    backToLibrary: "返回资料库",
    clipsRecorder: "Clips 录制器",
    preparingSources: "正在准备来源…",
    recordingScreen: "正在录制你的屏幕 — 请切换到要捕获的窗口",
    largeClipsNeedReencode: "较大的剪辑在上传前需要快速重新编码。",
    savingRecording: "正在保存你的录制…",
    sessionExpired: "会话已过期",
    sessionExpiredDescription: "你的登录会话已过期。请重新登录以开始录制。",
    logIn: "登录",
    betterInDesktop: "桌面应用体验更佳",
    desktopAppDescription: "菜单栏启动、全局快捷键、自动更新，重复录制更流畅。",
    downloadDesktopApp: "下载桌面应用",
    technicalDetails: "技术详情",
    whatToCheck: "需要检查的内容",
    downloadRecording: "下载录制",
    openRecorderInTab: "在标签页中打开录制器",
    connectStorageToFinish:
      "在下一屏连接存储：Builder.io（免费套餐存储 + AI）或 S3 兼容存储。Clips 将完成保存。",
    connectStorageToRetryLoom:
      "在下一屏连接存储：Builder.io（免费套餐存储 + AI）或 S3 兼容存储。Clips 将重试导入。",
  },
  dictateRoute: {
    pageTitle: "听写 · Clips",
    howToUse: "如何使用听写",
    browserDictation: "浏览器听写",
    browserDictationDescription:
      "使用本页面上的按钮，或在此标签页处于焦点时按快捷键。浏览器听写会保存在这里，方便复制和清理。",
    desktopShortcuts: "桌面快捷键",
    desktopShortcutsDescriptionSuffix: "，在桌面应用中。",
    holdFn: "按住 Fn",
    browserDictationDescriptionDesktop:
      "使用下方按钮直接在此页面记录笔记。不会粘贴到其他应用 — 如需粘贴，请使用右侧的桌面快捷键。",
    quickNoteTitle: "快速听写笔记",
    quickNoteHint:
      "在不离开此页面的情况下记录 — 不会粘贴到其他应用。用按钮开始和停止。",
    browserUnavailable:
      "此处无法使用浏览器语音识别。请使用 Chrome 或桌面应用进行全局听写。",
    browserUnavailableShort: "此处无法使用浏览器语音识别",
    startSpeaking: "开始说话...",
    replacedOriginal: "已用清理后的文本替换原文",
    noText: "没有文本",
    emptyTranscript: "转录为空",
    replaceOriginal: "用清理后的文本替换原文",
    cleanupWithAi: "使用 AI 清理",
    cleanupHint: "点击“使用 AI 清理”来修正标点、大小写和口头禅。",
    startFirst: "开始你的第一次听写",
    emptyDesktopDescription:
      "在 Mac 上的任何位置按住 {{fnKey}}，或按 {{modifierKey}} ⇧ Space。你的历史记录会保存在这里。",
    emptyWebDescription:
      "听写通过桌面应用运行，全局快捷键可在任何应用中使用 — Slack、你的编辑器，任何地方。",
    downloadDesktopApp: "下载 Clips 桌面应用",
    holdToDictate: "按住听写",
    toggle: "切换",
    desktopCtaTitle: "使用桌面应用随时随地听写",
    desktopCtaDescription:
      "在任何应用中按 {{modifierKey}} ⇧ Space。浏览器听写不够可靠 — 桌面应用才是正确的使用方式。",
    noSpeechCaptured: "未捕获到语音",
    dictationSaved: "听写已保存",
    voiceToTextDescription:
      "带 AI 清理的语音转文字听写。获取桌面应用即可通过全局快捷键随时随地听写。",
    loadFailed: "无法加载听写记录。",
    noFilterMatches: "没有符合此筛选条件的听写。",
    dictionaryTitle: "词典",
    dictionaryDescription:
      "这里的词条会让语音识别偏向你偏好的写法 — 从你的更正中自动学习，也可以手动添加。",
    dictionaryTermPlaceholder: "词条",
    dictionaryReplacementPlaceholder: "替换（可选）",
    dictionaryAdd: "添加",
    dictionaryLoading: "正在加载词典…",
    dictionaryEmpty: "还没有学习到的词条。",
    dictionaryUsesCount: "已使用 {{count}} 次",
    dictionaryRemove: "移除",
    vocabularyAddFailed: "无法添加词条",
    vocabularyRemoveFailed: "无法移除词条",
  },
  clipsFinalRaw: {
    splitAtPlayhead: "在播放头处分割 (S)",
    selectedCount: "已选择 {{count}} 项",
    move: "移动",
    moveSelected: "移动已选择的 {{count}} 项",
    current: "当前",
    clearSelection: "清除选择",
    waitingForStorage: "正在等待存储",
    savedLocally: "已保存到本地",
    uploadFailed: "上传失败",
    connectStorageToFinish: "打开以连接存储并完成保存。",
    retryFromClipsMenu:
      "从菜单栏的 Clips 图标重试——已保存剪辑的卡片上还有一个可打开本地文件的文件夹按钮。",
    removeFailedClip: "移除此失败剪辑。",
    remove: "移除",
    viewsCount: "{{count}} 次观看",
    recordingMenu: "录制菜单",
    moveToFolder: "移动到文件夹",
    unarchive: "取消归档",
    shareNamedMeeting: "分享“{{title}}”",
    shareMeeting: "分享会议",
    shareLink: "分享链接",
    alreadyHaveIt: "我已经有了",
    link: "链接",
    invite: "邀请",
    inviteFailed: "无法邀请此人",
    removePersonFailed: "无法移除此人",
    passwordProtectedDescription: "此视频受密码保护。请输入密码观看。",
    password: "密码",
    unlock: "解锁",
    noEngagementData: "暂无互动数据。",
    allCaughtUp: "你已经全部处理完了。",
    someone: "某人",
    view: "查看",
    reply: "回复",
    commentedOn: "评论了",
    reactedTo: "回应了",
    mentionedYouIn: "提到了你：",
    shared: "分享了",
    noCreatorsYet: "暂无创作者。",
    creator: "创作者",
    engagement: "互动",
    selectedMicrophoneWaveform: "所选麦克风波形",
    goHome: "返回首页",
    testMic: "测试麦克风",
    archivePageTitle: "归档 · Clips",
    insightsPageTitle: "洞察 · Clips",
    folderPageTitle: "文件夹 · Clips",
    spacePageTitle: "空间 · Clips",
    spacesPageTitle: "空间 · Clips",
    trashPageTitle: "废纸篓 · Clips",
  },
  bugReportRoute: {
    pageTitle: "错误报告 · Clips",
    donePageTitle: "错误报告已提交 · Clips",
    eyebrow: "错误报告",
    title: "录制错误报告",
    description: "为团队捕获一段包含屏幕、语音和已脱敏浏览器上下文的简短复现。",
    issueTitleLabel: "问题标题",
    issueTitlePlaceholder: "哪里出了问题？",
    detailsLabel: "详细信息",
    detailsPlaceholder: "你原本期望发生什么？",
    emailLabel: "你的邮箱",
    emailPlaceholder: "you@example.com",
    severityLabel: "严重程度",
    severityLow: "低",
    severityNormal: "普通",
    severityHigh: "高",
    severityUrgent: "紧急",
    sourceLabel: "来源",
    sourceUnknown: "当前产品页面",
    startRecording: "录制屏幕报告",
    privacyNote:
      "Clips 默认以工作区访问权限保存录制。URL 和元数据会在保存前脱敏。",
    doneTitle: "错误报告已提交",
    doneDescription: "录制已保存到 Clips，宿主产品也已收到通知。",
    openRecording: "打开录制",
    copyLink: "复制链接",
    copied: "已复制",
    returnToProduct: "返回产品",
    missingRecording: "未找到录制",
  },
  meetingsRoute: {
    pageTitle: "会议 · Clips",
    transcriptPending: "转录待生成",
    notesPending: "笔记待生成",
    pastRecordings: "过往录制",
    calendarNeedsReconnect:
      "需要重新连接 Google Calendar 才能继续显示你即将到来的会议。",
    connectGoogleCalendar: "连接 Google Calendar",
    desktopReminder:
      "连接 Google 日历，保持 Clips Desktop 打开，会议开始时在提醒或菜单栏中点击「开始笔记」。",
    getDesktopApp: "获取桌面应用",
    requiredForReminders: "桌面端为会议转写采集麦克风 + 系统音频。",
    calendarConnected: "日历已连接",
    calendarDisconnected: "日历已断开连接",
    calendarSettings: "日历设置",
    connectCalendarReminder: "连接 Google Calendar 以获取会议提醒。",
    disconnectGoogleCalendarTitle: "断开 Google Calendar 连接？",
    title: "会议",
    intro:
      "即将开始的日历会议和你录制的笔记。会议时间到时，从 Clips Desktop 开始实时笔记。",
    searchPlaceholder: "搜索会议...",
    clearSearch: "清除搜索",
    noMeetingsYet: "暂无会议",
    noMeetingsDescription:
      "连接日历并保持 Clips Desktop 打开。会议开始时，在提醒或菜单栏中使用「开始笔记」。",
    noMeetingsMatch: "没有符合“{{query}}”的会议",
    refreshing: "正在刷新…",
    howToTriggerTitle: "如何触发会议笔记",
    howToTriggerDescription:
      "会议笔记是 Clips 的 Granola 式流程：日历日程显示在这里，桌面应用采集麦克风和系统音频，转写和 AI 笔记会回到此历史记录。",
    guideCalendarTitle: "连接 Google 日历",
    guideCalendarDescription: "会议来自你的日历，Clips 因此知道何时提醒你。",
    guideDesktopTitle: "保持 Clips Desktop 打开",
    guideDesktopDescription: "转写麦克风加系统音频需要桌面端采集。",
    guideStartTitle: "点击「开始笔记」",
    guideStartDescription:
      "通话开始时，使用桌面提醒或菜单栏中的「开始会议笔记」。",
  },
  videoProjects: {
    listPageTitle: "视频项目 · Clips",
    editorPageTitle: "视频项目 · Clips",
    title: "视频项目",
    intro: "在完整编辑器中将录制、B-roll、字幕和文字组合在一起的多轨合成。",
    newProject: "新建项目",
    emptyTitle: "暂无视频项目",
    emptyDescription:
      "创建一个项目，或打开任意录制并选择“视频项目”，将其用作素材来源。",
    sourceCount_one: "{{count}} 个剪辑",
    sourceCount_other: "{{count}} 个剪辑",
    projectOptions: "项目选项",
    moveToTrash: "移到废纸篓",
    deleteTitle: "将项目移到废纸篓？",
    deleteDescription: "“{{title}}”将被移到废纸篓。其中使用的录制不受影响。",
    deleteFailed: "无法删除项目",
    createFailed: "无法创建项目",
    openInProject: "在视频项目中打开",
    openInProjectButton: "视频项目",
    openInProjectDescription: "将此录制添加为多轨视频项目中的素材来源。",
    respectEdits: "应用我在剪辑编辑器中的修剪",
    includeCaptions: "将转录添加为字幕",
    createAndOpen: "创建项目",
    addAndOpen: "添加到项目",
    addFailed: "无法添加到项目",
    cancel: "取消",
    back: "返回",
    backToLibrary: "返回资料库",
    saving: "保存中…",
    saved: "已保存",
    saveFailed: "保存失败",
    loadFailed: "无法加载此项目。它可能已被删除，或者你没有访问权限。",
    renameProject: "重命名项目",
  },
};

export default messages;
