...
- **Meetings** lists upcoming + past meetings synced from Google Calendar, with a two-pane detail view: live transcript (left) + AI summary / bullets / per-attendee action items (right). Use `list-meetings`, `get-meeting`, `finalize-meeting`, `connect-calendar`, etc. Navigation state exposes `view: "meetings" | "meeting"` and `meetingId`; `view-screen` includes `calendarAccounts` health so agents can distinguish an empty calendar from `needs-reauth` / sync failures. The visible Meetings UI is calendar-sourced only: no "New meeting" CTA.
