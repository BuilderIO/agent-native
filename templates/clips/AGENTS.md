...
2. **Transcription is the one exception.** Whisper runs directly because it takes an audio file, not a prompt. `request-transcript` prefers **Groq** (`whisper-large-v3-turbo` — typically 10x faster than OpenAI Whisper, ~$0.04/hr) via `GROQ_API_KEY`, and falls back to OpenAI `whisper-1` via `OPENAI_API_KEY`. Either key unlocks transcription; both are registered via `registerRequiredSecret` and surface in the sidebar settings UI. No other AI features may bypass the agent.
