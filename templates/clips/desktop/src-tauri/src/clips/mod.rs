...
pub async fn complete_voice_dictation(
    ...
    insert_text_for_frontmost(&app, &trimmed, "complete_voice_dictation", mode)
}
...
pub async fn paste_last_dictation(app: AppHandle) -> Result<(), String> {
    ...
    insert_text_for_frontmost(
        &app,
        text.trim(),
        "paste_last_dictation",
        TextDeliveryMode::PasteAndCopy,
    )
}
...
/// The delivery mode decides clipboard vs field insertion; paste-and-copy
/// deliberately leaves the transcript on the clipboard for manual re-paste,
/// so no prior-clipboard restore happens in that mode.
fn insert_text_for_frontmost(
    ...
    #[cfg(target_os = "macos")]
    eprintln!(
        "[clips-tray] {caller}: inserting {} chars, delivery={:?} via {:?} (frontmost={})",
        trimmed.chars().count(),
        mode,
        strategy,
    ...
    #[cfg(not(target_os = "macos"))]
    eprintln!(
        "[clips-tray] {caller}: inserting {} chars, delivery={:?}",
        trimmed.chars().count(),
        mode,
    );

    // Copy-only: put the transcript on the clipboard and stop.
    if mode == TextDeliveryMode::CopyOnly {
    ...
    match (mode, strategy) {
        ...
        // Unicode key events through AppKit text input.
        (_, TextInsertionStrategy::ClipboardPaste) => {
            write_clipboard(&trimmed)?;
            paste_clipboard(voice_target_bundle_id, trimmed.clone(), None);
        }
        // Terminal apps type directly via CGEventKeyboardSetUnicodeString —
