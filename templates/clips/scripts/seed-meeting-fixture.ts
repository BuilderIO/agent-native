 ...
 *     --participant="email|Name|organizer" --bullet="Summary bullet text"
 *
 * --bullet is repeatable and additive — it merges onto whatever bullets
 * already exist on the fixture meeting rather than replacing them, so you
 * can seed one more (e.g. a bullet with no matching transcript moment,
 * to test the BulletLink no-match indicator) without losing the rest.
 */
...
function bulletArguments(): string[] {
  return process.argv
    .filter((value) => value.startsWith("--bullet="))
    .map((value) => value.slice("--bullet=".length).trim())
    .filter(Boolean);
}
...
async function readExistingBullets(
  ...
  const [row] = await db
    .select({ bulletsJson: schema.meetings.bulletsJson })
    .from(schema.meetings)
    .where(eq(schema.meetings.id, FIXTURE_MEETING_ID))
    .limit(1);
  if (!row?.bulletsJson) return [];
  try {
    const parsed = JSON.parse(row.bulletsJson);
    return Array.isArray(parsed)
      ? parsed.filter((b): b is Bullet => typeof b?.text === "string")
      : [];
  } catch {
    return [];
  }
...
async function main(): Promise<void> {
  ...
  const newBulletTexts = bulletArguments();
  ...
  const existingBullets = await readExistingBullets(db);
  const existingBulletTexts = new Set(
    existingBullets.map((bullet) => bullet.text),
  );
  const mergedBullets: Bullet[] = [
    ...existingBullets,
    ...newBulletTexts
      .filter((text) => !existingBulletTexts.has(text))
      .map((text) => ({ text })),
  ];
  ...
  await db
    ...
    .values({
      ...
      ...(mergedBullets.length > 0
        ? { bulletsJson: JSON.stringify(mergedBullets) }
        : {}),
    })
    .onConflictDoUpdate({
