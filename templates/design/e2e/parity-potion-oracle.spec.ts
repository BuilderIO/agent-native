...
test("Potion final composition covers bubbles, glare, and Frame Selection", async ({
  page,
  request,
}) => {
  ...
  try {
    ...
    const bottleBody = await createNamedPrimitive(page, designId, {
      key: "o",
      primitive: "ellipse",
      name: "Bottle body",
      draw: { x: 100, y: 100, width: 200, height: 200 },
      geometry: { x: 0, y: 71, width: 200, height: 200 },
      fill: "#DEDCF9",
    });
    const bottleNeck = await createNamedPrimitive(page, designId, {
      key: "r",
      primitive: "rectangle",
      name: "Bottle neck",
      draw: { x: 150, y: 30, width: 100, height: 100 },
      geometry: { x: 50, y: 0, width: 100, height: 100 },
      fill: "#DEDCF9",
    });
    const bottleRim = await createNamedPrimitive(page, designId, {
      key: "r",
      primitive: "rectangle",
      name: "Bottle rim",
      draw: { x: 140, y: 60, width: 120, height: 40 },
      geometry: { x: 40, y: 40, width: 120, height: 40 },
      fill: "#DEDCF9",
      radius: 10,
    });
    await groupLayers(
      page,
      ["Bottle body", "Bottle neck", "Bottle rim"],
      "Bottle",
    );

    const potion = await createNamedPrimitive(page, designId, {
      key: "o",
      primitive: "ellipse",
      name: "Potion",
      draw: { x: 112, y: 112, width: 175, height: 175 },
      geometry: { x: 14, y: 112, width: 175, height: 175 },
      fill: "#E99BF4",
    });
    await selectLayer(page, "Potion");
    await page.keyboard.press("Enter");
    ...
    const top = page.locator("[data-vector-anchor]").nth(topIndex);
    const topBox = await top.boundingBox();
    if (!topBox) throw new Error("final Potion top anchor has no geometry");
    const beforeAnchorY = topBox.y + topBox.height / 2;
    await page.mouse.move(
      topBox.x + topBox.width / 2,
      topBox.y + topBox.height / 2,
    ...
    await page.mouse.up();
    const potionVector = designFrame(page).locator(
      `[data-agent-native-node-id="${potion.id}"]`,
    );
    await expect(potionVector).toHaveJSProperty("tagName", "svg");
    await expect(potionVector.locator("path")).toHaveCount(1);
    const afterAnchor = page
      .locator("[data-vector-anchor]")
      .nth(await topAnchorIndex(page));
    await expect
    ...
    await page.keyboard.press("Escape");
    await expect(page.locator("[data-vector-edit-overlay]")).toHaveCount(0);
    const editedPotion = await persistedSource(page, designId, (content) =>
      sourceTag(content, potion.id).includes('data-an-primitive="path"'),
    );
    expect(sourceTag(editedPotion, potion.id)).toMatch(
      /data-an-primitive=["']path["']/,
    );

    const bubbleSmall = await createNamedPrimitive(page, designId, {
      key: "o",
      primitive: "ellipse",
      name: "Bubble small",
      draw: { x: 80, y: 160, width: 6, height: 6 },
      geometry: { x: 80, y: 160, width: 6, height: 6 },
      fill: "#985DF6",
    });
    const bubbleLarge = await createNamedPrimitive(page, designId, {
      key: "o",
      primitive: "ellipse",
      name: "Bubble large",
      draw: { x: 120, y: 200, width: 12, height: 12 },
      geometry: { x: 120, y: 200, width: 12, height: 12 },
      fill: "#985DF6",
    });
    await groupLayers(
      page,
      ["Potion", "Bubble small", "Bubble large"],
      "Potion",
    );

    const glareShort = await createNamedPrimitive(page, designId, {
      key: "r",
      primitive: "rectangle",
      name: "Glare short",
      draw: { x: 90, y: 145, width: 20, height: 10 },
      geometry: { x: 90, y: 145, width: 20, height: 10 },
      fill: "#F2F2FF",
      radius: 20,
    });
    const glareLong = await createNamedPrimitive(page, designId, {
      key: "r",
      primitive: "rectangle",
      name: "Glare long",
      draw: { x: 120, y: 160, width: 40, height: 10 },
      geometry: { x: 120, y: 160, width: 40, height: 10 },
      fill: "#F2F2FF",
      radius: 20,
    });
    await groupLayers(page, ["Glare short", "Glare long"], "Glare");

    const cork = await createNamedPrimitive(page, designId, {
      key: "r",
      primitive: "rectangle",
      name: "Cork",
      draw: { x: 159, y: 8, width: 75, height: 75 },
      geometry: { x: 59, y: 0, width: 75, height: 75 },
      fill: "#CE856C",
      radius: 25,
    });
    ...
    for (const [index, name] of [
      ...
      if (index === 0) await layerButton(page, name).click({ force: true });
      else
    ...
    await expect(layerButton(page, "Frame")).toBeVisible();
    await renameLayer(page, "Frame", "Magic Potion");
    await selectLayer(page, "Magic Potion");
    await setGeometry(page, { x: 0, y: 0, width: 200, height: 271 });
    const finalSource = await persistedSource(page, designId, (content) => {
      const requiredNames = [
        "Bottle",
        "Potion",
        "Bubble small",
      ...
      return requiredNames.every((name) =>
        content.includes(`data-agent-native-layer-name="${name}"`),
      );
    });
    expect(sourceTag(finalSource, bottleBody.id)).toMatch(
      /data-an-primitive=["']ellipse["']/,
    );
    expect(sourceTag(finalSource, bottleNeck.id)).toMatch(
      /data-an-primitive=["']rectangle["']/,
    );
    expect(sourceTag(finalSource, bottleRim.id)).toMatch(
      /data-an-primitive=["']rectangle["']/,
    );
    expect(sourceTag(finalSource, potion.id)).toMatch(
      /data-an-primitive=["']path["']/,
    );
    expect(sourceTag(finalSource, bubbleSmall.id)).toMatch(
      /data-an-primitive=["']ellipse["']/,
    );
    expect(sourceTag(finalSource, bubbleLarge.id)).toMatch(
      /data-an-primitive=["']ellipse["']/,
    );
    expect(sourceTag(finalSource, glareShort.id)).toMatch(
      /data-an-primitive=["']rectangle["']/,
    );
    expect(sourceTag(finalSource, glareLong.id)).toMatch(
      /data-an-primitive=["']rectangle["']/,
    );
    expect(sourceTag(finalSource, cork.id)).toMatch(
      /data-an-primitive=["']rectangle["']/,
    );
    expect(sourceTag(finalSource, potion.id)).toMatch(
      /data-agent-native-layer-name=["']Potion["']/,
    );
    const magicPotionId = sourceNodes(finalSource).find(
      (node) => node.name === "Magic Potion",
    )?.id;
    if (!magicPotionId) throw new Error("Magic Potion frame has no source id");
    expect(sourceTag(finalSource, magicPotionId)).toMatch(
      /data-an-primitive=["']frame["']/,
    );
    expect(styleFor(finalSource, magicPotionId)).toMatch(/width:\s*200px/i);
    expect(styleFor(finalSource, magicPotionId)).toMatch(/height:\s*271px/i);
    ...
    const size = await downloadedPngSize(page, download);
    expect(size).toEqual({ width: 200, height: 271 });
    await test.info().attach("potion-final-composition.json", {
      body: JSON.stringify(
        {
          uniqueInteractions: [
            "two bubble ellipses at 6x6 and 12x12",
            "two glare rectangles at 20x10 and 40x10 with radius20",
            "Frame Selection into Magic Potion",
          ],
          fullFinalIllustration: true,
          size,
          source: finalSource,
        },
        null,
