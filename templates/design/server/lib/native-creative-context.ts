...
export const nativeDesignCreativeContextAdapter: NativeResourceCaptureAdapter = {
  ...
  async capture(reference) {
    ...
    return { artifactKey: `design:design:${design.id}`, source: { name: "Design", kind: "native-app", externalRef: design.id }, items: [{ externalId: `native:design:design:${design.id}`, kind: "design-project", title: design.title, canonicalUrl: `/design/${design.id}`, mimeType: "application/json", content: snapshot.files.map((file) => `${file.filename}\n${file.content.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ")}`).join("\n").slice(0, 40_000), summary: `${snapshot.files.length} saved design files captured as an immutable version.`, contentHash, sourceModifiedAt, sourceVersion: versionId, metadata: { preview: { type: "design", fileCount: snapshot.files.length } }, edges: snapshot.files.map((file) => ({ relation: "contains", toExternalId: `native:design:design:${design.id}:frame:${file.id}` })) }, ...snapshot.files.map((file) => { const content = file.content.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim(); return { externalId: `native:design:design:${design.id}:frame:${file.id}`, kind: "design-frame", title: file.filename, canonicalUrl: `/design/${design.id}`, mimeType: "text/html", content, summary: content.slice(0, 500), contentHash: createHash("sha256").update(file.content).digest("hex"), sourceModifiedAt, sourceVersion: versionId, metadata: { preview: { type: "design-frame", fileType: file.fileType } } }; })], privateMetadata: { nativeResource: { appId: "design", resourceType: "design", resourceId: design.id, expectedUpdatedAt: reference.expectedUpdatedAt }, clone: { handle, appId: "design", resourceType: "design", resourceId: design.id, contentHash, sourceVersion: versionId, updatedAt: design.updatedAt } } };
  },
};
