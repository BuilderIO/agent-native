...
describe("Design page agent discovery injection", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("keys token-authorized HTML by the full query string", async () => {
    const event = {
      url: "https://design.example.com/design/design-1?agent_access=sample-token",
    ...
    expect(mockVerifyScopedAgentAccessToken).toHaveBeenCalledWith(
      "sample-token",
      { resourceKind: "design:design", resourceId: "design-1" },
    );
  ...
  it("omits invalid tokens from cached HTML", async () => {
    ...
    const response = (await (handler as any)({
      url: "https://design.example.com/design/design-1?agent_access=invalid",
