...
export default function DesignSystemSetup() {
  ...
  const handleContinue = useCallback(() => {
    ...
    if (normalizedGithubLinks.length > 0) {
      parts.push(
        `\n## GitHub Repositories\nStart Builder design-system indexing for each repository with \`index-design-system-with-builder\`:\n${normalizedGithubLinks.map((l) => `- ${l.url}`).join("\n")}\n\nBuilder is the source of truth for repo/code design-system indexing. If Builder is not connected, stop and tell me to connect Builder from Settings instead of asking me to paste repository credentials into chat.`,
      );
    }
