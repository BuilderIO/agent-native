...
function buildDesignSystemAgentContext({
  ...
  if (builder) {
    lines.push(
      ...
      canRefreshBuilder
        ? "- If no usable DSI docs or tokens are returned, call get-design-system-index-status to check Builder progress (it uses docCount > 0 to verify indexing completed). Only call refresh-design-system-with-builder once after progress confirms completion, then call get-design-system again; if it is still empty, tell the user Builder indexing is not ready."
