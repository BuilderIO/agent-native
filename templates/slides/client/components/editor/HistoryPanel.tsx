...
export default function HistoryPanel({ open, onOpenChange, anchorRef }: HistoryPanelProps) {
  ...
  return createPortal(
    ...
    >
      <div className="px-3 py-2.5 border-b border-white/[0.06] flex items-center gap-2">
        ...
        <span className="text-[10px] text-white/30 ml-auto">Cmd+Z / Cmd+Shift+Z</span>
      </div>

      <div className="overflow-y-auto max-h-[50vh] p-1">
        {history.length === 0 && (
          <p className="text-xs text-white/30 text-center py-4">No history yet</p>
        )}
          ...
          return (
            ...
            >
              <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${
                isCurrent ? "bg-[#609FF8]" : realIdx < historyIndex ? "bg-white/20" : "bg-white/10"
              }`} />
