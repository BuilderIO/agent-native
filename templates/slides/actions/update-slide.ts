...
function styleOnlyGenericSuggestion(): string {
  return (
    ...
    '"edits": [{"find":"background:#111111","replace":"background:#f4f0e8","expectedMatches":1}].'
  );
}
...
function styleOnlyEditsSuggestion(args: {
  ...
  if (
    ...
    args.objectId.length <= SUGGESTION_ECHO_LIMIT
  ) {
    return resendAsEdits({ objectId: args.objectId, replace });
  }
  if (args.find !== undefined && args.find.length <= SUGGESTION_ECHO_LIMIT) {
    return resendAsEdits({ find: args.find, replace, expectedMatches: 1 });
  }
  return styleOnlyGenericSuggestion();
}
