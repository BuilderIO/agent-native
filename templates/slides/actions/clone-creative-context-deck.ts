...
export default defineAction({
  ...
  run: async (args) => {
    ...
    const raw = await readPrivateBlob(reference.cloneHandle as PrivateBlobHandle);
    const data = Buffer.from(raw.data).toString("utf8");
    const contentHash = createHash("sha256").update(data).digest("hex");
    if (raw.metadata?.appId !== "slides" || raw.metadata?.resourceType !== "deck" || raw.metadata?.resourceId !== args.resourceId || raw.metadata?.contentHash !== contentHash) throw new Error("Governed deck clone payload failed integrity verification.");
    ...
    return { id: saved.id, title: saved.title, slideCount: deck.slides.length, url: getDeckUrl(id), clonedExactVersion: reference.publishedItemVersionId };
  },
});
