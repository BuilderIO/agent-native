...
export default defineAction({
  ...
  description:
    "Granular deck patch used by the browser editor for concurrent-safe writes. Before adding or restyling slides from an external agent, read get-deck with compact=true once for designSystem, deckStyle, and representativeSlideId, and get-design-system once for the full linked context. For new deck generation, use one atomic batch of at most three add-slide operations only after the first one or two slides establish the deck-level visual contract; wait for the result and read get-deck compact=true before another batch. Never issue parallel writes to the same deck. " +
