 ...
 *
 * Usage:
 *   pnpm script navigate --view=list
 *   pnpm script navigate --deckId=abc123
 *   pnpm script navigate --deckId=abc123 --slideIndex=2
 *
 * Options:
...
export default async function main(): Promise<void> {
  ...
  if (!args.view && !args.deckId) {
    console.error(
      "Error: At least --view or --deckId is required. Usage: pnpm script navigate --deckId=abc123",
    );
    process.exit(1);
  }
