  ...
  const handleCreateDeckWithPrompt = async (
    ...
    const sourceModeInstructions = importedSourceDeck
      ...
      : [
          ...
          "Start a `manage-progress` run so progress appears in the app header. First make a compact outline and deck-level visual contract in working context, then add the first one or two slides with `add-slide` one at a time so the editor visibly establishes the direction.",
          ...
          "Add the first one or two slides ONE AT A TIME using the `add-slide` action with --deckId=" +
            deckId +
            "; wait for each result. After the first slide, call `get-deck` with compact=true and use its `deckStyle` and `representativeSlideId` to verify and lock the visual contract before continuing. For three or more remaining slides, use one `patch-deck` call with at most three `add-slide` operations when that is useful, then wait for the result and call `get-deck` with compact=true before the next batch. Never issue parallel writes to the same deck; if a batch fails, retry it in smaller bounded groups.",
    ...
    const context = [
      ...
      "If the user asked for a specific slide count, keep going until that count is reached unless a tool error blocks you. Use the sequential first-two-slide setup, then bounded patch-deck batches of at most three add-slide operations with a compact get-deck readback after each batch. If no explicit count was given (including when the guided slide-count question was skipped), infer the count from the distinct topics/sections implied by the request — one slide per section plus a title and closing slide — and add slides for every section before considering the deck done. Do not stop at an arbitrary round number (e.g. 10) if sections remain uncovered, and never call `generate-slides-ai` for this flow; it is a legacy single-shot helper capped at 10 slides.",
