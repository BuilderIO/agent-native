...
> {
  ...
  const sourceModeInstructions = importedSourceDeck
    ...
    : [
        ...
        `Add every generated slide ONE AT A TIME using the \`add-slide\` action with --deckId=${deckId}; wait for each result. After the first slide, call \`get-deck\` with compact=true and use its \`deckStyle\` and \`representativeSlideId\` to verify and lock the visual contract before continuing. Do not use \`patch-deck\` to append generated slides because \`add-slide\` records per-slide Creative Context provenance; use \`patch-deck\` for deck fields, existing-slide edits, ordering, or source-preserving work. Never issue parallel writes to the same deck.`,
