...
For new decks, preserve design-system precedence by letting create-deck resolve an explicit choice, the caller's personal default, then the workspace default. If its returned designSystemId is non-null, use the returned designSystem.agentContext when available, otherwise call get-design-system before adding slides, and never overwrite an explicit or personal choice with the workspace ID. If an existing deck has designSystemId, use the designSystem.agentContext returned by get-deck or view-screen when available, otherwise call get-design-system before writing and follow its exact tokens, assets, and custom instructions. If an existing deck has no design system and the user asks for on-brand styling, call get-workspace-defaults, link its usable design system with patch-deck, then use its returned designSystemContext when available or call get-design-system. Do not improvise a generic Builder-like palette when configured Builder.io design-system context is available.
