...
export default createAgentChatPlugin({
  ...
  mcp: {
    ...
    instructions:
      "For every new deck, including bare requests, call get-workspace-defaults before authoring. Preserve design-system precedence: an explicit designSystemId or create-deck's effective personal default wins over the workspace default. For a bare request, do not patch the workspace designSystemId after creation; create an empty deck with create-deck so it can resolve the effective default, then use its returned designSystemId to call get-design-system before adding slides. For an explicit design-system choice, pass that id to create-deck and use the returned effective designSystemId. Follow the exact agentContext tokens, assets, and custom instructions in slide HTML. For deck edits, call view-screen only when the active target IDs or selection are unknown. If it returns an exact selectedText browser range, make the focused change immediately with update-slide using one literal edits replacement and expectedMatches=1, optionally passing currentSlideContentHash as baseContentHash. An element text preview is context, not an exact replacement range. Do not call get-deck without slideId, enumerate slides, or inspect the full deck for that path. Use targeted get-deck with slideId only for missing, preview-only, truncated, ambiguous, or structural edits. Use patch-deck for slide deletion, reordering, deck-wide, or multi-slide changes, and delete-deck to remove an entire deck. Read back the same slide after writing when it still exists; a delegated ask_app or call-agent response is unverified until that readback confirms the persisted state.",
  },
