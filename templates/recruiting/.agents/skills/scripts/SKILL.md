...
Create `scripts/my-script.ts` following the pattern in `scripts/helpers.ts`:
...
- **Use `localFetch()`** from `./helpers.js` to call the local API.

## Related Skills

- **delegate-to-agent** — The agent invokes scripts via `pnpm script <name>`
- **sse-file-watcher** — Database writes from scripts trigger SSE events
