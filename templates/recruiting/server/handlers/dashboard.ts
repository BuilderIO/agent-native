...
async function dashboardFetch<T>(path: string, perPage = 100): Promise<T[]> {
  ...
  const qs = new URLSearchParams({ per_page: String(perPage), page: "1" });
  ...
  async (): Promise<DashboardStats> => {
    ...
    const [jobs, recentApps, interviews] = await Promise.all([
      ...
      dashboardFetch<GreenhouseApplication>(
        `/applications?created_after=${weekAgo.toISOString()}`,
        100,
      ),
      dashboardFetch<GreenhouseScheduledInterview>("/scheduled_interviews", 50),
    ]);
