...
2. **Transcription is direct to Deepgram.** The `request-transcript` action calls Deepgram Nova-3 with `diarize=true` using `DEEPGRAM_API_KEY`. No other AI features may bypass the agent. See the `transcription` skill.
