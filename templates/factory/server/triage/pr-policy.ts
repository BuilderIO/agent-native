...
const HTML_TAINT_SOURCE_PATTERN = String.raw`(?:(?:(?:user|attacker)[- ](?:controlled|supplied|provided|generated|injected)|malicious|arbitrary|raw|untrusted(?:\s+(?:user|attacker))?|external|remote|uploaded|webhook|third[- ]party|request[- ]body|api[- ]response|comment)\s+(?:html|markup|input|content|value)|(?:html|markup)\s+(?:supplied|provided|generated|injected)\s+by\s+(?:(?:the|a)\s+)?(?:user|attacker)|(?:html|markup)\s+(?:from\s+(?:the\s+)?(?:pr\s+(?:body|description)|request[- ]body|api[- ]response|comment|untrusted\s+source|user\s+input))|(?:user|attacker)\s+input)`;
