...
const COMPOUND_SAFETY_FINDING_PATTERN =
  /\b(?:auth(?:entication)?\s+bypass|(?:xss|cross-site scripting|csrf|cross-site request forgery|csp|content[- ]security[- ]policy|ssrf|rce|sanitiz(?:e|ers?|ations?|ed|ing))\s+vulnerabilit(?:y|ies)|tenant\s+isolation|access\s+control|privilege\s+escalation)\b/i;
