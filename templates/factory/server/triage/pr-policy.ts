...
const HTML_UNSAFE_SIGNAL_PATTERN = String.raw`(?:unescaped|unsanitized|without\s+(?:proper\s+)?(?:escaping|encoding|sanitiz(?:ation|ing))|(?:not|never|isn't|is\s+not)\s+(?:properly\s+)?(?:escaped|encoded|sanitized|sanitised)|no\s+(?:proper\s+)?(?:escaping|encoding|sanitiz(?:ation|ing))|(?:execute|run)\w*\b.{0,20}\b(?:javascript|scripts?)|(?:on[a-z]+\s*(?:=|\s+handler(?:\s+attribute)?).{0,50}\b(?:execute|run|fire|trigger)\w*|\b(?:execute|run|fire|trigger)\w*.{0,50}\bon[a-z]+\s*(?:=|\s+handler(?:\s+attribute)?)))`;
