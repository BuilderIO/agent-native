...
describe("database-scoped document properties", () => {
  it("enables Builder blocks only for direct children of a connected collection", async () => {
    const db = getDb();
    const now = new Date().toISOString();
    ...
    const unrelatedId = await createDocument({ title: "Unrelated" });
    const sourceId = nextId("builder_source");
    ...
    await expect(readContext(childId)).resolves.toMatchObject({
      authoringContext: { builderBlocks: true },
    });
    await expect(readContext(databaseDocumentId)).resolves.toMatchObject({
      authoringContext: { builderBlocks: false },
    });
    await expect(readContext(grandchildId)).resolves.toMatchObject({
      authoringContext: { builderBlocks: false },
    });
    await expect(readContext(unrelatedId)).resolves.toMatchObject({
      authoringContext: { builderBlocks: false },
    });

    await db
      .update(schema.documents)
      .set({ parentId: unrelatedId, updatedAt: new Date().toISOString() })
