...
postgresSuite("migrate-content-database-rows PostgreSQL locking", () => {
  ...
  it.each(["same-trash-root", "active"] as const)(
    ...
    async (rowState) => {
      ...
      // The original member survives Database trash; isolate the concurrently
      // inserted membership whose scope rebuilding this test exercises.
      await getDb()
        .delete(schema.contentDatabaseItems)
  ...
  it("refuses permanent deletion when restore wins the lifecycle locks", async () => {
    ...
    // The preserved live member would reject purge before the lifecycle race.
    await getDb()
      .delete(schema.contentDatabaseItems)
      .where(eq(schema.contentDatabaseItems.documentId, seed.documentId));
    let releaseHolder = () => {};
