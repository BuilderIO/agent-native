...
function PersonalNotificationSwitch({
  databaseId,
  subscriptionId,
  global = false,
  labelledBy,
  ...
  const target = global
    ...
    : subscriptionId && databaseId
      ? ({ scope: "rule", databaseId, subscriptionId } as const)
      : databaseId
    ...
    <div className="grid gap-4">
      ...
      {isOwner ? (
        <div className="flex items-center justify-between gap-3 border-b border-border pb-4">
          <div className="grid gap-0.5">
            <Label htmlFor={`database-schema-lock-${databaseId}`}>
              {t("database.lockDatabaseSchema")}
            </Label>
            <p className="text-xs leading-4 text-muted-foreground">
              {t("database.lockDatabaseSchemaDescription")}
            </p>
          </div>
          <Switch
                ...
                { databaseId, schemaLocked: nextSchemaLocked },
                {
                  onSuccess: () =>
                    toast.success(
                      t(
                        nextSchemaLocked
                          ? "database.databaseSchemaLocked"
                          : "database.databaseSchemaUnlocked",
                      ),
                    ),
                  onError: (error) =>
                    toast.error(t("database.databaseSchemaLockFailed"), {
                      description:
      ...
      {!draft ? (
        <>
          <div className="flex items-start justify-between gap-3">
            <div className="grid gap-1">
              ...
              <p className="text-xs leading-4 text-muted-foreground">
                {t("database.agentJudgmentUsesAutomations")}
              </p>
            </div>
            {canManage ? (
          ...
          <div className="flex items-center justify-between gap-3 rounded-md border border-border px-3 py-2.5">
            <div>
              <Label id={`database-notifications-${databaseId}`}>
                {t("database.personalDatabaseNotifications")}
              </Label>
              <p className="text-xs text-muted-foreground">
                {t("database.personalDatabaseNotificationsDescription")}
              </p>
            </div>
            <PersonalNotificationSwitch
              databaseId={databaseId}
              labelledBy={`database-notifications-${databaseId}`}
            />
          </div>

          <div className="flex items-center justify-between gap-3 rounded-md border border-border px-3 py-2.5">
            <div>
              <Label id="global-content-notifications">
                {t("database.personalContentNotifications")}
              </Label>
              <p className="text-xs text-muted-foreground">
                {t("database.personalContentNotificationsDescription")}
              </p>
            </div>
            <PersonalNotificationSwitch
              global
              labelledBy="global-content-notifications"
            />
          </div>
        ...
        <div className="grid gap-4">
              ...
              <SelectContent>
                <SelectGroup>
                  ...
                  <SelectItem
                    ...
                    disabled={
                      availabilityFor("property_changed")?.available !== true
                    }
                  >
          ...
          {draft.triggerKind === "property_changed" ? (
            <div className="grid gap-3 border-s border-border ps-3">
              <div className="grid gap-2">
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder={t("database.chooseAProperty")} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectGroup>
                    ...
                    <div className="grid gap-2">
                      ...
                      <Select
                        ...
                        onValueChange={(fromOptionId) =>
                          setDraft((current) =>
                            current ? { ...current, fromOptionId } : current,
                          )
                        }
                      >
                        <SelectTrigger>
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              {!draft.conditions ? (
                ...
                <Button
                  ...
                  onClick={() =>
                    setDraft((current) =>
                      current ? { ...current, conditions: undefined } : current,
                    )
                  }
                >
                  ...
                  >
                          ...
                          current?.conditions
                            ? current.conditions.clauses.length === 1
                              ...
                              : {
                                  ...
                                  conditions: {
                                    ...current.conditions,
                                    clauses: current.conditions.clauses.filter(
                                      (_, at) => at !== index,
                                    ),
                                  },
                                }
                    ...
                    {"value" in condition ? (
                      <Input
                        ...
                        placeholder={t("database.conditionValuePlaceholder")}
                        onChange={(event) => {
          ...
          <div className="grid gap-2">
            <Label>{t("database.hookTiming")}</Label>
            <Select
            ...
            {draft.effects.map((effect, index) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <Select
                    value={effect.kind}
                    onValueChange={(kind: ContentDatabaseHookEffect["kind"]) =>
                      setDraft((current) =>
                        current
                          ? {
                              ...current,
                              effects: current.effects.map((candidate, at) =>
                                at === index
                        ...
                        setDraft((current) =>
                          current
                            ? {
                                ...current,
                                effects: current.effects.map((candidate, at) =>
                                  at === index && candidate.kind === "notify"
                                    ? {
                                        ...candidate,
                                        recipientPersonPropertyId,
                                      }
                                  ...
                                  effects: current.effects.map(
                                    (candidate, at) =>
                                      ...
                                      candidate.kind === "webhook"
                                        ? {
                                            ...candidate,
                                            signatureKey: event.target.value,
                                          }
                                        : candidate,
                                  ),
                                }
          ...
          <div className="flex items-center justify-between gap-3 rounded-md border border-border px-3 py-2.5">
            <div>
              <Label htmlFor="hook-enabled">{t("database.ruleEnabled")}</Label>
              <p className="text-xs text-muted-foreground">
          ...
          <div className="flex items-center justify-between gap-2 pt-1">
            ...
            <div className="flex gap-2">
              ...
              >
                {manageHook.isPending ? <Spinner className="size-4" /> : null}
                {t("database.saveRule")}
              </Button>
            </div>
          </div>
