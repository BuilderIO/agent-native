...
export const nativeDocumentCreativeContextAdapter: NativeResourceCaptureAdapter = {
  ...
  async capture(reference) {
    ...
    return { artifactKey: `content:document:${document.id}`, source: { name: "Content", kind: "native-app", externalRef: document.id }, items: [{ externalId: `native:content:document:${document.id}`, kind: "document", title: document.title, canonicalUrl: `/page/${document.id}`, mimeType: "text/markdown", content: document.content.slice(0, 40_000), summary: document.description ?? "Immutable Markdown document.", contentHash, sourceModifiedAt: document.updatedAt, sourceVersion: versionId, metadata: { preview: { type: "markdown" } } }], privateMetadata: { clone: { handle, contentHash, sourceVersion: versionId, updatedAt: document.updatedAt } } };
  },
};
