export function cleanNotionProperties(properties: any): any {
  ...
  for (const [key, prop] of Object.entries(properties) as any[]) {
    ...
    if (['formula', 'rollup', 'created_by', 'created_time', 'last_edited_by', 'last_edited_time'].includes(prop.type)) {
      ...
      if (prop.type === 'relation' && (!prop.relation || prop.relation.length === 0)) continue;
      if (prop.type === 'people' && (!prop.people || prop.people.length === 0)) continue;
      ...
      if (prop.type === 'rich_text' && (!prop.rich_text || prop.rich_text.length === 0)) continue;
    ...
    if (prop.type === 'select') {
      ...
      if (prop.multi_select) cleanProps[key].multi_select = prop.multi_select.map((s: any) => ({ name: s.name }));
    } else if (prop.type === 'people') {
       if (prop.people) cleanProps[key].people = prop.people.map((p: any) => ({ id: p.id }));
    } else if (prop.type === 'relation') {
       if (prop.relation) cleanProps[key].relation = prop.relation.map((r: any) => ({ id: r.id }));
