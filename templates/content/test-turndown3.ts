...
const turndownService1 = new TurndownService({ emDelimiter: "*", strongDelimiter: "**" });

const turndownService2 = new TurndownService({ emDelimiter: "*", strongDelimiter: "**" });
...
turndownService2.addRule('emphasis', {
  filter: ['em', 'i'],
  replacement: (content, node, options) => wrapEmphasis(content, options.emDelimiter as string)
});

const html = "<p>Here is <em>some <code>code</code> block</em> and <strong>bold <code>code</code> too</strong>.</p>";
console.log("Default:");
