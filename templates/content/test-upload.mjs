...
async function loadAuth() {
  ...
  if (process.env.BUILDER_API_KEY && process.env.BUILDER_PRIVATE_KEY) {
    return { apiKey: process.env.BUILDER_API_KEY, privateKey: process.env.BUILDER_PRIVATE_KEY };
  }
...
async function main() {
  ...
  const uploadBase = "https://builder.io/api/v1/upload?apiKey=" + encodeURIComponent(apiKey);
