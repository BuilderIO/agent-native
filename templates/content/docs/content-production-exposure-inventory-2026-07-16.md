...
## PR 2: `production-privacy-inventory` action contract

The fork implements a reusable, explicitly operator-authorized, non-public,
non-agent-callable `production-privacy-inventory` action. It remains a fork-lab
control until the complete PR 2 gate and deployed adversarial proof are closed.

The action should:

- Accept no resource identifier, owner, principal, query, or free-form filter.
- Require an authenticated session allowlisted by
  `AGENT_NATIVE_PRIVACY_INVENTORY_ADMIN_EMAILS` and a separate high-entropy
  step-up token from `AGENT_NATIVE_PRIVACY_INVENTORY_ADMIN_TOKEN`, supplied in
  `X-Agent-Native-Operator-Token`. Missing deployment configuration fails
  closed, and A2A/network callers cannot satisfy the gate.
- Emit a timestamp, schema version, fixed authorization class, aggregate counts,
  explicit coverage booleans, and a SHA-256 hash of the unsigned result only.
...
- Produce a required durable audit event containing only the action version,
  caller authorization class, timestamp, and output hash. The result is not
  released if the receipt cannot be appended.
...
The action never accepts a caller-selected filter and never returns identifiers,
origins, principals, paths, or error text. Optional schema/configuration gaps
...
The reusable action is PR 2 scope so the baseline authority work owns its
authorization, audit, and long-term maintenance. It does not replace the
separate read-only provider/IAM exports required by the gate.
