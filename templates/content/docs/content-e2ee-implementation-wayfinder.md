...
broker replacement and cross-device ceremony transport beyond the same-device
first-beta path.
...
The attended endpoint now constructs the challenge inside the native boundary
as well. The authorizer builder derives its public identity from guarded
endpoint seeds, requires that exact key pair to name one active attended member
of the authenticated control state, verifies the candidate offer and key proof,
and signs the frozen head-, membership-, role-, lifetime-, and SAS-bound map.
Nothing is released until the production verifier accepts the generated bytes
against the same state. Its deterministic output matches the existing Core
vector byte-for-byte on Apple Silicon and Intel; malformed proofs, overlong
lifetimes, and custody keys that do not name the authorizer fail closed. The
production Apple Silicon XPC service builds with the authorizer linked.

After the attended person confirms the SAS, the endpoint now constructs the
authorization inside the same native custody boundary. It reopens the exact
registry-backed challenge evidence and bound confirmed receipt, borrows the
authorizer signing and agreement seeds plus the active EEK only into guarded
memory, creates the candidate endpoint certificate and recipient-bound EEK
wrap, signs the next control entry, and releases the public authorization only
after the production verifier accepts the entire bundle. The XPC protocol,
universal addon, typed Desktop client, and hosted transcript coordinator expose
