  ...
  try {
    ...
    for (const [key, prop] of Object.entries(existingPage.properties)) {
      ...
      if (prop.type === 'select') {
        ...
        else delete cleanProps[key].select; // empty select
      } else if (prop.type === 'multi_select') {
        if (prop.multi_select) cleanProps[key].multi_select = prop.multi_select.map(s => ({ name: s.name }));
      } else if (prop.type === 'people') {
         if (prop.people) cleanProps[key].people = prop.people.map(p => ({ id: p.id }));
      } else if (prop.type === 'relation') {
         if (prop.relation) cleanProps[key].relation = prop.relation.map(r => ({ id: r.id }));
      }
    }
