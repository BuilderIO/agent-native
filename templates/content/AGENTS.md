...
| `attach-content-database-source`             | `--databaseId <id>` or `--documentId <id> [--sourceType mock\|builder-cms] [--sourceName] [--sourceTable]`                   | Attach a source binding; Builder CMS remains read/diff/revision-only with live writes disabled       |
...
| `propose-content-database-source-change-set` | `--databaseId <id>` or `--documentId <id> [--itemDocumentId] [--propertyId] [--includeBodyChange true\|false]`               | Store a proposed local field/body diff record without executing writes                               |
