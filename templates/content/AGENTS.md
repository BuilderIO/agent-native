...
| `reset-document-collaboration` | `--id <document-id>` | Reset a closed document's stale collaboration snapshot so its next editor reseeds from canonical SQL; refuses while a human client is active |
...
| `lookup-builder-safe-model-intent`          | `--marker <exact-marker>` or `--exactTitle <exact-title> [--includeFidelity true]`                                                                                                                   | Read-only ambiguous-write proof in the safe Builder model; optionally returns bounded rich-content fidelity counts without raw article content                                                                                        |
...
| `builder-safe-fixture-repair`               | `[--batchSize 1-10] [--cursor <source-id>] [--apply true]`                                                                                                                                         | Exhaustive raw-data clone repair for `blog-article` → `agent-native-blog-article-test`; apply creates missing drafts or patches non-identical drafts in safe batches with webhooks disabled, then verifies exact data and provenance. |
