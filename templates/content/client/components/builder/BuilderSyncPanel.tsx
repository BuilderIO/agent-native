...
import { useBuilderAuthors, useBuilderArticles, useBuilderDocs, useUploadArticle } from "@/hooks/use-builder";
...
import { ArticleMetadataForm, type ArticleMetadata } from "./ArticleMetadataForm";
import { DocsMetadataForm, type DocsMetadata } from "./DocsMetadataForm";
import { ArticlePreviewCard } from "./ArticlePreviewCard";
import { Loader2, Upload, Download, AlertTriangle, ArrowRight, CheckCircle2, ExternalLink, X, RefreshCw } from "lucide-react";
import { toast } from "sonner";
import { cn } from "@/lib/utils";

import { getBuilderMetadata, getHeroImage, updateBuilderMetadata, parseFrontmatter, updateFrontmatter, updateHeroImage } from "@/lib/frontmatter";
...
function getFirstAuthorId(author: BuilderExistingArticle["data"]["author"], authors?: BuilderExistingArticle["data"]["authors"]) {
...
export function BuilderSyncPanel({
  ...
  const [conversion, setConversion] = useState<MarkdownConversionResult | null>(null);
  ...
  const sourceMarkdown = metadataMarkdown !== undefined ? metadataMarkdown : markdown;
  const builderMeta = useMemo(() => getBuilderMetadata(sourceMarkdown), [sourceMarkdown]);
  const existingHero = useMemo(() => getHeroImage(sourceMarkdown), [sourceMarkdown]);
  ...
  const handleMetadataChange = (next: ArticleMetadata | DocsMetadata) => {
    ...
    const targetMarkdown = metadataMarkdown !== undefined ? metadataMarkdown : markdown;
    if (targetMarkdown) {
      // Preserve the model and docsId fields from frontmatter
      const model = builderMeta.model || (builderMeta.docsId ? "docs-content" : "blog-article");
      const updates: any = {
        model,
        ...next,
      };
    ...
    const timer = setTimeout(() => {
      ...
      markdownToBuilder(contentBody)
        .then((result) => {
          ...
          const targetHandle = builderMeta.handle || propHandle || titleToHandle(result.title);
          const remoteMatch = articles.find(a => a.data?.handle === targetHandle);
          const remoteData: Partial<BuilderExistingArticle["data"]> = remoteMatch?.data || {};
          ...
          const metadataTitleFallback = metadataH1Match ? metadataH1Match[1] : result.title;
          ...
          setMetadata(prev => {
            ...
            if (builderMeta.model === "docs-content") {
              ...
              const next = {
                ...prev,
                url: localUrl,
                pageTitle: builderMeta.pageTitle || (prev as any).pageTitle || metadataTitleFallback,
                description: builderMeta.description || (prev as any).description || result.firstParagraph,
                hideNav: builderMeta.hideNav !== undefined ? builderMeta.hideNav : (prev as any).hideNav || false,
                shopifyApplicable: builderMeta.shopifyApplicable !== undefined ? builderMeta.shopifyApplicable : (prev as any).shopifyApplicable || false,
                referenceNumber: builderMeta.referenceNumber || (prev as any).referenceNumber || "",
                tags: (builderMeta.tags && builderMeta.tags.length > 0) ? builderMeta.tags : ((prev.tags && prev.tags.length > 0) ? prev.tags : detected.tags),
                redirectToUrl: builderMeta.redirectToUrl || (prev as any).redirectToUrl || "",
                redirectToPermanent: builderMeta.redirectToPermanent !== undefined ? builderMeta.redirectToPermanent : (prev as any).redirectToPermanent || false,
                image: builderMeta.image !== undefined ? builderMeta.image : (prev.image || existingHero || result.imageUrls[0] || ""),
                hideFeedbackColumn: builderMeta.hideFeedbackColumn !== undefined ? builderMeta.hideFeedbackColumn : (prev as any).hideFeedbackColumn || false,
                showToc: builderMeta.showToc !== undefined ? builderMeta.showToc : (prev as any).showToc || false,
                addNoIndex: builderMeta.addNoIndex !== undefined ? builderMeta.addNoIndex : (prev as any).addNoIndex || false,
              };
              const isEqual = Object.keys(next).every(k => (next as any)[k] === (prev as any)[k]);
            ...
            const next = {
              ...prev,
              title: builderMeta.title || (prev as any).title || remoteData.title || metadataTitleFallback,
              handle: builderMeta.handle || (prev as any).handle || remoteData.handle || propHandle || titleToHandle(metadataTitleFallback),
              blurb: builderMeta.blurb || (prev as any).blurb || remoteData.blurb || result.firstParagraph,
              metaTitle: builderMeta.metaTitle || (prev as any).metaTitle || remoteData.metaTitle || "",
              date: (builderMeta.date instanceof Date ? builderMeta.date.toISOString().split("T")[0] : builderMeta.date) || ((prev as any).date && (prev as any).date !== "" ? (prev as any).date : null) || (remoteData.date ? new Date(remoteData.date).toISOString().split("T")[0] : new Date().toISOString().split("T")[0]),
              readTime: builderMeta.readTime || ((prev as any).readTime > 0 ? (prev as any).readTime : null) || remoteData.readTime || estimateReadTime(result.wordCount),
              tags: (builderMeta.tags && builderMeta.tags.length > 0) ? builderMeta.tags : ((prev.tags && prev.tags.length > 0) ? prev.tags : ((remoteData.tags && remoteData.tags.length > 0) ? remoteData.tags : detected.tags)),
              topic: builderMeta.topic || (prev as any).topic || remoteData.topic || detected.topic,
              image: builderMeta.image !== undefined ? builderMeta.image : (prev.image || remoteData.image || existingHero || result.imageUrls[0] || ""),
              hideImage: builderMeta.hideImage !== undefined ? builderMeta.hideImage : ((prev as any).hideImage || remoteData.hideImage || false),
              authorId:
  ...
  }, [(metadata as any).handle, builderMeta.docsId, builderMeta.handle, builderMeta.model, contentList]);
  ...
  useEffect(() => {
    ...
    metadataImageChangedRef.current = false;
  }, [metadata.image, open, isEditingDraft, onHeroImageChange, currentHeroImage]);
  ...
  useEffect(() => {
    const model = builderMeta.model || (builderMeta.docsId ? "docs-content" : "blog-article");
    const identifier = model === "docs-content"
      ? builderMeta.docsId
  ...
  }, [matchingArticle?.data?.handle, matchingArticle?.lastUpdated, builderMeta.docsId, builderMeta.model, auth]);
    ...
    const timer = setTimeout(() => {
      ...
      markdownToBuilder(contentBody).then((result) => {
        ...
        if (model === "blog-article") {
          ...
          if (m.title !== blogMeta.title) issues.push(`title: '${m.title}' !== '${blogMeta.title}'`);
          if ((m.blurb || "") !== (blogMeta.blurb || "")) issues.push('blurb');
          if ((m.metaTitle || "") !== (blogMeta.metaTitle || "")) issues.push('metaTitle');
          ...
          const remoteDateStr = m.date ? new Date(m.date).toISOString().split("T")[0] : "";
          const localDateObj = typeof blogMeta.date === 'string' ? new Date(blogMeta.date) : (blogMeta.date || new Date());
          const localDateStr = blogMeta.date ? localDateObj.toISOString().split("T")[0] : "";
          if (remoteDateStr !== localDateStr) issues.push(`date: '${remoteDateStr}' !== '${localDateStr}'`);

          if ((m.readTime || 1) !== (blogMeta.readTime || 1)) issues.push(`readTime: ${m.readTime} !== ${blogMeta.readTime}`);

          const remoteAuthorId = getFirstAuthorId(m.author, m.authors);
          if (remoteAuthorId !== (blogMeta.authorId || "")) issues.push('authorId');
        } else {
          ...
          if ((m.pageTitle || "") !== (docsMeta.pageTitle || "")) issues.push('pageTitle');
          if ((m.description || "") !== (docsMeta.description || "")) issues.push('description');
          if (!!m.hideNav !== !!docsMeta.hideNav) issues.push('hideNav');
          if (!!m.shopifyApplicable !== !!docsMeta.shopifyApplicable) issues.push('shopifyApplicable');
          if ((m.referenceNumber || "") !== (docsMeta.referenceNumber || "")) issues.push('referenceNumber');
          if ((m.redirectToUrl || "") !== (docsMeta.redirectToUrl || "")) issues.push('redirectToUrl');
          if (!!m.redirectToPermanent !== !!docsMeta.redirectToPermanent) issues.push('redirectToPermanent');
          if ((m.image || "") !== (docsMeta.image || "")) issues.push('image');
          if (!!m.hideFeedbackColumn !== !!docsMeta.hideFeedbackColumn) issues.push('hideFeedbackColumn');
          if (!!m.showToc !== !!docsMeta.showToc) issues.push('showToc');
          if (!!m.addNoIndex !== !!docsMeta.addNoIndex) issues.push('addNoIndex');
        }
          ...
          try {
            ...
            if (remoteMarkdown.trim() !== localMarkdown.trim()) {
              ...
              if (firstDiff !== -1) {
                ...
                issues.push(`r: ${JSON.stringify(rmFull.substring(firstDiff - 10, firstDiff + 10))}`);
                issues.push(`l: ${JSON.stringify(lmFull.substring(firstDiff - 10, firstDiff + 10))}`);
              }
            }
          } catch (e) {
  ...
  }, [matchingArticle, metadata, markdown, remoteBlocks, lastSyncedMarkdown, storageKey]);
  ...
  const handleGenerateMetaDescription = async () => {
    ...
    if (!articleContent) {
      toast.error("Add some article content before generating a meta description");
      return;
    }
  ...
  const selectedAuthor = builderMeta.model !== "docs-content"
    ? authors.find((a) => a.id === (metadata as ArticleMetadata).authorId)
    : undefined;
  const authorName = selectedAuthor?.data?.fullName || selectedAuthor?.name || "";

  const isLocalAhead = useMemo(() => {
    if (!matchingArticle || !matchingArticle.lastUpdated || !localUpdatedAt) return true;
  ...
  const handleFetch = async () => {
    ...
    const model = builderMeta.model || (builderMeta.docsId ? "docs-content" : "blog-article");
    const identifier = builderMeta.docsId || builderMeta.handle || (metadata as any).handle || (metadata as any).url;
    ...
    try {
      ...
      if (model === "docs-content") {
        ...
        let fetchedUrl = data.url || fullData.url || (metadata as any).url || "";
        ...
        newMetadata = {
          ...metadata,
          url: fetchedUrl,
          pageTitle: fullData.pageTitle ?? data.title ?? (metadata as any).pageTitle ?? "",
          description: fullData.description ?? (metadata as any).description ?? "",
          hideNav: fullData.hideNav !== undefined ? fullData.hideNav : (metadata as any).hideNav ?? false,
          shopifyApplicable: fullData.shopifyApplicable !== undefined ? fullData.shopifyApplicable : (metadata as any).shopifyApplicable ?? false,
          referenceNumber: fullData.referenceNumber ?? (metadata as any).referenceNumber ?? "",
          tags: fullData.tags ?? [],
          redirectToUrl: fullData.redirectToUrl ?? (metadata as any).redirectToUrl ?? "",
          redirectToPermanent: fullData.redirectToPermanent !== undefined ? fullData.redirectToPermanent : (metadata as any).redirectToPermanent ?? false,
          image: fullData.image ?? "",
          hideFeedbackColumn: fullData.hideFeedbackColumn !== undefined ? fullData.hideFeedbackColumn : (metadata as any).hideFeedbackColumn ?? false,
          showToc: fullData.showToc !== undefined ? fullData.showToc : (metadata as any).showToc ?? false,
          addNoIndex: fullData.addNoIndex !== undefined ? fullData.addNoIndex : (metadata as any).addNoIndex ?? false,
        } as DocsMetadata;
        keysToCheck = ["url", "pageTitle", "description", "hideNav", "shopifyApplicable", "referenceNumber",
                       "redirectToUrl", "redirectToPermanent", "image", "hideFeedbackColumn", "showToc", "addNoIndex"];
      } else {
        newMetadata = {
          ...metadata,
          ...
          date: fullData.date ? new Date(fullData.date).toISOString().split("T")[0] : ((metadata as any).date || new Date().toISOString().split("T")[0]),
          readTime: fullData.readTime || ((metadata as any).readTime > 0 ? (metadata as any).readTime : 1),
          tags: fullData.tags ?? [],
        ...
        keysToCheck = ["title", "blurb", "metaTitle", "date", "readTime", "topic", "image", "hideImage", "authorId"];
      }

      const metadataDiff: Record<string, {old: any, new: any}> = {};
      for (const key of keysToCheck) {
        if ((metadata as any)[key] !== (newMetadata as any)[key]) {
          metadataDiff[key] = { old: (metadata as any)[key], new: (newMetadata as any)[key] };
        }
      }
      ...
      const displayTitle = model === "docs-content"
        ? (data.title || (metadata as DocsMetadata).pageTitle || (metadata as DocsMetadata).url)
  ...
  const handleApplyPull = () => {
    if (fetchedData) {
      ...
      const hasFrontmatter = markdown.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n?/);
      ...
      if (isEditingDraft) {
        ...
        let finalMarkdown = updateBuilderMetadata(markdownToUpdate, metaToApply as any);
        finalMarkdown = updateHeroImage(finalMarkdown, metaToApply.image || null);
        ...
        let updatedMetadataMarkdown = updateBuilderMetadata(targetMarkdown, metaToApply as any);

        // Also update the hero_image field to ensure both fields are in sync
        updatedMetadataMarkdown = updateHeroImage(updatedMetadataMarkdown, metaToApply.image || null);
        ...
        let updatedMetadataMarkdown = updateBuilderMetadata(markdownToUpdate, metaToApply as any);
        updatedMetadataMarkdown = updateHeroImage(updatedMetadataMarkdown, metaToApply.image || null);
  ...
  const handleUpload = async () => {
    ...
    if (model === "docs-content") {
      ...
      contentData = {
        ...
        image: docsMetadata.image ? docsMetadata.image.replace(/%2F/g, '/') : docsMetadata.image,
      ...
      const calculatedReadTime = Math.max(1, Math.ceil(conversion.wordCount / 225));
      contentData = {
        title: articleMetadata.title,
        handle: identifier,
        blurb: articleMetadata.blurb,
        date: (articleMetadata.date as any) instanceof Date ? (articleMetadata.date as any).getTime() : new Date(articleMetadata.date).getTime(),
        readTime: calculatedReadTime,
        tags: articleMetadata.tags,
        topic: articleMetadata.topic,
        image: articleMetadata.image ? articleMetadata.image.replace(/%2F/g, '/') : articleMetadata.image,
    ...
    const payload = {
      ...
      article: {
        ...
        ...(model === "docs-content" && transformedUrl ? { url: transformedUrl } : {}),
        data: contentData,
      },
      model,
  ...
  const isReady = !converting && conversion && (
    builderMeta.model === "docs-content"
      ...
      : (metadata as ArticleMetadata).title && (metadata as ArticleMetadata).handle
  );
  ...
  return (
    <div
      className={cn(
        ...
        open || embedded ? (embedded ? "w-full" : "w-[340px]") : "w-0 border-l-0"
      )}
    >
      {(open || embedded) && (
        <>
          ...
          {!embedded && (
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-border shrink-0">
              <h3 className="text-sm font-semibold text-foreground">Upload to Builder</h3>
              <button
          ...
          {converting ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-4 px-6 text-center">
              ...
              <div className="space-y-1">
                <p className="text-sm font-medium text-foreground">Builder conversion failed</p>
                <p className="text-sm text-muted-foreground">{conversionError}</p>
              </div>
              <Button
                variant="outline"
                size="sm"
            ...
            <div className="flex-1 flex items-center justify-center px-6 text-center">
              <span className="text-sm text-muted-foreground">Add content to start Builder conversion.</span>
            </div>
                    ...
                    <>
                      ...
                      {fetching && (
                        <div className="flex flex-col items-center justify-center py-8 gap-3 border rounded-md border-dashed">
                          ...
                          <p className="text-sm text-muted-foreground">Fetching from Builder...</p>
                        </div>
                      )}

                      {fetchedData && (
                        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                          {fetchedData.contentChanged || Object.keys(fetchedData.metadataDiff).length > 0 ? (
                            <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
                              <div className="flex items-start gap-2">
                                <AlertTriangle className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" />
                                <div className="text-xs text-amber-600 dark:text-amber-400">
                                  <p className="font-medium mb-1">Apply Changes</p>
                                  Applying will overwrite your local editor content with the latest from Builder.io.
                                </div>
                              </div>
                            </div>
                          ) : (
                            <div className="p-3 rounded-lg bg-green-500/10 border border-green-500/20">
                              <div className="flex items-start gap-2">
                                ...
                                <div className="text-xs text-green-600 dark:text-green-400">
                                  ...
                                  No changes found. Your local editor is already in sync with Builder.io.
                                </div>
                              </div>
                            </div>
                          )}
                            ...
                            <div className="text-xs space-y-2 border border-border rounded-md overflow-hidden">
                              ...
                              <div className="p-3 max-h-[200px] overflow-auto bg-background">
                                <ul className="space-y-3">
                                  {Object.entries(fetchedData.metadataDiff).map(([key, diff]) => (
                                    <li key={key} className="grid grid-cols-[80px_1fr] gap-3 items-start border-b border-border/50 pb-2 last:border-0 last:pb-0">
                                      <span className="font-semibold text-muted-foreground capitalize mt-1">{key}:</span>
                                      <div className="flex flex-col gap-1.5">
                                        <div className="flex flex-col gap-0.5">
                                          <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Current</span>
                                          <span className="text-red-500/80 line-through break-all">{JSON.stringify(diff.old) || "empty"}</span>
                                        </div>
                                        <div className="flex flex-col gap-0.5">
                                          <span className="text-[10px] text-muted-foreground uppercase tracking-wider">New</span>
                                          <span className="text-green-500 break-all">{JSON.stringify(diff.new) || "empty"}</span>
                                        </div>
                                      </div>
                                    </li>
                                  ))}
                          ...
                          {fetchedData.contentChanged && (
                            <div className="text-xs space-y-2 border border-border rounded-md overflow-hidden">
                              <div className="bg-muted px-3 py-2 font-medium border-b border-border flex justify-between">
                                ...
                                <span className="text-muted-foreground">{fetchedData.content.length} chars</span>
                              </div>
                              <div className="p-3 max-h-[250px] overflow-auto text-[11px] whitespace-pre-wrap font-mono text-muted-foreground">
                                {fetchedData.content.length > 500 ? fetchedData.content.substring(0, 500) + '...' : fetchedData.content}
                              </div>
                            </div>
                          )}
                        </div>
                    ...
                    <>
                      {fetchError && (
                        <div className="p-3 rounded-lg bg-destructive/10 border border-destructive/20 text-xs text-destructive flex justify-between items-center">
                          ...
                          <button onClick={() => setFetchError("")} className="text-destructive/80 hover:text-destructive">
                            <X size={14} />
                          </button>
                        </div>
                      )}
                      ...
                      {builderMeta.model === "docs-content" ? (
                        ...
                        <ArticleMetadataForm
                          ...
                          isGeneratingMetaDescription={generateMetaDescription.isPending}
                        />
                      )}

                      {conversion && (
                        <p className="text-[11px] text-muted-foreground">
                          {conversion.blocks.length} blocks · {conversion.wordCount} words · {conversion.imageUrls.length} images
                          {conversion.videoUrls.length > 0 && ` · ${conversion.videoUrls.length} videos`}
                        </p>
                      )}
              ...
              <div className="px-4 py-3 border-t border-border shrink-0 bg-background">
                {fetchedData ? (
                  <div className="flex flex-col gap-2">
                    <Button onClick={handleApplyPull} className="w-full" variant="default" disabled={!fetchedData.contentChanged && Object.keys(fetchedData.metadataDiff).length === 0}>
                      <ArrowRight className="w-4 h-4 mr-2" />
                      Apply Changes to Editor
                    </Button>
                    <Button onClick={() => setFetchedData(null)} className="w-full" variant="outline">
                      Cancel
                    </Button>
                  </div>
                ) : (
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-col gap-2">
                      <Button
                        onClick={handleFetch}
                        disabled={!(builderMeta.docsId || builderMeta.handle || (metadata as any).handle || (metadata as any).url) || !auth || fetching || isDataSynced}
                        className="w-full relative disabled:opacity-50"
                        variant={!isDataSynced && !isLocalAhead && matchingArticle ? "default" : "outline"}
                      >
                        <Download className="w-4 h-4 mr-2" />
                        {!isDataSynced && !isLocalAhead && matchingArticle ? "Pull updates from Builder" : "Pull from Builder"}
                      ...
                      <Button
                        ...
                        variant={(!isDataSynced && isLocalAhead) || !matchingArticle ? "default" : "outline"}
                        disabled={!isReady || uploadMutation.isPending || fetching || (isDataSynced && !!matchingArticle)}
                      >
                        {uploadMutation.isPending ? (
                          ...
                          <>
                            ...
                            {!isDataSynced && matchingArticle ? "Push updates to Builder" : "Push to Builder"}
                            {((!isDataSynced && isLocalAhead) || !matchingArticle) && (
                               <span className="absolute top-2 right-2 flex h-2 w-2 rounded-full bg-blue-500"></span>
                            )}
                          </>
