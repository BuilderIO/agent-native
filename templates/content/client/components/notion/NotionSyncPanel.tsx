...
export function NotionSyncPanel({
  ...
  const { data: pageMeta } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await authFetch(
        `/api/notion/page-meta?pageId=${notionMeta.page_id}`,
      );
      return res.json();
    },
    ...
    if (localLastEdited && remoteLastEdited !== localLastEdited) {
      const verifyChange = async () => {
        try {
          ...
          if (
            blocksToConvert &&
            blocksToConvert.length > 0 &&
            blocksToConvert[0].type === "image"
          ) {
          ...
          if (
            expectedPropsStr !== null &&
            !arePropsEqual(expectedProps, remoteProps)
          ) {
            isOutOfSync = true;
          }
  ...
  useEffect(() => {
    ...
    const pageExists =
      existingPageId && pagesArray.some((p) => p.id === existingPageId);
    ...
    if (!existingPageId || !pageExists) {
      ...
      const match = pagesArray.find((p) => {
        ...
        if (
          pubUrl &&
          expectedUrl &&
          pubUrl === expectedUrl.trim().toLowerCase()
        )
          return true;
        ...
        const titleObj =
          ...
          p.properties?.Name?.title ||
          p.properties?.Title?.title;
        const pageTitle = (titleObj?.[0]?.plain_text || "")
          .trim()
          .toLowerCase();
        ...
        if (pageTitle) {
          ...
          if (
            pageTitleSlug.includes(cleanHandle) ||
            cleanHandle.includes(pageTitleSlug)
          ) {
            return true;
          }
        }
    ...
    handle,
    notionMeta.page_id,
    notionMeta["Published URL"],
    notionMeta["URL"],
  ]); // Intentionally not including markdown/onChange to avoid loops

  const autoSync =
    notionMeta.auto_sync === true || notionMeta.auto_sync === "true";

  const handlePull = async () => {
    if (!notionMeta.page_id) {
      toast.error(
        "No Notion Page ID found in frontmatter. Please link to a page first.",
      );
      return;
    }
    ...
    try {
      ...
      if (fmEndIndex !== -1) {
        newStr =
          lines.slice(0, fmEndIndex + 1).join("\n") +
          "\n\n" +
          newMarkdownContent;
      } else {
    ...
    ({ silent = false }: { silent?: boolean } = {}) => {
      ...
      const pushTask = async () => {
        ...
        if (localImages.length > 0 && !isConnected) {
          setIsDataSynced(false);
          throw new Error(
            "Cannot upload local images because Builder.io is not connected.",
          );
        }
        ...
        const heroImage =
          parsedMd.data.hero_image || parsedMd.data.builder?.image;
        if (heroImage) {
        ...
        if (schema && isBlogArticle) {
          ...
          const titlePropEntry = Object.entries(schema).find(
          ...
          const pubUrlKey = Object.keys(schema).find(
            (k) =>
              k === "Published URL" ||
              k === "URL" ||
              k.toLowerCase() === "published url",
          );
          if (pubUrlKey && schema[pubUrlKey].type === "url") {
            notionProps[pubUrlKey] = {
              url: `https://www.builder.io/blog/${handle}`,
            };
          }
          ...
          for (const [key, schemaProp] of Object.entries(schema) as any) {
            ...
            if (schemaProp.type === "select") {
              ...
              notionProps[key] = {
                multi_select: (Array.isArray(val) ? val : [val]).map(
                  (v: string) => ({ name: v }),
                ),
              };
        ...
        const expectedRemoteBlocks = notionBlocksToMarkdown(
          blocksToConvertWithoutHero,
        );

        const expectedProps: any = {
          page_id: data.pageId || notionMeta.page_id,
        };
        ...
        localStorage.setItem(
          `notion-expected-props-${handle}`,
          JSON.stringify(expectedProps),
        );
        localStorage.setItem(
          `notion-expected-blocks-${handle}`,
          expectedRemoteBlocks,
        );
        ...
        if (data.pageId && notionMeta.page_id !== data.pageId) {
          finalMarkdown = updateNotionMetadata(currentMarkdown, {
            page_id: data.pageId,
          });
          onChange(finalMarkdown);
        }
      ...
      if (silent) {
        ...
        toast.promise(
          pushTask().finally(() => setIsPushing(false)),
          {
  ...
  useEffect(() => {
    ...
    if (
      lastPullMarkdownRef.current !== null &&
      markdown === lastPullMarkdownRef.current
    )
      return;
              ...
              {schemaRes?.error || pagesRes?.error ? (
                <div className="p-4 bg-destructive/10 text-destructive text-sm rounded-md space-y-2 border border-destructive/20">
                  <p className="font-semibold flex items-center gap-2">
                    <ExternalLink className="w-4 h-4" /> Failed to connect to
                    Notion
                  </p>
                  ...
                  <p className="text-xs mt-2 opacity-80">
                    Please check your Notion API key and ensure the database is
                    shared with your integration.
                  </p>
                  <Button
                    variant="outline"
                    size="sm"
                    className="mt-2 bg-background/50 hover:bg-background/80 text-foreground"
                    onClick={() => {
                          ...
                          <div className="flex items-start gap-2 min-w-0">
                            ...
                            <div className="flex flex-col gap-0.5 min-w-0">
                              <span className="font-medium text-sm">
                                {(() => {
                                  const selectedPage = pages.find(
                                    (p: any) => p.id === notionMeta.page_id,
                                  );
                                  if (selectedPage) {
                                    const titleObj =
                                      selectedPage.properties?.Topic?.title ||
                                      selectedPage.properties?.Name?.title ||
                                      selectedPage.properties?.Title?.title;
                                    return (
                                      titleObj?.[0]?.plain_text || "Untitled"
                                    );
                                  }
                              ...
                              {(() => {
                                ...
                                const pubUrl =
                                  selectedPage?.properties?.["Published URL"]
                                    ?.url ||
                                  selectedPage?.properties?.["URL"]?.url ||
                                  "";
                                return (
                                  pubUrl && (
                        ...
                        ([key, prop]: [string, any]) => {
                          ...
                          if (prop.type === "multi_select") {
                            ...
                            const availableOptions = options.filter(
                              (opt: any) => !selectedVals.includes(opt.name),
                            );
                              ...
                              >
                                <Checkbox
                                  ...
                                  onCheckedChange={(checked) =>
                                    updateProp(key, checked)
                                  }
                                />
                                <Label
                                  htmlFor={`notion-prop-${key}`}
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  checked={autoSync}
                  onCheckedChange={(checked) => {
                    const newMarkdown = updateNotionMetadata(markdown, {
                      auto_sync: !!checked,
                    });
                    onChange(newMarkdown);
                  }}
                ...
                <>
                  ...
                  <div className="flex flex-col gap-2">
                    <Button
                      ...
                      onClick={handlePull}
                      disabled={
                        isPulling || !notionMeta.page_id || isDataSynced
                      }
                    ...
                    <Button
                      ...
                      disabled={
                        isPulling || (isDataSynced && !!notionMeta.page_id)
                      }
                      className="w-full relative disabled:opacity-50"
                    >
