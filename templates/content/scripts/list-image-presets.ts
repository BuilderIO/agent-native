...
function listFolderPresets() {
  ...
  return fs.readdirSync(IMAGE_REFS_DIR)
    .filter(d => fs.statSync(path.join(IMAGE_REFS_DIR, d)).isDirectory())
    .map(name => {
      const images = fs.readdirSync(path.join(IMAGE_REFS_DIR, name))
...
export default async function main(_args: string[]) {
  ...
  if (!namedPresets.length && !folderPresets.length) {
    console.log("No image presets found. Create presets via the UI or add folders under content/shared-resources/image-references/.");
    return;
  }
