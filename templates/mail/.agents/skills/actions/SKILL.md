...
Use Drizzle's query builder for normal app data; use reviewed Postgres SQL via `getDbExec()` only when Drizzle cannot express the query. Never hardcode API keys/tokens/secrets — read via `readAppSecret` / `resolveCredential` / OAuth helpers; `process.env` is deploy-level config only.
