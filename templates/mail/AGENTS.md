...
| Action | Purpose |
| --- | --- |
| `list-inbox-threads` | Inbox tab bar, counts, and rows from one synced-store partition; All includes every inbox thread and is shown by default. Other views/search use `list-emails`/`search-emails`. |
