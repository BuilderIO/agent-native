...
/**
 * Regression guard mirroring the analytics template's db.spec.ts (see that
 * file for the incident this pattern is fixing — a schema.ts column added
 * without a matching migration silently 500s every read/write touching a
 * pre-existing production table's row).
 *
...
describe("mail db migrations cover every schema.ts column", () => {
 ...
 *   (b) every entry whose version is > 14 (mail's current max version, as of
 *       this guard) has a `name`.
 *
 * We deliberately do NOT require the existing v1-v14 entries to have a name —
 * naming them would make each re-apply by name on every existing database,
 * which is only safe if every one of those older SQL statements is
 * idempotent. That has not been verified/needed here since a live-DB audit
...
describe("mail db.ts migration entries follow the naming convention", () => {
  ...
  it("every migration entry with version > 14 has a name", () => {
    const missingNames = entries
      .filter((e) => e.version > 14)
      .filter((e) => !e.name)
      .map((e) => e.version);
...
describe("mail db.ts wires ensureAdditiveColumns after runMigrations", () => {
  ...
  it("keeps the composite hot-path indexes from v14", () => {
    expect(dbTsSource).toMatch(
