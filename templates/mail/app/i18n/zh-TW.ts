const messages = {
  sidebar: {
    collapseSidebar: "收合側邊欄",
    expandSidebar: "展開側邊欄",
  },
  mail: {
    appName: "郵件",
    pages: {
      team: "團隊",
    },
    views: {
      inbox: "收件箱",
      all: "全部",
      unread: "未讀",
      starred: "已加星號",
      snoozed: "已稍後提醒",
      sent: "已傳送",
      draftQueue: "草稿佇列",
      scheduled: "已計畫",
      drafts: "草稿",
      archive: "封存",
      trash: "垃圾桶",
      other: "其他",
      labels: "標籤",
      extensions: "擴充功能",
      noteToSelf: "給自己的備註",
    },
    inbox: {
      syncing: "收件箱同步中…",
    },
    sort: {
      label: "排序收件匣",
      newest: "最新",
      priority: "優先順序",
      priorityFailed: "無法排序收件匣。",
      priorityScoreHelp: "分數越高，代表 Jev 判斷這封郵件越重要。",
      priorityFeedbackSuggestion: "將此回饋模式設為重要性或自動封存規則。",
      priorityFeedbackAskAgent: "請代理建議規則",
      aiSetupTitle: "設定 AI 收件匣",
      aiSetupTagLabel: "建立 AI 標籤",
      aiSetupImportanceLabel: "重要郵件",
      aiSetupSpamLabel: "垃圾郵件",
      aiSetupArchiveLabel: "略過收件匣",
      aiSetupSave: "儲存設定",
      aiSetupSkip: "暫時略過",
      aiSetupImportantHeadline: "哪些內容重要",
      aiSetupSkipInboxHeadline: "哪些內容可以略過收件匣",
      aiSetupTagsHeadline: "選擇標籤",
      aiSetupArchiveSpamHeadline: "略過收件匣和垃圾郵件",
      aiSetupTagReceipts: "收據",
      aiSetupTagUpdates: "產品更新",
      aiSetupTagGitHub: "GitHub 人工留言",
      aiSetupPromptReceipts: "網路商店的收據和訂單確認郵件",
      aiSetupPromptUpdates: "我使用工具的產品更新和版本說明",
      aiSetupPromptGitHub: "有人留言的 GitHub 通知",
      aiSetupImportantPrompt:
        "需要回覆或有截止日期的郵件，包括 GitHub 上的人類留言。略過機器人留言。",
      aiSetupArchiveSpamPrompt:
        "自動封存：包含機器人留言或自動狀態更新的 GitHub 通知。\n垃圾郵件：我未要求的明顯促銷或不需要的郵件。",
      aiSetupCustomTag: "自訂",
      aiSetupDone: "完成",
      aiSetupRunAgain: "重新執行設定",
      aiSetupTagCalendar: "行事曆",
      aiSetupPromptCalendar: "我需要的行事曆邀請和活動更新",
      aiSetupTagTravel: "旅行",
      aiSetupPromptTravel: "我需要的旅行確認和預訂",
      aiSetupTagFinance: "財務",
      aiSetupPromptFinance: "我需要的帳單和帳戶明細",
      aiSetupArchiveExample: "GitHub、Vercel 和 Dependabot 的機器人與 CI 通知",
      aiSetupFilteredExample: "我尚未回覆的陌生銷售郵件和招募訊息",
      aiSetupSortingHeadline: "正在整理收件匣",
      aiSetupSortingProgress:
        "正在整理近期郵件：{{total}} 封中的 {{processed}} 封",
      aiSetupUndoing: "正在復原收件匣變更…",
      aiSetupSortingFailed: "無法整理收件匣。規則已儲存，請再試一次。",
      aiSetupUndoComplete: "已將 {{count}} 封郵件還原至原狀。",
      aiSetupRuleCount: "符合 {{count}} 封",
      aiSetupNoMatches: "過去 14 天內沒有郵件符合這些規則。",
      aiSetupChatTip: "你可以隨時在聊天中調整或新增規則。",
      aiSetupChatPrompt: "優先處理我主管的郵件…",
      aiSetupNoRules: "未選取規則。",
      aiSetupPartialFailure: "有 {{count}} 封郵件無法更新。",
      aiSetupSortInbox: "整理收件匣",
      aiSetupImportantExample: "我主管 Priya 寄來的所有郵件…",
      priorityFeedbackLabel: "重要性回饋",
      priorityScoreHigh: "高重要性",
      priorityScoreMedium: "中等重要性",
      priorityScoreLow: "低重要性",
      priorityEditRules: "編輯重要性規則",
      aiSetupContinue: "繼續",
    },
    toolbar: {
      toggleMenu: "切換選單",
      menu: "選單",
      configureTabs: "設定標籤頁面",
      refreshInbox: "重新整理收件箱",
      composeEmail: "撰寫郵件",
      compose: "撰寫",
      composeShortcut: "撰寫 (C)",
      accounts: "帳號",
      pinSidebar: "固定側邊欄",
      unpinSidebar: "取消固定側邊欄",
      closeSidebar: "關閉側邊欄",
      settings: "設定",
      aiSettings: "管理標籤與規則",
    },
    search: {
      label: "搜尋",
      placeholder: "搜尋...",
      clear: "清除搜尋 (Esc)",
      noMatches: "沒有匹配項",
      localResults: "此收件匣中",
      searchingGmail: "正在搜尋 Gmail...",
      saveAsTab: "儲存為分頁",
      saveAsTabPrompt: "為此分頁命名",
      saveAsTabFailed: "無法儲存此分頁。請再試一次。",
      filtersLimitReached: "最多可儲存 20 個篩選條件。",
    },
    tabSettings: {
      splitInbox: "分割收件匣",
      views: "檢視",
      categories: "分類",
      rename: "重新命名",
      renameTab: "重新命名標籤頁面",
      savedFilters: "已儲存的篩選條件",
      combinedInbox: "合併收件匣",
      allTab: "全部分頁",
      help: "合併收件匣會一起顯示所有帳戶；關閉後，郵件會依標籤分到不同分頁。",
      aiSetup: "設定 AI 標籤和規則",
    },
    accounts: {
      remove: "移除",
      connecting: "正在連線...",
      connectGoogle: "連線 Google",
      signInWithGoogle: "使用 Google 登入",
      addAccount: "新增帳號",
    },
    actions: {
      undo: "撤銷",
      archive: "封存",
      markRead: "標為已讀",
      markUnread: "標為未讀",
      moveTo: "行動到",
      moveToTrash: "移到垃圾桶",
      star: "加上星號",
      unstar: "移除星號",
    },
    compose: {
      addImage: "新增圖片",
      agentPromptLabel: "希望代理寫什麼？",
      agentPromptPlaceholder: "例如：寫一封禮貌的跟進郵件...",
      aiAssist: "AI 輔助",
      aiAssistPlaceholder: "例如：改得更正式...",
      apply: "應用",
      attachFile: "附加檔案",
      bold: "加粗",
      cancel: "取消",
      closeAllDrafts: "關閉所有草稿",
      code: "程式碼",
      deleteDraft: "刪除草稿",
      deleteDrafts: "刪除草稿",
      reopenDraft: "重新開啟",
      discardDraft: "丟棄草稿",
      enterLinkUrl: "輸入連結的 URL。",
      forward: "Forward",
      from: "寄件人",
      fullScreen: "全螢幕",
      fullScreenCompose: "全螢幕撰寫",
      generate: "生成",
      generateShortcut: "產生（{{shortcut}}）",
      generating: "正在生成...",
      insertLink: "插入連結",
      italic: "斜體",
      link: "連結",
      minimize: "Minimize",
      minimizeCompose: "最小化撰寫視窗",
      newDraft: "新草稿",
      newMessage: "新訊息",
      pasteLink: "貼上連結...",
      plainText: "純文字",
      popOut: "快顯到撰寫視窗",
      reply: "Reply",
      replyTo: "寄給 {{recipient}}",
      queuedDraft: "排隊中的草稿",
      restore: "恢復",
      restoreCompose: "恢復撰寫視窗",
      restoreComposeSize: "恢復撰寫視窗大小",
      restoreSize: "恢復大小",
      send: "傳送",
      sending: "正在傳送...",
      hideCcBcc: "隱藏副本與密件副本",
      showCcBcc: "顯示副本與密件副本",
      strikethrough: "刪除線",
      subject: "Subject",
      to: "收件人",
      toRecipients: "收件人清單",
      cc: "副本",
      ccRecipients: "副本收件人",
      bcc: "密件副本",
      bccRecipients: "密件副本收件人",
      toSubmit: "送出",
      uploading: "正在上傳...",
      writeMessagePlaceholder: "撰寫你的訊息...",
    },
    composeSlash: {
      text: "文字",
      plainTextBlock: "純文字塊",
      heading1: "標題 1",
      largeHeading: "大標題",
      heading2: "標題 2",
      mediumHeading: "中標題",
      heading3: "標題 3",
      smallHeading: "小標題",
      bulletList: "專案符號清單",
      unorderedList: "無序清單",
      numberedList: "編號清單",
      orderedList: "有序清單",
      quote: "引用",
      blockQuote: "塊引用",
      codeBlock: "程式碼塊",
      codeSnippet: "程式碼片段",
      divider: "分隔線",
      horizontalRule: "水平線",
      image: "圖片",
      uploadImage: "上傳圖片",
      generate: "生成",
      aiAssistedWriting: "AI 輔助寫作",
      blocks: "塊",
      media: "媒體",
      ai: "AI",
      snippets: "片段",
    },
    mobileActions: {
      archive: "封存",
      trash: "刪除",
      star: "加星號",
      reply: "回覆",
      replyAll: "全部回覆",
      forward: "轉發",
      unread: "未讀",
      prev: "上一個",
      next: "下一個",
      settings: "設定",
      customize: "自訂",
      customizeActions: "自訂操作",
      close: "關閉",
    },
    integrations: {
      addIntegration: "新增整合",
      checking: "正在檢查...",
      connect: "連線",
      disconnect: "斷開連線",
      getApiKey: "取得 API 金鑰：",
      newIntegration: "新建整合",
      newIntegrationPlaceholder:
        "例如 Salesforce CRM — 顯示聯絡人的交易階段和最近活動",
      openProviderSettings: "開啟 {{provider}} 設定",
      apolloDescription: "聯絡人補全和公司資料",
      hubspotDescription: "CRM 聯絡人、交易和工單",
      gongDescription: "近期通話和對話智慧",
      pylonDescription: "支援問題和帳戶資料",
      gongCalls: "Gong 通話",
      deals: "交易",
      experience: "經歷",
      tickets: "工單",
      save: "儲存",
      submit: "Submit",
      submitShortcut: "送出（{{shortcut}}）",
      updateKey: "更新金鑰",
    },
    recipients: {
      aliasName: "別名名稱",
      createReusableGroup: "建立包含 {{count}} 位收件者的可重複使用群組",
      editAlias: "編輯別名",
      expandToIndividualEmails: "展開為單一信箱地址",
      peopleCount: "{{count}} 人",
      recipientCount: "{{count}} 位收件者",
      removeRecipient: "移除收件者 {{recipient}}",
      saveAsAlias: "另存為別名",
      saving: "正在儲存...",
    },
    sendLater: {
      cancelScheduledSend: "取消定時傳送",
      dateInput: "日期和時間",
      noDateMatch: "沒有符合的未來時間",
      inputPlaceholder: "試試：上午 8 點、3 天後、8 月 7 日",
      scheduleAt: "{{date}}",
      scheduledFor: "已安排於 {{date}}",
      pickDateTime: "選取日期和時間...",
      scheduleSend: "定時傳送",
      sendNow: "立即傳送",
      laterToday: "今天晚些時候",
      tomorrowMorning: "明天早上",
      tomorrowAfternoon: "明天下午",
      weekdayMorning: "{{weekday}}早上",
      nextWeek: "下週",
    },
    snooze: {
      modalPlaceholder: "試試：上午 8 點、3 天後、8 月 7 日",
      parsing: "正在解析...",
      snooze: "Snooze",
      snoozeUntil: "稍後提醒至",
      snoozeUntilDate: "延後到 {{date}}",
      snoozing: "正在設為稍後提醒...",
      tryPlaceholder: "或試試：週五下午 5 點、兩週後...",
      laterToday: "今天晚些時候",
      tomorrow: "明天",
      nextWeek: "下週",
      thisWeekend: "本週末",
      someday: "某天",
    },
    thread: {
      alwaysFrom: "一律來自 {{domain}}",
      at: "於",
      back: "返回",
      cc: "副本",
      closeDetails: "關閉詳細資料",
      closeSearch: "關閉搜尋",
      downloadAll: "全部下載",
      emailContent: "郵件內容",
      from: "寄件人",
      hideQuotedText: "隱藏引用文字",
      imagesBlocked: "圖片已攔截。",
      maximize: "最大化",
      minimize: "最小化",
      nextConversation: "下一個對話",
      nextMatch: "下一個相符項目",
      previousConversation: "上一個對話",
      previousMatch: "上一個相符項目",
      remoteImagesHidden: "此嵌入中的遠端圖片已隱藏。",
      searchConversation: "在工作階段中搜尋...",
      searchConversationLabel: "在對話中搜尋",
      showImages: "顯示圖片",
      showQuotedText: "顯示引用文字",
      showSignature: "顯示簽名",
      to: "收件人",
      unsubscribe: "Unsubscribe",
      unsubscribeTooltip: "取消訂閱此郵件清單",
      unsubscribing: "正在取消訂閱...",
      viewPullRequest: "檢視 Pull Request",
    },
    selection: {
      clear: "清除選取",
      actions: "選取操作",
      selected: "已選取 {{count}} 項",
      select: "選取",
      selectEmail: "選取郵件",
      deselectEmail: "取消選取郵件",
      all: "全部",
      none: "無",
      read: "已讀",
      unstarred: "未加星號",
    },
    empty: {
      inboxZeroTitle: "你已清空收件箱",
      inboxZeroSubtitle: "所有郵件都處理完了",
      snoozed: "目前沒有稍後提醒的郵件。",
      drafts: "儲存的草稿會顯示在這裡。",
      starred: "給郵件加星號後會顯示在這裡。",
      sent: "你傳送的郵件會顯示在這裡。",
      scheduled: "定時傳送的郵件會顯示在這裡。",
      archive: "封存郵件會顯示在這裡。",
      trash: "垃圾桶郵件會在這裡保留 30 天。",
      spam: "Gmail 標記的垃圾郵件會顯示在這裡。",
      all: "所有郵件會顯示在這裡。",
      unread: "你已處理完畢，沒有未讀郵件。",
      default: "匹配此檢視的郵件會顯示在這裡。",
      noSearchResults: "沒有找到“{{query}}”的結果",
      tryDifferentKeywords: "試試其他關鍵詞",
      nothingHere: "這裡還沒有內容",
      loadingMore: "正在載入更多...",
    },
    toasts: {
      noEmailSelected: "未選取郵件。",
      reportedSpam: "已舉報為垃圾郵件。",
      reportedSpamBlocked: "已舉報為垃圾郵件並屏蔽 {{email}}。",
      noThreadSelected: "未選取工作階段。",
      threadMuted: "工作階段已靜音。",
      archived: "已封存。",
      archivedMany: "已封存 {{count}} 個工作階段。",
      archiveFailed: "封存失敗。工作階段已恢復。",
      moveFailed: "移動失敗。工作階段已恢復。",
      movePartialFailed:
        "已移動 {{total}} 個工作階段中的 {{succeeded}} 個；{{failed}} 個失敗。",
      moveSucceeded: "已移至「{{label}}」。",
      moveManySucceeded: "已將 {{count}} 個對話移至「{{label}}」。",
      trashed: "已移到垃圾桶。",
      trashedMany: "已將 {{count}} 個工作階段移到垃圾桶。",
      scheduledSent: "定時郵件已傳送。",
      scheduledSendFailed: "傳送定時郵件失敗",
      scheduledCancelled: "定時郵件已取消。",
      scheduledCancelFailed: "取消定時郵件失敗",
      failedToAttachFile: "附加檔案失敗",
      failedToUploadImage: "上傳圖片失敗",
      failedToSendEmail: "傳送郵件失敗",
      messageSent: "郵件已傳送。",
      failedToSaveDraft: "無法儲存草稿。",
      failedToDeleteDraft: "無法刪除草稿。",
      failedToScheduleEmailDraftKeptOpen: "定時傳送失敗 - 草稿仍保持開啟",
      finishRecipientInput: "請先新增收件人或清除輸入內容，再傳送。",
      pleaseAddRecipient: "請至少新增一個收件人",
      aiEngineRequired: "使用生成前，請先連線 Builder 或其他 AI 引擎。",
      couldNotUnsubscribe: "無法取消訂閱",
      unsubscribeSent: "取消訂閱請求已傳送",
      draftQueued: "草稿已排隊。",
      draftUpdated: "草稿已更新。",
      draftDismissed: "草稿已忽略。",
      openedInCompose: "已在撰寫視窗中開啟。",
      draftSent: "草稿已傳送。",
      draftClosed: "草稿已關閉。",
      draftsClosed: "已關閉 {{count}} 個草稿。",
      failedToSendDraft: "傳送草稿失敗。",
      snoozeDbNotReady:
        "稍後提醒資料庫未就緒。請在 mail 範本中執行：pnpm db:push。",
      couldNotSnooze: "無法稍後提醒 - 請檢查伺服器記錄。",
      snoozedUntil: "已稍後提醒至 {{time}}",
      snoozedConversationsUntil: "已將 {{count}} 個工作階段稍後提醒至 {{time}}",
    },
    error: {
      retrying: "正在重試...",
      tryAgainIn: "{{seconds}} 秒後重試",
      tryAgain: "重試",
      rateLimitTitle: "觸發 Gmail 速率限制",
      loadTitle: "無法載入郵件",
      rateLimitDescription: "最近對 Google 的請求過多。稍等片刻後會重試。",
      failedToConnect: "連線失敗。請重試。",
      failedToParseJson: "解析 JSON 失敗",
      missingGoogleCredentials: "在 JSON 中找不到 client_id 和 client_secret",
      failedToSaveCredentials: "儲存憑證失敗",
      someAccountsFailed: "無法載入：{{accounts}}",
    },
    googleConnect: {
      connectTitle: "連線你的 Google 帳號",
      heroDescription: "傳送和接收真實郵件。連線你的 Gmail 帳號即可開始。",
      setupIntro: "按照這些步驟連線你的 Google 帳號。大約需要 3 分鐘。",
      enableGmailApi: "啟用 Gmail API",
      enableGmailApiDescription:
        "開啟 Google Cloud Console，並在 Gmail API 上點選“啟用”。",
      enableGmailApiLink: "啟用 Gmail API",
      enablePeopleApi: "啟用 People API",
      enablePeopleApiDescription:
        "啟用 People API，以便撰寫郵件時自動補全聯絡人。",
      enablePeopleApiLink: "啟用 People API",
      configureConsent: "設定 OAuth 同意螢幕",
      configureConsentDescription:
        "應用名稱可任意填寫（例如“我的 Mail”），使用者型別選取“外部”，並將你的信箱新增為測試使用者。如果看到概覽頁面，說明同意螢幕已設定，可以跳到下一步。",
      configureConsentLink: "設定同意螢幕",
      createCredentials: "建立 OAuth 憑證",
      createCredentialsDescription:
        "1) 點選“+ 建立憑證”-> “OAuth 用戶端 ID”\n2) 選取“Web 應用”\n3) 新增此重新導向 URI：",
      createCredentialsLink: "建立憑證",
      uploadCredentialsJson: "上傳憑證 JSON",
      uploadCredentialsJsonDescription:
        "在憑證頁面點選“下載 JSON”，然後在這裡上傳。",
      uploadJson: "上傳 JSON",
      copy: "複製",
      copied: "已複製",
      saving: "正在儲存...",
      credentialsConfiguredSignIn:
        "憑證已設定。點選上方“使用 Google 登入”進行連線。",
      credentialsConfiguredConnect:
        "憑證已設定。點選上方“連線 Google”進行登入。",
      readyToConnect: "準備連線 - 使用你的 Google 帳號登入",
      connectBanner: "連線 Google 以傳送和接收真實郵件",
      ownerMismatch: "此帳號已連線到另一個登入",
      connectionFailed: "Google 連線失敗",
      signOutThenSignIn: "退出登入，然後使用 {{account}} 登入。",
      dismiss: "關閉",
      dismissNotice: "關閉 Google 登入提示",
      hideSetup: "隱藏設定",
      signOut: "退出登入",
    },
    gmailFilters: {
      account: "帳號",
      from: "發件人",
      to: "收件人",
      subject: "主題",
      gmailSearch: "Gmail 搜尋",
      selectAccount: "選取帳號",
      archive: "封存",
      markRead: "標為已讀",
      neverSpam: "永不標為垃圾郵件",
      neverImportant: "永不標為重要",
      important: "重要",
      notImportant: "不重要",
      star: "加星號",
      trash: "垃圾桶",
      applyLabel: "應用標籤",
      createLabel: "建立標籤",
      forwardTo: "轉發至",
      save: "儲存",
      cancel: "取消",
      editFilter: "編輯過濾器",
      deleteFilter: "刪除過濾器",
      deleteGmailFilter: "刪除 Gmail 過濾器",
      deleteGmailFilterDescription:
        "從 {{account}} 刪除此過濾器？這會直接更改 Gmail。",
      title: "Gmail 過濾器",
      description: "用於簡單發件人、主題和搜尋模式的伺服器端 Gmail 規則。",
      newFilter: "新過濾器",
      noFilters: "還沒有 Gmail 過濾器。",
    },
    aiFilter: {
      title: "AI 篩選",
      subtitle: "可復原的標籤，會根據你的決定學習。",
      rulesTitle: "規則",
      newRule: "新增規則",
      lunaBadge: "可用時使用 Luna",
      toggle: "切換 AI 篩選",
      autoFilterTitle: "自動篩選高信心郵件",
      autoFilterDescription:
        "只將明確符合的郵件移出收件匣，其餘郵件可在這裡審核。",
      autoFilterToggle: "自動篩選高信心郵件",
      thresholdLabel: "自動篩選信心門檻",
      labelName: "Gmail 標籤",
      labelHelp:
        "agent-native-filtered 會在 Gmail 和其他用戶端中顯示。這不是 Gmail 垃圾郵件。",
      reviewLabel: "審核已篩選郵件",
      instructionsTitle: "指示",
      instructionCount: "{{count}} 個指示",
      instructionPlaceholder:
        "例如：主旨包含「question for you, Steve」的郵件不需要",
      addInstruction: "新增",
      noInstructions: "尚無指示。",
      instructionExample: "試試「所有政治競選郵件都不需要」。",
      toggleInstruction: "切換指示：{{instruction}}",
      deleteInstruction: "刪除指示",
      activityTitle: "最近活動",
      suggestionCount: "{{count}} 個待審核",
      viewAll: "查看全部",
      noActivity: "尚無 AI 篩選活動。",
      unknownSender: "未知寄件者",
      noSubject: "（無主旨）",
      filterTitle: "使用 AI 篩選",
      keepTitle: "保留在收件匣",
      filterDescription:
        "這會封存 {{count}} 個對話，並新增 agent-native-filtered 標籤。",
      keepDescription: "這會將 {{count}} 個對話還原到收件匣，並讓篩選器學習。",
      labelNote: "新增 agent-native-filtered 標籤並封存對話。你可以隨時復原。",
      learningNote: "將郵件保留在收件匣，並讓篩選器不要重複這個錯誤。",
      learningProgress:
        "已確認 {{required}} 個範例中的 {{count}} 個。確認全部 {{required}} 個後會檢查近期郵件。",
      rememberLabel: "記住以處理未來郵件（選填）",
      correctLabel: "應該學習什麼？（選填）",
      rememberPlaceholder: "例如：來自政治競選的類似郵件都不需要",
      correctPlaceholder: "例如：這個寄件者是客戶，未來郵件請保留在收件匣",
      commentHint: "你的備註會成為可編輯的規則或學習範例。",
      filterButton: "篩選",
      keepButton: "保留",
      filteredToast: "已篩選 {{count}} 個對話。",
      keptToast: "已將 {{count}} 個對話保留在收件匣。",
      actionFailed: "無法更新 AI 篩選。",
      settingsFailed: "無法儲存 AI 篩選設定。",
      automationRulesLoadFailed: "無法載入分類規則。",
      instructionFailed: "無法儲存 AI 篩選指示。",
      skipInboxMode: "略過收件匣",
      spamMode: "垃圾郵件",
      tagMode: "標籤",
      aiTagsTitle: "AI 標籤",
      filteredMode: "已篩選",
      autoArchiveMode: "自動封存",
      manageSettings: "管理",
      askJev: "詢問 Jev",
      askJevPrompt:
        "幫我調整這條 Mail 規則：{{condition}}。先詢問我想變更什麼，再更新規則。",
      composerPlaceholder: "請 Jev 幫你整理收件匣…",
      ruleBackfillStarting: "正在將此規則套用到近期郵件…",
      ruleBackfillProgress:
        "正在套用到近期郵件：{{total}} 封中的 {{processed}} 封",
      ruleBackfillMatches: "近期有 {{count}} 封郵件符合",
      ruleBackfillNoMatches: "近期沒有郵件符合此規則。",
      ruleBackfillFailed: "無法將此規則套用到近期郵件。",
      ruleBackfillPartialFailure: "有 {{count}} 封郵件無法更新。",
      ruleBackfillUndoing: "正在還原近期郵件…",
      ruleBackfillUndoComplete: "已還原 {{count}} 封郵件",
      ruleBackfillReview: "檢視符合項目",
      importantMode: "重要",
      notImportantMode: "不重要",
      importantLabel: "AI 重要",
      reviewImportant: "查看重要郵件",
      importantPlaceholder: "例如：需要回覆或有截止日期的郵件",
      ruleAdded: "已新增 AI 規則。",
      instructionHelp: "使用自然語言",
      tagNamePlaceholder: "標籤名稱",
      spamLabelHint: "符合的郵件會新增標籤並移出收件匣。",
      spamPlaceholder: "例如：明顯是促銷或不需要的郵件",
      tagPlaceholder: "例如：網路商店的收據和訂單確認郵件",
      archivePlaceholder: "例如：無需關注的 GitHub 機器人通知",
      addShortcut: "按 ⌘ Enter 新增",
      previewTitle: "預覽最近郵件",
      previewDescription:
        "啟用新規則前，先檢查收件匣中的最近 20 封郵件。已封存或自動封存的郵件不會被評估。",
      previewScope: "僅限收件匣 · 已封存郵件排除",
      previewButton: "執行預覽",
      previewRunning: "正在檢查最近的收件匣郵件…",
      jevBadge: "Jev",
      jevMatchProbability: "Jev 符合機率 {{percent}}%",
      feedbackLabel: "勾選不應包含的符合項目",
      notSpamShort: "不是垃圾郵件",
      notMatchShort: "不符合",
      noMatch: "沒有符合項目",
      feedbackPlaceholder: "選填：告訴 AI 它誤解了什麼，或應該改為尋找什麼",
      refineButton: "更新規則並重新執行",
      instructionsUpdated: "規則已更新，預覽已重新執行。",
      noRecentMail: "沒有可供檢查的最近收件匣郵件。",
      addRuleToPreview: "新增 AI 規則以預覽最近郵件。",
      previewEmpty: "執行預覽後會顯示可能的符合項目。",
      previewFailed: "無法預覽最近郵件。",
      promptRulesCleared: "已移除分類規則。",
      tagTabsHelp: "每個標籤都會成為收件匣標籤頁",
      addTag: "新增標籤",
      triageTitle: "分類",
      connectJev: "連接 Jev",
      connectJevToRunTriage: "連接 Jev 以執行分類",
      freeBuilderOrApiKey: "透過 Builder.io 免費使用，或新增 API 金鑰。",
      jevAvailabilityFailed: "無法檢查 Jev 是否可用。",
      connectBuilder: "連接 Builder.io",
      addJevApiKey: "新增 API 金鑰",
    },
    draftQueue: {
      title: "草稿佇列",
      queueDraft: "排隊草稿",
      reviewer: "審核人",
      chooseMember: "選取成員",
      to: "收件人",
      cc: "抄送",
      bcc: "密送",
      subject: "主題",
      draft: "草稿",
      context: "脈絡",
      cancel: "取消",
      noSubject: "（無主題）",
      noQueuedDrafts: "沒有排隊草稿",
      emptyDescription: "隊友的新請求會顯示在這裡。",
      selectDraft: "選取一個排隊草稿進行審核。",
      requestedBy: "由 {{name}} 請求",
      requestedByMe: "我請求的",
      queuedFor: "{{time}} 排隊，給 {{owner}}",
      toRecipient: "收件人 {{recipient}}",
      fromRequester: "來自 {{requester}}",
      save: "儲存",
      compose: "撰寫",
      send: "傳送",
      awaitingApproval: "等待核准",
      newDraftRequest: "新草稿請求",
      needsOrg: "草稿佇列需要一個活動組織。",
      openSettings: "開啟設定",
      statusInReview: "審核中",
      awaitingApprovalCount: "{{count}} 個等待核准的草稿",
      requestedCount: "{{count}} 個已建立的草稿請求",
    },
    routeTitles: {
      notFound: "未找到 - Mail",
      draftQueue: "草稿佇列 - Mail",
      settings: "設定 - Mail",
      team: "團隊 - Mail",
      emailThread: "郵件工作階段",
      unableToLoadThread: "無法載入工作階段",
      openInApp: "在應用中開啟",
    },
    agentEngine: {
      activeEngine: "目前引擎",
      selectEngine: "選取引擎",
      engineSaved: "引擎已儲存。將在下一次對話中生效。",
      loadFailed: "無法載入引擎清單。請確認伺服器正在執行。",
    },
    contacts: {
      noContactSelected: "未選取聯絡人",
    },
  },
  agent: {
    emptyState: "可以問我任何關於郵件的問題",
    suggestionSummarize: "總結我的未讀郵件",
    suggestionReplies: "今天哪些郵件需要我回覆？",
    suggestionWidget: "為我的收件箱建立自訂小元件",
  },
  settings: {
    openAgentSettings: "管理代理",
    agentDescription: "管理代理的模型、API 金鑰、自動化、語音和其他控制項。",
    agentTitle: "管理代理",
    aliasName: "別名名稱",
    aliasNamePlaceholder: "例如：設計團隊",
    recipientsOnePerLine: "收件人（每行一個信箱）",
    save: "儲存",
    cancel: "取消",
    delete: "刪除",
    personSingular: "{{count}} 人",
    peoplePlural: "{{count}} 人",
    deleteAliasDescription: "刪除別名“{{name}}”？此操作無法撤銷。",
    aliasesDescription: "撰寫郵件時可使用的地址組。",
    newAlias: "新建別名",
    noAliases: "還沒有別名。建立一個即可開始。",
    applyLabel: "應用標籤",
    archive: "封存",
    markRead: "標為已讀",
    star: "加星號",
    trash: "移到垃圾桶",
    labelName: "標籤名稱",
    addAction: "+ 新增操作",
    ruleName: "規則名稱",
    ruleNamePlaceholder: "例如：自動標記新聞郵件",
    conditionNaturalLanguage: "條件（自然語言）",
    conditionPlaceholder:
      "例如：“來自新聞郵件或行銷郵件清單”\n“來自 alice@example.com”\n“主題包含 invoice 或 receipt”",
    actions: "操作",
    editRule: "編輯規則",
    deleteRule: "刪除規則",
    noEventAutomations: "還沒有郵件事件觸發的自動化。",
    eventAutomationsPrompt:
      "讓代理建立自動化，例如“當我收到老板的郵件時，加星號並通知我”。",
    disabled: "已停用",
    on: "在",
    when: "當",
    lastRun: "上次執行：",
    automationsDescription: "使用 AI 自動處理新收件箱郵件的規則。",
    allowAutomationSends: "允許自動化自動傳送郵件",
    allowAutomationSendsDescription:
      "預設為關閉。需要自動化每次傳送郵件時不必請求核准時，請開啟此選項。",
    automationSendSettingSaveFailed: "無法儲存自動郵件傳送設定。",
    loadingModels: "正在載入模型",
    newRule: "新建規則",
    noAutomationRules: "還沒有自動化規則。",
    noAutomationRulesDescription:
      "建立規則來自動標記郵件、封存新聞郵件、為重要郵件加星號等。你也可以讓 AI 代理為你設定。",
    eventTriggers: "事件觸發器",
    eventTriggersDescription:
      "郵件事件發生時觸發的自動化（例如收到新郵件）。由代理管理。",
    importedSignature: "已從 {{account}} 匯入簽名。",
    noGmailSignature: "未找到 {{account}} 的 Gmail 簽名。",
    importSignatureFailed: "匯入 Gmail 簽名失敗。",
    draftingSettingsSaved: "草稿設定已儲存。",
    draftingSettingsSaveFailed: "儲存草稿設定失敗。",
    draftingDescription: "撰寫和生成郵件草稿時使用的偏好設定。",
    signature: "簽名",
    importFromGmail: "從 Gmail 匯入",
    signatureHelp:
      "會新增到新草稿中，位於引用的回覆歷史之前。支援 Markdown 連結和圖片。",
    addSignatureImage: "新增圖片",
    signatureImageUploadFailed: "簽名圖片上傳失敗",
    writingStyle: "寫作風格",
    autocomplete: "自動完成",
    sendAndMarkDone: "傳送並標記為已完成",
    autocompleteSaveFailed: "無法儲存自動完成設定。",
    writingStylePlaceholder: "簡短、具體、溫暖。避免正式套話。",
    saveDraftingSettings: "儲存草稿設定",
    reset: "重設",
    trackingDescription:
      "了解收件人何時開啟你傳送的郵件以及點選連結。統計會顯示在每封已傳送郵件下方。",
    trackEmailOpens: "跟蹤郵件開啟",
    trackEmailOpensDescription:
      "在外發郵件中插入 1×1 像素，以檢視收件人何時開啟。",
    trackLinkClicks: "跟蹤連結點選",
    trackLinkClicksDescription:
      "重寫外發郵件中的外部連結，以統計收件人點選時間。",
    slackLoadFailed: "載入 Slack 狀態失敗",
    slackUpdateFailed: "更新 Slack 收件入口失敗",
    slackConfigured: "Slack 憑證已設定。",
    slackNeedsCredentials:
      "舊版自訂收件入口需要 SLACK_BOT_TOKEN 和 SLACK_SIGNING_SECRET。新的 Slack 訊息自動化請在設定 > 訊息中連接工作區。",
    slackIntake: "Slack 收件入口（舊版）",
    slackDescription: "允許組織成員從 Slack 排隊郵件草稿的舊版自訂整合。",
    enabled: "已啟用",
    disable: "停用",
    enable: "啟用",
    slackPostEndpoint: "Slack POST 端點",
    slackPostEndpointHelp:
      "用於 Slack Event Subscriptions。瀏覽器 GET 可能顯示 Not Found。",
    title: "設定",
    general: "通用",
    generalDescription: "Mail 的語言和帳號級偏好設定。",
    languageTitle: "語言",
    languageDescription:
      "選取此帳號的介面語言。Mail 會在不同設備上記住你的選取。",
    languageLabel: "介面語言",
    whatsNew: "最新變化",
    whatsNewDescription: "Agent-Native Mail 最近面向使用者的更新。",
    drafting: "起草",
    snippets: "片段",
    snippetsDescription:
      "可保存的常用回覆，輸入 / 和片段名稱即可插入撰寫中的草稿。",
    newSnippet: "新增片段",
    noSnippets: "尚無片段，建立一個開始使用。",
    snippetName: "片段名稱",
    snippetNamePlaceholder: "例如：報價後續跟進",
    snippetBody: "片段內容",
    snippetBodyPlaceholder: "使用此片段時插入的文字……",
    editSnippet: "編輯片段",
    deleteSnippet: "刪除片段",
    deleteSnippetDescription: "刪除片段「{{name}}」？此操作無法復原。",
    automations: "自動化",
    rules: "規則",
    rulesModel: "規則模型",
    rulesModelDescription: "將收到的郵件與你的規則進行比對。",
    slackDraftRequests: "草稿請求",
    slackDraftQueue: "從 Slack 將草稿加入佇列",
    slackDraftQueueDescription:
      "同事在 Slack 中提及代理即可請求郵件草稿。草稿會在你的草稿佇列中等待審閱。",
    openDraftQueue: "開啟草稿佇列",
    aiFilter: "分類",
    gmailFilters: "Gmail 篩選器",
    aliases: "別名",
    tracking: "跟蹤",
    slack: "Slack",
    deleteAlias: "刪除別名",
    editAlias: "編輯別名",
  },
  theme: {
    light: "淺色",
    dark: "深色",
  },
  notFound: {
    message: "找不到頁面",
    goToInbox: "前往收件箱",
  },
  commandPalette: {
    placeholder: "輸入指令或詢問 AI...",
    compose: "撰寫新郵件",
    reply: "回覆工作階段",
    snooze: "稍後提醒郵件",
    search: "搜尋郵件",
    refresh: "重新整理收件箱",
    reportSpam: "舉報垃圾郵件",
    reportSpamBlock: "舉報垃圾郵件並屏蔽發件人",
    muteThread: "靜音工作階段",
    navigate: "導覽",
    goToInbox: "前往收件箱",
    goToStarred: "前往星標",
    goToSent: "前往已傳送",
    goToDrafts: "前往草稿",
    goToAllMail: "前往所有郵件",
    goToArchive: "前往封存",
    goToTrash: "前往垃圾桶",
    enableAutocomplete: "啟用自動完成",
    disableAutocomplete: "關閉自動完成",
    privacy: "隱私",
    imagesShowAll: "圖片：全部顯示",
    imagesBlockTrackers: "圖片：屏蔽已知跟蹤器",
    imagesBlockAll: "圖片：屏蔽所有遠端圖片",
    appearance: "外觀",
    toggleLight: "切換到淺色模式",
    toggleDark: "切換到深色模式",
    actions: "操作",
    shortcuts: "鍵盤快速鍵",
    shortcutsGlobal: "全域",
    shortcutsList: "郵件清單",
    shortcutsThread: "對話",
    shortcutsCompose: "撰寫",
    backToCommands: "返回命令",
    backToMessageList: "返回郵件清單",
    cycleTabs: "切換分頁",
    extendSelection: "擴大選取範圍",
    moveSelection: "移動選取項目",
    nextPreviousConversation: "下一個／上一個對話",
    nextPreviousMessage: "下一封／上一封郵件",
    openMessage: "開啟郵件",
    selectAllConversations: "選取所有對話",
    sendAndMarkDone: "傳送並標記為完成",
    toggleMessageExpansion: "展開／收合郵件",
    toggleReadState: "切換已讀狀態",
  },
};

export default messages;
