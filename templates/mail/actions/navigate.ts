 ...
 *
 * Usage:
 *   pnpm action navigate --view=inbox
 *   pnpm action navigate --view=starred
 *   pnpm action navigate --threadId=thread-123
...
export default async function main(): Promise<void> {
  ...
  if (!args.view && !args.threadId) {
    fatal(
      "At least --view or --threadId is required. Usage: pnpm action navigate --view=inbox",
    );
  }
