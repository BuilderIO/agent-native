...
if (rel === 'templates/content/actions/update-document.ts' && stmt.line === 116) {
  console.error('  BLOCK at', cur.open, '-', cur.close, 'directText.len=', directText.length);
  console.error('  has assertAccess:', /\bassertAccess\s*\(/.test(directText));
  console.error('  has accessFilter:', /\baccessFilter\s*\(/.test(directText));
  console.error('  has resolveAccess:', /\bresolveAccess\s*\(/.test(directText));
  console.error('  blockHasAccessControl:', blockHasAccessControl(directText));
}
