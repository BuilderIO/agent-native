...
async function scanFiles(ownablesByDir) {
  ...
  if (rel === 'templates/content/actions/update-document.ts' && stmt.line === 116) {
    console.error('  INITIAL cur=', cur);
    const allWithOffset = blocks.filter(b => b.open <= stmt.queryStart && b.close >= stmt.queryStart);
    console.error('  all blocks containing query:', allWithOffset);
  }
      let levels = 0;
      while (cur && levels < 8) {
