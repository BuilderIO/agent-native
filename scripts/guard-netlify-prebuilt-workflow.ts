...
const betaMigrationStep = reusableSteps.find(
  (step) =>
    step?.name === "Run the beta release migration against the beta database",
);
...
if (
  ...
  betaMigrationEnv?.BUILD_CONTEXT !== "branch-deploy" ||
  ...
  !betaMigrationRun.includes("CONTEXT=branch-deploy") ||
  ...
  issues.push(
    `${reusablePath} must migrate each beta site's beta database after artifact validation and before publishing it`,
  );
}
