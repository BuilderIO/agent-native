# Appointment Blocker - Agent Guide

Appointment Blocker turns personal-inbox appointment details into buffered,
reviewable work-calendar blocks. Keep the three-step review surface primary:
capture, check, and confirm. Chat remains the handoff surface for connected
provider work.

## Workflow Contract

1. Read appointment lines from the connected personal inbox, or accept the
   clearly labeled local paste fallback.
2. Prepare one block per appointment with the requested buffer before and
   after. Duplicate invitation updates should be normalized before preparing.
3. Inspect the connected work calendar for overlaps and classify attendees as
   external when their email does not end in `@builder.io`.
4. Require an explicit confirmation before handing approved windows to the
   calendar agent. The local `apply-appointment-blocks` action records approval
   and never pretends that a provider event was created.

The current source format is one appointment per line:

```text
Appointment | Wed Oct 7, 2026 9am - 9:30am (PDT)
```

Calendar conflict checks accept one event per line with optional attendees:

```text
Internal sync | Wed Oct 7, 2026 8:30am - 9am | attendees: teammate@builder.io
```

ChatGPT and Claude import is intentionally a coming-soon boundary. Do not add
provider-specific import code to this app until that integration is designed.

## Appointment Actions

| Action                        | Purpose                                             |
| ----------------------------- | --------------------------------------------------- |
| `prepare-appointment-blocks`  | Parse appointments and persist the current plan     |
| `get-appointment-plan`        | Read the current reviewable plan                    |
| `check-appointment-conflicts` | Compare buffered windows with calendar events       |
| `apply-appointment-blocks`    | Record explicit approval for calendar-agent handoff |

All four actions are the shared surface for the UI and agent. The final
calendar write is intentionally delegated to the connected calendar agent so
the provider-specific capability remains visible and failure-reportable.
...
- `navigation` should describe the current view and selected entity ids. The
  workflow view is `workflow` at `/workflow`; the full-page chat view is
  `chat` at `/chat`.
...
The review selection is stored under `appointment-plan` with the plan id,
review status, and conflict status. Keep this state small and derived from the
