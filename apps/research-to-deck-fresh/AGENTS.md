...
## Research to deck brief

- Navigate to `/research-to-deck` for the notes-to-brief workflow. The full-page
  agent chat remains at `/` and `/chat/:threadId`.
- `build-deck-brief` accepts `{ sourceText }` and returns a deterministic local
  outline with an executive snapshot, themes, suggested flow, decisions/asks,
  and next steps. It does not call an external provider or create a deck.
- The page exposes a compact `selection` app-state record with the current
  source note count and whether a brief is available. Use `view-screen` before
  refining the visible brief. `Refine with agent` sends bounded notes and the
  current outline to the right AgentSidebar with `openSidebar: true`.
