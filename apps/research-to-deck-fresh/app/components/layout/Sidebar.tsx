...
function persistedActiveThreadId() {
  try {
    ...
    console.warn("Unable to read active chat thread preference", error);
    return null;
  }
}
...
function persistActiveThreadId(threadId: string) {
  try {
    ...
    console.warn("Unable to persist active chat thread preference", error);
  }
}
...
function threadIdFromPath(pathname: string) {
  ...
  try {
    ...
    // coercion-ok: malformed URL encodings have no active thread id.
    return null;
  }
}
