# email-automation-fresh — Development Guide
...
app/                   # React frontend
  ...
  routes/              # File-based page routes (auto-discovered)
    ...
    automations.tsx    # /automations (email preview console)
...
The `/automations` page is the durable workflow surface. Its shared actions
persist one daily-digest configuration per owner, create preview-only test
runs, and list recent runs. `server/plugins/db.ts` owns additive local/hosted
schema migrations, while `server/lib/email-automation.ts` keeps ownership
checks and preview shaping in one boundary.

The primary local action is **Run test now**: it saves the current draft,
creates a durable styled preview, refreshes run history, and opens the
AgentSidebar with only the automation/run identifiers and bounded status
context. **Ask agent** on a selected run uses the same sidebar handoff. No
email is sent by this MVP.
