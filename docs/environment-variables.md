...
| `APPLE_*` / `MACOSX_*` / `GGML_*` / `NEON_*` / `CORE_CLI_VERSION` / `RELEASE_VERSION` / `RELEASE_VERSION_INPUT` / `RELEASE_CHANNEL` / `RELEASE_DIST_TAG` / `RELEASE_NAME_PREFIX` / `RELEASE_POINTER_TAG` / `RELEASE_TAG_PREFIX` / `EP_PRE_RELEASE` | Desktop signing, native build, preview-database, and CLI release settings.                  |
...
| `CHAT_FIRST_SCREENSHOT_DIR` / `CHAT_FIRST_ELECTRON` / `CHAT_FIRST_ELECTRON_EXECUTABLE` / `CHAT_FIRST_COLOR_SCHEME`             | Chat-first workbench QA output, optional Electron lane toggle, binary override, and light/dark capture scheme. |
...
| `agent.runNoProgressTimeoutMs`                | `AGENT_RUN_NO_PROGRESS_TIMEOUT_MS`                                          | number  | `150000`  | Ceiling for the foreground no-progress backstop, in milliseconds. The effective foreground value is still clamped to a fraction of the chunk's soft timeout. 0 disables it. |
...
| `agent.backgroundSoftTimeoutCeilingMs`        | `AGENT_BACKGROUND_SOFT_TIMEOUT_CEILING_MS`                                  | number  | `780000`  | Largest chunk budget a background-function run may use, in milliseconds. Must stay under the host's real background-function wall.                                          |
...
| `agent.modelStreamNoProgressTimeoutMs`        | `AGENT_MODEL_STREAM_NO_PROGRESS_TIMEOUT_MS`                                 | number  | `90000`   | In-loop watchdog for silence between engine stream frames, in milliseconds.                                                                                                 |
| `agent.actionPreparationNoProgressTimeoutMs`  | `AGENT_ACTION_PREPARATION_NO_PROGRESS_TIMEOUT_MS`                           | number  | `90000`   | In-loop watchdog for silence while an action's arguments stream in, in milliseconds.                                                                                        |
| `agent.maxRunLoopContinuations`               | `AGENT_MAX_RUN_LOOP_CONTINUATIONS`                                          | number  | `6`       | Continuation rounds allowed inside one foreground agent-loop invocation.                                                                                                    |
| `agent.maxBackgroundRunLoopContinuations`     | `AGENT_MAX_BACKGROUND_RUN_LOOP_CONTINUATIONS`                               | number  | `20`      | Continuation rounds allowed inside one background agent-loop invocation.                                                                                                    |
| `agent.maxBackgroundRunContinuations`         | `AGENT_MAX_BACKGROUND_RUN_CONTINUATIONS`                                    | number  | `20`      | Server-driven background chunks a single logical turn may chain. A cost ceiling.                                                                                            |
| `agent.maxConsecutiveNoProgressContinuations` | `AGENT_MAX_CONSECUTIVE_NO_PROGRESS_CONTINUATIONS`                           | number  | `2`       | Consecutive chunks allowed to end on the same terminal code having produced nothing before the chain stops.                                                                 |
| `agent.maxTurnWallClockMs`                    | `AGENT_MAX_TURN_WALL_CLOCK_MS`                                              | number  | `5400000` | Absolute wall-clock ceiling on one logical turn across all its chunks, in milliseconds.                                                                                     |
