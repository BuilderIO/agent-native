...
| `AGENT_NATIVE_GOOGLE_OAUTH_RELAY_SECRET`                                       | Dedicated shared HMAC for the fixed Beta Dispatch callback used by immutable Netlify preview Google sign-in. Set the same 32+ character value on Beta Dispatch and preview contexts; never replace production app secrets. |
