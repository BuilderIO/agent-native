...
| `APPLE_*` / `MACOSX_*` / `GGML_*` / `NEON_*` / `CORE_CLI_VERSION` / `RELEASE_VERSION` / `RELEASE_CHANNEL` / `RELEASE_DIST_TAG`                | Desktop signing, native build, preview-database, and CLI release settings.                  |
