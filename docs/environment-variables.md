...
| `AUTO_*` / `CLEAR_*` / `DRY_RUN` / `MAX_AGE_MINUTES` / `REBUILD` / `PUBLISHED` / `PUBLISHED_PACKAGES` / `REDISPATCH` / `DIFF_*` / `IS_FORK` / `SHOT_*` / `SUPPRESSED` | Internal deployment and visual-recap workflow controls.                                     |
