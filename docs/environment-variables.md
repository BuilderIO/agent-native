...
| `APPLE_*` / `MACOSX_*` / `GGML_*` / `NEON_*` / `CORE_CLI_VERSION` / `RELEASE_VERSION` / `RELEASE_CHANNEL` / `RELEASE_DIST_TAG` / `RELEASE_NAME_PREFIX` / `RELEASE_POINTER_TAG` / `RELEASE_TAG_PREFIX` / `EP_PRE_RELEASE` | Desktop signing, native build, preview-database, and CLI release settings.                  |
