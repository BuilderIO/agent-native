...
| `observability.aiTelemetryEnabled`            | `AGENT_NATIVE_OBSERVABILITY`                                                                                             | boolean | `true`                                       | Capture agent run, model call, and tool call traces.                                                                                                                                                                                                      |
| `observability.enabled`                       | —                                                                                                                        | boolean | —                                            | Deprecated alias for `aiTelemetryEnabled`. Set that instead; this wins while both are present.                                                                                                                                                            |
...
| `observability.httpTelemetryDisabled`         | `AGENT_NATIVE_HTTP_TELEMETRY_DISABLED`                                                                                   | boolean | `false`                                      | Stop emitting `http.response` request telemetry entirely.                                                                                                                                                                                                 |
| `observability.httpTelemetrySampleRate`       | `AGENT_NATIVE_HTTP_TELEMETRY_SAMPLE_RATE`                                                                                | number  | `0.1`                                        | Fraction of fast successful requests that emit `http.response`, 0 to 1. Cold, slow, failing, and 4xx action requests are always kept regardless.                                                                                                          |
