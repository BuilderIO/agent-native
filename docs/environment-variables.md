...
| `OPENAI_*` / `ANTHROPIC_API_KEY` / `OPENROUTER_API_KEY` / `ORCAROUTER_API_KEY` / `GEMINI_API_KEY` / `GOOGLE_GENERATIVE_AI_API_KEY` / `GROQ_API_KEY` / `MISTRAL_API_KEY` / `COHERE_API_KEY` | Built-in agent provider keys and OpenAI-compatible endpoint configuration. Prefer scoped provider keys for multi-tenant apps.                                |
