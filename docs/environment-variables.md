...
| `AGENT_NATIVE_RELEASE_MIGRATIONS`                                  | Set by an app that runs its migrations at release time, which lets request functions skip schema work. Only set it where a release migration step actually exists — otherwise skipping means the migrations never run at all. |
