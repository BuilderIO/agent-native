...
| `CONCLUSION`                                                                   | GitHub Actions workflow-run conclusion passed to failure-notification jobs.                                                                                                 |
...
| `MIGRATION_ONLY`                                                               | Internal GitHub Actions marker for the separate release-migration job; do not set manually.                                                                                 |
| `SKIP_BUILD_MIGRATIONS`                                                        | Internal GitHub Actions marker that prevents a prebuilt docs build from rerunning its already-completed release migration.                                                  |
...
| `PUBLISH_AFTER_FRESHNESS`                                                      | Internal beta deploy marker that keeps a first publish as a draft until the source is revalidated against `main`.                                                           |
| `ARTIFACT_DOWNLOAD`                                                            | Internal marker selecting the downloaded GitHub Actions artifact for a trusted PR preview upload.                                                                           |
| `TARGET_FUNCTIONS_DIRECTORY`                                                   | Internal target path for serverless functions before an artifact-only preview upload selects its downloaded path.                                                           |
| `TARGET_PUBLISH_DIRECTORY`                                                     | Internal target path for static files before an artifact-only preview upload selects its downloaded path.                                                                   |
...
| `ACTION` / `ACTOR` / `CALLER` / `DEPLOY` / `METHOD` / `OPERATION` / `TEMPLATE` / `WORKSPACE` / `TARGET` / `ROUTE` / `REF` / `SHA` / `TRUSTED_REPOSITORY`                                                                                                                        | Trusted-acceptance and deployment workflow selectors.                                       |
...
| `access.signup`                               | `AUTH_SIGNUP`                                                                                                            | enum    | `"open"`                                     | Whether new accounts can sign up openly or need an organization invitation, an allowed email domain, or bootstrap-admin status.                                                                                                                           |
| `access.orgCreation`                          | `ORG_CREATION`                                                                                                           | enum    | `"open"`                                     | Whether authenticated users can create organizations. Closed deployments allow only configured bootstrap admins to create the canonical org.                                                                                                              |
| `access.autoCreateDefaultOrg`                 | `AUTO_CREATE_DEFAULT_ORG`                                                                                                | boolean | `true`                                       | Whether a user without memberships gets a personal default org. Ignored when organization creation is closed.                                                                                                                                             |
| `access.bootstrapAdmins`                      | `AUTH_BOOTSTRAP_ADMINS`                                                                                                  | array   | `[]`                                         | Comma-separated verified email addresses allowed to bootstrap the workspace organization.                                                                                                                                                                 |
| `access.sso.enabled`                          | `AUTH_SSO`                                                                                                               | boolean | `false`                                      | Enable opt-in organization SSO provider management.                                                                                                                                                                                                       |
| `access.scim.enabled`                         | `AUTH_SCIM`                                                                                                              | boolean | `false`                                      | Enable opt-in organization SCIM provisioning.                                                                                                                                                                                                             |
| `access.scim.credentialHashSecret`            | `AUTH_SCIM_CREDENTIAL_HASH_SECRET`                                                                                       | string  | —                                            | Independent secret used to hash framework-managed SCIM bearer credentials.                                                                                                                                                                                |
...
| `runtime.vercelBranchUrl`                     | `VERCEL_BRANCH_URL`                                                                                                      | string  | —                                            | Platform-provided Vercel branch URL used to address the current preview deployment.                                                                                                                                                                       |
