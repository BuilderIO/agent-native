# Deck Generator — Agent Architecture
...
┌─────────────────────┐         ┌─────────────────────┐
│  Frontend           │         │  Agent Chat         │
│  (React + Vite)     │◄───────►│  (this agent)       │
│                     │  files  │                     │
│  - reads/writes     │         │  - reads/writes     │
│    files via API    │         │    files + code     │
│  - sends prompts to │         │  - runs scripts     │
│    agent chat via    │         │    via pnpm script  │
│    agentChat util   │         │  - generates code   │
└────────┬────────────┘         └──────────┬──────────┘
         │                                 │
         │         ┌───────────────┐       │
         └────────►│  Backend      │◄──────┘
...
The app can delegate tasks to the agent chat using the `agentChat` utility (`shared/agent-chat.ts`). This lets any UI button or action trigger an agentic flow with full conversational follow-up.
...
The utility sends messages via two channels simultaneously:

1. **console.log** — `BUILDER_PARENT_MESSAGE:{"message":{"type":"builder.submitChat","data":{...}},"targetOrigin":"*"}`
   Works in any context (scripts, server, browser). The electron app picks this up from stdout.

2. **window.parent.postMessage** — `{type:"builder.submitChat", data:{...}}`
   Works from the iframe (browser context). The harness picks this up from the message event.
...
```typescript
{
  ...
  data: {
    message: string,    // Visible prompt text
    context?: string,   // Hidden context (not shown to user, sent to AI)
...
The editor exposes the current selection state so the agent can access it:
...
└── agent-chat.ts              # Utility to send messages to agent chat
...
3. **Agent chat for complex flows** — use `agentChat.submit()` to delegate multi-step operations to the agent, especially when follow-up conversation is valuable (image generation, content refinement, etc.)
