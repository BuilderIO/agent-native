...
const brands = [
  'greylock.com', 'microsoft.com',
  'zapier.com', 'jcrew.com', 'panasonic.com', 'scale.com',
  'ocbc.com', 'pendo.io', 'clickup.com', 'harrys.com'
];
...
async function fetchLogos() {
  ...
  for (const domain of brands) {
    try {
      ...
      let lightLogo = logos.find(l => l.type === 'logo' && l.theme === 'light');
      ...
      if (logo && logo.formats && logo.formats.length > 0) {
        ...
        results[domain] = { url: chosen.src, format: chosen.format, theme: logo.theme };
        console.log(domain, '-> theme:', logo.theme, 'format:', chosen.format, 'url:', chosen.src);
      } else {
        console.log(domain, 'NO LOGO FOUND. Available types:', logos.map(l => l.type + '/' + l.theme).join(', '));
      }
    } catch (e) {
