...
This release provides the four headless protocol, client, adapters, and
conformance packages. React composition and Core chat UI migration below
describe follow-on presentation work; they are not included in this release.
...
Applications install the focused `@agent-native/agentkit-protocol`,
`@agent-native/agentkit-client`, `@agent-native/agentkit-adapters`, and
`@agent-native/agentkit-conformance` packages as needed. The protocol is
dependency-free; the other packages depend only on the protocol at runtime.
Servers, native clients, and alternate renderers can use these contracts
without a React binding or facade package.
...
Package APIs follow each package's version. The four headless packages are
tested together. Network discovery offers
