...
function MembershipEditor({
  ...
  const [rank, setRank] = useState<CreativeContextMembershipRank>(
    membership.rank,
  );
  ...
  return (
    <article className="rounded-md border border-border p-3">
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-2">
        ...
        >
          <SelectTrigger aria-label="Context role">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <Button
          ...
          size="sm"
          disabled={disabled}
          onClick={() => onSave({ rank, purpose, note })}
        >
          Save
        </Button>
...
export function CreativeContextShareTab({
  ...
  const availableContexts = contexts.filter(
    (context) => !activeContextIds.has(context.id),
  );
      ...
      <Tabs defaultValue="contexts" className="mt-4">
              ...
              onSave={(input) =>
                void manageMembership
                  .mutateAsync({
                    ...
                    expectedUpdatedAt: membership.updatedAt ?? undefined,
                    ...input,
                  })
                  .then(refresh)
                  .catch(() => setError("Could not update this membership."))
              }
              onWithdraw={() =>
                void manageMembership
                  .mutateAsync({
                  ...
                  .then(refresh)
                  .catch(() => setError("Could not withdraw this membership."))
              }
              onRemove={() =>
                void manageMembership
                  .mutateAsync({
                  ...
                  .then(refresh)
                  .catch(() => setError("Could not remove this membership."))
              }
            />
          ))}
          {!memberships.length && !membershipsQuery.isLoading ? (
            <p className="text-sm text-muted-foreground">
              This resource has not been added to a context.
            </p>
          ) : null}
          {canManage ? (
        ...
        >
          <div className="flex gap-2 rounded-md border border-border p-3">
            ...
            <p>
              Open contexts accept members immediately. Review contexts keep
              additions pending. Admins-only contexts can only be changed by
              administrators.
            </p>
          </div>
          {memberships.map((membership) => (
            <p key={membership.id}>
              <span className="font-medium text-foreground">
                {membership.context?.name ?? membership.contextId}:
              </span>{" "}
              {policyCopy(membership.context?.policy)}
            </p>
          ))}
        </TabsContent>
      </Tabs>
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-full overflow-y-auto sm:max-w-lg">
        <SheetHeader>
          ...
          <SheetDescription>
            Place this resource in the contexts where it should guide future
            work.
          </SheetDescription>
        </SheetHeader>
