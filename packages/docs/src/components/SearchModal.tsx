...
function highlightMatch(text: string, query: string) {
  ...
  return (
    <>
      ...
      <mark className="rounded-sm bg-[var(--accent)]/20 px-0.5 text-[var(--accent)]">{match}</mark>
    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[15vh]" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center gap-3 border-b border-[var(--border)] px-4 py-3">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--fg-secondary)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0">
              ...
              {results.map((entry, i) => (
                <button
                  ...
                  className={`flex w-full flex-col gap-1 px-4 py-3 text-left transition ${
                    i === activeIdx ? 'bg-[var(--accent)]/10' : 'hover:bg-[var(--bg-secondary)]'
                  }`}
                >
                  <div className="flex items-center gap-2">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={i === activeIdx ? 'var(--accent)' : 'var(--fg-secondary)'} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="shrink-0">
                      <path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z" />
                      <polyline points="13 2 13 9 20 9" />
                    </svg>
                    <span className="text-xs text-[var(--fg-secondary)]">{entry.page}</span>
                    <span className="text-xs text-[var(--fg-secondary)]">›</span>
                    <span className={`text-sm font-medium ${i === activeIdx ? 'text-[var(--accent)]' : 'text-[var(--fg)]'}`}>
                      {entry.section}
                    </span>
                    {i === activeIdx && (
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--fg-secondary)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="ml-auto shrink-0">
        ...
        {results.length > 0 && (
          <div className="flex items-center gap-4 border-t border-[var(--border)] px-4 py-2 text-[10px] text-[var(--fg-secondary)]">
            ...
            <span className="inline-flex items-center gap-1">
              <kbd className="rounded border border-[var(--border)] px-1 py-0.5">esc</kbd>
              close
            </span>
          </div>
        )}
