...
function AppResourceEffectiveStack({
  appId,
  resource,
}: {
  appId: string;
            ...
            >
              ...
              {layer.resource ? (
                <div className="mt-2 text-[11px] text-muted-foreground">
                  Updated {formatTimestamp(layer.resource.updatedAt)}
                </div>
              ) : (
                <div className="mt-2 text-[11px] text-muted-foreground">
...
function appAvailabilityLabel(value?: string) {
  switch (value) {
    case "all-apps":
...
function appLayerState(layer: any): { label: string; className: string } {
  if (layer.effective) {
    return {
      label: "Wins",
