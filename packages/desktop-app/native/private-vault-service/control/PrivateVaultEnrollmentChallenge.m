...
AncPrivateVaultEnrollmentChallengeVerify(
  ...
  if (encodedChallenge.length == 0 || encodedChallenge.length > 64 * 1024 ||
      state == nil || nowSeconds == 0 || authenticatedHeadSignedAtSeconds == 0 ||
  ...
  NSSet<NSNumber *> *keys = [NSSet setWithArray:@[
    @1, @2, @3, @4, @5, @170, @171, @172, @173, @174, @175, @176, @177,
    @178, @179, @180, @181, @182
  ]];
  ...
  BOOL valid = suite != nil && [suite.textValue isEqualToString:@"anc/v1"] &&
               ...
               Exact(authorizerSigning.bytesValue, 32) &&
               authorizerAgreement != nil &&
  ...
  AncPrivateVaultEnrollmentOfferResult *offer =
      AncPrivateVaultEnrollmentOfferVerify(encodedOffer, proof.bytesValue,
                                           vaultId, &offerStatus);
  if (offer == nil) {
    SetStatus(status, offerStatus == AncPrivateVaultEnrollmentOfferStatusCryptoFailed
  ...
  if (authorizer == nil || ![authorizer.role isEqualToString:@"endpoint"] ||
      ...
      !Same(authorizer.keyAgreementPublicKey, authorizerAgreement.bytesValue) ||
  ...
  NSMutableData *signedMessage =
      [NSMutableData dataWithBytes:kChallengeDomain length:sizeof kChallengeDomain];
  [signedMessage appendData:unsignedChallenge];
  BOOL signatureValid = anc_pv_ed25519_verify(
                            signature.bytesValue.bytes, signedMessage.bytes,
                            signedMessage.length,
                            authorizerSigning.bytesValue.bytes) ==
                        ANC_PV_CRYPTO_OK;
  anc_pv_zeroize(signedMessage.mutableBytes, signedMessage.length);
  if (!signatureValid) {
    SetStatus(status, AncPrivateVaultEnrollmentChallengeStatusInvalidSignature);
    return nil;
  }
  NSDictionary<NSNumber *, AncPrivateVaultCanonicalValue *> *sasMap = @{
    ...
    @328 : [AncPrivateVaultCanonicalValue bytes:authorizerAgreement.bytesValue],
  ...
  NSData *challengeHash = DomainHash(kChallengeDomain, sizeof kChallengeDomain,
                                     encodedChallenge);
  ...
  AncPrivateVaultEnrollmentChallengeResult *result = class_createInstance(
  ...
  result.candidateKeyAgreementPublicKey =
      [offer.keyAgreementPublicKey copy];
  ...
  result.authorizerKeyAgreementPublicKey = [authorizerAgreement.bytesValue copy];
