...
static int anc_pv_any_range_overlap(const AncPrivateVaultMemoryRange *ranges,
  ...
  for (size_t left = 0; left < count; left += 1) {
    for (size_t right = left + 1; right < count; right += 1) {
      if (anc_pv_ranges_overlap(ranges[left], ranges[right]))
        return 1;
    }
  }
  return 0;
...
static int anc_pv_valid_id(const uint8_t *value, size_t length, int required) {
  if (length > ANC_PV_CUSTODY_ID_BYTES || (required && length == 0))
    return 0;
  ...
  @autoreleasepool {
    ...
    return string != nil &&
           [string dataUsingEncoding:NSUTF8StringEncoding].length == length;
  }
}
...
static int anc_pv_decode_id(const uint8_t *field, uint8_t *output,
  ...
  size_t length = 0;
  while (length < ANC_PV_CUSTODY_ID_BYTES && field[length] != 0)
    length += 1;
  for (size_t index = length; index < ANC_PV_CUSTODY_ID_BYTES; index += 1) {
    if (field[index] != 0)
      return 0;
  }
  if (!anc_pv_valid_id(field, length, required))
    return 0;
...
static int
anc_pv_valid_numbers(const AncPrivateVaultCustodySnapshot *snapshot) {
  const uint64_t values[] = {
      snapshot->custody_generation, snapshot->active_epoch,
      snapshot->pending_epoch,      snapshot->recovery_generation,
      snapshot->anchored_sequence,  snapshot->signed_at_ms,
      snapshot->freshness_ms,       snapshot->expected_next_sequence,
      snapshot->removal_sequence,   snapshot->removal_time_ms,
  };
  for (size_t index = 0; index < sizeof values / sizeof values[0]; index += 1) {
    if (!anc_pv_safe_integer(values[index]))
      return 0;
  }
...
static int
anc_pv_valid_public_state(const AncPrivateVaultCustodySnapshot *snapshot) {
  if ((snapshot->record_version != ANC_PV_CUSTODY_LEGACY_VERSION &&
       snapshot->record_version != ANC_PV_CUSTODY_VERSION) ||
      snapshot->authority_anchor_present > 1 ||
      snapshot->expected_edge_present > 1 || !anc_pv_valid_enums(snapshot) ||
      !anc_pv_valid_numbers(snapshot) ||
      !anc_pv_valid_id(snapshot->vault_id, snapshot->vault_id_length, 1) ||
      !anc_pv_valid_id(snapshot->endpoint_id, snapshot->endpoint_id_length,
                       1) ||
  ...
  if (!snapshot->authority_anchor_present) {
    if (snapshot->anchored_sequence != 0 ||
        anc_pv_is_nonzero(snapshot->anchored_head, ANC_PV_HASH_BYTES) ||
        anc_pv_is_nonzero(snapshot->membership_digest, ANC_PV_HASH_BYTES) ||
      ...
      return 0;
             ...
             anc_pv_is_zero(snapshot->membership_digest, ANC_PV_HASH_BYTES) ||
             snapshot->signed_at_ms == 0 ||
  ...
  if (!snapshot->authority_anchor_present) {
    ...
    return snapshot->expected_next_sequence == expectedGenesisSequence &&
           anc_pv_is_zero(snapshot->expected_previous_head, ANC_PV_HASH_BYTES);
  }
  if (snapshot->anchored_sequence == kAncPrivateVaultMaxSafeInteger ||
      snapshot->expected_next_sequence != snapshot->anchored_sequence + 1) {
    return 0;
  }
  return anc_pv_memcmp(snapshot->expected_previous_head,
                       snapshot->anchored_head,
                       ANC_PV_HASH_BYTES) == ANC_PV_CRYPTO_OK;
}

static int
anc_pv_valid_state_matrix(const AncPrivateVaultCustodySnapshot *snapshot,
  ...
  if (!pending) {
    return snapshot->lifecycle == ANC_PV_CUSTODY_LIFECYCLE_ACTIVE &&
           snapshot->rotation_phase == ANC_PV_CUSTODY_ROTATION_NONE &&
           snapshot->enrollment_phase == ANC_PV_CUSTODY_ENROLLMENT_NONE &&
           snapshot->ceremony_id_length == 0 && snapshot->active_epoch > 0 &&
           has_active_key && snapshot->authority_anchor_present &&
  ...
  if (snapshot->pending_kind == ANC_PV_CUSTODY_PENDING_ADD_DEVICE ||
    ...
    if (snapshot->enrollment_phase == ANC_PV_CUSTODY_ENROLLMENT_OFFER_PENDING) {
...
static void anc_pv_clear_secret_outputs(
  ...
  if (secrets == NULL)
    return;
  if (secrets->signing_seed != NULL)
    anc_pv_zeroize(secrets->signing_seed, 32);
  if (secrets->box_seed != NULL)
...
static void anc_pv_clear_disjoint_decode_outputs(
  ...
  if (snapshot != NULL &&
      !anc_pv_ranges_overlap(
          source, (AncPrivateVaultMemoryRange){snapshot, sizeof *snapshot})) {
...
anc_pv_custody_record_encode(const AncPrivateVaultCustodySnapshot *snapshot,
  ...
  const int top_level_valid = snapshot != NULL && record != NULL &&
                              secrets != NULL &&
  ...
  if (top_level_valid) {
    ...
    if (anc_pv_valid_secret_pointers(&captured_inputs)) {
      ...
      const AncPrivateVaultMemoryRange non_record_ranges[] = {
          snapshot_range,  descriptor_range, input_ranges[0], input_ranges[1],
          input_ranges[2], input_ranges[3],  input_ranges[4],
      };
      if (anc_pv_any_range_overlap(non_record_ranges,
                                   sizeof non_record_ranges /
                                       sizeof non_record_ranges[0])) {
        anc_pv_zeroize(record, record_length);
        return ANC_PV_CUSTODY_INVALID_ARGUMENT;
      }
    }
  ...
  record[ANC_PV_OFF_FLAGS] = (snapshot->authority_anchor_present
                                  ? ANC_PV_CUSTODY_FLAG_AUTHORITY_ANCHOR_PRESENT
                                  : 0) |
                             (snapshot->expected_edge_present
                                  ? ANC_PV_CUSTODY_FLAG_EXPECTED_EDGE_PRESENT
                                  : 0);
  ...
  memcpy(record + ANC_PV_OFF_SIGNING_PUBLIC_KEY, snapshot->signing_public_key,
         32);
  ...
  memcpy(record + ANC_PV_OFF_ACTIVE_EPOCH_KEY, captured_inputs.active_epoch_key,
         32);
  ...
  anc_pv_write_u64(record + ANC_PV_OFF_REMOVAL_TIME, snapshot->removal_time_ms);
  AncPrivateVaultCustodyRecordStatus status =
      anc_pv_checksum(record + ANC_PV_OFF_CHECKSUM, record);
  if (status != ANC_PV_CUSTODY_OK)
    anc_pv_zeroize(record, record_length);
  return status;
}
...
AncPrivateVaultCustodyRecordStatus anc_pv_custody_record_decode(
  ...
  const int top_level_valid = record != NULL && snapshot != NULL &&
                              secret_outputs != NULL &&
                              record_length == ANC_PV_CUSTODY_RECORD_BYTES;
  if (!top_level_valid) {
    ...
    if (anc_pv_ranges_overlap(source, descriptor_range) ||
      ...
      if (snapshot != NULL && !anc_pv_ranges_overlap(source, snapshot_range)) {
        anc_pv_custody_snapshot_zero(snapshot);
      }
      return ANC_PV_CUSTODY_INVALID_ARGUMENT;
    }
    AncPrivateVaultCustodySecretOutputs captured_outputs = {0};
    if (secret_outputs != NULL)
      captured_outputs = *secret_outputs;
  ...
  const AncPrivateVaultMemoryRange snapshot_range = {snapshot,
                                                     sizeof *snapshot};
  const AncPrivateVaultMemoryRange descriptor_range = {secret_outputs,
                                                       sizeof *secret_outputs};
  if (anc_pv_ranges_overlap(source, descriptor_range)) {
  ...
  const uint16_t record_version = anc_pv_read_u16(record + ANC_PV_OFF_VERSION);
  const uint8_t flags = record[ANC_PV_OFF_FLAGS];
  if (memcmp(record + ANC_PV_OFF_MAGIC, kAncPrivateVaultCustodyMagic, 4) != 0 ||
      (record_version != ANC_PV_CUSTODY_LEGACY_VERSION &&
       record_version != ANC_PV_CUSTODY_VERSION) ||
      anc_pv_read_u16(record + ANC_PV_OFF_LENGTH) !=
          ANC_PV_CUSTODY_RECORD_BYTES ||
      (record_version == ANC_PV_CUSTODY_LEGACY_VERSION && flags != 0) ||
      (record_version == ANC_PV_CUSTODY_VERSION && (flags & 0xfc) != 0) ||
      !anc_pv_is_zero(record + ANC_PV_OFF_FLAGS + 1, 2) ||
  ...
  if (checksum_status != ANC_PV_CUSTODY_OK)
  ...
  if (!anc_pv_decode_id(record + ANC_PV_OFF_VAULT_ID, snapshot->vault_id,
      ...
      !anc_pv_decode_id(record + ANC_PV_OFF_CEREMONY_ID, snapshot->ceremony_id,
                        &snapshot->ceremony_id_length,
                        snapshot->pending_kind !=
                            ANC_PV_CUSTODY_PENDING_NONE)) {
    goto invalid;
  }
  memcpy(captured_outputs.signing_seed, record + ANC_PV_OFF_SIGNING_SEED, 32);
  memcpy(snapshot->signing_public_key, record + ANC_PV_OFF_SIGNING_PUBLIC_KEY,
         32);
  ...
  memcpy(captured_outputs.local_state_key, record + ANC_PV_OFF_LOCAL_STATE_KEY,
         32);
  ...
  snapshot->removal_time_ms = anc_pv_read_u64(record + ANC_PV_OFF_REMOVAL_TIME);

  if (record_version == ANC_PV_CUSTODY_VERSION) {
    snapshot->authority_anchor_present =
        (flags & ANC_PV_CUSTODY_FLAG_AUTHORITY_ANCHOR_PRESENT) != 0;
    snapshot->expected_edge_present =
        (flags & ANC_PV_CUSTODY_FLAG_EXPECTED_EDGE_PRESENT) != 0;
  } else {
    snapshot->authority_anchor_present = snapshot->anchored_sequence != 0;
    snapshot->expected_edge_present = snapshot->expected_next_sequence != 0;
    if (!snapshot->authority_anchor_present &&
        (anc_pv_is_nonzero(snapshot->anchored_head, ANC_PV_HASH_BYTES) ||
         anc_pv_is_nonzero(snapshot->membership_digest, ANC_PV_HASH_BYTES) ||
         snapshot->signed_at_ms != 0 ||
