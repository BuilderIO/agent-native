...
static NSString *_Nullable AncAccount(NSString *service, NSString *vaultId,
  ...
  NSMutableData *input = [NSMutableData dataWithBytes:kAccountDomain
...
- (AncPrivateVaultKeychainStatus)copyDataForService:(NSString *)service
                                           ...
                                           recordId:(NSString *)recordId
                                               data:(NSData **)data {
  if (data == NULL) return AncPrivateVaultKeychainStatusInvalid;
  *data = nil;
  NSDictionary *base = [self baseQueryForService:service
                                         vaultId:vaultId
                                        recordId:recordId];
  if (base == nil) return AncPrivateVaultKeychainStatusInvalid;
  LAContext *context = self.contextFactory();
  if (context == nil) return AncPrivateVaultKeychainStatusFailed;
  ...
  OSStatus result = self.functions.copyMatching((__bridge CFDictionaryRef)query,
                                                 &rawResult);
...
- (AncPrivateVaultKeychainStatus)addData:(NSData *)data
  ...
  NSDictionary *base = [self baseQueryForService:service
                                         vaultId:vaultId
                                        recordId:recordId];
  if (base == nil || data.length == 0 || data.length > kMaximumRecordBytes) {
    return AncPrivateVaultKeychainStatusInvalid;
  }
