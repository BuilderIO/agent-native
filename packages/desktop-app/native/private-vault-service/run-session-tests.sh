...
OUTPUT="$(mktemp -d "${TMPDIR:-/tmp}/agent-native-private-vault-session-tests.XXXXXX")"
trap 'rm -rf "$OUTPUT"' EXIT
ARCHITECTURES="${PRIVATE_VAULT_BUILD_ARCHITECTURES:-universal}"
if [[ "$ARCHITECTURES" != "universal" && "$ARCHITECTURES" != "arm64" ]]; then
  echo "Session test architectures must be universal or arm64" >&2
  exit 1
fi

PRIVATE_VAULT_BUILD_ARCHITECTURES="$ARCHITECTURES" \
PRIVATE_VAULT_BUILD_SESSION_TESTS=1 \
  bash "$ROOT/native/build-private-vault-service.sh" "$OUTPUT" >/dev/null

ARM64="$OUTPUT/.session-tests/private-vault-session-tests-arm64"
X86_64="$OUTPUT/.session-tests/private-vault-session-tests-x86_64"
