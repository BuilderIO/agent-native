...
bool PVTrustedRecoveryCollectPhrase(const char *vaultID,
  ...
  @autoreleasepool {
    ...
    NSAlert *alert = [[NSAlert alloc] init];
    alert.alertStyle = NSAlertStyleWarning;
    alert.messageText = @"Recover this Private Vault?";
    alert.informativeText = [NSString
        stringWithFormat:
            @"Vault: %@\n\nEnter all 24 recovery words. The phrase stays "
             "inside the signed native vault boundary and is cleared after "
             "the complete encrypted history verifies.",
            vault];
    [alert addButtonWithTitle:@"Verify and Recover"];
    [alert addButtonWithTitle:@"Cancel"];
    NSSecureTextField *field = [[NSSecureTextField alloc]
        initWithFrame:NSMakeRect(0, 0, 520, 28)];
    field.placeholderString = @"word1 word2 … word24";
...
bool PVTrustedExportVerifyPhrase(const char *vaultID,
  ...
  @autoreleasepool {
    ...
    NSAlert *alert = [[NSAlert alloc] init];
    alert.alertStyle = NSAlertStyleWarning;
    alert.messageText = @"Run a recovery drill on this encrypted export?";
    alert.informativeText = [NSString
        stringWithFormat:
            @"Vault: %@\n\nEnter all 24 recovery words. The archive will be "
             "opened and verified inside the signed native vault boundary. "
             "Neither the phrase nor recovered document text is returned to "
             "Content.",
            vault];
    [alert addButtonWithTitle:@"Verify Recovery Export"];
    [alert addButtonWithTitle:@"Cancel"];
    NSSecureTextField *field = [[NSSecureTextField alloc]
        initWithFrame:NSMakeRect(0, 0, 520, 28)];
    field.placeholderString = @"word1 word2 … word24";
