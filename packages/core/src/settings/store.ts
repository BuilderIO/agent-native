...
/**
 * Mutate a setting inside a caller-owned PostgreSQL transaction. The caller's
 * work and the setting share one serialization boundary; cache and event side
 * effects happen only after the transaction runner confirms the commit.
 */
export async function mutateSettingTransaction<
...
>(
  ...
  options?: StoreWriteOptions & { fallbackKey?: string },
  ...
  const mutation = await runTransaction(async (tx) => {
    ...
    const currentResult = await tx.execute(
      sql`SELECT value FROM public.settings WHERE key = ${key} FOR UPDATE`,
    );
    ...
    if (
      ...
      const fallbackResult = await tx.execute(
        sql`SELECT value FROM public.settings WHERE key = ${options.fallbackKey} FOR UPDATE`,
      );
    ...
    if (next.value === null) {
      await tx.execute(sql`DELETE FROM public.settings WHERE key = ${key}`);
    } else {
      await tx.execute(sql`
        INSERT INTO public.settings (key, value, updated_at)
        VALUES (${key}, ${JSON.stringify(next.value)}, ${Date.now()})
    ...
    if (options?.fallbackKey && options.fallbackKey !== key) {
      await tx.execute(
        sql`DELETE FROM public.settings WHERE key = ${options.fallbackKey}`,
      );
    }
    return next;
  });

  const raw = mutation.value === null ? null : JSON.stringify(mutation.value);
  requestSettingsCache()?.set(key, raw);
  if (options?.fallbackKey && options.fallbackKey !== key) {
    requestSettingsCache()?.set(options.fallbackKey, null);
  }
