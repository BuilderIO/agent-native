...
describe("Fusion container dev-server reload behavior", () => {
  const fusionEnvKeys = [
    "FUSION_ENVIRONMENT",
    "FUSION_ENV_ORIGIN",
    "VITE_FUSION_ENV_ORIGIN",
  ] as const;
  const previousValues = fusionEnvKeys.map((key) => process.env[key]);

  afterEach(() => {
    fusionEnvKeys.forEach((key, i) => {
  ...
  function clearFusionEnv() {
    fusionEnvKeys.forEach((key) => delete process.env[key]);
  }

  it("is not detected as a Fusion runtime when no Fusion env vars are set", () => {
    clearFusionEnv();
    expect(_isFusionContainerDevRuntime()).toBe(false);
  });

  it.each(fusionEnvKeys)(
    "is detected as a Fusion runtime when %s is set",
    (key) => {
      clearFusionEnv();
      process.env[key] = "1";
      expect(_isFusionContainerDevRuntime()).toBe(true);
    },
  );

  it("leaves Nitro's default full-reload-on-server-edit behavior alone for ordinary local dev", () => {
    clearFusionEnv();
    const options = _buildNitroDevPluginOptions({}, "/");
    expect(options.experimental).toBeUndefined();
  });

  it("opts out of Nitro's serverReload full-page-reload only inside a Fusion container", () => {
    clearFusionEnv();
    process.env.FUSION_ENVIRONMENT = "1";
    const options = _buildNitroDevPluginOptions({}, "/");
    expect(options.experimental.vite.serverReload).toBe(false);
  });

  it("still merges caller-supplied nitro.experimental.vite options inside a Fusion container", () => {
    clearFusionEnv();
    process.env.FUSION_ENVIRONMENT = "1";
    const options = _buildNitroDevPluginOptions(
      { nitro: { experimental: { vite: { hmr: false } } } },
      "/",
    );
    expect(options.experimental.vite.serverReload).toBe(false);
    expect(options.experimental.vite.hmr).toBe(false);
  });

  it("does not clobber caller-supplied nitro.experimental outside a Fusion container", () => {
    clearFusionEnv();
    const options = _buildNitroDevPluginOptions(
      { nitro: { experimental: { someOtherFlag: true } } },
      "/",
    );
    expect(options.experimental).toEqual({ someOtherFlag: true });
  });
});
