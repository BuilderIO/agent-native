...
export function MultiTabAssistantChat({
  ...
  const {
    ...
    pinThread,
    renameThread,
    // `scope` is what stops one resource's chat from surfacing on another's
    // page: a chat started while viewing a resource is tagged with it, and
    // useChatThreads then keeps general (unscoped) chats visible everywhere
    // while swapping out a chat scoped to a different resource. Passing null
    // here made every chat global, so opening a second design restored — and
    // typed into — the first design's thread.
  ...
  // follow the resource in view, so one design's tabs never stay mounted (and
  // rebroadcasting their run state) while another design is open.
  ...
  // `useChatThreads`: on a scope change we need to read the new key BEFORE the
  // persistence effect writes the current (now-wrong) tab list under that key.
