...
export function createExtensionActionEntries(): Record<string, ActionEntry> {
  return {
    ...
    "render-inline-extension": {
      tool: {
        description:
          "Render a one-time, transient sandboxed Alpine.js mini-app directly inside the chat. Use this for generated UI that should answer the current turn inline without saving anything to the Extensions view: calculators, adjustable controls, knobs, pickers, visualizers, temporary dashboards, and interactive results. The content must be a self-contained Alpine.js HTML body snippet that can use appAction(), appFetch(), dbQuery(), dbExec(), extensionFetch(), extensionData, and agentNative.chat.send()/sendToAgentChat() to pass outputs back to chat. Use semantic Tailwind colors (bg-background, text-foreground, bg-primary, etc.) so it inherits the parent app theme. Use create-extension instead when the user wants the UI saved or reusable.",
        parameters: {
          type: "object",
          properties: {
            name: {
    ...
    "show-extension-inline": {
      tool: {
        description:
          "Render an existing saved extension inline in the chat. Use this when the user asks to load, reopen, reuse, or show a saved extension/widget/dashboard/calculator/mini-app in the conversation. Pass id when known; otherwise pass a search string and the action will use the best visible extension match.",
        parameters: {
          type: "object",
          properties: {
            id: {
