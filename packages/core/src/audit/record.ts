...
async function appendActionAudit(
  ...
  if (isAuditDisabled()) return false;
  const ctx = input.ctx;
  ...
  if (!shouldRecordAudit(input.config, actionName)) return false;
...
/**
 * Record one best-effort audit event. Resolves the actor, target, ownership,
 * and redacted inputs. Never throws and preserves historical behavior.
 */
export async function recordActionAudit(
  input: RecordActionAuditInput,
): Promise<void> {
  try {
    await appendActionAudit(input);
  } catch {
    // Best-effort — auditing must never break the audited action.
  }
}

/** Append a mandatory receipt or throw before the action result is returned. */
export async function recordRequiredActionAudit(
  input: RecordActionAuditInput,
): Promise<void> {
  const recorded = await appendActionAudit(input);
  if (!recorded) {
    throw new Error("Required audit receipt is unavailable");
  }
}
