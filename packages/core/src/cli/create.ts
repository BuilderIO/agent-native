...
function postProcessStandalone(
  ...
  seedDevAuthSecret(targetDir);
  setupAgentSymlinks(targetDir);
}

/** Pre-seed `.env.local` so first `pnpm dev` does not write mid-boot and restart Vite. */
function seedDevAuthSecret(targetDir: string): void {
  ...
  fs.writeFileSync(
    envLocalPath,
    [
      "# Auto-generated by agent-native create. Gitignored.",
