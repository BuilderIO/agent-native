...
describe("durable-background Netlify function emit (single-template, flag-gated)", () => {
  ...
  it("does not duplicate generated redirect comments across rebuilds", () => {
    const cwd = setupNetlifyOutput();
    const redirectsPath = path.join(cwd, "dist", "_redirects");
    ...
    expect(redirects.split(comment).length - 1).toBe(1);
    expect(
      redirects.split("/* /.netlify/functions/server 200").length - 1,
    ).toBe(1);
  });

  it("replaces an earlier path catch-all so the server fallback is first-match", () => {
    const cwd = setupNetlifyOutput();
    const redirectsPath = path.join(cwd, "dist", "_redirects");
    fs.writeFileSync(
      redirectsPath,
  ...
  it("fails when an earlier path catch-all would shadow the server fallback", () => {
    const cwd = setupNetlifyOutput();
    const redirectsPath = path.join(cwd, "dist", "_redirects");
    fs.writeFileSync(
      redirectsPath,
      ["/* /index.html 200", "/* /.netlify/functions/server 200"].join("\n") +
        "\n",
    );
