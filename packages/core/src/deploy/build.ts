...
export function assertSingleTemplateNetlifyBuildOutput(
  ...
  if (!fs.existsSync(publishDir)) {
    ...
    const fallbackRedirect = "/* /.netlify/functions/server 200";
    const firstPathCatchAll = redirectLines.find((line) =>
      line.startsWith("/* "),
    );
    if (firstPathCatchAll !== fallbackRedirect) {
      if (!redirectLines.includes(fallbackRedirect)) {
        failures.push(
          'dist/_redirects is missing "/* /.netlify/functions/server 200"',
        );
      } else {
        failures.push(
          'dist/_redirects has an earlier "/*" catch-all that would shadow "/* /.netlify/functions/server 200"',
        );
      }
    }
  }
...
export function writeSingleTemplateNetlifyRedirects(projectCwd: string): void {
  ...
  const fallbackRedirect = "/* /.netlify/functions/server 200";
  const knownCommentSet = new Set(fallbackCommentLines);

  function normalizeRedirectLine(line: string): string {
    return line.trim().replace(/\s+/g, " ");
  }

  function isPathCatchAllRedirect(line: string): boolean {
    const normalized = normalizeRedirectLine(line);
    return normalized.startsWith("/* ") && !normalized.startsWith("#");
  }

  // Keep non-catch-all redirects (and unrelated comments). Drop prior copies of
  // our generated comments/fallback and any earlier path catch-all so Netlify's
  // first-match semantics can't shadow the server fallback.
  const lines = existingLines.filter((line) => {
    const normalized = normalizeRedirectLine(line);
    if (!normalized) return false;
    if (knownCommentSet.has(normalized)) return false;
    if (normalized === fallbackRedirect) return false;
    if (isPathCatchAllRedirect(line)) return false;
    return true;
  });

  for (const line of [...fallbackCommentLines, fallbackRedirect]) {
    lines.push(line);
  }
