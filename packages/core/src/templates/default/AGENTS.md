...
Match effort to the size of the request. A small, well-specified change -- one screen, one field, one style fix -- is a short read, the edit, and the app's existing checks (`pnpm typecheck`, formatter, existing tests). Do not survey the whole codebase, add tests nobody asked for, or drive a browser for it. Save deep exploration for ambiguous, cross-cutting, or data-model work, and ask a question instead of guessing when the request is unclear.
...
3. **Actions for app operations** -- `pnpm action <name>` dispatches to callable action files in `actions/`; `defineAction` also auto-exposes those operations at `/_agent-native/actions/:name` for the UI. Do not create custom REST routes that re-export actions. **If you are about to add a file under `server/routes/api/`, stop and write a `defineAction` instead** -- unless it is a file upload, a streaming response, an inbound webhook, an OAuth callback, a public unauthenticated URL, or a non-JSON/binary response. Nothing else qualifies, including auth, settings, search, and CRUD. Needing new middleware to scope a route to the current user means it should have been an action; actions already run inside request context with access checks.
