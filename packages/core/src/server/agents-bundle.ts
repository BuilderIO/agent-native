...
export interface AgentsBundle {
  ...
  agentsMd: string;
  /** Map from skill name → { meta, content, dir, extraFiles }. */
  skills: Record<string, Skill>;
}

const EMPTY: AgentsBundle = { agentsMd: "", skills: {} };
 ...
 * Returns an empty bundle if nothing is found. Used by both the Vite plugin
 * (at build time) and the runtime fallback (in dev / Node prod).
 *
 * Synchronous — the Vite plugin's load hook calls it inline during the build.
 */
export function readAgentsBundleFromFs(cwd: string): AgentsBundle {
  let agentsMd = "";
  try {
    ...
    if (fs.existsSync(skillsDir)) {
      ...
      for (const entry of entries) {
        ...
        try {
          ...
          if (skills[name]) continue; // First one wins
...
export async function loadAgentsBundle(): Promise<AgentsBundle> {
  ...
  // 2. Filesystem fallback — only works in dev / Node prod.
  try {
    cached = readAgentsBundleFromFs(process.cwd());
    return cached;
  } catch {
    cached = EMPTY;
    return cached;
