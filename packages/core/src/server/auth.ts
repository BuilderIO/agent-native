...
export async function getSession(event: H3Event): Promise<AuthSession | null> {
  ...
  if (customGetSession) {
    ...
    try {
      ...
      if (ba) {
        console.log(
          "[auth-debug] calling ba.api.getSession, cookie header:",
          event.headers.get("cookie")?.substring(0, 80),
        );
        const baSession = await ba.api.getSession({
          headers: event.headers,
        });
        console.log(
          "[auth-debug] baSession result:",
          baSession ? `email=${baSession.user?.email}` : "null",
        );
        if (baSession?.user?.email) {
