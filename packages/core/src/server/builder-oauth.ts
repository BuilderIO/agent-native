...
function ownerOptions(ownerEmail: string) {
  ...
  return { key: BUILDER_OAUTH_KEY, scope: "user" as const, scopeId };
}
...
function isBuilderDiscoveryState(value: unknown): boolean {
  ...
  return (
    ...
    discovery.resourceMetadata?.resource === BUILDER_OAUTH_RESOURCE &&
...
function isBuilderCredential(credentials: McpOAuthCredentialBundle): boolean {
  return (
    credentials.serverUrl === BUILDER_OAUTH_RESOURCE &&
...
export async function deleteBuilderOAuthSession(
  ...
  try {
    ...
    const clientId = (
      credentials?.clientInformation as { client_id?: string } | undefined
    )?.client_id;
    if (refreshToken && clientId) {
      const response = await fetch(BUILDER_OAUTH_REVOCATION_ENDPOINT, {
        method: "POST",
  ...
  const localDeleted = await deleteMcpOAuthCredentials(options);
  return { localDeleted, remoteRevoked };
}
