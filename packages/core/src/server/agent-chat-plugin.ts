...
Generated UI content can use appAction(), appFetch(), dbQuery(), dbExec(), extensionFetch(), and extensionData. It can receive chat inputs through slotContext/window.onSlotContext and send results back to chat through agentNative.chat.send(...) or sendToAgentChat(...). Use semantic Tailwind classes like bg-background, text-foreground, bg-primary, border-border, and text-muted-foreground so the UI inherits the parent app theme.
...
Generated UI can read chat inputs from slotContext/window.onSlotContext, see/update app state through appFetch/appAction, use extensionData, and send results back through agentNative.chat.send(...) or sendToAgentChat(...). Use semantic Tailwind theme classes.
