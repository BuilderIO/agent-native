 ...
 * default plugins. The bootstrap is fire-and-forget — plugins push their
 * routes synchronously into `~middleware`, and Nitro plugin loaders run
 * before the first request anyway.
 */
export function getH3App(nitroApp: any): H3AppShim {
  ...
  if (!BOOTSTRAPPED.has(nitroApp)) {
    ...
    void bootstrapDefaultPlugins(nitroApp).catch((err) => {
      console.warn(
