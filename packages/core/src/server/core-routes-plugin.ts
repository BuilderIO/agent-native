...
export async function consumeBuilderConnectPendingState(
  ...
  try {
    ...
    await dependencies.remove(pendingKey).catch(() => false);
    return pending;
  } catch {
    return null;
  }
...
export function getFrameworkRouteRequestUrl(event: H3Event): URL {
  ...
  // Node request URL still has the OAuth query string, so preserve it before
  // validating the flow.
  const rawUrl =
    event.node?.req?.url ??
...
export function createCoreRoutesPlugin(
  ...
  return async (nitroApp: any) => {
    ...
    try {
      ...
      // Builder OAuth callback when the user reconnects.
      try {
      ...
      const builderStatusHandler = defineEventHandler(async (event) => {
        ...
        return runWithRequestContext(
          ...
          async () => {
            ...
            if (userEmail) {
              const oauthAccess = await resolveBuilderOAuthRequestAccess({
              ...
              if (await hasBuilderOAuthSession(userEmail).catch(() => false)) {
                return withConnectToken({
                  ...requestStatus,
                  configured: false,
                  credentialSource: "user" as const,
      ...
      getH3App(nitroApp).use(
        ...
        defineEventHandler(async (event) => {
          ...
          if (!isBuilderConnectCallbackOriginAllowed(previewOrigin)) {
            const msg =
              "This deployment does not have a Builder-approved callback origin.";
            setResponseStatus(event, 400);
            setResponseHeader(
              event,
              "Content-Type",
            ...
            return createBuilderBrowserCallbackErrorPage(msg, {
              ...
              body: "Open this app on its Agent Native or Builder preview URL, then try again.",
              parentOrigin: previewOrigin,
            });
          }
          const callbackUrl = new URL(
            `${getAppBasePath()}${BUILDER_CALLBACK_PATH}`,
            previewOrigin,
          ).toString();
        ...
        defineEventHandler(async (event: H3Event) => {
          ...
          try {
            ...
            try {
              ...
              // Org support is optional; legacy user scope remains valid.
            }
