 ...
 * screen; roster and identity migrations run by an agent reach this setting
 * with no notice anywhere in their path.
 *
...
async function warnOnCrossOrgRepoint(
  ...
  try {
    ...
    console.warn(
      `[agent-native][org] Could not read the previous active organization before ` +
  ...
  console.warn(
    `[agent-native][org] Repointed an account's active organization from ` +
