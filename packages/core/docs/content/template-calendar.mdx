...
<DataModel id="doc-block-1bkv9ic" title="Calendar data model" summary={"Only non-event data is stored locally — events live in Google Calendar. Booking links use ownableColumns so the sharing system applies."} entities={[
  {
    "id": "booking_links",
    "name": "booking_links",
