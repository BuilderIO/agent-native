...
<DataModel id="doc-block-1hb5m0c" title="Design data model" summary={"A design owns its files and versioned snapshots, and optionally links a reusable design system. Both designs and systems are ownable, each with a framework shares table."} entities={[
  {
    "id": "designs",
    "name": "designs",
