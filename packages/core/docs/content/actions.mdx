...
`get-feature-flags`, `list-feature-flags`, and `set-feature-flag`; the shared
Feature flags Settings tab uses those same actions for exact-user,
organization, percentage, and global rollouts. Management is permission-checked
and audited. Never route flag writes through generic settings endpoints or raw
SQL, and never put authentication, authorization, audit, secrets, or SSR cache
behavior behind a runtime flag. Read the `feature-flags` skill for rollout and
retirement guidance.
