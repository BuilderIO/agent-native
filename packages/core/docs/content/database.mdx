...
Instead of pushing directly, schema changes should be applied via additive SQL
migrations. For local development, implement them in a server plugin (e.g.
`server/plugins/db.ts`) by invoking the framework's `runMigrations()` helper.
For production or serverless deployments, run migrations in a release or deploy
step instead of on the first request. The Chat template provides
`pnpm migrate:production` for this.
  ...
  annotations={[
    {
      lines: "6-7",
      label: "Additive only",
      note: "Migration entries are tracked and applied once. Keep changes additive: add columns or tables, and never drop or rename them.",
    },
    {
      lines: "13-16",
