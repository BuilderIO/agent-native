...
The local stack intentionally uses a fake auth secret and a local Postgres volume. It is for `localhost`, not an internet-facing deployment.
...
### 2. Add a local Compose file

Create `docker-compose.yml` in the app root. It uses the [Dockerfile](#docker) in the next section and starts Postgres only on the private Compose network:

You can also download the [Dockerfile](https://www.agent-native.com/examples/self-hosted-chat/Dockerfile), [Compose file](https://www.agent-native.com/examples/self-hosted-chat/docker-compose.yml), and [environment template](https://www.agent-native.com/examples/self-hosted-chat/env.example) as a starter fixture.
...
services:
  app:
    ...
    ports:
      - "3000:3000"
    environment:
      DATABASE_URL: postgres://agent_native:local-development-only@postgres:5432/agent_native
...
Open [http://localhost:3000](http://localhost:3000). Set `ANTHROPIC_API_KEY` in a local `.env` file if you want to use the embedded agent; leave it empty if you only want to verify that the app and database start. Stop the stack with `docker compose down`; add `-v` when you intentionally want to reset the local database.

Before putting this on the public internet, replace the example secret, configure HTTPS and OAuth callback URLs, use a persistent production database, and complete the [production environment checklist](#env-required-prod).
