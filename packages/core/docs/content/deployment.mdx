...
| `AGENT_NATIVE_BUILDER_RELAY_TARGET_ORIGINS` | Corporate callback-only, comma-separated allowlist of exact trusted preview origins (for example, `https://deploy-preview-42--content.netlify.app`). Set it only on the corporate callback deployment. Wildcards and domain suffixes are not supported. |
