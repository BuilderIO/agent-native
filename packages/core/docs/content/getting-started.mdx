        ...
        {result.points.map((point) => (
          ...
          >
            <div
              ...
              style={{ height: `${Math.max(Math.round((point.value / max) * MAX_BAR_PX), 2)}px` }}
            />
...
When the agent opened `/text-analyses` in the previous step, it used two
built-in actions that ship with the Chat template:

- **`view-screen`** reads the current route and returns a compact summary of
  what the user is looking at. The agent calls this when it needs UI context
  before deciding what to do next.
- **`navigate`** writes a same-origin path to the browser's history. The
  agent calls this when a visual page is the right response to what the user
  asked.

You didn't write either of these; they're included in the template. As you add
more pages, keep them updated so the agent stays aware of what's in the app:

- `view-screen` should return the current route, any selected item or active
  filters, and enough page context for the agent to answer follow-up questions
  without the user describing the screen.
- `navigate` should handle every route in your app. Document available paths in
  `AGENTS.md` so the agent knows what destinations exist.
- Action results can include `href: "/text-analyses"` so a chat reply renders
  an explicit button rather than relying on the agent to call `navigate`
  automatically.
