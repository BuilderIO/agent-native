...
| `<OrgSwitcher>`          | Account menu for the sidebar footer: organizations, Settings, Usage, app downloads, and Log out. |
...
the persistent shell. Organization management lives in Settings: the
Organization group's General, Members, Usage, Authentication, Apps,
Infrastructure, and Audit log pages (the last four for owners and admins).
`/team` and `/settings/organization` redirect there.

`OrgSwitcher` is the account menu (also exported as `AccountMenu`). Its button
shows the signed-in person's photo, name, and current organization, and
`compact` renders the photo alone for a collapsed rail. The menu lists the
person's email, their organizations, pending invitations, email-domain matches,
and Create organization, then Settings, Usage, and Log out. Member invitations
and organization settings live in Settings. Pass
`utilityLinks` to add a "Get apps and extensions" list for the app's downloads;
links with `external: true` open in a new tab.
...
<OrgSwitcher
  ...
  utilityLinks={[
    ...
    { id: "desktop-app", label: "Get desktop app", href: "/download" },
  ]}
/>
```
...
With the `settings-redesign` flag on, the Settings shell renders the
Organization pages from `@agent-native/core/client/org`: `OrgGeneralPage`,
`OrgMembersPage({ appRoles? })`, `OrgAuthenticationPage`, and `OrgAppsPage`.
With the flag off, `TeamPage` passed as `team` renders the earlier Team tab.
...
declare it with `defineAppRoles` and pass the descriptor to `TeamPage`, and to
`OrgMembersPage` by replacing the Members page (see
[Settings Kit → Registering a page](/docs/toolkit-settings#registering-a-page)). The
...
- Invites and roles live on Settings › Organization › Members, not in
  per-resource dialogs.
...
- Owners and admins change the org domain, and the control explains who
  can auto-join.
