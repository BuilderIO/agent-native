...
Yes. If you want a basic agentic app, run `npx @agent-native/core@latest create my-app --template base`; you get actions, auth, SQL-backed runtime state, and room to add your own screens. If you are building a no-browser automation, run `npx @agent-native/core@latest create my-agent --headless`. See [Getting Started](/docs/getting-started) and [Automation-First Apps](/docs/pure-agent-apps). Use `--template chat` only when you want the Chat product.
