...
- **Act on other tools from a message.** An @mention can end in a row in Notion, an issue in Jira, or a record in HubSpot — see [@mention in Slack → row in Notion](#slack-to-notion) for the full worked example.
...
Finish [Set up Slack](#slack) and confirm that an `@mention` in a channel gets a reply before you touch Notion. Two behaviors matter for the rest of this example:
...
Files in `actions/` are discovered automatically — there is no registry to edit. See [Reuse the Core action factories](/docs/integrations#provider-api-factories) for narrowing the provider list or exposing these over HTTP.
...
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ |
| Slack never replies at all                                   | The mention never reached the app. Re-check the Request URL and signing secret in [Set up Slack](#slack) — this is not a Notion problem.   |
