...
The built-in runtime adapters are **Slack**, **Telegram**, **WhatsApp Cloud API**, and **provider-webhook email** (Resend or SendGrid). They are not blueprints: each ships an adapter, webhook verification, queue processing, and reply delivery. The catalog does not make Discord or Microsoft Teams connectable; add and register a verified adapter before offering either in a UI.
