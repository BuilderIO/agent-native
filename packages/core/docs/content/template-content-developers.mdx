...
Content's schema (`server/db/schema.ts`) currently defines 22 tables, not the nine a much earlier version of this doc described. Nine cover the document lifecycle, collaboration, and sync surfaces below; the other 13 belong to the inline-collection and external-source subsystem, documented in full on [Collections, properties & forms](/docs/template-content-databases#data-model).
...
- [Collections, properties & forms](/docs/template-content-databases) — the full inline-collection and external-source data model
