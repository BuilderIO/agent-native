...
Content provisions your personal space and its Files database automatically,
...
Content's schema (`server/db/schema.ts`) currently defines 22 tables, not the nine a much earlier version of this doc described. Nine cover the document lifecycle, collaboration, and sync surfaces below; the other 13 belong to the inline-database and external-source subsystem, documented in full on [Databases, properties, and forms](/docs/template-content-databases#data-model).
...
Content's database table view composes the reusable `DataGrid` component from
...
columns, then keeps database-specific property rendering, grouping, filters,
...
Do not move Content database models, access checks, grouping, drag and drop,
...
- **`app/components/editor/database/DatabaseView.tsx`** — the Content database
  table adapter. It composes `@agent-native/toolkit/data-grid` while keeping
  database-specific rendering and actions local.
...
- [Databases, properties, and forms](/docs/template-content-databases) — the full inline-database and external-source data model
