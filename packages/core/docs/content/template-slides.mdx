...
Deck and slide CRUD uses shared actions such as `create-deck`, `add-slide`, `update-slide`, and `delete-deck`, callable by both the agent and UI. REST routes under `templates/slides/server/routes/api/` are reserved for uploads, streaming, exports, and other route-only flows.
