...
adding another standalone environment setting. If a public config value must
vary by deployment, use its deterministic `AGENT_NATIVE_CONFIG_<PATH>` alias,
which also becomes part of the public browser bundle. Keep credentials and
user-, organization-, or workspace-scoped values in the appropriate secret or
connection store.
