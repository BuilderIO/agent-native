...
| `NITRO_PRESET`                                 | Build and deployment adapter.                                                                                                                                                  | A preset supported by the target host, such as `node`, `netlify`, `vercel`, or `cloudflare_module`.                     |
...
| `AGENT_NATIVE_SELF_DISPATCH_URL`               | Where this deployment sends its own background work, ahead of the platform URLs and `APP_URL`. Lets a long-lived server dispatch over loopback instead of its edge.            | An absolute `http(s)` URL such as `http://127.0.0.1:3000`; the app base path is appended.                               |
