...
comments, history, search, database properties, and the same action surface
...
<Diagram id="doc-block-content11" title="A local folder attaches to one space's Files database" summary={"Every page belongs to exactly one space and exactly one Files database. A local folder either joins an existing space's Files database or creates a new folder-backed space with its own."}>
...
<div class="diagram-flow">
  ...
  <div class="diagram-panel center">
    <span class="diagram-pill accent">Files database</span
...
or create a private folder-backed space with its own Files database.
...
- The folder appears as a source on the intended Files database.
