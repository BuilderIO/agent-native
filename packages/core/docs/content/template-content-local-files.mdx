...
does not replace the collection.

This means local files can keep their familiar repo workflow while Content still
provides collaboration, sharing, comments, history, search, collection properties,
...
- create a private folder-backed space with its own Files collection
...
1. the folder appears as a source on the intended Files collection
...
- [**Database**](/docs/server-database) — the SQL model every Content page and space is backed by, mode switch or not
- [**Content**](/docs/template-content) — the app this connects to: spaces, the Files collection, and the editor built on top of it
- [**Content: Local Files, Notion & Builder CMS Sync**](/docs/template-content-sync) — connecting a folder day-to-day, local MDX components, and conflict handling
