...
Some resources opt in to group sharing. A group must belong to the resource's
organization, and each current group member gets the selected share role on
that resource. See [Organizations, Teams & Permissions → User groups](/docs/organizations-teams-permissions#user-groups)
for what groups do and do not scope.
