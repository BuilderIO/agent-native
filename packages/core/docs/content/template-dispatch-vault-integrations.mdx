...
Saving a key in the Vault writes it to the workspace's shared credential store under the key's `credential_key`, so you never re-enter it per app. Each app's **Settings → API keys** tab shows the value the app actually uses and where it comes from, in this order:
