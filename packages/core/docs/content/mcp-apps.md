...
```an-diagram title="Two host bridge paths, one signed route" summary="Claude transplants the hydrated route and uses the direct `ui/_`bridge; ChatGPT gets a controlled iframe via window.openai and relays host actions over postMessage. Both point at the same signed app route."
{
