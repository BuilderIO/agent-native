...
<DataModel id="doc-block-1goc7zn" title="The connection model" summary={"A connection records safe provider metadata and credentialRefs (pointers, not secrets). Each grant unlocks one app — one connection, many grants."} entities={[
  {
    "id": "conn",
