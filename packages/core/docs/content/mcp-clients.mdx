...
`vercel`, `github`, `slack`, `asana`, and `hubspot`.
...
`agentNative.providerApi.request(...)`; those helpers call the template's
`provider-api-*` actions and resolve the granted connection server-side. The
same helpers are available under `agentNative.connectors.mcp` and
`agentNative.connectors.providerApi`.

Local-file extensions must explicitly allow `list-mcp-tools`,
`call-mcp-tool`, and any `provider-api-*` action they use in `extension.json`.
