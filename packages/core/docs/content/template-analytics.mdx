...
<DataModel id="doc-block-unvmkd" title="Analytics data model" summary={"Dashboards and analyses are the resources; views, shares, and a query cache hang off them. Org tables come from @agent-native/core/org."} entities={[
  {
    "id": "dashboards",
    "name": "dashboards",
