...
| `PUT /_agent-native/org/domain`                  | Set or clear the allowed email domain (owner/admin)             |
| `GET /_agent-native/org/a2a-secret`              | Reveal the active org's A2A secret (owner only, on demand)      |
| `PUT /_agent-native/org/a2a-secret`              | Set or regenerate the active org's A2A secret (owner only)      |
| `POST /_agent-native/org/a2a-secret/sync`        | Push the org's A2A secret to every connected app (owner only)   |
...
| -------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `owner`  | Full control of the organization | Manage the organization, members, admins, domain policy, and the cross-app A2A secret                                                                                                             |
| `admin`  | Team administrator               | Invite/remove members, manage ordinary member roles, and set email domain auto-join; admins cannot change the owner's role or manage the A2A secret, and only the owner manages admin transitions |
...
Organizations can have reusable user groups made up of current organization
members. Owners and admins manage group membership, and one person can belong
to multiple groups. Apps can use a group to share a resource that opts into
group sharing or to limit access to a workspace connection.

A group is an access list, not a child team. It has no separate organization or
app roles, data boundary, or team-specific settings, instructions, context,
skills, memory, or work view. A group share applies its resource role to every
current group member. Access still requires membership in the resource's
organization.

See [Sharing & Privacy → Group grants](/docs/sharing#group-grants) and
[Workspace Connections](/docs/workspace-connections) for details.
