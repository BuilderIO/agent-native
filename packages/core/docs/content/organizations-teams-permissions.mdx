...
export function InviteButton() {
  const { canInviteOrgMembers } = useOrgRole();
  return canInviteOrgMembers ? <button>Invite members</button> : null;
}
```
...
| `admin` | Team administrator | Invite/remove members and manage ordinary member roles; admins cannot change the owner's role, and only the owner manages admin-to-admin transitions |
...
| `<APP_NAME>_DATABASE_URL` | Per-app database override in a multi-app workspace; it wins over `DATABASE_URL`. |
| `<APP_NAME>_DATABASE_URL_UNPOOLED` | Per-app unpooled connection used for migrations where the provider exposes one. |
| `APP_URL` | Canonical app/workspace origin used for generated links and callbacks. |
| `BETTER_AUTH_URL` | Public auth origin used for session and OAuth URL construction. |
...
| `APP_BASE_PATH` | Mounts a standalone app under a path such as `/mail`. |
...
2. Set `BETTER_AUTH_SECRET` and `BETTER_AUTH_URL` before production.
