...
A text box sends a prompt, the AI returns a string, you display it. Impressive in demos; breaks the moment reality gets messy.

### Rung 2 — A chat with tools

The AI has tools and a chat interface where it works in front of you. Real step up — but still just a chat window with no dashboards, forms, or shortcuts.

### Rung 3 — Agent + UI as equal partners

Every agent action is also a button; every button runs the same logic the agent uses. The agent sees what you're looking at, writes to the same database, and its work shows up in the UI immediately.
