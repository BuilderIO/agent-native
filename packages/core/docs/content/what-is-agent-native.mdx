...
```mermaid
graph BT
  R1["🔴 Rung 1 — A single LLM call<br/>Prompt in, string out. No persistence or audit trail."]
  R2["🟡 Rung 2 — A chat with tools<br/>The AI can act — but only through a chat window."]
  R3["🟢 Rung 3 — Agent + UI as equal partners<br/>Every agent action is also a button. One implementation."]
  R1 --> R2 --> R3
```
