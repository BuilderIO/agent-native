...
deployments. Cloudflare Workers/Pages and D1 have no deployable filesystem, so
keep generated SQL in entries embedded in the worker bundle and pass them to
`runMigrations()`. Keep using `runMigrations()` for existing handwritten history
and cases that need dialect-specific SQL, D1-specific behavior, data backfills,
or deferred JavaScript steps. Never use destructive SQL, `drizzle-kit push`,
