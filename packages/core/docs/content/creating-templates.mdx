...
`runDrizzleMigrations()` from the release migration command. Keep
`runMigrations()` for existing handwritten history and cases that need
dialect-specific SQL, D1-specific behavior, data backfills, or deferred
JavaScript steps. Never use destructive SQL, `drizzle-kit push`, table renames,
or column drops.
