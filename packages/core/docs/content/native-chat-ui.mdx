...
### Choose when action UI appears {#conditional-native-ui}

An action can have several operations, while only some results need a card.
Add `when(args, result)` to `chatUI` to select those calls. The server calls it
after a successful action. When it returns `false`, the action result stays in
the ordinary collapsible tool history. The predicate is not stored or sent to
the browser.

Mail uses this on `manage-draft` so create results get a draft card, while
updates and deletes remain ordinary tool calls:
...
For actions with multiple write operations, check both the arguments and the
successful result. Mail's Gmail filter action only selects the rule card for a
created or replaced filter with its summary fields:
