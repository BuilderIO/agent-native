...
In the app, open **Settings → Connections → Integrations** and search for the host name you
already know: Claude, Claude Cowork, Claude Code, Anthropic, ChatGPT, OpenAI,
Codex, Grok, or xAI. The one external-host result opens **Settings → MCP server** with
...
Settings includes an **MCP server** page in the app's group. It shows
the app's canonical MCP URL, an A2A agent card when available, and the same
client setup instructions used by the full connect page. Use **Settings → MCP server**
