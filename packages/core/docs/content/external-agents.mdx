...
already has access. If the MCP server is connected but `get-document` returns
an access denial, the document owner must grant that account access. Reinstalling
the connector or changing the public share link does not resolve an account
permission denial.
