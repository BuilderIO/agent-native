...
External agents get the app's real create/update tools — the same named actions the in-app agent calls — so when a user asks for new content (a design, a deck, a form, a document), author it directly and persist it with those tools instead of delegating the work through `ask_app`. Actions marked `endsTurn: true`, such as an in-app question form that waits for the user's answer in that app's own chat, are hidden from MCP, WebMCP, and A2A by default; they are the in-app agent's tool, not an external caller's, unless the action explicitly opts back in with `mcpTool: true`. The WebMCP bridge accepts tool inputs and results up to 500,000 characters, so a whole hand-authored screen, deck, or document fits in one write instead of needing to be chunked. Page-local WebMCP calls also carry the browser tab id, so per-tab context (navigation, selection) resolves to the tab the user is actually looking at.
