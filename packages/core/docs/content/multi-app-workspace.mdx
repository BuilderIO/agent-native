...
You have two options: **unified deploy** (the default for workspaces) or per-app independent deploy. [Workspace Deployment — One Origin, Many Apps](/docs/workspace-deployment#workspace-deploy) covers the full command, per-preset details (Netlify and Vercel), and a diagram of the shape; the tradeoff:
...
- **Per-app independent deploy** — prefer each app on its own domain (`mail.company.com`, `calendar.company.com`)? Every app in the workspace is still an independent deployable — `cd apps/mail && pnpm build` behaves exactly like a standalone scaffold. Cross-app A2A then goes through the standard JWT-signed path with a shared `A2A_SECRET`. Cross-domain SSO between separately-deployed apps is handled by identity federation with Dispatch as the hub — see [Cross-App SSO](/docs/cross-app-sso); the unified single-origin deploy avoids needing it. For a self-hosted VPS or Docker Compose setup on one origin instead, see [Workspace Deployment — Self-hosted Workspace with Docker Compose](/docs/workspace-deployment#workspace-docker-compose).

To share user accounts, organizations, settings, and other cross-app data, point every app at the same `DATABASE_URL`. During local development, set it in the workspace root `.env` so every app uses the same PostgreSQL database. If unset, each app uses its own process-local PGlite database, so users and organizations are not shared. The shared package is never deployed on its own. pnpm symlinks it into each app's `node_modules/` at build time.

Connection demand adds up across processes that share a database. If five long-lived Node.js app processes each use one shared pool for the same `DATABASE_URL`, they can hold up to 100 connections at the default of 20 per pool (`5 × 1 pool × 20`). For serverless workspaces, estimate warm function instances per app using the runtime-specific pool defaults. See [Server database](/docs/server-database) for `AGENT_NATIVE_DB_POOL_MAX` and the full budgeting guidance.
