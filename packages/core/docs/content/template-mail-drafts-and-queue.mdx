...
- **Your signature.** If you've configured one in Settings → Mail → General → Drafting, it's appended automatically and used exactly as written — the agent never invents one from your name or profile.
...
Configure aliases — named groups of email addresses you can send as — from Settings → Mail → General → Aliases. Aliases are managed from that Settings page directly; the agent can read and use an alias you've set up, but adding or editing one is a manual step in Settings today.
