...
Settings page that owns the credential instead of building bespoke credential
forms: model provider keys live on Agent › Model, other keys on
Connections › API keys, and Builder.io on Connections › Integrations.
  ...
  methods: [
    {
      ...
      label: "Open settings",
      primary: true,
      payload: { url: buildSettingsRoute("model") },
    },
  ],
...
standard [Settings Kit](/docs/toolkit-settings) pages so keys, connections,
Builder connect, and team settings live in one predictable place.
...
- App setup pages and Settings pages use the same readiness source.
