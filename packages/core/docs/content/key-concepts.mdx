...
| Live sync between agent and UI       | On a long-lived host, same-process writes stream immediately over `/_agent-native/events`; a lightweight poll keeps cron and cross-process writes convergent, and is the app's only path on production serverless (at the same cadence) unless it opts in to the [Hosted Realtime Sync Gateway](/docs/real-time-collaboration#hosted-gateway). Mutating actions invalidate action-backed queries automatically, so agent-created records appear without a manual refresh. See [Live Sync](#polling-sync) below. |
...
When the agent changes data, the UI needs to reflect that without a manual refresh. `useDbSync()` is what makes that automatic. On a long-lived host, same-process writes stream over `/_agent-native/events`; `/_agent-native/poll` remains the cross-process fallback. When the agent writes to the database (application state, settings, or domain data), a version counter increments and the client invalidates the relevant React Query caches.

SSE covers the common case instead of WebSockets because it gives same-process writes an immediate path to the browser with plain HTTP, while the version-counter poll stays as a fallback that works in every deployment environment. On a production serverless host (Netlify, Lambda, Vercel functions), the app's own `/_agent-native/events` isn't a slower path — it's off. Holding that stream open would tie up a function instance that serves one request at a time (so it could never see another invocation's write anyway) until the platform kills it, recycling the container; the server answers with an immediate 204 instead and `/_agent-native/poll` carries all sync, at the same cadence, unless the app has opted in to the [Hosted Realtime Sync Gateway](/docs/real-time-collaboration#hosted-gateway), which streams from its own long-lived process instead.
