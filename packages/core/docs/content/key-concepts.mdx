...
<FileTree id="doc-block-1ae57y4" title="Guidance and behavior" entries={[
  {
    "path": "AGENTS.md",
...
<DataModel id="doc-block-4w0fu8" title="Core SQL stores" summary={"Auto-created in every template — the agent and UI both read and write these."} entities={[
  {
