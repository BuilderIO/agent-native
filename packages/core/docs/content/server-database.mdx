...
Set `DATABASE_URL` in your `.env` file or deploy-provider environment to connect a hosted PostgreSQL database. Common managed options include [Neon](/docs/neon), [Supabase](/docs/supabase), Railway, Render, [Amazon RDS for PostgreSQL](https://aws.amazon.com/rds/postgresql/), [Cloud SQL for PostgreSQL](https://cloud.google.com/sql/postgresql), and [Azure Database for PostgreSQL](https://learn.microsoft.com/en-us/azure/postgresql/overview). You can also use a self-hosted PostgreSQL server:
...
DATABASE_URL=postgres://user:pass@your-rds-endpoint.region.rds.amazonaws.com:5432/mydb?sslmode=require

# Cloud SQL for PostgreSQL
DATABASE_URL=postgres://user:pass@your-cloud-sql-host:5432/mydb?sslmode=require

# Azure Database for PostgreSQL
DATABASE_URL=postgres://user:pass@your-server.postgres.database.azure.com:5432/mydb?sslmode=require
...
The filesystem-backed loader is not available in Cloudflare Workers/Pages or
other filesystem-free edge runtimes, including D1. Keep using `runMigrations()`
there with SQL entries embedded in the worker bundle. A single Drizzle output
folder has one dialect-specific metadata baseline, so generate against the
primary deployment dialect and do not switch the same folder between SQLite and
PostgreSQL.
...
two independent migration owners. Generate against the primary deployment
dialect and keep using handwritten `runMigrations()` entries for framework
tables, D1-only or dialect-specific SQL, data backfills, and deferred JavaScript
steps.
...
`drizzle.config.ts` at the root of each app configures drizzle-kit for local development schema inspection and migration generation. If one migration folder serves multiple runtime backends, pass its primary dialect explicitly, for example `createDrizzleConfig({ dialect: "postgresql" })`:
