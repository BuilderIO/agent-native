...
Import schema helpers from `@agent-native/core/db/schema`. The framework helpers produce PostgreSQL definitions.
...
| Helper    | Purpose                                                        |
| --------- | -------------------------------------------------------------- |
| `table`   | Define a PostgreSQL table                                      |
| `text`    | Text column, supports `{ enum: [...] }`                        |
| `integer` | Integer column, `{ mode: "boolean" }` maps to Postgres boolean |
| `real`    | Floating-point column                                          |
| `now`     | Current timestamp for `.default(now())`                        |

<DataModel
  id="doc-block-3hbven"
  title="The tasks table"
  summary={
    "Defined once with the framework helpers for PostgreSQL and local PGlite."
  }
  entities={[
    {
      id: "tasks",
