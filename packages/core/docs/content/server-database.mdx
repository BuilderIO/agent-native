...
The framework bounds connections per database pool. Defaults are 20 for
long-lived Node.js processes, 2 for serverless foreground functions, 4 for
serverless background functions, and 1 for low-connection scheduled workers.
Set `AGENT_NATIVE_DB_POOL_MAX` to override the per-pool maximum for both Neon
and standard PostgreSQL clients:

```bash
AGENT_NATIVE_DB_POOL_MAX=4
```

Estimate the aggregate connection budget across all warm function instances or
Node.js processes, apps, and distinct database URLs. Each process or function
instance owns its own pools. Keep the sum below the database or pooler's
connection limit, leaving capacity for migrations, administration, and other
clients. Use a provider's pooled endpoint when available. For self-hosted
PostgreSQL with many app processes, consider transaction pooling with
PgBouncer.
...
<Callout tone="info">
  Apps that share one database used to have to run `migrate:production` one
  at a time (e.g. `pnpm -r --workspace-concurrency=1 --filter './apps/*'
  migrate:production`), because every app applies the same shared framework
  migrations to the same database. `runFrameworkReleaseMigrations` now holds a
  cross-process Postgres advisory lock around that shared work, so apps
  pointed at the same database can migrate concurrently — raise
  `--workspace-concurrency` above `1` and each app's own process startup and
  private-table migrations overlap, serialized only on the framework
  migrations they share.
</Callout>
