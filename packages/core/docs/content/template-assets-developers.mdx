...
database across every deploy context, so add columns with dialect-compatible SQL
in a new migration version in `server/plugins/db.ts` (Postgres supports
`ADD COLUMN IF NOT EXISTS`; SQLite migrations should use plain `ADD COLUMN`
because each entry is tracked and applied once). Never rename or drop a column,
and never run `drizzle-kit push` against production. See
[Database](/docs/database#migrations) for the full migration pattern.
