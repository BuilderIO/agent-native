...
Create an internal integration at **[notion.so/my-integrations](https://www.notion.so/my-integrations)** and copy its secret. Then, in your app, open **Settings → Connections → API keys**, add a key named exactly `NOTION_API_KEY`, paste the value, and pick a scope:
