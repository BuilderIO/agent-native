...
Builder CMS models can also be attached as read-only inline-database sources — see [Databases, properties, and forms](/docs/template-content-databases#connecting-an-external-source) for that workflow, which is a separate path from pulling/pushing a single document.
...
- [Databases, properties, and forms](/docs/template-content-databases) — syncing rows inside a database, rather than whole documents
