...
Builder CMS models can also be attached as read-only inline-collection sources — see [Collections, properties & forms](/docs/template-content-databases#connecting-an-external-source) for that workflow, which is a separate path from pulling/pushing a single document.
...
- [Collections, properties & forms](/docs/template-content-databases) — syncing rows inside a collection, rather than whole documents
