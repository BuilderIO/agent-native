...
files. That is appropriate for local development and Agent Native Desktop, but
it is not the default production security model.

The framework refuses `local-files` mode in known hosted, serverless, or remote
database-backed runtimes. This check cannot be bypassed by
`AGENT_NATIVE_ALLOW_LOCAL_FILES_IN_PRODUCTION`; an environment flag cannot prove
that a deployment is single-tenant. Production mode is limited to Agent Native
Desktop child runtimes. Local development, including local SQLite, continues to
work without a flag. For hosted or multi-user apps, use database mode and
SQL-backed sharing.
