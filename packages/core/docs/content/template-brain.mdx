...
<DataModel id="doc-block-qwo0b1" title="Brain data model" summary={"Connectors produce raw captures; distillation turns captures into reviewable knowledge; proposals gate sensitive entries. Sync runs and the ingest queue track background work."} entities={[
  {
    "id": "sources",
    "name": "brain_sources",
