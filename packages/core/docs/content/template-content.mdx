...
Documents are private by default, with viewer/editor/admin sharing and a public read-only mode when you want one out in the world. Multiple people (and the agent) can edit the same page at once without clobbering each other. See [Writing & organizing documents](/docs/template-content-editing), [Collections, properties & forms](/docs/template-content-databases), and [Local files, Notion & Builder CMS sync](/docs/template-content-sync) for each of those in depth.
