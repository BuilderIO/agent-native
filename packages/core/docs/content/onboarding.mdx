...
In a multi-app workspace, set `ONBOARDING_SHARED_COMPLETION` to reuse completed
onboarding across apps. If you do not set it, each app tracks onboarding
...
Setup cards show the available setup choices. They do not list every possible
key or credential.
