...
<div class="ob">
  ...
  <div class="diagram-card">
    ...
    ><small class="diagram-muted"
      ><code>DATABASE_URL</code> in host env</small
    >
  </div>
  <div class="diagram-card">
...
- **Paste an API key or fill a form** — e.g. choose an LLM or email provider,
  paste the value(s), click **Save**. Secret fields use a password input so
  the value isn't shown on screen. Saved values are encrypted and stored in
  your account or the workspace, never in a local `.env` file — see
  [Security](/docs/security) for where they live.
- **Set in the host environment** — for a value the running app reads from
  its own hosting environment and cannot save, like its database connection
  or a sign-in OAuth client. The step names the exact variable(s) and links to
  setup docs. There's no Save button, because there's nothing this checklist
  can save.
...
- **File uploads and storage** (optional) — Connect Builder for managed
  storage, or fill in S3-compatible storage keys (AWS S3, Cloudflare R2,
  MinIO). See [File Uploads](/docs/file-uploads).
- **Database** (optional) — Agent-Native stores app data in SQL.
  `DATABASE_URL` is set in the host environment, not from this checklist;
  without one, the app uses local PGlite.
- **Authentication** (optional) — built-in email/password accounts work by
  default. Google and GitHub sign-in are set in the host environment
  (`GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`,
  `GITHUB_CLIENT_ID`/`GITHUB_CLIENT_SECRET`).
- **Email delivery** (optional) — the Resend or SendGrid form saves a
  workspace key that sends team invitations and share notifications. Sign-in
  links, password resets, and scheduled reports read `RESEND_API_KEY` or
  `SENDGRID_API_KEY` and `EMAIL_FROM` from the host environment only, so the
  step lists those separately from the form.
...
the built-in steps listed below. To add app-specific steps (Gmail, Slack,
Notion, etc.), call `registerOnboardingStep()` from a server plugin.
