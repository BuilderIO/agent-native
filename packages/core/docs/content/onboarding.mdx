...
In a multi-app workspace, set `ONBOARDING_SHARED_COMPLETION` to reuse completed onboarding across apps. If you do not set it, each app tracks onboarding completion separately.
...
requirement. The primary path is shown first. Secondary paths stay behind a picker or disclosure when a step has several equivalent providers. Setup cards show the available setup choices. They do not list every possible key or credential.
