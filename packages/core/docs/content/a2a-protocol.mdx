...
- `isA2AProductionRuntime()` returns `true` on Netlify, AWS Lambda, Cloudflare Workers, Vercel, Render, Fly, and Cloud Run — even when `NODE_ENV` isn't `"production"`. Some serverless providers don't set `NODE_ENV` consistently, so the policy reads provider-specific flags too.
