...
- **Approval flow.** Destructive or external actions — sending money, shipping an outbound email, posting to Slack at scale — can require an admin OK before they fire.
...
- **Gate risky actions.** Sending money, mass-emailing customers, or posting to a public Slack channel can require an admin OK first — see the approval flow on [Messaging, routing, and approvals](/docs/template-dispatch-messaging-routing).
