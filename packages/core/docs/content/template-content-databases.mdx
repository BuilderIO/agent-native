---
title: "Content: Databases, Properties, and Forms"
...
# Databases, Properties, and Forms
...
Point the database at a Notion database, a Builder CMS model, or a local table.

### Choose an attach mode

**Add more rows** turns the source's entries into new rows in your database. **Add details** joins the source's fields onto your _existing_ rows by a shared key, for example matching a Notion database row to a local row by email or slug, adding read-only columns without duplicating rows.
...
- [Writing and organizing documents](/docs/template-content-editing) — the editor, page tree, and sharing model a database's row pages build on
- [Local files, Notion, and Builder CMS sync](/docs/template-content-sync) — syncing whole documents, as opposed to database rows
