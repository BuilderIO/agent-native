...
### 選擇動作介面的顯示時機 {#conditional-native-ui}

一個動作可以處理多種操作，但只有部分結果需要顯示卡片。請在 `chatUI` 加入 `when(args, result)` 來選擇這些呼叫。動作成功後，伺服器會執行此述詞。如果回傳 `false`，結果會保留在一般可摺疊的工具記錄中。述詞不會儲存或傳送至瀏覽器。

Mail 的 `manage-draft` 使用此方式在建立草稿時顯示卡片，並讓更新和刪除保留為一般工具呼叫：
...
對於包含多種寫入操作的動作，請同時檢查參數和成功結果。Mail 的 Gmail 篩選器動作只會為包含摘要欄位的新建或取代篩選器選取規則卡片：
