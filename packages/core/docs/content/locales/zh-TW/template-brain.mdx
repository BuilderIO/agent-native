...
<DataModel id="doc-block-eqz7qn" title={"大腦資料模型"} summary={"Connectors produce raw captures; distillation turns captures into reviewable knowledge; proposals gate sensitive entries. Sync runs and the ingest queue track background work."} entities={[
  {
    "id": "sources",
    "name": "brain_sources",
    "note": "連線器設定",
