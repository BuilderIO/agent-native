...
<DataModel id="doc-block-kwbrxf" title={"內容資料模型"} summary={"Nine tables in server/db/schema.ts. documents is the page tree; the rest hang off it for versions, comments, Notion sync, inline databases, and sharing."} entities={[
  {
    "id": "documents",
    "name": "documents",
