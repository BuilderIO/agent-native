...
移轉請參閱[本機資料夾來源](/docs/template-content-local-files)。
...
- **`server/db/schema.ts`** — 資料模型。此處新增列或表。內容範本沒有 `db:push` 指令碼；它依賴於在啟動時執行的嚴格附加遷移。編輯 `server/db/schema.ts`，編寫匹配的附加遷移，並在下次應用啟動時應用更改 - 架構更新絕不能刪除、重新命名或破壞性地更改現有表或列（有關準則，請參閱 [Database](/docs/server-database#migrations)）。
