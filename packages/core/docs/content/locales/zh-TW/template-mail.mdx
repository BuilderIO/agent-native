...
<DataModel id="doc-block-hk7agl" title={"郵件 SQL 表"} summary={"電子郵件本身位於 Gmail 中。 SQL 表包含 Gmail 不包含的內容：排隊草稿、傳送跟蹤事件和 OAuth 權杖。設定和臨時狀態位於 settings 和 application_state 儲存中。"} entities={[
  {
