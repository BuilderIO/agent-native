---
title: "工作區"
description: "Claude - 每個使用者的程式碼級自訂 - skills、記憶、指令、自訂代理、計畫作業、MCP 伺服器 - 由 SQL 支援，而不是檔案系統。"
---

# 工作區

> **哪個工作區檔案？** 此頁面涵蓋 **自訂層** — 工作區*是什麼*。對於部署形狀（一個單一儲存庫，許多應用程式），請參閱 [Multi-App Workspaces](/docs/multi-app-workspace)；有關治理（誰審查、核准和擁有什麼），請參閱 [Workspace Governance](/docs/workspace-management)。

每個 Agent-Native 應用程式都附帶一個**工作區**：使代理成為您的自訂層。它包含團隊指令 (`AGENTS.md`)、共用學習內容 (`LEARNINGS.md`)、個人結構化記憶 (`memory/MEMORY.md`)、代理按需拉入的 skills、自訂子代理、計畫作業和連線的 MCP 伺服器 — 您期望從 Claude 程式碼 / Codex 設定獲得的一切。

變化：**它是 SQL 行，而不是檔案系統檔案。**每個使用者都將自己的工作區儲存在資料庫中。沒有要啟動的開發盒，沒有每個使用者的容器，沒有要安裝的檔案。多租戶 SaaS 可以為每個使用者提供基本上免費的完全可自訂的代理，因為所有這些都是行——個人記憶、個人 MCP 伺服器、個人 skills、個人子代理——並且共用程式碼庫同時託管所有這些。

```an-diagram title="Claude-Code 工作區，但儲存在 SQL 中" summary="相同的定制層——指令、技能、記憶、代理、作業、MCP——除了每個檔案都是共用多租戶資料庫中的一行。"
{
...
應用內工作區面板顯示所有三個範圍。個人和共用/組織資源可以在那裡編輯。工作區範圍的資源在應用面板中是唯讀的，並透過 Dispatch 進行集中編輯，因此每個應用都可以看到相同的規範檔案，而無需同步步驟。
...
代理面板包括一個 **工作區** 分頁以及聊天和 CLI。它顯示了所有資源的資料夾組織樹、任何文字檔案的行內編輯器（Markdown、JSON、YAML、純文字）以及 `+` 選單的型別化建立流程（檔案、Skills、代理、計畫工作）。使用者可以瀏覽繼承的工作區預設設定並建立/編輯/刪除個人或組織資源。
...
| `mcp-servers/…`    | 工作區/共用 | 人類透過 Dispatch 或應用程式工作區 | MCP 設定重新整理             |

使用者可以直接在“工作空間”分頁中編輯這些記憶檔案 - 它們是一般資源。刪除代理出錯的行，將個人偏好保留在 `memory/MEMORY.md` 中，或將團隊範圍的規則推廣到 `AGENTS.md` 中。

這些表面中的每一個 - `AGENTS.md`、skills、記憶、自訂代理、MCP 伺服器 - 都是相同的底層資源形狀：`path` + `scope` + `content`，以相同的方式尋址和解析。

<DataModel id="doc-block-nilj9i" title={"工作區資源模型"} summary={"一種資源形狀支援每個工作區檔案。執行時按路徑和範圍對其進行鍵控，並解析讀取時的有效值。"} entities={[
  {
    "id": "resource",
    "name": "workspace resource",
    "note": "使用者工作區中的單一檔案 - 說明、技能、記憶、代理、MCP 設定或作業。",
    "fields": [
      {
        "name": "path",
        "type": "string",
...
Skills 是 `skills/` 路徑（最好是 `skills/<name>/SKILL.md`）下的 Markdown 資源檔案，為代理提供按需領域知識，在與 `/skill-name` 聊天時呼叫。從“工作空間”分頁新增它們，或者在“程式碼”模式下從 `.agents/skills/` 新增它們。
...
<AnnotatedCode id="doc-block-tnf0r5" title={"自訂代理設定檔案"} filename="agents/design.md" language="markdown" code={"---\nname: Design\ndescription: >-\n  Reviews layouts, interaction patterns, and product UX decisions.\nmodel: inherit\ntools: inherit\ndelegate-default: false\n---\n\n# 角色\n\n您是一名專注的設計代理。\n\n##職責\n\n-審查布局和互動流程\n-提出更強的視覺方向\n-簡潔且有主見"} annotations={[
  {
    "lines": "2",
...
在聊天輸入中鍵入 `@` 以引用工作區專案。游標處會出現一個下拉清單，顯示匹配的代理和檔案。使用箭頭鍵進行導覽並使用 Enter 進行選取。所選專案在輸入中顯示為行內標籤。

當您傳送訊息時，**檔案/資源**作為代理可以讀取的引用進行傳遞，**自訂代理**使用其設定檔案指令在本機執行，並且**連線的代理**透過 A2A 進行呼叫。

## / 斜線指令 {#slash-commands}

在行首鍵入 `/` 以呼叫技能。下拉清單顯示可用的 skills 及其名稱和描述；選取一個會新增一個行內標籤，並在傳送訊息時將其內容作為脈絡包含在內。如果未設定 skills，則下拉清單連結到這些檔案。
...
工作區連線允許應用程式共用相同的提供者帳戶（Slack、GitHub、HubSpot 等），而無需重複憑證。連線在 SQL 中紀錄提供者身分、帳戶標籤、狀態、範圍、應用程式授權和憑證引用。秘密保留在憑證儲存中；連線僅指向憑證金鑰名稱，例如 `SLACK_BOT_TOKEN`。
