...
輕量級閘道會另外使用 `ubuntu-latest`。停用 GitHub 託管執行器的儲存庫可以將 `VISUAL_RECAP_GATE_RUNS_ON` 設為一個不含 JSON 的純文字標籤，或使用 `--gate-runs-on visual-recap-gate`。請使用專用且最好是暫時性的執行器；閘道只評估中繼資料，不會簽出或執行 PR 程式碼。
