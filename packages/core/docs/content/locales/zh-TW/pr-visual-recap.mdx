...
JSON 引號不可省略。輕量級 `gate` 工作和完整的 `recap` 工作都會使用所選執行器。若要在引導設定期間進行設定，請將相同的 JSON 值傳給 `--runs-on`，然後使用 `doctor` 驗證工作流程和執行器可用性：
...
適用 [Secrets and variables](#secrets-and-variables) 中描述的相同秘密和變數 - 在儲存庫設定中以與複製變體相同的方式設定它們。可重複使用的呼叫端會透過 `runs-on` 輸入轉送 `VISUAL_RECAP_RUNS_ON`；被呼叫的工作流程會剖析 JSON，並將結果用於兩個工作。省略該輸入時會保留 `ubuntu-latest` 預設值。
