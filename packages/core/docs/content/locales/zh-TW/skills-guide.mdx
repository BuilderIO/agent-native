...
```an-diagram title="漸進式披露" summary="只有每個技能的名稱+描述始終處於脈絡中。當工作匹配時，全身會按需載入。"
{
