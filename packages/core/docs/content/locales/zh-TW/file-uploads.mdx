---
title: "檔案上傳"
description: "為檔案上傳設定持久物件儲存 - Builder.io 或自訂 S3 相容提供者。"
---
...
2. **Builder.io 提供者**（提供免費方案）— 內建，連線 Builder.io 時自動啟用
3. **需要儲存空間** - 未設定提供者時，上傳會安全失敗並顯示設定卡片；二進位資料不會存入 SQL

<Diagram id="doc-block-2q5re2" title={"提供者決議順序"} summary={"uploadFile() 按順序選取第一個設定的提供者。未設定提供者時，上傳會安全失敗並顯示設定卡片。"}>
...
<div class="diagram-upload">
  ...
  <div class="diagram-col">
    ...
    <div class="diagram-step">
      ...
      <div class="diagram-node">
        需要儲存空間<br /><small class="diagram-muted"
          >設定 Builder.io 或 S3 相容儲存</small
        >
      </div>
    </div>
  </div>
...
當沒有設定提供者時，上傳會安全失敗，並顯示用於設定 Builder.io 或 S3 相容提供者的設定卡片。二進位資料不會以 base64 儲存在 SQL 中。

位元組只能用於目前請求，不會儲存到聊天記錄。設定儲存空間後，上傳會回傳穩定的公開 URL，可在後續回合中重複使用。
