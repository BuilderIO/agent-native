...
從此範本本機路徑匯入 `getDb` - 路由中的 `../../server/db/index.js`，actions 中的 `../server/db/index.js` - 而不是直接從 `@agent-native/core` 匯入。核心匯出返回一個通用的無型別執行個體；範本的 `getDb()` 包含您的架構型別。請參閱 [Server](/docs/server-routes#request-context) 了解 actions 和自訂路由如何匯入它。
  ...
  annotations={[
    {
      lines: "6-7",
      label: "僅新增劑",
      note: "遷移項目會被追蹤並只套用一次。保持變更為附加式：新增欄位或資料表，不要刪除或重新命名它們。",
    },
    {
      lines: "13-16",
      label: "方言門控",
