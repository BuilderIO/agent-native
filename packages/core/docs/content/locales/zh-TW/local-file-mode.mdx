...
檔案。這適合本機開發和 Agent Native Desktop，但不是預設的正式環境安全模型。

框架會在已知的託管、無伺服器或遠端資料庫支援的執行環境中拒絕
`local-files` 模式。此檢查無法透過
`AGENT_NATIVE_ALLOW_LOCAL_FILES_IN_PRODUCTION` 規避；環境變數無法證明單一租戶隔離。
正式環境模式僅限 Agent Native Desktop 子程序。本機開發（包括本機 SQLite）仍可
在沒有旗標的情況下運作。對於託管或多使用者應用程式，請使用資料庫模式和
SQL 支援的共用。
