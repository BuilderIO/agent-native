...
## 均衡目錄 {#balanced-catalog}

目錄也涵蓋基礎、編輯器與畫布、智慧代理脈絡與知識、協作與治理、工作區外殼、營運以及能力套件。豐富編輯器歸 Toolkit；Core 保留相容性重新匯出。Blocks 在本階段仍歸 Core。Design 是具 `design-tweaks` 的範本模式，而非完整即插即用編輯器。Context X-Ray 引擎歸 Core，可重用呈現歸 Toolkit。Scheduling 與 Creative Context 是獨立能力套件；請參閱[套件生命週期](/docs/package-lifecycle)。
