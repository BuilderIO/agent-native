...
```an-diagram title="設定清單" summary="只需要連線一個 AI 引擎。該面板會跟蹤完成情況，並在完成所需的所有操作後自動隱藏。"
{
...
| `database` | 沒有 | 預設資料庫或任何 SQL `DATABASE_URL`  |
