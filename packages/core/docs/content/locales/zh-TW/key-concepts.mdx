...
| 代理與 UI 之間的即時同步      | 在長期執行的主機上，同行程的寫入會立即透過 `/_agent-native/events` 串流；輕量級輪詢則讓 cron 和跨行程的寫入保持一致，除非應用程式已啟用[託管即時同步閘道](/docs/real-time-collaboration#hosted-gateway)，否則在正式環境無伺服器環境中，這是應用程式以相同節奏運作的唯一路徑。會變更資料的 actions 會自動讓以 action 為基礎的查詢失效，因此代理建立的紀錄無需手動重新整理即可出現。請參閱下方的 [Live Sync](#polling-sync)。 |
...
當代理變更資料時，UI 需要在不手動重新整理的情況下反映出來。`useDbSync()` 就是讓這件事自動發生的機制。在長期執行的主機上，同行程的寫入會透過 `/_agent-native/events` 串流；`/_agent-native/poll` 仍然是跨行程情境下的回退方案。當代理寫入資料庫（應用程式狀態、設定或領域資料）時，版本計數器會遞增，用戶端也會讓相關的 React Query 快取失效。

SSE 之所以能涵蓋一般情境而不必使用 WebSockets，是因為它能讓同行程的寫入透過單純的 HTTP 立即傳遞到瀏覽器；而版本計數器輪詢則作為回退方案，在每一種部署環境中都能運作。在正式環境無伺服器主機（Netlify、Lambda、Vercel）上，應用程式自身的 `/_agent-native/events` 並不是較慢的路徑——它是關閉的。讓該串流保持開啟會佔用一個一次只服務一個請求的函式實例（反正它也永遠看不到另一次呼叫的寫入），直到平台將其終止、回收容器；伺服器改為立即回傳 204，其餘同步以相同節奏全部交由 `/_agent-native/poll` 負責，除非應用程式已啟用[託管即時同步閘道](/docs/real-time-collaboration#hosted-gateway)，該閘道會改從自己的長期執行程序串流。
