---
title: "嵌入 SDK"
description: "使用頁面脈絡和主機指令將 Agent-Native sidecar 嵌入到現有 SaaS 應用中。"
---
...
已在使用。如果您仍在無頭代理、豐富聊天和
嵌入式邊車，或完整的應用程式，從
[Agent Surfaces](/docs/agent-surfaces).

<Diagram id="doc-block-g4vq16" title={"包埋膜"} summary={"主機應用程式提供伺服器端驗證和即時頁面脈絡； Agent-Native 執行持久 sidecar 並透過用戶端操作和主機指令到達開啟的分頁。"}>

```html
<div class="diagram-embed">
  <div class="diagram-box" data-rough>
    <strong>主機 SaaS 應用</strong
    ><small class="diagram-muted">你的介面，你的認證</small>
  </div>
  <div class="diagram-col">
    <div class="diagram-pill accent">getContext &rarr;</div>
    <div class="diagram-pill">&larr; 用戶端操作</div>
    <div class="diagram-pill">&larr; 主機指令</div>
  </div>
  <div class="diagram-panel center" data-rough>
    <strong>Agent-Native sidecar</strong
    ><small class="diagram-muted">持久聊天 · 應用狀態 · 擴充功能</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    SQL<br /><small class="diagram-muted">框架表</small>
  </div>
</div>
```
...
元件。這是推薦的預設值：它重用整個框架
（actions、SQL 支援的應用程式狀態、擴充功能、瀏覽器工作階段工具）並給出
代理檢視使用者正在使用的頁面並進行操作的能力。

主機將 Agent-Native 伺服器路由掛載到其現有應用程式中，傳遞其
使用者登入 Agent-Native，並在產品 UI 中渲染 React 側邊欄。
Agent-Native 使用主機部署、主機工作階段和設定
`DATABASE_URL` 管理自己的框架表：聊天對話串、設定，
...
此模式是建議的預設模式，因為它重用了完整的框架：後端 actions 安裝在 `/_agent-native/actions` 下，代理可以呼叫與 UI 相同的 actions，使用者建立的擴充功能儲存在 SQL 中，`extensionData` 是持久的且具有使用者/組織範圍，瀏覽器工作階段工具允許後端代理檢查或操作目前開啟的分頁。

主機驗證是伺服器端的。不要將瀏覽器的身分作為事實來源傳遞；使用主機的請求/工作階段物件或短期伺服器驗證的權杖。如果主機不公開電子郵件，則返回穩定的 `userId`，Agent-Native 將使用它作為所有者金鑰。

### 資料庫隔離

嵌入模式管理 SQL 中的 Agent-Native 表。對於成熟的 SaaS 產品，最安全的預設設定是**相同的託管和驗證、專用的 Agent-Native 資料庫/架構**：
...
支援使用主機產品的主要 `DATABASE_URL`，但請明確選取。 Agent-Native 建立 `settings`、`application_state`、`tools`、`tool_data` 等框架表、瀏覽器工作階段表、秘密、聊天對話串和相關索引。專用的資料庫/模式可以避免表名衝突，保持託管表的所有權清晰，並使備份/保留策略更容易推理。如果您有意共用主機資料庫，請首先檢查現有表名稱，並將 Agent-Native 表視為框架擁有的表。
...
| 模式                     | 什麼時候使用它                                                                        | 包                                      |
| ------------------------ | ------------------------------------------------------------------------------------- | --------------------------------------- |
| **嵌入式應用程式選取器** | 啟動完整的 Agent-Native 應用程式作為重點 iframe（資產選取器、表單生成器、核准面板）。 | `@agent-native/embedding`               |
| **`<AgentNative>` 主橋** | 手動連線頁面脈絡和用戶端 actions 的獨立 sidecar 應用或跨來源 iframe。                 | `@agent-native/core/client`             |
| **便攜式擴充功能**       | 當 SaaS 已擁有擴充功能儲存/核准時，讓主機使用者建置沙盒迷你應用程式。                 | `@agent-native/core/client`擴充功能插槽 |

較低階別的 `@agent-native/embedding` 包公開：
...
表單建置器、行事曆時段選取器、核准面板或任何其他特定於工作的
工作流程。這是故意小於下面的 sidecar 主橋：
iframe 宣布準備就緒，主機可以傳送命名訊息，並且嵌入
...
資產還發出 `chooseImage` 作為舊圖片選取器的相容性別名
主機；新的整合應該偵聽 `chooseAsset`。
...
> 首選上面包含電池的外掛。使用這個較低階別的橋
> 僅適用於您連線頁面的獨立 sidecar 應用程式或跨來源 iframe
> 脈絡和用戶端 actions 自己。

對於獨立的 sidecar 應用程式或跨來源 iframe，請使用較低階別的 `<AgentNative />`。它將 iframe sidecar 和連線頁面脈絡、即時用戶端 actions 以及主機重新整理/導覽指令呈現在一個位置：
...
如果您只需要顯式語義脈絡，請使用 `screen={false}`。使用 `screen={{ includeDomHtml: true }}` 作為尚未將 UI 對應到語義 ID 和選取狀態的應用程式的後備。預設情況下，主橋僅接受來自 `agentUrl` 來源的訊息。如果 iframe URL 是可信來源不同的路由/代理 URL，則傳遞 `agentOrigin`。

對於非 React 主機，直接呼叫 `createAgentNativeHostBridge()` 並傳遞相同的 `getContext`、`actions` 和 `commands` 選項。

### Iframe 側

在 Agent-Native sidecar 內，使用框架助手請求主機脈絡，發現即時瀏覽器工作階段 actions，執行它們，或要求主機執行 UI 工作。在正式環境中始終透過預期的 `hostOrigin`：
...
對於 CLAW 風格的同事，iframe 還可以向 sidecar 後端註冊其即時瀏覽器分頁。然後，代理取得正常的後端工具，將請求排入佇列，iframe 聲明它，主機頁面執行它，後端將結果返回給代理。

<Diagram id="doc-block-m1fbf9" title={"伺服器介導的瀏覽器工作階段橋"} summary={"後端工具將工作排隊；註冊的分頁聲明它，在即時頁面上執行它，並將結果返回給代理 - 因此 backend/Slack/A2A 代理仍然可以觸摸開啟的分頁。"}>

```html
<div class="diagram-bridge">
  <div class="diagram-node" data-rough>
    後端代理<br /><small class="diagram-muted">聊天 · Slack · A2A</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    enqueue request<br /><small class="diagram-muted"
      >/_agent-native/browser-sessions</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node" data-rough>
    活動標籤頁面接管它<br /><small class="diagram-muted"
      >registered bridge</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-pill ok">result &rarr; agent</div>
</div>
```
...
| `view-browser-session`         | 向即時分頁詢問目前頁面脈絡和螢幕截圖。             |
| `list-browser-session-actions` | 向即時分頁詢問目前用戶端操作清單。                 |
| `run-browser-session-action`   | 透過即時分頁執行一項目前用戶端操作。               |
| `send-browser-session-command` | 請求主機重新整理、導覽、重新安裝、重新載入或核准。 |

這是當代理在後端、Slack/Telegram/email 中執行或作為 A2A 被呼叫者執行時使用的橋，但在開啟時仍需要觸摸使用者目前的瀏覽器分頁。如果瀏覽器關閉，後端 actions 仍應處理持久工作，並且瀏覽器工作階段工具將報告沒有連線活動分頁。
...
| ---------- | ----------------------------------------------------- | ------------------------ | -------------------------------------------------------------------------------- |
| 後端操作   | Sidecar 應用、後端 API、MCP 或整合轉接器              | 是                       | 持久的工作，例如建立、更新、發布、同步、傳送、匯入。                             |
| 用戶端操作 | 透過 `<AgentNative actions={...} />` 的目前瀏覽器分頁 | 否                       | 臨時 UI 的工作方式類似於選取元素、讀取編輯器狀態、滾動到一行、複製目前畫布狀態。 |

後端 actions 應該是任何必須在重新整理、關閉瀏覽器、重試或整合觸發執行後仍然存在的預設設定。它們屬於 sidecar 應用程式的正常 Agent-Native 操作/工具層，代理可以在其中透過聊天、自動化、Slack/Telegram/電子郵件整合和背景作業呼叫它們。

用戶端 actions 是一個瀏覽器分頁的即時橋梁。主機使用 `source: "client"` 和 `availability: "browser-session"` 來通告它們，並且 sidecar 應該將該清單視為臨時的。當路由或選取發生變化時重新列出 actions，並在分頁消失時回退到後端 actions。

### 便攜式擴充功能

> 當您希望 Agent-Native 進行管理時，首選包含電池的外掛
> 擴充功能定義、核准、儲存和代理建立的擴充功能。使用
> 僅當 SaaS 已經擁有這些問題時才使用下面的可移植插槽。

SDK 還支援使用者定義的擴充功能：主機 SaaS 可以在命名槽中呈現的沙盒 Alpine.js 迷你應用程式。當客戶想要針對代理使用的相同操作/脈絡介面建置自己的小面板、計算器、儀表板或工作流程助手時，請使用此選項。
...
清單是安裝合同。當存在 `requestedActions`、`requestedCommands` 或 `storageScopes` 時，SDK 在 iframe 請求到達操作橋或儲存轉接器之前在主機中強制執行它們。當 `slots` 存在時，`AgentNativeExtensionSlot` 僅在匹配插槽中渲染擴充功能。主機仍然可以使用 `allowedActions`、`allowedCommands` 和 `allowedStorageScopes` 覆蓋每個插槽的策略。
...
| `agentNative.command(name, p)` | 要求主機導覽、重新整理、重新安裝或開啟。   |
| `agentNative.refresh(payload)` | `refreshData` 的快捷方式。                 |
| `extensionData.*`              | 透過主機轉接器保留擴充功能本機資料。       |

預設情況下，`extensionData` 使用瀏覽器 `localStorage`，這對於原型和本機小工具很有用。正式環境 SaaS 主機應傳遞後端支援的 `storage` 轉接器，以便使用者和組織範圍的擴充功能資料持久、可審核並受應用程式權限管理。通用 HTTP 轉接器傳送 POST 主體（如 `{ operation, extensionId, slotId, collection, id, data, options, context }`），並直接期望 `{ result }` 或結果 JSON。

這個可移植的 SDK 層與框架的內建 SQL 支援的擴充功能儲存是分開的。在 Agent-Native 應用程式中，使用現有的 `ExtensionSlot`/`EmbeddedExtension` 元件和 `create-extension` 操作。在託管 SaaS 嵌入場景中，當您希望 Agent-Native 開箱即用地管理擴充功能定義、核准、儲存和代理建立的擴充功能時，首選 `createAgentNativeEmbeddedPlugin()` 加上 `AgentNativeEmbedded`。僅當 SaaS 已擁有擴充功能定義、核准、市場、儲存和計費時才使用 `AgentNativeExtensionSlot`。

安全模型：

- 擴充功能 iframe 是沙盒的，沒有 `allow-same-origin`；小應用程式無法直接讀取父DOM、cookies或應用程式執行時。
- 擴充功能只能呼叫主機和擴充功能清單允許的 actions 和指令。
- 有風險的 actions 應設定 `destructive` 或 `requiresApproval`，以便主機可以顯示核准流程。
- 將使用者建立的擴充功能 HTML 視為不受信任。按使用者/組織檢視市場安裝、紀錄操作使用情況和範圍後端儲存。

### 工作階段和分頁

主機橋的作用域為一對 iframe/主機視窗。如果同一使用者開啟多個分頁，則每個分頁都有自己的 `session`、脈絡、選取、用戶端 actions 和暫停的指令回應。不要假設在一個分頁中發現的用戶端操作可以在另一分頁中執行，或者它在導覽後仍然存在。

對於多分頁產品，在 SQL/後端 actions 中保持持久狀態，並僅將用戶端 actions 用於分頁本機部分：聚焦行、複製可見編輯器狀態、選取畫布元素或重新整理目前 React 查詢快取。包含足夠的 `route`、`resource` 和 `selection` 脈絡，以便 sidecar 決定目前分頁是否是執行瀏覽器工作階段操作的正確位置。

### 指令模型

內建指令名稱故意采用應用程式形式，而不是資料庫形式：

| 指令                                   | 目的                                            |
| -------------------------------------- | ----------------------------------------------- |
| `navigate`                             | 將主機 UI 行動到路徑/檢視/資源。                |
| `refreshData` / `refresh-data`         | 要求主機使用戶端資料無效。                      |
| `remountView` / `remount-view`         | 要求主機重新掛載子樹，例如`<App key={key} />`。 |
| `hardReload` / `hard-reload`           | 完全重新載入瀏覽器。                            |
| `openResource` / `open-resource`       | 在主機 UI 中開啟特定域物件。                    |
| `requestApproval` / `request-approval` | 要求主持人展示確認流程。為此註冊一個處理常式。  |

如果未提供處理常式，安全預設值將調度瀏覽器事件，例如 `agentNative:refresh-data` 和 `agentNative:remount-view`。 `requestApproval`沒有預設處理常式；在依賴它之前先註冊一個。

### 核准指南

在其清單中將有風險的用戶端 actions 標記為 `destructive: true`，並在執行刪除、發布、傳送、收費、邀請、共用或以其他方式影響目前檢視之外的使用者的操作之前需要主機核准。後端 actions 也應該執行自己的授權和核准檢查；主機核准是有用的使用者體驗，而不是安全邊界。

更喜歡這個形狀：

- 持久突變在後端操作中執行，包括驗證、驗證、審核記錄紀錄和重試。
- 主機指令開啟核准 UI 或聚焦受影響的資源。
- 用戶端操作僅處理無法在後端發生的即時 UI 步驟。
...
| 工具                | 目的                                           |
| ------------------- | ---------------------------------------------- |
| `view-host-screen`  | 讀取語義主機脈絡和螢幕截圖。                   |
| `list-host-actions` | 列出目前分頁公開的即時瀏覽器工作階段 actions。 |
| `run-host-action`   | 按名稱執行一個即時用戶端操作。                 |
| `send-host-command` | 傳送重新整理、導覽、重新安裝或核准等主機指令。 |

助手有意返回普通的 `{ name, description, parameters, execute }` 物件，以便 sidecar 可以將它們調整為 AI SDK、Anthropic、OpenAI 函式呼叫或 Agent-Native `ActionEntry` 形狀，而無需將此 SDK 耦合到一個執行時。

## 推薦產品形狀

首先啟動 iframe。它適用於 Builder.io、客戶 SaaS 應用程式和內部管理工具，無需耦合發布週期或 CSS/執行時假設。

sidecar 本身應該仍然是 Agent-Native 應用程式/範本：actions 是後端 API 表面，SQL 支援的應用程式狀態是代理的記憶，並且 Slack 或 Telegram 等整合可以路由到相同的持久聊天。嵌入的 SDK 在該 sidecar 和目前主機頁面之間提供活動膜。
