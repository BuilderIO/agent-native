...
對於應用程式操作](/docs/server-overview#actions-first)。
...
- **`readOnly: true`** — 即使對於不變異的 POST actions 也顯式跳過輪詢重新整理。 它**不是**存取控制：它不會限制誰可以呼叫該 action，也不會影響 `useActionQuery`、`useActionMutation` 或任何用戶端 hook。它唯一的作用是抑制呼叫後的重新整理事件，並讓該 action 不參與寫入稽核。要對模型隱藏某個 action，請使用 `agentTool: false`；要限制呼叫方，請使用上面的驗證/存取選項。
