...
使用 `@agent-native/core/client` 中的 `useFeatureFlag(FULL_APP_BUILDING.key)` 隱藏或顯示用戶端介面。Core 提供 `get-feature-flags`、`list-feature-flags` 和 `set-feature-flag`；共用的功能旗標設定分頁使用相同的 actions，支援依特定使用者、組織、百分比和全域逐步推出。管理操作會檢查權限並記錄稽核日誌。切勿透過通用設定 endpoint 或原始 SQL 寫入旗標，也不要將驗證、授權、稽核、secret 或 SSR 快取行為置於 runtime flag 之後。如需推出與移除的指引，請閱讀 `feature-flags` skill。
