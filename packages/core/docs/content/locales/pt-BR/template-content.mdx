...
<DataModel id="doc-block-u1j4ob" title={"Modelo de dados de conteúdo"} summary={"Nine tables in server/db/schema.ts. documents is the page tree; the rest hang off it for versions, comments, Notion sync, inline databases, and sharing."} entities={[
  {
    "id": "documents",
    "name": "documents",
