...
O catálogo também cobre fundamentos, editores e canvases, contexto e conhecimento do agente, colaboração e governança, estrutura do workspace, operações e pacotes de capacidade. O editor rico pertence ao Toolkit; o Core mantém reexportações de compatibilidade. Os blocos permanecem no Core. Design é um padrão de template com `design-tweaks`, não um editor completo instalável. O mecanismo Context X-Ray é do Core e sua apresentação reutilizável é do Toolkit. Scheduling e Creative Context são pacotes de capacidade separados; consulte [Ciclo de vida de pacotes](/docs/package-lifecycle).
