...
O gate leve usa `ubuntu-latest` separadamente. Repositórios sem executores hospedados pelo GitHub podem definir `VISUAL_RECAP_GATE_RUNS_ON` como um único rótulo de texto simples, sem JSON, ou usar `--gate-runs-on visual-recap-gate`. Use um executor dedicado e, de preferência, efêmero: o gate avalia somente metadados e nunca baixa nem executa código do PR.
