...
As aspas JSON são obrigatórias. Tanto o trabalho leve `gate` quanto o trabalho completo `recap` usam o executor selecionado. Para configurá-lo durante a instalação guiada, passe o mesmo valor JSON para `--runs-on` e depois valide o fluxo de trabalho e a disponibilidade do executor com `doctor`:
...
Aplicam-se os mesmos segredos e variáveis descritos em [Secrets and variables](#secrets-and-variables) — defina-os nas configurações do seu repositório da mesma forma que para a variante de cópia. O chamador reutilizável encaminha `VISUAL_RECAP_RUNS_ON` por meio da entrada `runs-on`; o fluxo de trabalho chamado analisa o JSON e usa o resultado para ambos os trabalhos. Omitir a entrada preserva o padrão `ubuntu-latest`.
