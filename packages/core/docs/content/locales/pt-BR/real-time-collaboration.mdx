...
distribui os eventos de sincronização entre várias instâncias sem servidor em vez
de cada instância manter suas próprias conexões de longa duração.

- `AGENT_NATIVE_REALTIME_TRANSPORT=hosted` é a única variável que você define, e
  defini-la é o consentimento explícito: nada disso acontece em uma implantação
  que não a definiu. `AGENT_NATIVE_REALTIME_GATEWAY_URL` substitui o endpoint do
  gateway para homologação; sem valor, ela é derivada de
  `BUILDER_GATEWAY_BASE_URL`.
- Em um aplicativo hospedado pelo Builder, o id do canal e o segredo de
  assinatura (`AGENT_NATIVE_REALTIME_HMAC_SECRET`) são variáveis de ambiente
  reservadas injetadas pelo caminho de provisionamento do Builder — o pipeline já
  provisionou o banco de dados que o gateway lê.
- **Em qualquer outro lugar, o aplicativo se registra sozinho, e isso significa
  enviar ao Builder a URL do seu banco de dados.** Com a variável de transporte
  definida, uma implantação apoiada em Postgres que tenha `BUILDER_PRIVATE_KEY`
  envia uma vez sua string de conexão e sua origem pública ao gateway e recebe de
  volta um id de canal e um segredo de assinatura. O Builder armazena essa string
  de conexão e se conecta ao seu banco de dados para acompanhar `sync_events`; a
  conexão não é somente leitura, já que tanto a alocação de versões quanto a
  detecção de alterações fora de banda gravam. Se você não quiser isso, deixe a
  variável de transporte sem definir — essa é toda a decisão, e não há outro
  caminho que compartilhe a credencial. O registro é refeito quando a URL do
  banco de dados ou a origem do aplicativo muda, de modo que a rotação de
  credenciais é aplicada sem reimplantar mais nada.
- A organização também precisa estar habilitada para o gateway. Até que esteja, o
  registro é recusado e o aplicativo continua com o polling local.
- Apenas implantações de produção se registram. Uma prévia de implantação
  mantém a sincronização local, de modo que sua credencial de banco de dados
  descartável nunca sai da máquina e não se cria um canal por pull request.
- **A organização é o limite de confiança.** Qualquer credencial que possa
  registrar também pode registrar novamente uma origem existente, o que devolve
  o segredo de assinatura daquele canal e reaponta seu banco de dados. Ter esse
  segredo permite emitir um token de inscrição para qualquer usuário do
  aplicativo, expondo o *fluxo de alterações* dele — quais registros mudaram e
  quando — mas não os registros em si, já que os clientes releem o conteúdo do
  aplicativo com a própria sessão. Trate uma credencial de registro como algo
  que vale para toda a organização.
- `GET /_agent-native/realtime-token` emite um token de inscrição de curta duração
  (10 minutos) e com escopo de sessão que o navegador usa para abrir a conexão com
  o gateway. Ele aceita apenas a mesma origem e responde 401 sem uma sessão ativa.
- `GET /_agent-native/can-see` é a verificação de visibilidade por evento do
  gateway: ele valida um token de acesso assinado e então reexecuta o próprio
  `resolveAccess` do aplicativo, para que o gateway nunca encaminhe um evento a
  alguém que já perdeu o acesso. Tipos de recurso desconhecidos ou falhas de
  consulta falham de forma segura (`{ allowed: false }`).
- No Postgres, aplicativos hospedados alocam as versões de sincronização a partir
  do banco de dados (`dbAssignedVersions`) em vez de um contador em processo,
  porque as instâncias do gateway e as instâncias sem servidor do próprio
  aplicativo compartilham um banco de dados e precisam concordar sobre a ordem de
  gravação.

Todas as falhas aqui são silenciosas por design: sem variável de transporte, sem
chave do Builder, com um banco de dados que não seja Postgres, com uma
organização ainda não habilitada ou com um gateway inacessível, o aplicativo
permanece no polling local em vez de gerar erro. Se o tempo real hospedado não
estiver ativo quando você espera, verifique esses pontos nessa ordem — um 404 em
`GET /_agent-native/realtime-token` significa que nenhum canal foi resolvido.
