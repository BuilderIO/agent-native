...
arquivos. Isso é apropriado para desenvolvimento local e Agent Native Desktop,
mas não é o modelo de segurança de produção padrão.

A estrutura recusa o modo `local-files` em ambientes conhecidos de hospedagem,
serverless ou com banco de dados remoto. Essa verificação não pode ser ignorada
com `AGENT_NATIVE_ALLOW_LOCAL_FILES_IN_PRODUCTION`; uma variável de ambiente não
prova o isolamento de um único locatário. O modo de produção é limitado aos
processos filhos do Agent Native Desktop. O desenvolvimento local, incluindo
SQLite local, continua funcionando sem uma variável. Para aplicativos hospedados
ou multiusuário, use o modo de banco de dados e o compartilhamento apoiado por SQL.
