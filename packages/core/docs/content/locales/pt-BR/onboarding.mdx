...
Em um espaço de trabalho com vários aplicativos, defina
`ONBOARDING_SHARED_COMPLETION` para reutilizar a integração concluída entre os
aplicativos. Se você não definir isso, cada aplicativo rastreará a conclusão
da integração separadamente.
...
Os cartões de configuração mostram as opções de configuração disponíveis. Eles
não listam todas as chaves ou credenciais possíveis.
