...
## Devo usar uma extensão ou editar o código do aplicativo? {#vs-app-code}

A estrutura permite que o agente edite diretamente o código-fonte do aplicativo — componentes, rotas, estilos e lógica de negócio. Use esta pergunta quando qualquer um dos dois caminhos puder resolver a solicitação:

- **Comece com uma extensão** para um widget autocontido e relativamente pontual: uma visualização ou interação personalizada, um bloco de dashboard, um utilitário independente ou um complemento em um slot existente. Ela é rápida, não exige alteração no código-fonte, build ou deploy e pode ser privada ou compartilhada.
- **Use o código do aplicativo** para um recurso central do template, mudanças na UI nativa, layout, rotas ou lógica de negócio, ou qualquer coisa que deva ser amplamente reutilizada entre dashboards ou usuários. Se o host não tiver um slot adequado ou a solicitação precisar de um novo slot, use código.
- **Promova extensões repetidas.** Se a mesma extensão for recriada continuamente, ou se manter várias cópias for pior do que uma implementação compartilhada, mova-a para o código do aplicativo como um recurso nativo reutilizável.
...
Regra geral: **se uma extensão puder resolver o problema, normalmente comece por ela.** Use código quando o limite da extensão não for suficiente ou quando a reutilização ampla e a disponibilidade em todo o produto justificarem uma implementação nativa. Por exemplo, uma visualização personalizada pontual em um dashboard é uma boa extensão; um novo tipo de gráfico de pizza, barras múltiplas ou linhas que deva estar disponível em todos os dashboards pertence ao código do template.
