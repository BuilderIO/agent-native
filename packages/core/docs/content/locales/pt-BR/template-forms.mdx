...
<DataModel id="doc-block-jq5mo7" title={"Modelo de dados de formulários"} summary={"Três tabelas. Os campos e integrações são colunas JSON nos formulários, portanto, as edições do agente são correções cirúrgicas, em vez de alterações de linhas entre tabelas."} entities={[
  {
    "id": "forms",
    "name": "forms",
