...
> **A maioria das pessoas instala o Plan como uma skill, não como um app scaffolded.** Um comando CLI adiciona as skills `/visual-plan`, `/visual-recap` e `/visualize-repo`, além do conector Plan hospedado, ao seu agente de código — consulte [Plan plugin & marketplace](/docs/plan-plugin) para as rotas de plugin e marketplace. Fazer fork do template Plan (coberto em [For developers](#for-developers)) é o caminho secundário para auto-hospedagem ou para construir sobre o próprio Plan.
...
<Diagram id="doc-block-woqr2s" title={"Dois comandos, uma superfície de revisão"} summary={"Ambos os comandos são publicados por meio do conector do Plano MCP hospedado na mesma superfície de anotação e comentário."}>
...
<div class="diagram-plan">
  <div class="diagram-col">
    <div class="diagram-node">
      ...
      ><small class="diagram-muted"
        >antes do codigo: arquitetura, UI, refatoracao</small
      >
    </div>
    <div class="diagram-node">
      <span class="diagram-pill">/visual-recap</span
      ><small class="diagram-muted"
        >depois do codigo: PR, commit, branch, diff</small
      >
    </div>
  </div>
  ...
  <div class="diagram-box">
    Superficie de revisao<br /><small class="diagram-muted"
      >diagramas · wireframes · codigo anotado · comentarios</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&harr;</div>
  <div class="diagram-node">
    Agente de codigo<br /><small class="diagram-muted"
...
<WireframeBlock id="doc-block-114m1dg">
  <Screen
    surface="desktop"
    html={
...
O comando instala três comandos: `/visual-plan`, `/visual-recap` e `/visualize-repo`.
...
`https://plan.agent-native.com/mcp` — consulte [MCP Clients](/docs/mcp-clients) para configuração específica do cliente.
...
Gerar um plano ou uma recapitulação executa uma sessão completa de agente contra
o LLM do seu agente de codificação, não um preenchimento leve de modelo, então o
custo escala com o tamanho do diff ou do plano e o modelo que você escolher.

- O modelo de chat padrão do aplicativo é o GPT-5.6 Luna. Conexões diretas com a
  Anthropic continuam usando Claude Sonnet 5 como padrão. Altere o modelo a
  qualquer momento em **Configurações**; Haiku é a opção Claude mais barata.
- As recapitulações automáticas de PR da GitHub Action são executadas
  separadamente do aplicativo interativo e têm seus próprios controles de
  modelo e custo — veja [PR Visual Recap](/docs/pr-visual-recap) para
...
O conector de planos hospedados fica em `https://plan.agent-native.com/mcp`.
...
## Documentação visual do repositório {#visualize-repo}

Use `/visualize-repo` quando quiser uma camada durável de documentação visual para uma base de código, em vez de um plano ou recap pontual. Ele cria ou abre um workspace local de Plan MDX, atualiza `agent-native.json` com uma seção `apps.visualize-repo` em modo local-files e serve os docs pela mesma ponte localhost:

```bash
npx @agent-native/core@latest visualize-repo --open
```

Se `agent-native.json` não existir, o comando cria um arquivo inicial e recomenda um pequeno conjunto de partida a partir da estrutura do repositório, como `actions/`, `app/components/`, `app/pages/`, `server/db/schema.ts`, `src/`, `packages/`, `templates/`, `docs/` ou `content/`. Você pode mirar âncoras de código específicas:
...
A pasta inicial fica em `.agent-native/visual-docs/repo-overview` por padrão. Os comentários permanecem locais em `comments.json`, para que revisores marquem seções para um agente de código e o agente atualize código e MDX na mesma mudança. Execute `visualize-repo check` para lint local e `visualize-repo verify` quando quiser que o schema do renderizador Plan valide o MDX.
...
- "Execute `/visualize-repo` e comece pelas nossas ações de API públicas e pelo schema."
...
<DataModel
  id="doc-block-jbmdmw"
  title="Modelo de dados do plano"
  summary={
    "Uma linha do plano possui seções ordenadas, além de comentários, eventos, versões, compartilhamentos e ativos inline."
  }
  entities={[
    {
      id: "plans",
