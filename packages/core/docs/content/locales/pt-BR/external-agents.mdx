...
No aplicativo, abra **Settings → Connections → Integrations** e pesquise pelo nome de host que
você já conhece: Claude, Claude Cowork, Claude Code, Anthropic, ChatGPT, OpenAI,
Codex, Grok ou xAI. O único resultado de host externo abre **Settings → MCP server**
...
As configurações incluem uma página **MCP server** no grupo do
aplicativo. Ela mostra a URL MCP canônica do aplicativo, um cartão de agente A2A
quando disponível e os mesmos guias da página de conexão completa. Use **Settings → MCP server**
