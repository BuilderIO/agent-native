...
A autenticação identifica a conta conectada e não amplia suas permissões. Um documento público continua legível pelo link. Um documento privado só pode ser lido via MCP quando essa conta já tem acesso. Se `get-document` retornar acesso negado após a conexão, o proprietário precisa conceder acesso a essa conta.

Reinstalar o conector ou alterar o link de compartilhamento público não resolve uma negação de permissão da conta conectada.
