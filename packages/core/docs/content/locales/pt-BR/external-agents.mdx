...
Os agentes externos recebem as ferramentas reais de criação/atualização do aplicativo — as mesmas ações nomeadas que o agente interno chama — então, quando um usuário pedir novo conteúdo (um design, uma apresentação, um formulário, um documento), crie-o diretamente e salve-o com essas ferramentas em vez de delegar o trabalho por meio do `ask_app`. Ações marcadas com `endsTurn: true`, como um formulário de perguntas interno que espera a resposta do usuário no chat do próprio aplicativo, ficam ocultas do MCP, WebMCP e A2A por padrão; elas são a ferramenta do agente interno, não a de um chamador externo, a menos que a ação opte explicitamente por reativá-las com `mcpTool: true`. A ponte WebMCP aceita entradas e resultados de ferramentas com até 500.000 caracteres, de modo que uma tela, apresentação ou documento inteiro cabe em uma única gravação em vez de precisar ser dividido. As chamadas WebMCP locais à página também carregam o id da aba do navegador, de modo que o contexto próprio de cada aba (navegação, seleção) é resolvido para a aba que o usuário está realmente vendo.
