...
Cada aplicativo é construído com `APP_BASE_PATH=/<name>` e `VITE_APP_BASE_PATH=/<name>` e emitido por meio do preset Nitro selecionado. **Netlify** é o preset padrão para o comando unificado `deploy`; ele emite funções de aplicativo em `.netlify/functions-internal/<app>-server` e gera redirecionamentos que deixam ativos estáticos não forçados para que o CDN sirva os arquivos primeiro. **Vercel** é compatível com `npx @agent-native/core@latest deploy --preset vercel`; ele grava um pacote raiz `.vercel/output` usando a Build Output API do Vercel.
...
Prefere cada aplicativo em seu próprio domínio (`mail.company.com`, `calendar.company.com`)? Cada aplicativo no espaço de trabalho ainda pode ser implementado de forma independente – `cd apps/mail && pnpm build` se comporta exatamente como um andaime independente. O A2A entre aplicativos passa pelo caminho padrão assinado pelo JWT com um `A2A_SECRET` compartilhado. O SSO entre domínios entre aplicativos implantados separadamente é gerenciado pela federação de identidade com o Dispatch como hub — consulte [Cross-App SSO](/docs/cross-app-sso); a implantação unificada de origem única evita a necessidade dela.

### Banco de dados compartilhado, credenciais compartilhadas

Para compartilhar contas de usuário, organizações, configurações e outros dados entre aplicativos, defina o mesmo `DATABASE_URL` para todos eles. No desenvolvimento local, defina-o no `.env` raiz do workspace para que todos os aplicativos usem o mesmo banco de dados PostgreSQL. Se ele não estiver definido, cada aplicativo usará seu próprio banco de dados PGlite local ao processo, e usuários e organizações não serão compartilhados.

A demanda de conexões se soma entre os processos que compartilham um banco de dados. Se cinco processos de apps Node.js de longa duração usarem um pool compartilhado cada um para o mesmo `DATABASE_URL`, eles podem manter até 100 conexões no padrão de 20 por pool (`5 × 1 pool × 20`). Para workspaces serverless, estime as instâncias de função aquecidas por app usando os limites padrão específicos do runtime. Consulte [Banco de dados do servidor](/docs/server-database) para `AGENT_NATIVE_DB_POOL_MAX` e as orientações completas de dimensionamento.
