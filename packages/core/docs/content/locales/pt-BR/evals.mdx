...
## A partir de um trace de produção {#from-trace}

Quando uma execução real surpreende — falhou em produção depois de funcionar na demo, ou chamou a ferramenta errada — promova esse trace concluído a um caso de CI em vez de inventar um prompt.

```bash
agent-native eval promote <runId> --write evals/from-trace.eval.ts
```

Esse comando persiste uma linha `EvalDataset` (o mesmo helper do painel e da action `promote-trace-eval`) e grava um fixture `defineEval` que o gate `agent-native eval` já descobre. Scorers padrão são `usesTool(...)` para cada ferramenta bem-sucedida, mais `--must-contain` opcional. Execuções truncadas, abortadas ou sem mensagem do usuário falham fechadas.

A action hospedada e `POST /_agent-native/observability/traces/:runId/promote` nunca escrevem a árvore do app. Use `--write` para o CI. O detalhe do trace no painel Observability tem **Promote to eval**.
