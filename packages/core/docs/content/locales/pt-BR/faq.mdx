---
title: "FAQ"
description: "Perguntas comuns sobre Agent-Native: o que é, para quem se destina, o que você pode criar e como funciona."
---

# FAQ

Perguntas comuns sobre Agent-Native, organizadas de "Estou apenas olhando" até "Estou conectando a autenticação agora".

## O básico {#general}

### O que é Agent-Native? {#what-is-agent-native}

Agent-Native é uma estrutura para criar aplicativos onde o agente de IA e a superfície do produto ao seu redor são parceiros iguais. O caminho comum começa com chat, adiciona actions tipadas, renderiza resultados estruturados inline e cresce para páginas duráveis sobre o mesmo estado SQL. A invariante é que agentes e humanos compartilham o mesmo actions, banco de dados e estado. Consulte [O que é Agent-Native?](/docs/what-is-agent-native) para obter a explicação completa.

### Para quem é isso? {#who-is-this-for}

Agent-Native é para pessoas que desejam que um aplicativo real e um agente de IA trabalhem a partir dos mesmos dados e actions. Os caminhos comuns são:
...
- **Conecte outro agente ou ferramenta de código** se desejar que Claude, ChatGPT, Codex, Cursor ou GitHub Copilot / VS Code usem um app Agent-Native — consulte [External Agents](/docs/external-agents) e [Skills Guide](/docs/skills-guide).

### Qual a diferença entre isso e adicionar IA a um aplicativo existente? {#how-is-this-different}

A maioria dos aplicativos utiliza a IA como uma reflexão tardia que não pode realmente _fazer_ coisas no aplicativo. Em um app Agent-Native, o agente é um cidadão de primeira classe que compartilha o mesmo actions, banco de dados e estado que a UI, portanto, pode fazer qualquer coisa que os botões puderem. Consulte [Por que criar apps dessa forma](/docs/what-is-agent-native#why-build-apps-this-way).

<Diagram id="doc-block-gefzrm" title="IA acoplada depois vs. Agent-Native" summary={"Uma barra lateral de chat acoplada depois vive em seu próprio mundo. Um agente Agent-Native compartilha as mesmas ações, banco de dados e estado que a UI."}>
...
Esse é o ponto principal. Comece com um template e personalize-o no seu próprio repositório e fluxo de desenvolvimento. O agente incorporado então opera o aplicativo resultante por meio de suas actions e dados.
...
Sim, através do [protocolo A2A (Agent-to-Agent)](/docs/a2a-protocol). Cada app Agent-Native obtém automaticamente um endpoint A2A. No aplicativo de e-mail, você pode marcar o agente de análise para consultar dados. Um agente descobre quais outros agentes estão disponíveis, chama-os pelo protocolo e mostra os resultados na UI. Não é necessária configuração: o cartão do agente é gerado automaticamente a partir das actions do seu modelo.
...
### Quais ferramentas de codificação de IA funcionam com Agent-Native? {#which-ai-tools-work}
