---
title: "Doctor (Verificações de código)"
description: "agent-native doctor verifica o código-fonte do aplicativo quanto a segurança e arquitetura."
---

# Doctor (Verificações de código)
...
Guards run: no-drizzle-push, no-unscoped-credentials, no-unscoped-queries, no-env-credentials, db-tool-scoping, no-env-mutation, no-localhost-fallback, migration-manifest
Clean — no findings.
```
...
## The eight guards {#guards}

v1 ships 8 guards. Each is a pure scan over the app's source tree — `node_modules`, `.git`, `dist`, `build`, and other build-output directories are always skipped.
