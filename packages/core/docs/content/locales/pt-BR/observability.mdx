...
| **Revisão humana** | Revise o pedido e a resposta, registre feedback e elabore uma atualização das instruções                           |

### Detalhes das conversas {#conversations}

A guia Conversas mostra as execuções registradas. Selecione uma para inspecionar seus períodos e expanda qualquer período para ver entradas, saídas, erros e metadados de ferramentas capturados. **Abrir conversa completa** leva à conversa de chat correspondente.

Os argumentos e resultados das ferramentas não são capturados por padrão. Ative `captureToolArgs` e `captureToolResults` na configuração `observability` para armazená-los. Campos sensíveis e mensagens de erro das ferramentas são ocultados antes do armazenamento.

### Revisão humana {#review}

A guia Revisão humana mostra o pedido e a resposta de cada execução, permite registrar feedback e salvar um rascunho de atualização das instruções para revisão humana. Salvar o rascunho não altera as instruções automaticamente.
