...
A observabilidade e o CI Eval Gate pontuam _qualidade_, mas de extremos opostos - pontuação post-hoc passiva de tráfego real versus verificações ativas de aprovação/reprovação em entradas fixas. A memória observacional não está relacionada à qualidade; trata-se do custo do token e da pressão da janela de contexto. Promova um trace de produção concluído a um caso `*.eval.ts` (painel **Promote to eval**, action `promote-trace-eval`, ou `agent-native eval promote <runId> --write evals/from-trace.eval.ts`). Veja [A partir de um trace de produção](/docs/evals#from-trace).
...
| POST   | `/traces/:runId/promote`   | Promover uma execução concluída a eval de CI |
