...
Os apps hospedados canônicos reconhecem solicitações do Desktop SSO Canary empacotado sem configuração de hub de identidade por app. Navegadores comuns e apps auto-hospedados ainda exigem `AGENT_NATIVE_IDENTITY_HUB_URL`. Builds estáveis do Desktop não inicializam o intermediário até que uma versão posterior seja aprovada separadamente.
