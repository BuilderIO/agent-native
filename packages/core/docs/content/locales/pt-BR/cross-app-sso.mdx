...
Implemente primeiro o flag de identidade do navegador. Depois de implantar as colunas de migração da organização no aplicativo e no Dispatch, ative `organization.cross-app-federation` para uma organização e verifique o ID canônico e a função. Organizações existentes sem mapeamento permanecem inalteradas até uma migração explícita.
