...
### Contexto da organização {#organization-context}

A flag desativada por padrão `organization.cross-app-federation` adiciona uma
etapa somente no servidor para o contexto das organizações do workspace. Quando
um aplicativo cria ou lê uma organização, ele a registra no Dispatch e guarda o
mapeamento exato `(identity_authority, identity_id)`. Um login posterior em outro
aplicativo habilitado leva o mesmo `org_id` canônico, o nome da organização e a
função do membro na asserção assinada. O aplicativo de destino cria ou vincula
sua organização local usando esse ID.

Nomes de organizações, domínios de e-mail e cookies do navegador não são chaves
de vínculo. Uma organização local existente sem mapeamento permanece inalterada
e não é mesclada automaticamente. Use uma migração explícita depois de confirmar
a correspondência desejada.

Organizações locais não vinculadas continuam utilizáveis durante uma interrupção
temporária do Dispatch. Associações federadas vinculadas falham com segurança até
que a autoridade esteja acessível, e o registro é tentado novamente na próxima
...
<Diagram id="doc-block-v0atqe" title={"Fluxo de federação de identidade"} summary={"O Dispatch autentica a pessoa e retorna uma declaração assinada de curta duração com o e-mail verificado e, quando ativado, o contexto exato da organização. O aplicativo vincula a conta por e-mail, mapeia a organização pelo ID e cria sua própria sessão local."}>
...
3. **Dispatch → Aplicativo (token de identidade assinado).** O Dispatch valida `redirect_uri` em uma lista de permissões estrita e redireciona 302 de volta para o `redirect_uri` do aplicativo carregando uma identidade de curta duração **`A2A_SECRET` assinada por JWT**. As reivindicações do token são intencionalmente mínimas:
   ...
   | `org_id`      | ID canônico da organização, somente com a federação ativada                |
   | `org_name`    | Nome da organização associado a `org_id`                                   |
   | `org_role`    | Função de membro verificada associada a `org_id`                           |
   | `scope`       | Sempre `"identity"`. Campos de organização só são aceitos com o flag ativo |
   | `exp`         | **≤ 2 minutos** a partir da edição                                         |

4. **O aplicativo verifica e vincula JIT por e-mail.** A rota `GET /_agent-native/identity/callback` verifica a assinatura do token com `A2A_SECRET`, confere `scope: "identity"` e `exp` e realiza **vinculação just-in-time por e-mail verificado**:
   - Se existir um usuário local com esse e-mail → reutilize-o inalterado.
   - Caso contrário → crie um usuário local para esse e-mail.
     Quando `organization.cross-app-federation` está ativo, o aplicativo também valida juntos os campos assinados `org_id`, `org_name` e `org_role`. Ele mapeia a organização local somente pela autoridade e pelo ID exatos, nunca pelo nome ou domínio de e-mail.
...
- **Cliente hospedado canônico** → o aplicativo mostra a opção **"Entrar com Agent-Native"** para a origem registrada; apps auto-hospedados continuam precisando definir a variável. Colunas de mapeamento aditivas podem existir após a implantação, mas ficam sem uso quando o flag está desativado.
...
- **Lista de permissões estrita de `redirect_uri` vinculada ao aplicativo.** Cada ID de aplicativo canônico corresponde a uma origem de produção e um caminho de callback exatos; localhost continua disponível para desenvolvimento. Subdomínios desconhecidos, IDs incompatíveis, caminhos arbitrários e destinos de origem cruzada são rejeitados.
- **Vinculação por e-mail verificado e mapeamento exato da organização.** O e-mail verificado continua sendo a única chave de junção das contas de usuário. Com o flag de organização ativo, o aplicativo aceita `org_id`, `org_name` e `org_role` somente de uma declaração Dispatch verificada criptograficamente e mapeia organizações apenas pela autoridade e pelo ID exatos.
- **Gravações de identidade e organização somente aditivas.** A vinculação reutiliza uma conta de mesmo e-mail existente intacta ou insere uma nova. O mapeamento da organização apenas é acrescentado. Nenhuma renomeação, reatribuição ou exclusão de linhas existentes ocorre nesse caminho.
- **Clientes hospedados canônicos habilitados por origem.** Apps hospedados canônicos com uma origem registrada são habilitados automaticamente; apps auto-hospedados continuam inativos sem `AGENT_NATIVE_IDENTITY_HUB_URL`.

<Callout id="doc-block-6dy3lq" tone="success">

**Seguro para ativar, seguro para reverter.** As gravações de identidade e os mapeamentos de organização são **apenas aditivos**. Uma conta existente é reutilizada sem alterações, um novo e-mail insere uma linha e a migração adiciona colunas de mapeamento anuláveis. Desativar qualquer flag não exclui usuários, organizações, associações ou credenciais locais.

</Callout>

O link just-in-time é uma decisão única digitada inteiramente no e-mail verificado:

<Diagram id="doc-block-15y8gtf" title={"Decisão JIT-link"} summary={"A conta do usuário é vinculada pelo e-mail verificado. Quando ativada, a organização é mapeada pela autoridade e pelo ID exatos. Todas as gravações são aditivas."}>
...
- **Padrão:** somente pares exatos de ID/origem de aplicativos Agent-Native registrados e localhost (`CANONICAL_IDENTITY_SSO_APP_ORIGINS`).
...
Implemente primeiro o flag de identidade do navegador. Depois de implantar as colunas de migração da organização no aplicativo e no Dispatch, ative `organization.cross-app-federation` para uma organização e verifique o ID canônico e a função. O SSO do navegador continua usando `A2A_SECRET`, mas a federação de organizações usa uma credencial separada por aplicativo: defina `AGENT_NATIVE_IDENTITY_FEDERATION_SECRET` no aplicativo de origem e o `AGENT_NATIVE_IDENTITY_FEDERATION_SECRET_<APP_ID>` correspondente no Dispatch. Nunca reutilize o `A2A_SECRET` compartilhado para este limite de autoridade. Organizações existentes sem mapeamento permanecem inalteradas até uma migração explícita.
...
Os apps hospedados canônicos reconhecem solicitações do Desktop SSO Canary empacotado e navegadores comuns sem configuração de hub de identidade por app. Apps auto-hospedados ainda exigem `AGENT_NATIVE_IDENTITY_HUB_URL`. Builds estáveis do Desktop não inicializam o intermediário até que uma versão posterior seja aprovada separadamente.
