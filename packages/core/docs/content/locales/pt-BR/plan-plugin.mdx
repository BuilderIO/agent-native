...
description: "Instale as skills do Agent-Native Plan (/visual-plan, /visual-recap, /visualize-repo) mais o conector do Plan MCP hospedado como plug-in Claude Code ou Codex, ou com o CLI universal. Como funcionam as atualizações e se você precisa enviar algo."
---

# Planejar plug-in e mercado

O aplicativo Agent-Native **Plan** é enviado como um pacote instalável. Uma única instalação adiciona as skills de comando slash do Plan **e** conecta o conector do Plan MCP hospedado, para que o agente possa gerar planos e as skills possam publicá-los diretamente no aplicativo Plan.

## O que você ganha {#what-you-get}

Uma instalação oferece:

- **Três skills** — `/visual-plan` (o ponto de entrada canônico), `/visual-recap` e `/visualize-repo`.
- **O conector do Plano MCP** — registrado no aplicativo hospedado em `https://plan.agent-native.com` (endpoint MCP `https://plan.agent-native.com/mcp`, nome do servidor `plan`).

<Diagram id="doc-block-1uxomn4" title={"Três rotas, um pacote"} summary={"O CLI universal, o plug-in Claude Code e o plug-in Codex instalam as mesmas skills do Plan mais o conector do Plan hospedado."}>
...
Por padrão, `/visual-plan` e `/visual-recap` publicam no aplicativo Plan hospedado — eles criam um plano via
...
<Diagram id="doc-block-rf25cs" title="Modo hospedado vs. modo de arquivos locais" summary={"Por padrão, as habilidades são publicadas por meio do conector; O modo local-files grava MDX no disco e visualiza por meio de uma ponte localhost."}>

```html
<div class="diagram-modes">
  <div class="diagram-card">
    <span class="diagram-pill accent">Padrão · hospedado</span
    ><strong>Publicar no app Plan</strong
  ...
  <div class="diagram-card">
    ...
    ><strong>Gravar MDX no disco</strong
    ><small class="diagram-muted"
      >plan.mdx + canvas.mdx + prototype.mdx &rarr; localhost bridge &rarr;
      hosted Plan UI reads local source. No Escrita no DBs until
...
Funciona para qualquer host — Claude Code, Codex, Cursor, Cline, Goose, aplicativos ChatGPT custom MCP, Claude Cowork e qualquer outra coisa compatível com MCP. O Agent-Native CLI instala as skills do Plan, registra o conector do Plan MCP hospedado **e executa a autenticação para os clientes locais selecionados na mesma etapa**, para que sua primeira chamada de ferramenta não encontre uma parede OAuth:

```bash
npx @agent-native/core@latest skills add visual-plan
```

Isso instala `visual-plan` mais as skills complementares `visual-recap` e `visualize-repo`, registra o conector `plan` e executa a autenticação (prompt OAuth para compartilhamento hospedado/apoiado por conta). Flags úteis:
...
`/plugin install` adiciona as skills do Plan e uma configuração MCP **somente URL** (sem segredos no pacote); `/mcp` → **Authenticate** conclui o handshake OAuth. Use a rota CLI universal quando quiser arquivos locais ou modo auto-hospedado.
...
codex mcp login plan   # OAuth no navegador
```

Após a instalação, **inicie um novo thread Codex** para que as ferramentas skills e MCP sejam carregadas na sessão. O plugin vem com um conector somente URL (`[mcp_servers.plan]` → `https://plan.agent-native.com/mcp`); `codex mcp login plan` executa o fluxo OAuth. A rota universal CLI acima também funciona para Codex (`npx @agent-native/core@latest skills add visual-plan --client codex`) se você preferir um comando que instale e autentique juntos ou quando desejar arquivos locais ou modo auto-hospedado.
