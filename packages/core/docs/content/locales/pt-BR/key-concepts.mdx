...
| Sincronização ao vivo entre agente e UI   | Em um host de longa duração, gravações do mesmo processo são transmitidas imediatamente via `/_agent-native/events`; um polling leve mantém as gravações de cron e entre processos convergentes, e é o único caminho do app em serverless de produção (no mesmo ritmo), a menos que ele tenha ativado o [Gateway de sincronização em tempo real hospedado](/docs/real-time-collaboration#hosted-gateway). Ações que fazem mutação invalidam automaticamente as queries baseadas em ação, de modo que registros criados pelo agente aparecem sem uma atualização manual. Veja [Live Sync](#polling-sync) abaixo. |
...
Quando o agente altera dados, a UI precisa refletir isso sem uma atualização manual. `useDbSync()` é o que torna isso automático. Em um host de longa duração, gravações do mesmo processo são transmitidas via `/_agent-native/events`; `/_agent-native/poll` continua sendo o substituto para entre processos. Quando o agente grava no banco de dados (estado da aplicação, configurações ou dados de domínio), um contador de versão é incrementado e o cliente invalida os caches relevantes do React Query.

O SSE cobre o caso comum em vez de WebSockets porque oferece às gravações do mesmo processo um caminho imediato até o navegador usando HTTP simples, enquanto o polling por contador de versão permanece como um substituto que funciona em qualquer ambiente de implantação. Em um host serverless de produção (Netlify, Lambda, Vercel), o próprio stream `/_agent-native/events` do app não é um caminho mais lento — está desligado. Manter esse stream aberto ocuparia uma instância de função que atende uma requisição por vez (e por isso nunca poderia ver a gravação de outra invocação de qualquer forma) até a plataforma encerrá-la, reciclando o container; em vez disso, o servidor responde imediatamente com 204, e `/_agent-native/poll` cuida de toda a sincronização, no mesmo ritmo, a menos que o app tenha ativado o [Gateway de sincronização em tempo real hospedado](/docs/real-time-collaboration#hosted-gateway), que transmite a partir do seu próprio processo de longa duração.
