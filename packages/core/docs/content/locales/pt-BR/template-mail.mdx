...
<DataModel id="doc-block-8jxgi9" title="Enviar tabelas SQL" summary={"O próprio e-mail reside em Gmail. As tabelas SQL contêm o que Gmail não contém: rascunhos em fila, eventos de rastreamento de envio e tokens OAuth. As configurações e o estado efêmero residem nas configurações e nos armazenamentos application_state."} entities={[
  {
