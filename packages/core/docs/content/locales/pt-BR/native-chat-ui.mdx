...
### Escolha quando a interface da ação aparece {#conditional-native-ui}

Uma ação pode ter várias operações, mas só alguns resultados precisam de um cartão. Adicione `when(args, result)` a `chatUI` para selecionar essas chamadas. O servidor chama esse predicado após uma ação bem-sucedida. Se ele retornar `false`, o resultado continua no histórico comum de ferramentas recolhível. O predicado não é salvo nem enviado ao navegador.

No Mail, `manage-draft` usa isso para mostrar um cartão nos resultados de criação, enquanto atualizações e exclusões continuam como chamadas comuns:
...
Para ações com várias operações de gravação, verifique os argumentos e o resultado bem-sucedido. A ação de filtros do Gmail no Mail seleciona o cartão da regra somente para filtros criados ou substituídos com os campos de resumo:
