...
<DataModel id="doc-block-blbynx" title="Modelo de dados cerebrais" summary={"Connectors produce raw captures; distillation turns captures into reviewable knowledge; proposals gate sensitive entries. Sync runs and the ingest queue track background work."} entities={[
  {
    "id": "sources",
    "name": "brain_sources",
