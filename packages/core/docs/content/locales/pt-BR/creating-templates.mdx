...
`runDrizzleMigrations()` na etapa de release. Mantenha `runMigrations()` para o
histórico escrito à mão e casos especiais. Nunca use SQL destrutivo,
`drizzle-kit push`, renomeações de tabelas ou eliminações de colunas.
