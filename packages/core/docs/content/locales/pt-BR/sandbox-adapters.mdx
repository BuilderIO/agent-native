...
<FileTree id="doc-block-1j5ataj" title={"A junção do sandbox no core"} title="packages/core/src/coding-tools/sandbox/" entries={[
  {
    "path": "adapter.ts",
    "note": "O contrato SandboxAdapter (SandboxRunRequest / SandboxRunResult)"
  },
  {
    "path": "index.ts",
    "note": "Seleção: getSandboxAdapter() / registerSandboxAdapter()"
  },
  {
    "path": "local-child-process-adapter.ts",
    "note": "O backend padrão: processo filho Node bloqueado"
  },
  {
    "path": "../run-code.ts",
    "note": "Conecta a junção; nunca muda quando você troca backends"
  }
]} />
