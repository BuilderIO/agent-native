...
Para operações de aplicativos](/docs/server-overview#actions-first).
...
- **`readOnly: true`** — ignora explicitamente a atualização da enquete, mesmo para POST actions que não sofre mutação. **Não** é controle de acesso: não restringe quem pode chamar a action e não afeta `useActionQuery`, `useActionMutation` nem qualquer hook de cliente. Seus únicos efeitos são suprimir o evento de atualização após a chamada e excluir a action da auditoria de escrita. Para ocultar uma action do modelo use `agentTool: false`; para restringir quem chama use as opções de auth/acesso acima.
