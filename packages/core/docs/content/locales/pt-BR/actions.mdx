...
Use `useFeatureFlag(FULL_APP_BUILDING.key)` de `@agent-native/core/client` para ocultar ou revelar a superfície do cliente. O Core disponibiliza `get-feature-flags`, `list-feature-flags` e `set-feature-flag`; a aba compartilhada de configurações de feature flags usa essas mesmas actions para rollouts por usuário específico, organização, percentual e global. O gerenciamento verifica permissões e é auditado. Nunca encaminhe gravações de flags por endpoints genéricos de configurações ou SQL bruto, nem coloque autenticação, autorização, auditoria, segredos ou o comportamento do cache SSR atrás de uma flag de runtime. Leia a skill `feature-flags` para obter orientações sobre rollout e remoção.
