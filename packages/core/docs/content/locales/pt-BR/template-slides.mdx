...
<DataModel id="doc-block-1gbgyel" title={"Modelo de dados do Apresentações"} summary={"A deck owns its slides as JSON in decks.data; comments, versions, shares, and design systems hang off it."} entities={[
  {
    "id": "decks",
    "name": "decks",
