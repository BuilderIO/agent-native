...
<AnnotatedCode
  ...
  annotations={[
    ...
    },
    {
      lines: "7-12",
      label: "O prompt",
      note: "O corpo é apenas um prompt — o agente o executa a cada disparo com todas as suas ferramentas normais e contexto de espaço de trabalho.",
    },
  ]}
/>
...
Trabalhos recorrentes podem usar ferramentas MCP remotas ou locais que já estejam conectadas ao app. Peça ao agente para descobrir os nomes exatos com `tool-search` e vincule apenas as ferramentas necessárias em `mcpTools`:
...
Encontre gravações novas desde a última execução bem-sucedida. Extraia apenas
acompanhamentos acionáveis e chame uma vez a action de importação limitada do
app com IDs de origem e externos estáveis, uma citação curta e a URL da
gravação. Não importe a transcrição completa.
```

O trabalho armazena apenas nomes de ferramentas, não URLs MCP, tokens de acesso ou tokens de atualização OAuth. Em tempo de execução, o framework resolve a concessão conectada no contexto de requisição do criador do trabalho ou da organização e expõe apenas as ferramentas permitidas. Mantenha a lista pequena e use uma action idempotente do app na importação final para que execuções sobrepostas não criem duplicatas. Consulte [MCP Clients](/docs/mcp-clients) para configurar conexão e OAuth.
