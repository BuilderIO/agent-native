...
Builder é o caminho mais fácil: conecte Builder.io (nível gratuito disponível) em **Settings → Connections → Integrations** e o provedor padrão se tornará Builder Gemini Flash-Lite. Para provedores BYOK, adicione a chave correspondente do Gemini, Groq ou OpenAI.

### Por usuário (recomendado para SaaS)

O usuário adiciona a própria chave em **Personal providers** na página **Settings → Agent → Model**. Ela é armazenada como um segredo criptografado no escopo do usuário (via `readAppSecret`). Cada usuário paga pela própria transcrição; custo zero para o anfitrião.

### Compartilhado (para ferramentas internas)

Defina `GEMINI_API_KEY`, `GROQ_API_KEY` ou `OPENAI_API_KEY` como variável de ambiente, ou peça a um proprietário ou administrador para adicioná-la para a organização em **Organization providers** na página **Settings → Agent → Model**. A linha **Voice input** em **Settings → Organization → Infrastructure** escolhe qual provedor responde primeiro. A transcrição de cada usuário usa a chave compartilhada.
