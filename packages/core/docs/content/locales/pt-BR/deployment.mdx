...
- `extensionTools: true` — habilita no aplicativo as actions e orientações de
  gerenciamento de extensões (`create-extension`, `update-extension` etc.). As
...
[Builder.io](https://www.builder.io?utm_source=agent-native&utm_medium=docs&utm_campaign=framework&utm_content=deployment) resolve isso fornecendo um ambiente de nuvem gerenciado onde o agente mantém a capacidade de modificar o UI do seu aplicativo em produção. Conecte seu repositório ao Builder.io e solicite alterações no UI diretamente, sem necessidade de reimplantação.
