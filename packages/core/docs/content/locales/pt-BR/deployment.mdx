...
Um Worker não tem sistema de arquivos, então o banco de dados é uma
`DATABASE_URL` externa ou o Cloudflare D1. Defina
`CLOUDFLARE_D1_DATABASE_NAME` e `CLOUDFLARE_D1_DATABASE_ID` juntas no
ambiente de build para que o build escreva o binding `DB` no
`wrangler.json` gerado; definir apenas uma delas é um erro, e não um binding
omitido em silêncio. A sequência completa de inicialização local está no guia
de deploy em inglês.


### Browser Rendering {#cloudflare-browser-rendering}

Um Worker não consegue iniciar um processo do Chromium: o navegador é um
binding, e o `wrangler.json` gerado declara
`{ "browser": { "binding": "BROWSER" } }`. As ações que dependem do navegador
resolvem esse binding através do `@cloudflare/playwright`. Se o binding estiver
ausente, ele é nomeado no erro em vez de se culpar um binário do Chromium
inexistente. Localmente funciona com `wrangler dev`, com uma limitação: o
runtime local rejeita requisições maiores que 1 MB.

### Cloudflare R2 {#cloudflare-r2}

Um Worker não tem sistema de arquivos e o conteúdo dos arquivos não pode ficar
em SQL, então os uploads vão para o R2. Defina `CLOUDFLARE_R2_BUCKET_NAME` no
ambiente de build para que o vínculo `UPLOADS` seja escrito no `wrangler.json`
gerado, e `CLOUDFLARE_R2_PUBLIC_BASE_URL` com a URL pública do bucket. O
provedor já vem registrado no registro de uploads. Um vínculo ausente ou
inutilizável falha com orientação de configuração em vez de gravar o payload em
SQL. Detalhes no guia de deploy em inglês.
...
- `extensionTools: true` — habilita no aplicativo as actions e orientações de
