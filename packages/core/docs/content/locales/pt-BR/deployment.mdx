...
| `AGENT_NATIVE_BUILDER_RELAY_TARGET_ORIGINS` | Lista separada por vírgulas de origens de prévia confiáveis e exatas, exclusiva do callback corporativo (por exemplo, `https://deploy-preview-42--content.netlify.app`). Defina-a somente na implantação do callback corporativo. Curingas e sufixos de domínio não são compatíveis.                                         |
