...
| `A2A_SECRET`             | HMAC compartilhado para A2A JSON-RPC entre apps. Sem `SECRETS_ENCRYPTION_KEY`, o cofre compartilhado usa uma chave derivada desta finalidade; girar esse valor também gira a chave do cofre.                                                                                                                                                                       |
| `SECRETS_ENCRYPTION_KEY` | Chave AES-256-GCM estável e preferida para o cofre compartilhado. Defina o mesmo valor em todo app que o lê. Leituras recorrem ao `A2A_SECRET` derivado e depois a chaves antigas do Better Auth e específicas do app. Antes de girar A2A, adicione esta chave em todos os lugares e leia/migre todas as linhas enquanto o material A2A antigo estiver disponível. |
