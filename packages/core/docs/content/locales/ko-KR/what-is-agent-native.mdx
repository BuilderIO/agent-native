...
에이전트 네이티브는 개발자만을 위한 것이 아닙니다. 임베디드 에이전트는 팀 전체에 앱을 운영하는 공통 방식을 제공하고, 소스 변경은 일반 개발 워크플로에 남습니다. 소스를 편집할 수 있는 것은 호스트가 repository/workspace 쓰기 도구를 의도적으로 부여한 경우뿐입니다.
...
## 템플릿에서 시작하고 맞춤 설정하기 {#fork-and-customize}

에이전트 기반 앱은 작동하는 app에서 시작하는 모델을 따릅니다. 달력, 콘텐츠, 슬라이드, 분석, 메일, 클립, 디자인, 양식, 발송 등 **템플릿**에서 시작하여 나만의 것으로 만들 수 있습니다. 각각은 빈 발판이 아닌 완전하고 작동하는 SaaS 제품입니다.

1. [agent-native.com/templates](/templates)에서 템플릿을 선택하세요
2. 호스팅 앱으로 즉시 사용하세요(예: mail.agent-native.com)
3. 맞춤 설정을 원할 때 템플릿에서 나만의 app을 만드세요 — "Stripe 계정 연결", "코호트 차트 추가"
4. 자체 저장소와 개발 워크플로에서 소스를 사용자 지정합니다.
5. app을 자신의 도메인에 배포하거나 Agent-native.com을 유지하세요

공유 인프라가 아닌 _귀하의_ 앱이므로 소스를 소유하고 공유 프레임워크를 변경하지 않고 발전시킬 수 있습니다. 임베디드 에이전트는 actions와 데이터를 통해 작업하며, 쓰기 도구가 의도적으로 부여된 경우에만 소스 변경을 지원할 수 있습니다. 전체 내용을 보려면 [Templates](/docs/cloneable-saas)를 참조하세요.

전체 템플릿에서 시작할 준비가 되지 않았습니까? 이미 사용하고 있는 코딩 에이전트에 **스킬**을 추가하여 에이전트 네이티브를 사용해 볼 수도 있습니다. `npx @agent-native/core@latest skills add visual-plan`로 Plans 스킬을 설치하세요. [Skills Guide](/docs/skills-guide#app-backed-skills)를 참조하세요.
