...
일부 리소스는 그룹 공유를 지원합니다. 그룹은 리소스와 같은 조직에 속해야 하며, 현재 각 구성원에게 해당 리소스에서 선택한 공유 역할이 적용됩니다. 그룹의 범위와 한계는 [조직, 팀 및 권한 → 사용자 그룹](/docs/organizations-teams-permissions#user-groups)에서 확인하세요.
