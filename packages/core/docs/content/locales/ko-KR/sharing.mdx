...
<DataModel id="doc-block-f64u84" title={"리소스 + 컴패니언 공유 테이블"} summary={"Coarse visibility lives on the resource; each fine-grained grant is a row in the shares table."} entities={[
  {
    "id": "deck",
    "name": "decks",
    "note": "...ownableColumns()",
