...
<DataModel id="doc-block-1rkl11j" title={"캘린더 데이터 모델"} summary={"이벤트가 아닌 데이터만 로컬에 저장됩니다. 이벤트는 Google Calendar에 실시간으로 저장됩니다. 예약 링크는 ownableColumns를 사용하므로 공유 시스템이 적용됩니다."} entities={[
  {
    "id": "booking_links",
    "name": "booking_links",
