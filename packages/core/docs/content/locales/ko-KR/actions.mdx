...
클라이언트 화면을 숨기거나 표시하려면 `@agent-native/core/client`의 `useFeatureFlag(FULL_APP_BUILDING.key)`를 사용하세요. Core는 `get-feature-flags`, `list-feature-flags`, `set-feature-flag`를 제공하며, 공통 feature flag 설정 탭도 특정 사용자, 조직, 비율 및 전체 rollout에 동일한 actions를 사용합니다. 관리 작업에는 권한 검사가 적용되고 감사 기록이 남습니다. flag 쓰기를 일반 설정 endpoint나 원시 SQL을 통해 처리하지 말고, 인증, 권한 부여, 감사, secret 또는 SSR 캐시 동작을 runtime flag 뒤에 두지 마세요. rollout 및 제거 지침은 `feature-flags` skill을 참고하세요.
