...
앱 운영용](/docs/server-overview#actions-first).
...
- **`readOnly: true`** — 변형되지 않는 POST actions에 대해서도 폴링 새로 고침을 명시적으로 건너뜁니다. 이것은 접근 제어가 **아닙니다**. 누가 action을 호출할 수 있는지 제한하지 않으며, `useActionQuery`, `useActionMutation` 등 어떤 클라이언트 hook에도 영향을 주지 않습니다. 유일한 효과는 호출 후 새로고침 이벤트를 억제하고 action을 쓰기 감사 대상에서 제외하는 것입니다. 모델에서 action을 숨기려면 `agentTool: false`를, 호출자를 제한하려면 위의 인증/접근 옵션을 사용하세요.
