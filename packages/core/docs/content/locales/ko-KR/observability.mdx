...
| **사람 검토** | 요청과 답변을 검토하고 피드백을 기록한 뒤 지침 업데이트 초안을 작성합니다    |

### 대화 세부 정보 {#conversations}

대화 탭에는 기록된 실행이 표시됩니다. 실행을 선택해 스팬을 확인하고, 스팬을 펼쳐 캡처된 도구 입력, 출력, 오류 및 메타데이터를 볼 수 있습니다. **전체 대화 열기**를 선택하면 연결된 채팅 스레드로 이동합니다.

도구 인수와 결과는 기본적으로 캡처되지 않습니다. 저장하려면 `observability` 설정에서 `captureToolArgs`와 `captureToolResults`를 활성화하세요. 민감한 필드와 도구 오류 메시지는 저장 전에 마스킹됩니다.

### 사람 검토 {#review}

사람 검토 탭에서는 각 실행의 요청과 답변을 확인하고, 피드백을 기록하며, 사람이 검토할 지침 업데이트 초안을 작성할 수 있습니다. 초안을 저장해도 지침이 자동으로 바뀌지는 않습니다.
