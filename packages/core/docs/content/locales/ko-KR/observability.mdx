...
관찰 가능성과 CI 평가 게이트는 둘 다 _품질_ 점수를 매기지만 반대쪽 끝, 즉 실제 트래픽의 수동적 사후 점수와 고정 입력에 대한 활성 통과/실패 확인의 점수를 매깁니다. 관찰 기억은 품질과 관련이 없습니다. 토큰 비용과 컨텍스트 창 압박에 관한 것입니다. 완료된 프로덕션 트레이스를 `*.eval.ts` 케이스로 승격하세요(대시보드 **Promote to eval**, `promote-trace-eval`, 또는 `agent-native eval promote <runId> --write evals/from-trace.eval.ts`). [프로덕션 트레이스에서](/docs/evals#from-trace)를 보세요.
...
| POST | `/traces/:runId/promote`   | 완료된 실행을 CI eval로 승격 |
| POST | `/feedback`                | 의견 제출                  |
| GET  | `/feedback`                | 피드백 나열                |
