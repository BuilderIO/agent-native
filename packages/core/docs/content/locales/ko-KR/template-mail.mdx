...
<DataModel id="doc-block-q77xut" title={"메일 SQL 테이블"} summary={"이메일 자체는 Gmail에 있습니다. SQL 테이블에는 Gmail에 없는 내용(대기 중인 초안, 전송 추적 이벤트, OAuth 토큰)이 들어 있습니다. 설정 및 임시 상태는 settings 및 application_state 저장소에 있습니다."} entities={[
  {
