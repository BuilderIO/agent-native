...
<DataModel id="doc-block-1bls1jz" title={"설계 데이터 모델"} summary={"디자인은 해당 파일과 버전이 지정된 스냅샷을 소유하고 선택적으로 재사용 가능한 디자인 시스템을 연결합니다. 디자인과 시스템 모두 소유 가능하며 각각 프레임워크 공유 테이블이 있습니다."} entities={[
  {
    "id": "designs",
    "name": "designs",
