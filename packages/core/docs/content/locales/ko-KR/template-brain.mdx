...
<DataModel id="doc-block-4tvj3q" title={"뇌 데이터 모델"} summary={"Connectors produce raw captures; distillation turns captures into reviewable knowledge; proposals gate sensitive entries. Sync runs and the ingest queue track background work."} entities={[
  {
    "id": "sources",
    "name": "brain_sources",
    "note": "커넥터 구성",
