...
인증은 연결된 계정을 식별할 뿐 문서 권한을 넓히지 않습니다. 공개 문서는 공유 페이지에서 계속 읽을 수 있습니다. 비공개 문서는 해당 계정에 기존 접근 권한이 있을 때만 MCP로 읽을 수 있습니다. 연결 후 `get-document`가 접근 거부를 반환하면 소유자가 해당 계정에 접근 권한을 부여해야 합니다.

커넥터를 다시 설치하거나 공개 공유 링크를 변경해도 연결된 계정의 권한 거부는 해결되지 않습니다.
