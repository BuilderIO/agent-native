...
외부 에이전트는 앱 내 에이전트가 호출하는 것과 동일한 이름의 실제 생성/수정 도구에 접근할 수 있습니다. 따라서 사용자가 새로운 콘텐츠(디자인, 덱, 폼, 문서 등)를 요청하면, 작업을 `ask_app`으로 위임하지 말고 직접 작성한 뒤 해당 도구로 저장하세요. `endsTurn: true`로 표시된 액션 ― 예를 들어 사용자의 답변을 해당 앱 자체의 채팅에서 기다리는 앱 내 질문 폼 ― 은 액션이 `mcpTool: true`로 명시적으로 다시 활성화하지 않는 한 기본적으로 MCP, WebMCP, A2A에서 숨겨집니다. 이는 외부 호출자가 아니라 앱 내 에이전트의 도구입니다. WebMCP 브리지는 도구 입력과 결과를 최대 500,000자까지 허용하므로, 직접 작성한 화면, 덱, 문서 전체를 나누지 않고 한 번의 쓰기로 저장할 수 있습니다. 페이지 로컬 WebMCP 호출도 브라우저 탭 ID를 함께 전달하므로, 탭별 컨텍스트(내비게이션, 선택 항목)가 사용자가 실제로 보고 있는 탭으로 해석됩니다.
