...
앱에서 **Settings → Connections → Integrations**를 열고 알고 있는 호스트 이름을 검색하세요.
Claude, Claude Cowork, Claude Code, Anthropic, ChatGPT, OpenAI, Codex, Grok,
xAI 중 어떤 이름으로도 검색할 수 있습니다. 외부 호스트 결과 하나를 선택하면 가장 알맞은
가이드가 선택된 **Settings → MCP server**가 열립니다. **MCP server URL**을 복사해 AI
...
Settings에는 앱 그룹에 **MCP server** 페이지가 있습니다. 앱의 표준 MCP
URL, 가능한 경우 A2A 에이전트 카드, 전체 연결 페이지와 같은 설정 가이드를 보여줍니다.
앱에서 시작하려면 **Settings → MCP server**를 사용하거나, 전체 흐름과 토큰 대체 경로를
