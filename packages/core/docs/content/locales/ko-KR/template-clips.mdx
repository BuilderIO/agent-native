...
<DataModel id="doc-block-1kk6g43" title={"클립 핵심 데이터 모델"} summary={"recordings is the source of truth for media; transcripts, meetings, and dictations compose with it rather than duplicating video. (Engagement and org tables omitted for clarity — see the full table below.)"} entities={[
  {
    "id": "recordings",
    "name": "recordings",
