...
JSON 따옴표는 필수입니다. 경량 `gate` 작업과 전체 `recap` 작업 모두 선택한 러너를 사용합니다. 안내식 설정 중 구성하려면 동일한 JSON 값을 `--runs-on`에 전달한 다음 `doctor`로 워크플로와 러너 가용성을 검증하세요.
...
- **`VISUAL_RECAP_MODEL`**는 CLI(`--model`)에 전달된 모델을 고정합니다. 예를 들어 Codex의 경우 `gpt-5.6-sol` 또는 Claude 모델 ID입니다. CLI 자체 기본 모델을 사용하려면 설정하지 않은 상태로 두세요.
...
| `VISUAL_RECAP_MODEL`     | 각 CLI의 기본값                 | 변수입니다. 모델을 고정합니다. Codex의 경우 `gpt-5.6-sol` 또는 Claude 모델 ID입니다. Unset은 CLI 자체 기본값을 사용합니다.                          |
...
[Secrets and variables](#secrets-and-variables)에 설명된 것과 동일한 비밀 및 변수가 적용됩니다. 복사 변형과 동일한 방식으로 저장소 설정에서 설정하세요. 재사용 가능한 호출자는 `VISUAL_RECAP_RUNS_ON`을 `runs-on` 입력으로 전달하고, 호출된 워크플로는 JSON을 파싱하여 두 작업 모두에 그 결과를 사용합니다. 입력을 생략하면 `ubuntu-latest` 기본값이 유지됩니다.
