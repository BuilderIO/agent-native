...
반복 작업은 앱에 이미 연결된 원격 또는 로컬 MCP 도구를 사용할 수 있습니다. 에이전트에게 `tool-search`로 정확한 이름을 찾게 한 다음 `mcpTools`에 작업에 필요한 도구만 지정하세요.
...
마지막 성공 실행 이후의 새 녹음을 찾습니다. 실행 가능한 후속 조치만
추출한 뒤 안정적인 source 및 external ID, 짧은 인용, 녹음 URL을 포함하여
앱의 제한된 import action을 한 번 호출합니다. 전체 transcript는 import하지
않습니다.
```

작업에는 도구 이름만 저장하며 MCP URL, access token 또는 OAuth refresh token은 저장하지 않습니다. 실행 시 프레임워크는 작업 생성자 또는 조직의 request context에서 연결된 grant를 해석하고 allowlist에 지정된 도구만 노출합니다. 목록을 작게 유지하고 최종 import에는 멱등적인 app action을 사용하여 겹치는 실행이 중복을 만들지 않게 하세요. 연결 및 OAuth 설정은 [MCP Clients](/docs/mcp-clients)를 참고하세요.
