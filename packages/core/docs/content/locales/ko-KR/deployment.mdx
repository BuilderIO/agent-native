...
Worker에는 파일 시스템이 없으므로 데이터베이스는 외부 `DATABASE_URL`이거나
Cloudflare D1입니다. 빌드 환경에서 `CLOUDFLARE_D1_DATABASE_NAME`과
`CLOUDFLARE_D1_DATABASE_ID`를 함께 설정하면 빌드가 생성된
`wrangler.json`에 `DB` 바인딩을 기록합니다. 둘 중 하나만 설정하는 것은
바인딩을 조용히 생략하는 대신 오류로 처리됩니다. 전체 로컬 기동 순서는 영어
배포 가이드를 참고하세요.


### Browser Rendering {#cloudflare-browser-rendering}

Worker는 Chromium 프로세스를 실행할 수 없으므로 브라우저는 바인딩으로
제공되며, 생성된 `wrangler.json`에
`{ "browser": { "binding": "BROWSER" } }`가 선언됩니다. 브라우저에 의존하는
액션은 `@cloudflare/playwright`를 통해 이 바인딩을 해석합니다. 바인딩이 없으면
없는 Chromium 바이너리를 탓하는 대신 그 바인딩 이름을 밝혀 보고합니다. 로컬
에서도 `wrangler dev`로 동작하지만, 로컬 런타임은 1 MB보다 큰 요청을
거부합니다.

### Cloudflare R2 {#cloudflare-r2}

Worker에는 파일 시스템이 없고 파일 내용은 SQL에 둘 수 없으므로 업로드는 R2에
저장됩니다. 빌드 환경에 `CLOUDFLARE_R2_BUCKET_NAME`을 설정하면 생성된
`wrangler.json`에 `UPLOADS` 바인딩이 기록되며, `CLOUDFLARE_R2_PUBLIC_BASE_URL`
에는 버킷의 공개 URL을 설정합니다. 프로바이더는 업로드 레지스트리에 이미
등록되어 있습니다. 바인딩이 없거나 사용할 수 없으면 페이로드를 SQL에 저장하는
대신 설정 안내와 함께 실패합니다. 자세한 내용은 영문 배포 가이드를 참고하세요.
...
- `extensionTools: true` — 앱에서 프레임워크 확장 관리 actions와 안내
...
[Builder.io](https://www.builder.io?utm_source=agent-native&utm_medium=docs&utm_campaign=framework&utm_content=deployment)는 별도의 관리형 코드 지원 워크플로이며 에이전트가 프로덕션 앱 UI를 변경할 수 있습니다. 저장소를 연결하고 UI 변경을 직접 요청할 수 있으며 재배포가 필요하지 않습니다.
