...
| `AGENT_NATIVE_BUILDER_RELAY_TARGET_ORIGINS` | 기업 콜백 전용으로, 정확히 일치하는 신뢰할 수 있는 미리 보기 origin을 쉼표로 구분한 허용 목록입니다(예: `https://deploy-preview-42--content.netlify.app`). 기업 콜백 배포에만 설정하세요. 와일드카드와 도메인 접미사는 지원되지 않습니다. |
