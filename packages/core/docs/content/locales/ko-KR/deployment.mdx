...
| `A2A_SECRET`             | 앱 간 A2A JSON-RPC의 공유 HMAC. `SECRETS_ENCRYPTION_KEY`가 없으면 공유 vault가 이 값에서 용도별로 파생한 키를 사용하므로 이 값의 회전은 vault 키도 회전시킵니다.                                                                                                   |
| `SECRETS_ENCRYPTION_KEY` | 공유 vault에 권장되는 안정적인 AES-256-GCM 키. 읽는 모든 앱에 같은 값을 설정하세요. 읽기는 파생된 `A2A_SECRET`, 이후 legacy Better Auth 및 앱별 키로 fallback합니다. A2A 회전 전에 모든 곳에 추가하고 이전 A2A 자료가 있는 동안 모든 행을 읽고 마이그레이션하세요. |
