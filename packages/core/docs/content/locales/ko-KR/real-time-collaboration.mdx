...
| 요구 사항 | 이유 | 확인 방법 |
| --------- | ---- | --------- |
| **공개적으로 접근 가능한 호스트의 Postgres** | 게이트웨이는 자체 네트워크에서 데이터베이스에 접속합니다. PGlite, SQLite, `localhost`, `*.internal`, 순수 IP 주소는 연결 문자열이 사용자의 머신을 떠나기도 전에 거부됩니다. | `DATABASE_URL`이 Neon 같은 관리형 Postgres를 가리킬 것 |
...
| **플랫폼이 배포를 식별할 것** | 등록은 앱의 공개 오리진을 키로 삼으므로, 실제로 그 배포가 아닌 프로세스가 오리진을 주장해서는 안 됩니다. | Netlify, Vercel, AWS Lambda, Cloud Run, Render, Cloudflare Pages는 모두 스스로를 표시합니다. 순수 컨테이너나 VM에서는 `URL`에 해당 배포 자신의 오리진을 설정하세요 |
...
이후 등록은 채널이 필요한 첫 요청에서 온디맨드로 이루어집니다. 앱은 연결 문자열과
...
**자체 등록은 Builder에 데이터베이스 연결 문자열을 보낸다는 뜻입니다.** Builder는
...
그 사용자의 *변경 스트림* — 어떤 레코드가 언제 바뀌었는지 — 이 노출됩니다. 다만
