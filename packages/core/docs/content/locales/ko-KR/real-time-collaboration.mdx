...
앱은 기본적으로 위에서 설명한 poll/SSE 전송을 사용합니다. 모든 앱은 대신 호스팅형
실시간 동기화 게이트웨이를 선택할 수 있습니다. 이 게이트웨이는 각 인스턴스가 자체
장기 연결을 유지하는 대신, 동기화 이벤트를 여러 서버리스 인스턴스로 팬아웃합니다.

- 설정하는 변수는 `AGENT_NATIVE_REALTIME_TRANSPORT=hosted` 하나뿐이며, 이를
  설정하는 것이 곧 명시적 동의입니다. 이 변수를 설정하지 않은 배포에서는 여기
  설명된 어떤 일도 일어나지 않습니다. `AGENT_NATIVE_REALTIME_GATEWAY_URL` 은
  스테이징용으로 게이트웨이 엔드포인트를 덮어씁니다. 설정하지 않으면
  `BUILDER_GATEWAY_BASE_URL` 에서 파생됩니다.
- Builder 가 호스팅하는 앱에서는 채널 id 와 서명 시크릿
  (`AGENT_NATIVE_REALTIME_HMAC_SECRET`) 이 Builder 프로비저닝 경로가 주입하는 예약
  환경 변수입니다. 게이트웨이가 읽는 데이터베이스는 파이프라인이 이미
  프로비저닝했습니다.
- **그 외의 환경에서는 앱이 스스로 등록하며, 이는 Builder 에 데이터베이스 URL 을
  보낸다는 뜻입니다.** 전송 변수가 설정되어 있으면, `BUILDER_PRIVATE_KEY` 를 가진
  Postgres 기반 배포가 연결 문자열과 공개 오리진을 게이트웨이에 한 번 전송하고 채널
  id 와 서명 시크릿을 돌려받습니다. Builder 는 그 연결 문자열을 저장하고
  `sync_events` 를 추적하기 위해 여러분의 데이터베이스에 연결합니다. 버전 할당과
  대역 외 변경 감지가 모두 쓰기를 수행하므로 이 연결은 읽기 전용이 아닙니다. 원하지
  않는다면 전송 변수를 설정하지 않은 채로 두십시오. 결정은 그것이 전부이며, 자격
  증명을 공유하는 다른 경로는 없습니다. 데이터베이스 URL 이나 앱 오리진이 바뀌면
  등록이 다시 실행되므로, 자격 증명 교체는 다른 것을 재배포하지 않아도 반영됩니다.
- 조직도 게이트웨이에 대해 활성화되어 있어야 합니다. 활성화되기 전까지 등록은
  거부되고 앱은 로컬 폴링을 유지합니다.
- 프로덕션 배포만 등록합니다. 배포 미리보기는 로컬 동기화를 유지하므로, 일회성
  데이터베이스 자격 증명이 머신을 떠나지 않고 풀 리퀘스트마다 채널이 만들어지지도
  않습니다.
- **조직이 신뢰 경계입니다.** 등록할 수 있는 자격 증명은 기존 오리진을 다시 등록할
  수도 있으며, 그 경우 해당 채널의 서명 시크릿이 반환되고 데이터베이스가 다시
  지정됩니다. 그 시크릿을 가지면 앱의 어떤 사용자에 대해서도 구독 토큰을 발급할 수
  있어 그 사용자의 *변경 스트림*(어떤 레코드가 언제 바뀌었는지)이 노출됩니다. 다만
  레코드 자체는 노출되지 않습니다. 클라이언트가 자신의 세션으로 앱에서 내용을 다시
  읽기 때문입니다. 등록 자격 증명은 조직 전체에 미치는 것으로 취급하십시오.
- `GET /_agent-native/realtime-token` 은 브라우저가 게이트웨이 연결을 여는 데
  사용하는, 수명이 짧고(10분) 세션 범위인 구독 토큰을 발급합니다. 동일 오리진만
  허용하며 활성 세션이 없으면 401 을 반환합니다.
- `GET /_agent-native/can-see` 는 게이트웨이의 이벤트별 가시성 검사입니다. 서명된
  게이트웨이 액세스 토큰을 검증한 뒤 앱 자체의 `resolveAccess` 를 다시 실행하므로,
  이미 접근 권한을 잃은 공유 대상에게 게이트웨이가 이벤트를 전달하는 일은 없습니다.
  알 수 없는 리소스 유형이나 조회 실패는 안전하게 실패합니다
  (`{ allowed: false }`).
- Postgres 에서 호스팅형 앱은 동기화 버전을 프로세스 내 카운터가 아니라 데이터베이스
  (`dbAssignedVersions`) 에서 할당합니다. 게이트웨이 인스턴스와 앱 자체의 서버리스
  인스턴스가 하나의 데이터베이스를 공유하며 쓰기 순서에 합의해야 하기 때문입니다.

여기서 모든 실패는 의도적으로 조용합니다. 전송 변수 없음, Builder 키 없음, Postgres
가 아닌 데이터베이스, 아직 활성화되지 않은 조직, 도달할 수 없는 게이트웨이 — 모두
오류를 내는 대신 앱을 로컬 폴링에 남겨 둡니다. 기대와 달리 호스팅형 실시간이
활성화되지 않는다면 이 순서대로 확인하십시오.
`GET /_agent-native/realtime-token` 이 404 를 반환하면 채널이 해석되지 않았다는
뜻입니다.
