...
| **플랫폼이 배포를 식별할 것**                       | 등록은 앱의 공개 오리진을 키로 삼으므로, 실제로 그 배포가 아닌 프로세스가 오리진을 주장해서는 안 됩니다.                                                                       | Netlify, Vercel, AWS Lambda, Cloud Run, Render는 스스로를 표시합니다. Cloudflare Workers는 표시하지 않으므로 `AGENT_NATIVE_REALTIME_APP_URL`에 해당 배포 자신의 오리진을 설정하세요. 순수 컨테이너나 VM에서도 같은 변수를 설정하세요 |
