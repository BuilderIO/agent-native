...
- **피어 편집자로서의 에이전트** — 에이전트는 동일한 Yjs를 통해 흐름을 편집합니다
  ...
  위치, 선택 또는 실행 취소 스택
- **어디서나 작동** — Drizzle가 지원하는 모든 SQL 데이터베이스(PGlite, Postgres).
...
| 요구 사항                                           | 이유                                                                                                                                                                        | 확인 방법                                                                                                                                                                                    |
| --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **공개적으로 접근 가능한 호스트의 Postgres**        | 게이트웨이는 자체 네트워크에서 데이터베이스에 접속합니다. PGlite, `localhost`, `*.internal`, 순수 IP 주소는 연결 문자열이 사용자의 머신을 떠나기도 전에 거부됩니다. | `DATABASE_URL`이 Neon 같은 관리형 Postgres를 가리킬 것                                                                                                                                       |
