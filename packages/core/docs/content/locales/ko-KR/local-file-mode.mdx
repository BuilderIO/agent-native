...
파일. 로컬 개발과 Agent Native Desktop에 적합하지만 기본 프로덕션 보안
모델은 아닙니다.

프레임워크는 알려진 호스팅, 서버리스 또는 원격 데이터베이스 기반 런타임에서
`local-files` 모드를 거부합니다. 이 검사는
`AGENT_NATIVE_ALLOW_LOCAL_FILES_IN_PRODUCTION`으로 우회할 수 없습니다. 환경 변수는
단일 테넌트 격리를 증명하지 못하기 때문입니다. 프로덕션 모드는 Agent Native
Desktop 하위 프로세스로 제한됩니다. 로컬 SQLite를 포함한 로컬 개발은 플래그 없이
계속 작동합니다. 호스팅 또는 다중 사용자 앱에서는 데이터베이스 모드와 SQL 지원
공유를 사용하세요.
