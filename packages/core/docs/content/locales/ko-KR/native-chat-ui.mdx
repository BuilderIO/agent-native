...
### 작업 UI가 표시되는 시점 선택 {#conditional-native-ui}

작업 하나가 여러 작업 유형을 처리해도 카드가 필요한 결과는 일부일 수 있습니다. 해당 호출을 선택하려면 `chatUI`에 `when(args, result)`를 추가하세요. 서버는 작업이 성공한 후 이 조건 함수를 실행합니다. `false`를 반환하면 결과는 일반 접을 수 있는 도구 기록에 남습니다. 조건 함수는 저장하거나 브라우저로 보내지 않습니다.

Mail의 `manage-draft`는 이 방식을 사용해 초안 생성 결과에 카드를 표시하고 업데이트와 삭제는 일반 도구 호출로 남깁니다.
...
여러 쓰기 작업이 있는 작업에서는 인수와 성공 결과를 함께 확인하세요. Mail의 Gmail 필터 작업은 요약 필드가 있는 생성 또는 교체 결과에만 규칙 카드를 선택합니다.
