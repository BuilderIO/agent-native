...
## 균형 잡힌 카탈로그 {#balanced-catalog}

카탈로그는 기반, 에디터와 캔버스, 에이전트 컨텍스트와 지식, 협업과 거버넌스, 워크스페이스 크롬, 운영 및 Capability 패키지도 다룹니다. 리치 에디터는 Toolkit 소유이며 Core는 호환성 재내보내기를 제공합니다. Blocks는 이 단계에서 Core에 남습니다. Design은 `design-tweaks`가 있는 템플릿 패턴이지 완전한 드롭인 에디터가 아닙니다. Context X-Ray 엔진은 Core, 재사용 가능한 표현은 Toolkit이 소유합니다. Scheduling과 Creative Context는 별도의 Capability 패키지입니다. [Package Lifecycle](/docs/package-lifecycle)를 참조하세요.
