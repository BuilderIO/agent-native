...
- **버전에 묶인 권한** — 확장 프로그램은 정확한 action, 프레임워크 경로와 HTTP 메서드, SQL 접근, 외부 HTTPS origin을 선언합니다. 사용자는 그중 일부를 명시적으로 승인하며 manifest가 바뀌면 이전 승인은 무효가 됩니다.
- **레거시 확장 프로그램의 fail-closed 동작** — 현재 manifest를 승인하지 않으면 읽기 전용 action과 애플리케이션 상태 및 자체 데이터 읽기만 허용됩니다. SQL, 쓰기, 외부 통신은 꺼진 상태로 유지됩니다.
- **서버 측 적용** — action, SQL, 애플리케이션 상태, 데이터, proxy 경로가 승인을 각각 검증합니다. 취소는 다음 요청부터 즉시 적용됩니다.
