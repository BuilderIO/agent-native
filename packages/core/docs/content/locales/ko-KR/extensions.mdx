...
## 확장 프로그램을 사용할까요, 아니면 앱 코드를 편집할까요? {#vs-app-code}

이 프레임워크에서는 에이전트가 구성 요소, 경로, 스타일, 비즈니스 로직 등 앱의 소스 코드를 직접 편집할 수 있습니다. 두 방법 중 어느 쪽으로도 요청을 해결할 수 있을 때는 다음 기준을 사용하세요.

- **확장 프로그램으로 시작하세요.** 독립적이고 비교적 일회성인 위젯, 즉 맞춤형 시각화나 인터랙션, 대시보드 타일, 독립 유틸리티, 기존 슬롯의 추가 기능에 적합합니다. 소스 변경, 빌드, 배포가 필요하지 않아 빠르고, 비공개 또는 공유로 사용할 수 있습니다.
- **앱 코드를 사용하세요.** 템플릿의 핵심 기능, 네이티브 UI, 레이아웃, 경로, 비즈니스 로직을 변경하거나 여러 대시보드와 사용자에게 폭넓게 재사용해야 하는 동작에 적합합니다. 호스트에 적절한 슬롯이 없거나 새 슬롯이 필요하다면 코드로 처리하세요.
- **반복되는 확장 프로그램은 승격하세요.** 같은 확장 프로그램을 계속 다시 만들게 되거나 여러 복사본을 유지하는 것보다 하나의 공유 구현이 낫다면, 재사용 가능한 네이티브 기능으로 앱 코드에 옮기세요.
...
경험 법칙: **확장 프로그램으로 해결할 수 있다면 일반적으로 먼저 확장 프로그램으로 시작하세요.** 확장 프로그램의 경계로 해결할 수 없거나, 폭넓은 재사용과 제품 전체 제공을 위해 네이티브 구현이 필요할 때는 코드를 사용하세요. 예를 들어 대시보드의 일회성 맞춤 시각화는 확장 프로그램에 적합하지만, 모든 대시보드에서 사용할 수 있어야 하는 새 파이, 다중 막대, 꺾은선 차트 유형은 템플릿 코드에 속합니다.
