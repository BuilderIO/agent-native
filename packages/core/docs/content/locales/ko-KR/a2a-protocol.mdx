...
- `isA2AProductionRuntime()`는 `NODE_ENV`가 `"production"`가 아닌 경우에도 Netlify, AWS Lambda, Cloudflare Workers, Vercel, Render, Fly 및 Cloud Run에서 `true`를 반환합니다. 일부 서버리스 제공업체는 `NODE_ENV`를 일관되게 설정하지 않으므로 정책은 제공업체별 플래그도 읽습니다.
