...
먼저 브라우저 ID flag를 출시하세요. 앱과 Dispatch에 조직 매핑 migration을 배포한 후 한 조직에서 `organization.cross-app-federation`을 활성화하고 canonical ID와 역할을 확인하세요. 매핑이 없는 기존 조직은 명시적 migration 전까지 변경되지 않습니다.
