...
기본적으로 꺼져 있는 `organization.cross-app-federation` 플래그는 워크스페이스
조직을 위한 서버 전용 단계를 추가합니다. 앱에서 조직을 만들거나 읽을 때 Dispatch에
등록하고 정확한 `(identity_authority, identity_id)` 매핑을 저장합니다. 이후 다른 활성화된
앱에 로그인하면 동일한 정식 `org_id`, 조직 이름과 멤버 역할이 서명된 assertion으로
전달됩니다. 대상 앱은 이 ID를 기준으로 로컬 조직을 만들거나 연결합니다.

조직 이름, 이메일 도메인과 브라우저 쿠키는 연결 키가 아닙니다. 영구 매핑이 없는 기존
로컬 조직은 변경되지 않으며 자동으로 병합되지 않습니다. 의도한 일치를 확인한 뒤 명시적인
마이그레이션을 사용하세요.

일시적인 Dispatch 장애 중에도 연결되지 않은 로컬 조직은 계속 사용할 수
있습니다. 연결된 페더레이션 멤버십은 권한 기관에 연결될 때까지 안전하게
거부되며, 이후 조직을 읽을 때 등록을 다시 시도합니다.

## 작동 방식 {#how-it-works}

흐름은 표준 승인 → 서명된 토큰 → 콜백 리디렉션이며, 신뢰 경계를 넘는 유일한 것은 이메일입니다.

<Diagram id="doc-block-1axbtj8" title={"ID 페더레이션 흐름"} summary={"Dispatch는 사용자를 인증하고 확인된 이메일과 활성화된 경우 정확한 조직 컨텍스트를 포함한 단기 서명 어설션을 반환합니다. 앱은 이메일로 계정을 연결하고 ID로 조직을 매핑한 뒤 자체 로컬 세션을 시작합니다."}>
...
3. **디스패치 → 앱(서명된 ID 토큰).** 디스패치는 엄격한 허용 목록에 대해 `redirect_uri`를 검증하고 단기 **`A2A_SECRET` 서명 ID JWT**를 전달하는 앱의 `redirect_uri`로 다시 302 리디렉션합니다. 토큰의 클레임은 의도적으로 최소화됩니다.
   ...
   | `org_id`     | 조직 페더레이션이 활성화된 경우에만 제공되는 정식 조직 ID          |
   | `org_name`   | `org_id`와 함께 제공되는 조직 이름                                 |
   | `org_role`   | `org_id`와 함께 제공되는 확인된 멤버십 역할                        |
   | `scope`      | 항상 `"identity"`. 조직 필드는 flag가 활성화된 경우에만 허용됩니다 |
   | `exp`        | **≤ 2분** 문제 발생 후                                             |

4. **앱은 이메일로 확인하고 JIT 연결합니다.** 앱의 `GET /_agent-native/identity/callback` 경로는 자체 `A2A_SECRET`로 토큰 서명을 확인하고 `scope: "identity"`와 `exp`를 검사한 다음 **확인된 이메일로 적시 연결**을 수행합니다.
   - 해당 이메일을 가진 로컬 사용자가 존재하는 경우 → 변경하지 않고 다시 사용하세요.
   - 그렇지 않은 경우 → 해당 이메일에 대한 로컬 사용자를 생성하세요.
     `organization.cross-app-federation`이 활성화되면 서명된 `org_id`, `org_name` 및 `org_role`도 함께 검증합니다. 로컬 조직은 정확한 authority와 ID로만 매핑하며 이름이나 이메일 도메인으로 매핑하지 않습니다.
...
- **정식 호스팅 클라이언트** → 등록된 오리진의 앱에 **"Agent-Native로 로그인"** 옵션이 표시됩니다. 자체 호스팅 앱은 계속 변수를 설정해야 하며, 배포 후 추가 매핑 열이 존재할 수 있지만 flag가 꺼져 있으면 사용되지 않습니다.
...
- **앱에 바인딩된 엄격한 `redirect_uri` 허용 목록.** 각 정식 앱 ID는 하나의 정확한 프로덕션 오리진과 콜백 경로에 매핑되며 localhost는 개발용으로 계속 사용할 수 있습니다. 알 수 없는 하위 도메인, 일치하지 않는 앱 ID, 임의 경로 및 교차 출처 대상은 거부됩니다.
- **확인된 이메일 연결 및 정확한 조직 매핑.** 확인된 이메일은 사용자 계정의 유일한 조인 키입니다. 조직 flag가 활성화되면 앱은 암호학적으로 확인된 Dispatch assertion의 `org_id`, `org_name` 및 `org_role`만 허용하고 정확한 authority와 ID로만 조직을 매핑합니다.
- **ID 및 조직의 추가 전용 쓰기.** 연결하면 기존의 동일한 이메일 계정을 그대로 재사용하거나 새 계정을 삽입합니다. 조직 매핑은 추가만 합니다. 이 경로에서는 기존 ID 또는 조직 행의 이름 변경, 재할당 또는 삭제가 발생하지 않습니다.
- **정식 호스팅 클라이언트는 오리진에 따라 활성화됩니다.** 등록된 오리진을 사용하는 정식 호스팅 앱은 자동으로 활성화되고, 자체 호스팅 앱은 `AGENT_NATIVE_IDENTITY_HUB_URL`이 없으면 비활성 상태로 유지됩니다.

<Callout id="doc-block-1ccuipn" tone="success">

**활성화해도 안전하고 되돌려도 안전합니다.** 신원 및 조직 매핑 쓰기는 **추가만 가능합니다**. 기존 계정은 그대로 재사용되고 새 이메일은 새 행을 삽입하며 마이그레이션은 nullable 매핑 열을 추가합니다. 어느 flag를 끄더라도 로컬 사용자, 조직, 멤버십 또는 자격 증명은 삭제되지 않습니다.

</Callout>

적시 링크는 확인된 이메일에 전적으로 기반한 단일 결정입니다.

<Diagram id="doc-block-9m3ozw" title={"JIT-link 결정"} summary={"사용자 계정은 확인된 이메일로 연결됩니다. 활성화된 경우 조직은 정확한 authority와 ID로 매핑됩니다. 모든 쓰기는 추가만 수행합니다."}>
...
- **기본값:** 정확히 등록된 Agent-Native 앱 ID/오리진 쌍과 localhost만 허용합니다(`CANONICAL_IDENTITY_SSO_APP_ORIGINS`).
...
먼저 브라우저 ID flag를 출시하세요. 앱과 Dispatch에 조직 매핑 migration을 배포한 후 한 조직에서 `organization.cross-app-federation`을 활성화하고 canonical ID와 역할을 확인하세요. 브라우저 SSO는 계속 `A2A_SECRET`을 사용하지만 조직 federation은 앱별 별도 credential을 사용합니다. source 앱에 `AGENT_NATIVE_IDENTITY_FEDERATION_SECRET`을, Dispatch에 일치하는 `AGENT_NATIVE_IDENTITY_FEDERATION_SECRET_<APP_ID>`를 설정하세요. 이 federation authority 경계에서 공유 `A2A_SECRET`을 재사용하지 마세요. 매핑이 없는 기존 조직은 명시적 migration 전까지 변경되지 않습니다.
...
앱을 되돌리려면 **해당 앱 환경에서 `AGENT_NATIVE_IDENTITY_HUB_URL`를 제거하고 다시 배포/다시 시작하세요.** 앱은 즉시 이전 인증 동작으로 돌아갑니다. ID 및 조직 매핑 쓰기는 추가만 수행하며 변수를 설정 해제해도 로컬 사용자, 조직, 멤버십 또는 자격 증명은 삭제되지 않습니다. 각 앱의 컷오버 및 롤백은 독립적이며 되돌릴 수 있습니다.
...
정식 호스팅 앱은 앱별 identity hub 환경 설정 없이 패키지된 Desktop SSO Canary 요청과 일반 브라우저 로그인을 인식합니다. 자체 호스팅 앱에는 계속 `AGENT_NATIVE_IDENTITY_HUB_URL`이 필요합니다. 안정적인 Desktop 빌드는 이후 릴리스가 별도로 승인될 때까지 브로커를 초기화하지 않습니다.
