...
각 앱은 `APP_BASE_PATH=/<name>` 및 `VITE_APP_BASE_PATH=/<name>`로 빌드되며 선택한 Nitro 프리셋을 통해 배포됩니다. 통합 `deploy` 명령의 기본 프리셋은 **Netlify**입니다. `.netlify/functions-internal/<app>-server` 아래에 앱 함수를 내보내고 정적 자산을 강제로 적용하지 않는 리디렉션을 생성하여 CDN이 파일을 먼저 제공합니다. **Vercel**은 `npx @agent-native/core@latest deploy --preset vercel`로 지원되며 Vercel Build Output API를 사용해 루트 `.vercel/output` 번들을 작성합니다.
...
자체 도메인(`mail.company.com`, `calendar.company.com`)에 있는 각 앱을 선호하시나요? 작업 공간의 모든 앱은 여전히 ​​독립적으로 배포 가능합니다. `cd apps/mail && pnpm build`는 ​​독립형 스캐폴드처럼 정확하게 작동합니다. 그런 다음 크로스 앱 A2A는 공유 `A2A_SECRET`를 사용하여 표준 JWT 서명 경로를 통과합니다. 별도로 배포된 앱 간의 도메인 간 SSO는 Dispatch를 허브로 사용하는 ID 페더레이션에 의해 처리됩니다. [Cross-App SSO](/docs/cross-app-sso)를 참조하세요. 통합된 단일 원본 배포를 사용하면 필요하지 않습니다.

### 공유 데이터베이스, 공유 자격 증명

사용자 계정, 조직, 설정 및 기타 앱 간 데이터를 공유하려면 모든 앱이 동일한 `DATABASE_URL`을 사용하도록 설정하세요. 로컬 개발 중에는 워크스페이스 루트 `.env`에 설정하여 모든 앱이 동일한 PostgreSQL 데이터베이스를 사용하게 하세요. 설정하지 않으면 앱마다 프로세스 내 PGlite 데이터베이스를 사용하므로 사용자와 조직이 공유되지 않습니다.

같은 데이터베이스를 공유하는 프로세스의 연결 수를 합산해야 합니다. 예를 들어, 오래 실행되는 Node.js 앱 프로세스 5개가 같은 `DATABASE_URL`에 대해 각각 공유 런타임 풀 하나를 사용하면 풀당 기본값 20에서 최대 100개의 연결을 사용할 수 있습니다 (`5 × 1 풀 × 20`). 서버리스 워크스페이스는 런타임별 풀 기본값과 앱마다 유지되는 웜 함수 인스턴스 수를 기준으로 계산하세요. `AGENT_NATIVE_DB_POOL_MAX`와 자세한 연결 예산 안내는 [Server database](/docs/server-database)를 참조하세요.
