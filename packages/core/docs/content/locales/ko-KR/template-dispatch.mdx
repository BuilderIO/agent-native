...
<DataModel id="doc-block-13tzff3" title={"비밀 저장소 스키마"} summary={"Secrets are stored once; grants give a named app access; requests + reviews gate sensitive access; the audit log records who used which secret when. Defined in @agent-native/dispatch (packages/dispatch/src/db/schema.ts)."} entities={[
  {
    "id": "secrets",
    "name": "vault_secrets",
    "note": "저장된 자격 증명 값",
