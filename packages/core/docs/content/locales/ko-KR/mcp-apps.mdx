...
ngrok를 통해 Claude를 테스트할 때 프로덕션 빌드(`pnpm build` 다음 `npx @agent-native/core@latest start`) 또는 배포된 미리 보기/프로덕션 URL를 사용하세요. Claude의 단일 프레임 이식 경로는 생산 자산 청크와 함께 작동합니다. `/app/root.tsx`와 같은 원시 Vite 개발 모듈은 앱 인증으로 보호될 수 있으며 Claude 리소스 원본에서 동적 가져오기가 실패합니다.
