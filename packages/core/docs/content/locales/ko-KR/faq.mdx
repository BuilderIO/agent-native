...
그렇습니다. Node(Netlify, Vercel, Cloudflare, AWS, Deno Deploy, 자체 서버) 및 SQL 데이터베이스(Postgres, SQLite, Turso, D1)를 실행하는 호스트를 선택하세요. 프레임워크는 이식 가능하도록 구축되었습니다. 먼저 [로컬 Docker 빠른 시작](/docs/deployment#self-host-quickstart)을 사용한 다음 프로덕션에는 [Deployment](/docs/deployment)를 참조하세요.
