...
<DataModel id="doc-block-40bwis" title={"분석 데이터 모델"} summary={"Dashboards and analyses are the resources; views, shares, and a query cache hang off them. Org tables come from @agent-native/core/org."} entities={[
  {
    "id": "dashboards",
    "name": "dashboards",
    "note": "탐색기 및 SQL 대시보드",
