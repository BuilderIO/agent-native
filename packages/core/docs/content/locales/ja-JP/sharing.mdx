...
一部のリソースはグループ共有に対応しています。グループはリソースと同じ組織に属している必要があり、現在の各メンバーにそのリソースで選択した共有ロールが適用されます。グループでできることとできないことは、[組織、チーム、権限 → ユーザーグループ](/docs/organizations-teams-permissions#user-groups)を参照してください。
