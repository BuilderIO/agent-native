...
<DataModel id="doc-block-1e89g7v" title={"リソース + コンパニオン共有テーブル"} summary={"Coarse visibility lives on the resource; each fine-grained grant is a row in the shares table."} entities={[
  {
    "id": "deck",
    "name": "decks",
    "note": "...ownableColumns()",
