...
まずブラウザー ID フラグをロールアウトします。アプリと Dispatch に組織マッピングのマイグレーションをデプロイした後、1 つの組織で `organization.cross-app-federation` を有効にし、正規 ID とロールを確認します。対応付けのない既存組織は明示的なマイグレーションまで変更されません。
