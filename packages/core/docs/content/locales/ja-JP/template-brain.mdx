...
<DataModel id="doc-block-4puwh5" title={"脳データモデル"} summary={"Connectors produce raw captures; distillation turns captures into reviewable knowledge; proposals gate sensitive entries. Sync runs and the ingest queue track background work."} entities={[
  {
    "id": "sources",
    "name": "brain_sources",
    "note": "コネクタ構成",
