...
<WireframeBlock id="doc-block-1mdxkyx">
  <Screen
    surface="desktop"
    html={
...
<Diagram id="doc-block-1h5swzg" title={"構築、公開、収集"} summary={"エージェントとビジュアル エディターは 1 つの SQL-backed フォーム定義を編集します。公開入力ページは認証されておらず、送信はサーバー側から宛先にルーティングされます。"}>

```html
<div class="diagram-flow">
  <div class="diagram-col">
    <div class="diagram-node">
      エージェントプロンプト<br /><small class="diagram-muted"
  ...
  <div class="diagram-col">
    <div class="diagram-box">
      公開入力ページ<br /><small class="diagram-muted">unauthenticated</small>
    </div>
    <div class="diagram-box">
      responses<br /><small class="diagram-muted"
        >Slack / webhook / Sheetsを追加</small
      >
    </div>
  </div>
</div>
...
<DataModel
  id="doc-block-1wk7pa0"
  title={"フォームデータモデル"}
  summary={
    "テーブルが3つ。フィールドと統合はフォーム上の JSON 列であるため、エージェントの編集はテーブル間の行変更ではなく、外科的パッチとなります。"
  }
  entities={[
    {
      id: "forms",
      ...
      fields: [
        ...
        },
        {
          name: "fields",
          type: "json",
          note: "FormField[] — すべてのフィールド タイプ",
        },
        {
          name: "settings",
          type: "json",
          note: "FormSettings — 統合など",
        },
        {
          name: "status",
          type: "enum",
          note: "ドラフト |公開された |閉まっている",
        },
        {
          name: "deleted_at",
          type: "datetime",
...
- [**Actions**](/docs/actions-overview) — ビルダーを動かすアクション システム
