...
<DataModel id="doc-block-hoij3c" title={"データモデルの設計"} summary={"デザインはそのファイルとバージョン管理されたスナップショットを所有し、オプションで再利用可能なデザイン システムをリンクします。デザインとシステムは両方とも所有可能であり、それぞれにフレームワーク共有テーブルがあります。"} entities={[
  {
    "id": "designs",
    "name": "designs",
