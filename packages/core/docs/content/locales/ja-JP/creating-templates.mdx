...
`server/db/migrations/<id_name>.sql` にレビュー可能な SQL を生成し、
リリース手順から `runDrizzleMigrations()` で読み込みます。手書きの履歴や特殊な
ケースには `runMigrations()` を使います。破壊的な SQL、`drizzle-kit push`、
テーブル名の変更、または列の削除は決して使用しないでください。
