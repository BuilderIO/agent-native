...
正規のホスト型アプリは、アプリごとの ID ハブ環境設定なしでパッケージ版 Desktop SSO Canary のリクエストを認識します。通常のブラウザとセルフホスト型アプリでは、引き続き `AGENT_NATIVE_IDENTITY_HUB_URL` が必要です。安定版 Desktop は、後続リリースが別途承認されるまでブローカーを初期化しません。
