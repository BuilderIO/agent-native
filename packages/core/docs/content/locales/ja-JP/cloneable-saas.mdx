...
<Diagram id="doc-block-1m85zma" title={"ユーザーごとのカスタマイズがスケールする理由"} summary={"動く app から始めるモデルは 2 つのアイデアによって実用的になります。エージェントがメンテナンスを行い、ユーザーごとのカスタマイズはユーザーごとのコードではなく SQL で行われます。"}>
