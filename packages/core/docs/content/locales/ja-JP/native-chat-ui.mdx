...
### アクション UI を表示するタイミングを選ぶ {#conditional-native-ui}

アクションが複数の操作を扱っていても、カードが必要なのは一部の結果だけの場合があります。`chatUI` に `when(args, result)` を追加して、対象の呼び出しを選べます。サーバーはアクションの成功後にこの述語を実行します。`false` の場合、結果は通常の折りたたみ可能なツール履歴に残ります。この述語は保存もブラウザーへの送信もされません。

Mail の `manage-draft` はこの仕組みを使い、作成結果にはカードを表示し、更新と削除は通常のツール呼び出しとして残します。
...
複数の書き込み操作があるアクションでは、引数と成功結果の両方を確認します。Mail の Gmail フィルターアクションは、要約フィールドがある作成または置換結果にだけルールカードを選びます。
