...
マルチアプリのワークスペースでは、`ONBOARDING_SHARED_COMPLETION` を設定すると、
完了したオンボーディングをアプリ間で再利用できます。設定しない場合、各アプリは
オンボーディングの完了状況を個別に追跡します。
...
セットアップカードには利用可能なセットアップの選択肢が表示されます。考えられる
すべてのキーや認証情報を列挙するものではありません。
