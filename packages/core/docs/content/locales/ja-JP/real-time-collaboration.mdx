...
| **プラットフォームがデプロイを識別できること** | 登録はアプリの公開オリジンをキーにするため、実際にはそのデプロイではないプロセスがオリジンを主張してはいけません。 | Netlify、Vercel、AWS Lambda、Cloud Run、Render、Cloudflare Pages はいずれも自身を示します。素のコンテナや VM では、`URL` にそのデプロイ自身のオリジンを設定してください |
