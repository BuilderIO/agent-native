...
アプリは既定で上記の poll/SSE トランスポートを使用します。どのアプリでも、代わりに
ホスト型リアルタイム同期ゲートウェイを選択できます。これは、各インスタンスが独自の
長時間接続を保持する代わりに、同期イベントを複数のサーバーレスインスタンスへ配信し
ます。

- 設定する変数は `AGENT_NATIVE_REALTIME_TRANSPORT=hosted` のみで、これを設定する
  ことが明示的なオプトインです。設定していないデプロイでは、ここに書かれたことは
  一切起こりません。`AGENT_NATIVE_REALTIME_GATEWAY_URL` はステージング用に
  ゲートウェイのエンドポイントを上書きします。未設定の場合は
  `BUILDER_GATEWAY_BASE_URL` から導出されます。
- Builder がホストするアプリでは、チャネル ID と署名シークレット
  (`AGENT_NATIVE_REALTIME_HMAC_SECRET`) は Builder のプロビジョニング経路が注入する
  予約済み環境変数です。ゲートウェイが読み取るデータベースは、パイプラインが
  すでにプロビジョニング済みです。
- **それ以外の環境では、アプリが自分自身を登録します。これは Builder に
  データベース URL を送信することを意味します。** トランスポート変数が設定されて
  いる場合、`BUILDER_PRIVATE_KEY` を持つ Postgres バックエンドのデプロイは、接続
  文字列と公開オリジンを一度ゲートウェイへ送信し、チャネル ID と署名シークレットを
  受け取ります。Builder はその接続文字列を保存し、`sync_events` を追跡するために
  あなたのデータベースへ接続します。バージョン割り当てと帯域外の変更検出はいずれも
  書き込みを行うため、この接続は読み取り専用ではありません。これを望まない場合は、
  トランスポート変数を未設定のままにしてください。判断はそれだけであり、認証情報を
  共有する経路は他にありません。データベース URL やアプリのオリジンが変わると登録は
  再実行されるため、認証情報のローテーションは他を再デプロイしなくても反映されます。
- 組織側でもゲートウェイが有効化されている必要があります。有効化されるまで登録は
  拒否され、アプリはローカルポーリングのままです。
- 登録するのは本番デプロイのみです。デプロイプレビューはローカル同期のままなので、
  使い捨てのデータベース認証情報がマシンの外に出ることはなく、プルリクエストごとに
  チャネルが作られることもありません。
- **信頼境界は組織です。** 登録できる認証情報は、既存のオリジンを再登録することも
  できます。再登録はそのチャネルの署名シークレットを返し、データベースの向き先を
  変更します。このシークレットを持てばアプリのどのユーザー向けにも購読トークンを
  発行でき、そのユーザーの*変更ストリーム*（どのレコードがいつ変わったか）が露出
  します。ただしレコードの内容そのものは露出しません。クライアントは自分の
  セッションでアプリから内容を読み直すためです。登録用の認証情報は組織全体に及ぶ
  ものとして扱ってください。
- `GET /_agent-native/realtime-token` は、ブラウザーがゲートウェイ接続を開くために
  使う、短命 (10 分) でセッションスコープの購読トークンを発行します。同一オリジン
  のみを受け付け、アクティブなセッションがない場合は 401 を返します。
- `GET /_agent-native/can-see` はゲートウェイのイベント単位の可視性チェックです。
  署名済みのゲートウェイアクセストークンを検証したうえで、アプリ自身の
  `resolveAccess` を再実行するため、すでにアクセス権を失った共有相手へゲートウェイが
  イベントを転送することはありません。未知のリソース種別や参照失敗はフェイル
  クローズします (`{ allowed: false }`)。
- Postgres では、ホスト型アプリは同期バージョンをプロセス内カウンターではなく
  データベース (`dbAssignedVersions`) から採番します。ゲートウェイのインスタンスと
  アプリ自身のサーバーレスインスタンスが 1 つのデータベースを共有しており、書き込み
  順序について一致する必要があるためです。

ここでの失敗はすべて設計上サイレントです。トランスポート変数がない、Builder キーが
ない、Postgres 以外のデータベース、まだ有効化されていない組織、到達できない
ゲートウェイ — いずれの場合もエラーにはならず、アプリはローカルポーリングのままに
なります。期待どおりにホスト型リアルタイムが有効にならない場合は、この順に確認して
ください。`GET /_agent-native/realtime-token` が 404 を返す場合は、チャネルが解決
されていないという意味です。
