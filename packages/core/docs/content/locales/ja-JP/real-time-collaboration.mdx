...
| **プラットフォームがデプロイを識別できること**               | 登録はアプリの公開オリジンをキーにするため、実際にはそのデプロイではないプロセスがオリジンを主張してはいけません。                                                           | Netlify、Vercel、AWS Lambda、Cloud Run、Render は自身を示します。Cloudflare Workers は示さないため、`AGENT_NATIVE_REALTIME_APP_URL` にそのデプロイ自身のオリジンを設定してください。素のコンテナや VM でも同じ変数を設定してください |
