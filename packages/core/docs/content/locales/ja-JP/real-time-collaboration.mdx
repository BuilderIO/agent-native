...
- **ピア編集者としてのエージェント** — エージェントの編集フローは同じ Yjs を介して行われます
  ...
  位置、選択、または元に戻すスタック。
- **どこでも動作** — Drizzle がサポートするあらゆる SQL データベース (PGlite、Postgres)。
...
| 要件                                                         | 理由                                                                                                                                                                      | 確認方法                                                                                                                                                                                          |
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **公開到達可能なホスト上の Postgres**                        | ゲートウェイは自身のネットワークからデータベースに接続します。PGlite、`localhost`、`*.internal`、生の IP アドレスは、接続文字列がマシンから出る前に拒否されます。 | `DATABASE_URL` が Neon などのマネージド Postgres を指していること                                                                                                                                 |
