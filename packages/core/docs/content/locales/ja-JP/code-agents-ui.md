...
Agent-Native コードは同じ `Agent作成rFrame` + `Prompt作成r` を使用します /
`Tiptap作成r` スタックは `@agent-native/core/client/composer` から
...
standard sidebar composer. Code uses `Prompt作成r` directly because the host
