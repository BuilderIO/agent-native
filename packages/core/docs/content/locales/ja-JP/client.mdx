...
<Diagram id="doc-block-14hiq5q" title={"ブラウザのデータループ"} summary={"フックはアクションを通じて読み取りと書き込みを行います。 useDbSync はデータベースを監視し、エージェントとバックグラウンドの書き込みが同じキャッシュを自動的に再取得するようにします。"}>
...
<div class="diagram-client">
  ...
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&harr;</div>
  <div class="diagram-panel" data-rough><strong>SQLデータベース</strong></div>
...
<Diagram id="doc-block-prwgbw" title={"再フェッチへの 2 つのパス"} summary={"ローカルな突然変異は、それ自体のキャッシュを即座に無効にします。リモート書き込みは、SSE、またはフォールバックとしてのポーリング ティックを介してこのタブに到達します。"}>
...
<div class="diagram-latency">
  <div class="diagram-col">
    ...
    <div class="diagram-card" data-rough>
      <span class="diagram-pill accent">エージェント · webhook · 別タブ</span
      ><strong>リモート書き込み</strong
      ><small class="diagram-muted"
