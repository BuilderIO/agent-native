...
軽量なゲートは別途 `ubuntu-latest` を使用します。GitHub ホスト型ランナーを無効にしたリポジトリでは、`VISUAL_RECAP_GATE_RUNS_ON` に JSON ではない単一のプレーンラベルを設定するか、`--gate-runs-on visual-recap-gate` を使用できます。専用で、できればエフェメラルなランナーを使用してください。ゲートはメタデータのみを評価し、PR コードをチェックアウトまたは実行しません。
