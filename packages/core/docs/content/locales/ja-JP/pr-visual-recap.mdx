...
JSON の引用符は必須です。軽量な `gate` ジョブと完全な `recap` ジョブの両方が、選択したランナーを使用します。ガイド付きセットアップ中に構成するには、同じ JSON 値を `--runs-on` に渡し、`doctor` でワークフローとランナーの可用性を検証します。
...
[Secrets and variables](#secrets-and-variables) で説明されているものと同じシークレットと変数が適用されます。コピー バリアントと同じ方法でリポジトリ設定に設定します。再利用可能な呼び出し元は `VISUAL_RECAP_RUNS_ON` を `runs-on` 入力として転送し、呼び出されたワークフローは JSON を解析して両方のジョブにその結果を使用します。入力を省略すると、`ubuntu-latest` のデフォルトが維持されます。
