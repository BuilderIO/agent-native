...
## 拡張機能を使うべきか、アプリコードを編集すべきか？ {#vs-app-code}

このフレームワークでは、エージェントがアプリのソースコード（コンポーネント、ルート、スタイル、ビジネスロジック）を直接編集できます。どちらの方法でも解決できそうなときは、次の質問を使って判断してください。

- **拡張機能から始める**のは、自己完結した比較的単発のウィジェットです。たとえば、カスタムの可視化や操作、ダッシュボードタイル、単独のユーティリティ、既存スロットへの追加などです。ソース変更、ビルド、デプロイが不要で、すばやく作成でき、非公開にも共有にもできます。
- **アプリコードを使う**のは、テンプレートのコア機能、ネイティブ UI、レイアウト、ルート、ビジネスロジックを変更する場合、または複数のダッシュボードやユーザーで広く再利用する場合です。ホストに適切なスロットがない、または新しいスロットが必要な場合もコードを使います。
- **繰り返し作られる拡張機能は昇格させる**べきです。同じ拡張機能を何度も作ることになった場合や、多くのコピーを保守するより共有実装の方がよい場合は、再利用可能なネイティブ機能としてアプリコードに移します。
...
経験則: **拡張機能で解決できるなら、基本的にはまず拡張機能から始めます。** 拡張機能の境界に収まらない場合、または幅広い再利用と製品全体での提供にネイティブ実装が必要な場合はコードを使います。たとえば、ダッシュボード用の単発のカスタム可視化は拡張機能に適していますが、すべてのダッシュボードで使える新しい円グラフ、複合バー、または折れ線グラフの種類はテンプレートコードに属します。
