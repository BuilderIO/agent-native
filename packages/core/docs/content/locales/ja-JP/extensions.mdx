...
- **バージョンに結び付いた権限** — 拡張機能は、正確なアクション、フレームワークのパスと HTTP メソッド、SQL アクセス、外部 HTTPS オリジンを宣言します。利用者はその一部を明示的に承認し、マニフェストが変わると以前の許可は無効になります。
- **従来拡張機能のフェイルクローズ** — 現在のマニフェストが承認されていない場合、読み取り専用アクションとアプリ状態、自身のデータの読み取りだけが可能です。SQL、書き込み、外部通信は無効です。
- **サーバー側の強制** — アクション、SQL、アプリ状態、データ、プロキシの各ルートが許可を独立して検証します。取り消しは次のリクエストから有効です。
