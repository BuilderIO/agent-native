  ...
  <div class="diagram-row">
    <div class="diagram-card">
      ...
      ><small class="diagram-muted"
        >データを読み書きし、actionsを実行し、コードを変更</small
      >
    </div>
    <div class="diagram-card">
...
(`databaseTools: "read"`)。エージェントは `db-schema` / `db-query` で
アプリ所有データを検査し、書き込みは型付きアプリ actions 経由で行います。
スコープ付きの `db-exec` / `db-patch` も公開する必要がある意図的な
メンテナンス面だけで `databaseTools: "write"` (または `true`) を設定します。
すべてのデータアクセスに型付きアプリ actions を要求するには
`databaseTools: "off"` / `false` を設定します。
