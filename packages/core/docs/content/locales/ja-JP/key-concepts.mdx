...
| エージェントと UI 間のライブ同期         | 長期稼働ホストでは、同一プロセスの書き込みは `/_agent-native/events` 経由で即座にストリームされます。軽量なポーリングが cron とクロスプロセスの書き込みを収束させ続け、アプリが[ホスト型リアルタイム同期ゲートウェイ](/docs/real-time-collaboration#hosted-gateway)を有効にしていない限り、本番のサーバーレスでは、同じペースでそれがアプリの唯一の経路になります。変更を行うアクションはアクションに基づくクエリを自動的に無効化するため、エージェントが作成したレコードは手動更新なしで表示されます。下記の [Live Sync](#polling-sync) を参照してください。 |
...
エージェントがデータを変更したとき、UI は手動での更新なしにそれを反映する必要があります。`useDbSync()` がそれを自動化します。長期稼働ホストでは、同一プロセスの書き込みは `/_agent-native/events` 経由でストリームされます。`/_agent-native/poll` は引き続きクロスプロセス向けのフォールバックです。エージェントがデータベース (アプリケーション状態、設定、またはドメインデータ) に書き込むと、バージョンカウンターが増加し、クライアントは関連する React Query のキャッシュを無効化します。

WebSockets ではなく SSE が一般的なケースをカバーしているのは、通常の HTTP だけで同一プロセスの書き込みをブラウザへ即座に届けられるからです。一方でバージョンカウンターによるポーリングはフォールバックとして残り、あらゆるデプロイ環境で機能します。本番のサーバーレスホスト（Netlify、Lambda、Vercel）では、アプリ自身の `/_agent-native/events` は遅い経路ではなく、無効化されています。そのストリームを開いたままにすると、一度に 1 つのリクエストしか処理しない関数インスタンス（そもそも別の呼び出しによる書き込みを見ることはできません）を、プラットフォームが強制終了してコンテナを再構築するまで占有してしまいます。そのため代わりにサーバーは即座に 204 を返し、`/_agent-native/poll` が同じペースですべての同期を担いますが、アプリが[ホスト型リアルタイム同期ゲートウェイ](/docs/real-time-collaboration#hosted-gateway)を有効にしている場合は、代わりにその専用の長期稼働プロセスからストリームされます。
