...
<FileTree id="doc-block-1kpeiur" title={"ガイダンスと振る舞い"} entries={[
  {
    "path": "AGENTS.md",
...
<DataModel id="doc-block-1c6ozmb" title={"コア SQL ストア"} summary={"すべてのテンプレートで自動作成されます。エージェントと UI は両方ともこれらの読み取りと書き込みを行います。"} entities={[
  {
