...
アプリ操作用](/docs/server-overview#actions-first)。
...
- **`readOnly: true`** — 変異しない POST actions の場合でも、ポーリング更新を明示的にスキップします。 これはアクセス制御では **ありません**。誰が action を呼び出せるかを制限するものではなく、`useActionQuery`、`useActionMutation`、その他のクライアント hook にも影響しません。効果は、呼び出し後のリフレッシュイベントを抑制することと、action を書き込み監査の対象外にすることだけです。モデルから action を隠すには `agentTool: false` を、呼び出し元を制限するには上記の認証/アクセスオプションを使用してください。
