...
クライアント側の表示を隠したり表示したりするには、`@agent-native/core/client` の `useFeatureFlag(FULL_APP_BUILDING.key)` を使用します。Core は `get-feature-flags`、`list-feature-flags`、`set-feature-flag` を提供し、共通の feature flag 設定タブも、特定ユーザー、組織、割合、全体へのロールアウトに同じ actions を使用します。管理操作では権限が確認され、監査ログに記録されます。flag の書き込みを汎用設定 endpoint や生の SQL 経由にしたり、認証、認可、監査、secret、SSR キャッシュの動作を runtime flag の背後に置いたりしないでください。ロールアウトと廃止の指針については `feature-flags` skill を参照してください。
