...
<Diagram id="doc-block-17olwgs" title={"1 つのドキュメントに多数の編集者"} summary={"あなたとエージェントは両方とも同じ Yjs パイプラインを通じて書き込みます。 SQL は正規ストアです。ローカル ファイルと Notion はオプションの同期サーフェスです。"}>
...
<div class="diagram-flow">
  ...
  <div class="diagram-panel center">
    ...
    ><small class="diagram-muted">ライブ、競合なしマージ</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  ...
  <div class="diagram-col">
    ...
    <div class="diagram-box">
      Notion ページ<br /><small class="diagram-muted">pull · push</small>
    </div>
  </div>
</div>
```
...
[ローカルフォルダーソース](/docs/template-content-local-files) を参照してください。
...
- **`server/db/schema.ts`** — データ モデル。ここに列またはテーブルを追加します。コンテンツ テンプレートには `db:push` スクリプトがありません。これは、起動時に実行される厳密に追加的な移行に依存しています。 `server/db/schema.ts` を編集し、一致する追加移行を作成すると、次回アプリが起動するときに変更が適用されます。スキーマの更新では、既存のテーブルや列を削除したり、名前を変更したり、破壊的に変更したりしてはなりません (ガイドラインについては、[Database](/docs/server-database#migrations) を参照してください)。
