...
## バランスの取れたカタログ {#balanced-catalog}

このカタログは、基盤、エディターとキャンバス、エージェントのコンテキストと知識、共同作業とガバナンス、ワークスペースのクローム、運用、Capability パッケージも扱います。リッチエディターは Toolkit が所有し、Core は互換性のために再エクスポートします。Blocks はこの段階では Core に残ります。Design は `design-tweaks` を持つテンプレートパターンであり、完全なドロップインエディターではありません。Context X-Ray のエンジンは Core、再利用可能な表示は Toolkit が所有します。Scheduling と Creative Context は分離された Capability パッケージです。 [Package Lifecycle](/docs/package-lifecycle) を参照してください。
