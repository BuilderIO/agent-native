...
<DataModel id="doc-block-1jqbpcd" title={"タスクテーブル"} summary={"Defined once with the framework helpers; the dialect is chosen at runtime from DATABASE_URL."} entities={[
  {
    "id": "tasks",
    "name": "tasks",
