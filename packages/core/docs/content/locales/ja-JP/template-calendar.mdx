...
<DataModel id="doc-block-lpnq12" title={"カレンダーデータモデル"} summary={"イベント以外のデータのみがローカルに保存されます。イベントは Google カレンダーに保存されます。予約リンクは ownableColumns を使用するため、共有システムが適用されます。"} entities={[
  {
    "id": "booking_links",
    "name": "booking_links",
