...
エージェントネイティブは開発者だけを対象としたものではありません。埋め込みエージェントはチーム全体にアプリを操作する共通の方法を提供し、ソース変更は通常の開発ワークフローに残ります。ソースを編集できるのは、ホストが repository/workspace の書き込みツールを意図的に付与した場合だけです。
...
エージェント ネイティブ アプリは、動く app から始めるモデルに従います。 **テンプレート** (カレンダー、コンテンツ、スライド、アナリティクス、メール、クリップ、デザイン、フォーム、ディスパッチ) から始めて、それを自分のものにします。それぞれは、空の足場ではない、完全に機能する SaaS 製品です。

1. [agent-native.com/templates](/templates) でテンプレートを選択してください
2. ホストされたアプリ (例: mail.agent-native.com) としてすぐに使用します
3. カスタマイズしたいときはテンプレートから自分の app を作成してください — 「Stripe アカウントに接続する」、「コホート チャートを追加する」
4. 独自のリポジトリと開発ワークフローでソースをカスタマイズします
5. app を独自のドメインにデプロイするか、agent-native.com に留まる

これは共有インフラストラクチャではなく自分のアプリなので、ソースを所有し、共有フレームワークを変更せずに進化させられます。埋め込みエージェントは actions とデータを通じて動作し、意図的に書き込みツールが付与された場合に限りソース変更を支援できます。全文については、[Templates](/docs/cloneable-saas) を参照してください。

テンプレート全体から始める準備ができていませんか?すでに使用しているコーディング エージェントに **スキル** を追加して、エージェント ネイティブを試すこともできます。`npx @agent-native/core@latest skills add visual-plan` で計画スキルをインストールします。 [Skills Guide](/docs/skills-guide#app-backed-skills) を参照してください。
