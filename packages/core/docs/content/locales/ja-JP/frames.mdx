...
<Diagram id="doc-block-fw0naf" title={"3 つのフレーム、1 つのランタイム"} summary={"アプリとエージェント パネルはどのフレームでも同じです。それらを囲むラッパーのみが変更されます。"}>

```html
<div class="diagram-frames">
  <div class="diagram-card" data-rough>
    <span class="diagram-pill accent">埋め込みパネル</span
    ><small class="diagram-muted">すべてのアプリに同梱 · dev + prod</small>
  </div>
  <div class="diagram-card" data-rough>
    <span class="diagram-pill">ローカル開発フレーム</span
    ><small class="diagram-muted">iframe内のアプリ + パネル + CLI端末</small>
  </div>
  <div class="diagram-card" data-rough>
    <span class="diagram-pill">Builder.ioクラウドフレーム</span
    ><small class="diagram-muted"
      >ホスト版: 共同作業 · ビジュアル編集 · 並列実行</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
  <div class="diagram-box" data-rough>
    同じruntime<br /><small class="diagram-muted"
      >あなたのアプリ · アクション · アプリ状態</small
    >
  </div>
</div>
```
...
- 完全なチャット / CLI / エージェント リソース サーフェスを提供するため、エージェントの入力はオンのままです
...
<Diagram id="doc-block-1aw3n0h" title={"コードリクエストゲート"} summary={"コード型メッセージには、コード対応フレームが必要です。 1 つが接続されている場合、リクエストはそこにルーティングされます。これがない場合、パネルはコード変更には Desktop または Builder が必要であると説明します。"}>
...
[Builder.io](https://www.builder.io?utm_source=agent-native&utm_medium=docs&utm_campaign=framework&utm_content=frames) は、
