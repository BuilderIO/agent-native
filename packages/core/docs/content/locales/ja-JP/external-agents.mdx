...
認証は接続アカウントを識別するだけで、文書権限を拡張しません。公開文書は共有ページから引き続き読めます。非公開文書は、そのアカウントに既存のアクセス権がある場合にのみ MCP で読めます。接続後に `get-document` がアクセス拒否を返す場合は、所有者がそのアカウントに権限を付与する必要があります。

コネクタを再インストールしたり公開共有リンクを変更したりしても、接続済みアカウントの権限拒否は解決しません。
