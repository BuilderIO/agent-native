...
外部エージェントはアプリの実際の作成・更新ツール ― アプリ内エージェントが呼び出すのと同じ名前付きアクション ― を利用できます。そのため、ユーザーが新しいコンテンツ(デザイン、デッキ、フォーム、ドキュメントなど)を求めた場合は、`ask_app` に作業を委任するのではなく、直接それを作成し、それらのツールで保存してください。`endsTurn: true` が付いたアクション ― そのアプリ自身のチャットでユーザーの回答を待つアプリ内質問フォームなど ― は、アクションが明示的に `mcpTool: true` で再度有効にしない限り、デフォルトで MCP、WebMCP、A2A から隠されます。それらはアプリ内エージェントのためのツールであり、外部の呼び出し元のためのものではありません。WebMCP ブリッジはツールの入力と結果を最大 500,000 文字まで受け付けるため、手作業で作成した画面、デッキ、ドキュメント全体を分割することなく一度の書き込みに収めることができます。ページローカルの WebMCP 呼び出しはブラウザのタブ ID も伝えるため、タブごとのコンテキスト(ナビゲーションや選択状態)はユーザーが実際に見ているタブに解決されます。
