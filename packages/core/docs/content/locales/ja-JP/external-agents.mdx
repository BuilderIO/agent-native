...
Settings にはアプリのグループに **MCP server** ページがあります。
アプリの正規 MCP URL、利用可能な場合は A2A エージェント カード、完全な接続ページと
同じ設定ガイドを表示します。アプリ内の開始点として **Settings → MCP server** を使うか、
