...
- `isA2AProductionRuntime()` は、`NODE_ENV` が `"production"` でない場合でも、Netlify、AWS Lambda、Cloudflare Workers、Vercel、Render、Fly、Cloud Run で `true` を返します。一部のサーバーレス プロバイダーは `NODE_ENV` を一貫して設定しないため、ポリシーはプロバイダー固有のフラグも読み取ります。
