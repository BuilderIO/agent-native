...
<DataModel id="doc-block-s83p8i" title={"メール SQL テーブル"} summary={"メール自体は Gmail 内に存在します。 SQL テーブルには、キューに入れられた下書き、送信追跡イベント、OAuth トークンなど、Gmail にはないものが保持されます。設定と一時的な状態は、settings ストアと application_state ストアに存在します。"} entities={[
  {
