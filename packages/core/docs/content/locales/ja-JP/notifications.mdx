...
<Endpoint id="doc-block-wg8xf2" title={"リスト通知"} summary={"The route behind listNotifications() — scoped to the authenticated session's email."} method="GET" path={"/_agent-native/notifications?unread=true&limit=50"} summary={"現在のユーザーに対する最近の通知を一覧表示します"} auth={"Authenticated session; results are scoped to the session's email."} params={[
  {
    "name": "unread",
    "in": "query",
    "type": "boolean",
