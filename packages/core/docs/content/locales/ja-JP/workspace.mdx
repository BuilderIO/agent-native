...
<DataModel id="doc-block-vudfrb" title={"ワークスペースのリソースモデル"} summary={"1 つのリソース シェイプがすべてのワークスペース ファイルをサポートします。ランタイムはパスとスコープによってキーを設定し、読み取り時に有効な値を解決します。"} entities={[
  {
    "id": "resource",
...
<AnnotatedCode id="doc-block-1wqrzag" title={"カスタムエージェントプロファイル"} filename="agents/design.md" language="markdown" code={"---\nname: Design\ndescription: >-\n  Reviews layouts, interaction patterns, and product UX decisions.\nmodel: inherit\ntools: inherit\ndelegate-default: false\n---\n\n# 役割\n\nあなたは、重点を置いたデザイン エージェントです。\n\n## 責任\n\n- レイアウトとインタラクション フローを確認する\n- より強力な視覚的方向性を提案する\n- 簡潔かつ意見を述べてください。"} annotations={[
  {
    "lines": "2",
