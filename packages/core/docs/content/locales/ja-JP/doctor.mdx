...
生成されたアプリのソースコードは、リポジトリの開発ワークフローで変更できます。意図的に workspace と書き込みツールを与えられたエージェント frame も変更できます。`agent-native doctor` はソースのセキュリティ上重要なコード不変条件を検査します。読み取り専用であり、コードを編集せず結果だけを報告します。
