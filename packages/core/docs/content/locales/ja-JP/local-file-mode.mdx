...
ファイル。これはローカル開発と Agent Native Desktop に適していますが、
デフォルトの実稼働セキュリティ モデルではありません。

フレームワークは、既知のホスト環境、サーバーレス環境、またはリモート
データベースを使用する環境では `local-files` モードを拒否します。この検査は
`AGENT_NATIVE_ALLOW_LOCAL_FILES_IN_PRODUCTION` では回避できません。環境変数だけでは
シングルテナント分離を証明できないためです。本番モードは Agent Native Desktop
の子プロセスに限定されます。ローカル SQLite を含むローカル開発は、フラグなしで
引き続き動作します。ホスト型またはマルチユーザーのアプリでは、データベース
モードと SQL による共有を使用してください。
