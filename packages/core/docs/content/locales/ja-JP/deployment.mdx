...
Worker にはファイルシステムがないため、データベースは外部の
`DATABASE_URL` か Cloudflare D1 のどちらかになります。ビルド環境で
`CLOUDFLARE_D1_DATABASE_NAME` と `CLOUDFLARE_D1_DATABASE_ID` の両方を
設定すると、生成される `wrangler.json` に `DB` バインディングが書き込ま
れます。どちらか一方だけの設定はエラーであり、バインディングが黙って省かれる
ことはありません。ローカル起動の完全な手順は英語版のデプロイガイドを参照して
ください。


### Browser Rendering {#cloudflare-browser-rendering}

Worker は Chromium プロセスを起動できないため、ブラウザーはバインディングと
して提供され、生成される `wrangler.json` に
`{ "browser": { "binding": "BROWSER" } }` が宣言されます。ブラウザーに依存
するアクションは `@cloudflare/playwright` を通じてこのバインディングを解決
します。バインディングが無い場合は、存在しない Chromium バイナリのせいに
せず、そのバインディング名を挙げて報告します。ローカルでも `wrangler dev`
で動作しますが、ローカルランタイムは 1 MB を超えるリクエストを拒否します。

### Cloudflare R2 {#cloudflare-r2}

Worker にはファイルシステムがなく、ファイルの中身を SQL に置くこともできないた
め、アップロードは R2 に保存されます。ビルド環境で `CLOUDFLARE_R2_BUCKET_NAME`
を設定すると、生成される `wrangler.json` に `UPLOADS` バインディングが書き込ま
れます。あわせて `CLOUDFLARE_R2_PUBLIC_BASE_URL` にバケットの公開 URL を設定し
てください。プロバイダーはアップロードレジストリに登録済みです。バインディング
が無い、または使用できない場合は、ペイロードを SQL に保存せずセットアップ手順を
示して失敗します。詳細は英語版のデプロイガイドを参照してください。
...
- `extensionTools: true` — フレームワークの拡張管理 actions と
...
[Builder.io](https://www.builder.io?utm_source=agent-native&utm_medium=docs&utm_campaign=framework&utm_content=deployment) は独立したマネージドなコード対応ワークフローで、エージェントが本番環境のアプリ UI を変更できます。リポジトリを接続して UI の変更を直接依頼でき、再デプロイは不要です。
