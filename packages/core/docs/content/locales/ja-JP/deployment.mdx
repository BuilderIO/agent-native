...
| `A2A_SECRET`             | アプリ間 A2A JSON-RPC の共有 HMAC。`SECRETS_ENCRYPTION_KEY` が未設定なら、共有 Vault はこの値から用途別に導出したキーを使うため、この値のローテーションは Vault キーも変更します。                                                                                                                          |
| `SECRETS_ENCRYPTION_KEY` | 共有 Vault に推奨される安定した AES-256-GCM キー。読むすべてのアプリに同じ値を設定します。読み取りは導出済み `A2A_SECRET`、次に旧 Better Auth とアプリ固有キーへフォールバックします。A2A のローテーション前に全環境へ追加し、旧 A2A 素材が残る間に全行を読み取り・移行してください。                       |
