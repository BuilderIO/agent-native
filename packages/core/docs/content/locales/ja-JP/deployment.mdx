...
| `AGENT_NATIVE_BUILDER_RELAY_TARGET_ORIGINS` | 企業コールバック専用の、完全一致する信頼済みプレビューオリジンをカンマ区切りで指定する許可リスト（例: `https://deploy-preview-42--content.netlify.app`）。企業コールバックのデプロイメントにのみ設定します。ワイルドカードとドメインサフィックスはサポートされません。 |
