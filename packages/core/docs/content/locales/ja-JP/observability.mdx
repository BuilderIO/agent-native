...
| **人によるレビュー** | 依頼と回答を確認し、フィードバックを記録して、指示の更新案を作成します              |

### 会話の詳細 {#conversations}

会話タブには記録された実行が表示されます。実行を選ぶとスパンを確認でき、各スパンを展開すると、記録されたツールの入力、出力、エラー、メタデータを表示できます。**会話全体を開く**を選ぶと、対応するチャットスレッドに移動します。

ツールの引数と結果は既定では記録されません。保存するには `observability` 設定で `captureToolArgs` と `captureToolResults` を有効にしてください。機密フィールドとツールのエラーメッセージは保存前にマスキングされます。

### 人によるレビュー {#review}

人によるレビュータブでは、各実行の依頼と回答を確認し、フィードバックを記録して、人が確認する指示更新案を作成できます。下書きを保存しても指示は自動で変更されません。
