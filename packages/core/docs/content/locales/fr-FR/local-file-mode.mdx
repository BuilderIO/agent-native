...
fichiers. Cela convient au développement local et à Agent Native Desktop, mais
ce n'est pas le modèle de sécurité de production par défaut.

Le framework refuse le mode `local-files` dans les environnements connus
d'hébergement, serverless ou adossés à une base de données distante. Ce contrôle
ne peut pas être contourné avec
`AGENT_NATIVE_ALLOW_LOCAL_FILES_IN_PRODUCTION` : une variable d'environnement
ne prouve pas l'isolation d'un locataire unique. Le mode production est limité
aux processus enfants d'Agent Native Desktop. Le développement local, y compris
SQLite local, continue de fonctionner sans variable. Pour les applications
hébergées ou multi-utilisateurs, utilisez le mode base de données et le partage SQL.
