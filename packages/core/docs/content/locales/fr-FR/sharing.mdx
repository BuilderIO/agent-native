...
### Partage avec des groupes {#group-grants}

Certaines ressources prennent en charge le partage avec des groupes. Le groupe doit appartenir à l’organisation de la ressource, et chaque membre actuel reçoit le rôle choisi pour cette ressource. Voir [Organisations, équipes et autorisations → Groupes d’utilisateurs](/docs/organizations-teams-permissions#user-groups) pour connaître le périmètre des groupes et ses limites.
