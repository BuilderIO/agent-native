...
La plupart des applications intègrent l'IA après coup, ce qui ne permet pas réellement de _faire_ des choses dans l'application. Dans une application Agent-Native, l'agent est un citoyen de première classe qui partage le même actions, la même base de données et le même état que le UI, il peut donc faire tout ce que les boutons peuvent faire. Voir [Pourquoi créer des applications de cette manière](/docs/what-is-agent-native#why-build-apps-this-way).
