...
Le tableau de bord comporte 6 onglets :
...
| **Revue humaine**  | Examiner la demande et la réponse, noter un retour et préparer un brouillon de mise à jour des instructions |

### Détails des conversations {#conversations}

L’onglet Conversations affiche les exécutions enregistrées. Sélectionnez-en une pour examiner ses étendues, puis développez une étendue pour voir les entrées, sorties, erreurs et métadonnées des outils qui ont été capturées. **Ouvrir la conversation complète** ouvre le fil de discussion associé.

La capture des arguments et résultats des outils est désactivée par défaut. Activez `captureToolArgs` et `captureToolResults` dans la configuration `observability` pour les stocker. Les champs sensibles et les messages d’erreur des outils sont expurgés avant l’enregistrement.

### Revue humaine {#review}

L’onglet Revue humaine présente la demande et la réponse de chaque exécution, permet de noter un retour et d’enregistrer un brouillon de modification des instructions pour examen humain. L’enregistrement du brouillon ne modifie pas automatiquement les instructions.
