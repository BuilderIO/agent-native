...
| Synchronisation en direct entre l'agent et l'UI | Sur un hôte de longue durée, les écritures dans le même processus sont diffusées immédiatement via `/_agent-native/events` ; une interrogation légère maintient la convergence des écritures cron et inter-processus, et c'est l'unique voie de l'app en serverless de production (au même rythme), sauf si elle a activé la [passerelle de synchronisation temps réel hébergée](/docs/real-time-collaboration#hosted-gateway). Les actions de mutation invalident automatiquement les requêtes adossées aux actions, si bien que les enregistrements créés par l'agent apparaissent sans actualisation manuelle. Voir [Live Sync](#polling-sync) ci-dessous. |
...
Lorsque l'agent modifie des données, l'UI doit le refléter sans actualisation manuelle. C'est `useDbSync()` qui rend cela automatique. Sur un hôte de longue durée, les écritures dans le même processus sont diffusées via `/_agent-native/events` ; `/_agent-native/poll` reste la solution de repli inter-processus. Lorsque l'agent écrit dans la base de données (état de l'application, settings ou données de domaine), un compteur de version s'incrémente et le client invalide les caches React Query pertinents.

Le SSE couvre le cas courant à la place des WebSockets car il offre aux écritures dans le même processus un chemin immédiat vers le navigateur en HTTP simple, tandis que l'interrogation par compteur de version reste une solution de repli qui fonctionne dans tout environnement de déploiement. Sur un hôte serverless de production (Netlify, Lambda, Vercel), le flux `/_agent-native/events` propre à l'app n'est pas une voie plus lente — il est désactivé. Garder ce flux ouvert monopoliserait une instance de fonction qui ne sert qu'une requête à la fois (elle ne pourrait de toute façon jamais voir l'écriture d'une autre invocation) jusqu'à ce que la plateforme la tue, recyclant le conteneur ; le serveur répond à la place immédiatement par un 204, et `/_agent-native/poll` assure toute la synchronisation, au même rythme, sauf si l'app a activé la [passerelle de synchronisation temps réel hébergée](/docs/real-time-collaboration#hosted-gateway), qui diffuse depuis son propre processus de longue durée.
