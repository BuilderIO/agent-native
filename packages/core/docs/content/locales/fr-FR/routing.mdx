...
C'est tout : le serveur de développement lit les fichiers de route au démarrage.
Si `pnpm dev` est déjà en cours d'exécution, redémarrez-le après avoir ajouté le
fichier. Aucune inscription n'est nécessaire.
...
- [**Server**](/docs/server-overview) — routes de serveur basées sur des fichiers et espace de noms `/_agent-native/`
