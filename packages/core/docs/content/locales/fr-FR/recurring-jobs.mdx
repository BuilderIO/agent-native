...
Les tâches récurrentes peuvent utiliser des outils MCP distants ou locaux déjà connectés à l'application. Demandez à l'agent de découvrir les noms exacts avec `tool-search`, puis liez uniquement les outils nécessaires via `mcpTools :`
...
Recherchez les nouveaux enregistrements depuis la dernière exécution réussie.
Extrayez uniquement les suivis exploitables, puis appelez une fois l'action
d'import bornée de l'application avec des identifiants source et externes
stables, une courte citation et l'URL de l'enregistrement. N'importez pas la
transcription complète.
```

La tâche ne stocke que les noms d'outils, pas les URL MCP, les jetons d'accès ni les jetons de renouvellement OAuth. À l'exécution, le framework résout l'autorisation connectée dans le contexte de requête du créateur de la tâche ou de l'organisation et n'expose que les outils autorisés. Gardez la liste courte et utilisez une action d'application idempotente pour l'import final afin que les exécutions qui se chevauchent ne créent pas de doublons. Consultez [MCP Clients](/docs/mcp-clients) pour la connexion et la configuration OAuth.
