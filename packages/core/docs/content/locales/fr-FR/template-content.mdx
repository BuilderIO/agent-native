...
<DataModel id="doc-block-wsk3tc" title={"Modèle de données Content"} summary={"Neuf tables dans server/db/schema.ts. documents est l’arborescence des pages ; le reste s’y rattache pour les versions, commentaires, synchronisation Notion, bases inline et partage."} entities={[
  {
    "id": "documents",
    "name": "documents",
