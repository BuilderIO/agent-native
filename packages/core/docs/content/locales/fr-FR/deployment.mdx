...
Un Worker n'a pas de système de fichiers : la base de données est donc soit une
`DATABASE_URL` externe, soit Cloudflare D1. Définissez ensemble
`CLOUDFLARE_D1_DATABASE_NAME` et `CLOUDFLARE_D1_DATABASE_ID` dans
l'environnement de build pour que celui-ci écrive le binding `DB` dans le
`wrangler.json` généré ; n'en définir qu'une seule est une erreur et non un
binding silencieusement omis. La séquence complète de démarrage local figure
dans le guide de déploiement en anglais.


### Browser Rendering {#cloudflare-browser-rendering}

Un Worker ne peut pas lancer de processus Chromium : le navigateur est un
binding, et le `wrangler.json` généré déclare
`{ "browser": { "binding": "BROWSER" } }`. Les actions qui dépendent du
navigateur résolvent ce binding via `@cloudflare/playwright`. Si le binding est
absent, il est nommé dans le message d'erreur plutôt que d'invoquer un binaire
Chromium manquant. En local, cela fonctionne avec `wrangler dev` à une
limitation près : le runtime local refuse les requêtes de plus de 1 Mo.

### Cloudflare R2 {#cloudflare-r2}

Un Worker n'a pas de système de fichiers et le contenu des fichiers n'a pas sa
place en SQL : les téléversements vont donc dans R2. Définissez
`CLOUDFLARE_R2_BUCKET_NAME` dans l'environnement de build pour que la liaison
`UPLOADS` soit écrite dans le `wrangler.json` généré, et
`CLOUDFLARE_R2_PUBLIC_BASE_URL` sur l'URL publique du bucket. Le fournisseur
est déjà enregistré dans le registre de téléversement. Une liaison absente ou
inutilisable échoue avec les instructions de configuration au lieu d'écrire la
charge utile en SQL. Détails dans le guide de déploiement anglais.
...
- `extensionTools: true` — active pour l’application les actions et les
