...
| `A2A_SECRET`             | HMAC partagé pour A2A JSON-RPC entre apps. Sans `SECRETS_ENCRYPTION_KEY`, le coffre partagé utilise une clé dérivée de cette valeur pour cet usage ; sa rotation change donc aussi la clé du coffre.                                                                                                                                                          |
| `SECRETS_ENCRYPTION_KEY` | Clé AES-256-GCM stable préférée du coffre partagé. Définissez la même valeur dans chaque app qui le lit. Les lectures retombent sur `A2A_SECRET` dérivé, puis sur les anciennes clés Better Auth et propres à l’app. Avant de faire tourner A2A, ajoutez cette clé partout et lisez/migrez toutes les lignes tant que l’ancien matériau A2A reste disponible. |
