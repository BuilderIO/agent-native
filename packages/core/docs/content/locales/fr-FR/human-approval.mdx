...
<AnnotatedCode
  id="doc-block-1i5mqok"
  title={"Gating la seule action conséquente"}
  ...
  annotations={[
    {
      lines: "10",
      label: "Toute la porte",
      note: "Un drapeau. Avec la vérité et l'appel non approuvé, la boucle s'arrête avant `run` — le modèle n'atteint jamais l'effet secondaire de lui-même.",
    },
    {
      lines: "11-13",
      label: "run() is untouched",
      note: "Le gestionnaire reste le même. L'approbation est appliquée par la boucle qui l'entoure, et non par quoi que ce soit à l'intérieur de `run`.",
    },
  ]}
/>
...
<Diagram id="doc-block-olgste" title={"L'interruption d'approbation"} summary={"Un appel fermé interrompt le tour avant que run() ne se déclenche. L'approbation réémet le tour porteur de la clé d'appel ; ce n’est qu’alors que l’effet secondaire se produit."}>
...
<div class="diagram-approve">
  ...
  <div class="diagram-panel warn" data-rough>
    <strong>Gate vrai, appel pas encore approuvé</strong
  ...
  <div class="diagram-box" data-rough>
    L’humain clique sur Approuver dans le chat<br /><small class="diagram-muted"
      >le client relance le tour avec approvedToolCalls: [approvalKey]</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
...
- [**Actions**](/docs/actions-access-control#needs-approval) : la surface `defineAction` complète, y compris `outputSchema` pour valider les valeurs de retour.
