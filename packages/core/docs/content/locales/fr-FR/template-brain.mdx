...
<DataModel id="doc-block-1gx8o4" title={"Modèle de données Brain"} summary={"Les connecteurs produisent des captures brutes ; la distillation les transforme en connaissances révisables ; les propositions contrôlent les entrées sensibles. Les exécutions de synchronisation et la file d’ingestion suivent le travail en arrière-plan."} entities={[
  {
    "id": "sources",
    "name": "brain_sources",
