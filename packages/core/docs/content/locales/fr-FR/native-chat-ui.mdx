...
### Choisir quand afficher l'interface d'une action {#conditional-native-ui}

Une action peut gérer plusieurs opérations, alors que seules certaines réponses ont besoin d'une carte. Ajoutez `when(args, result)` à `chatUI` pour sélectionner ces appels. Le serveur exécute ce prédicat après la réussite de l'action. S'il renvoie `false`, le résultat reste dans l'historique d'outils standard et repliable. Le prédicat n'est ni enregistré ni envoyé au navigateur.

Mail utilise ce mécanisme dans `manage-draft` pour afficher une carte après la création d'un brouillon. Les mises à jour et suppressions restent des appels d'outils standard :
...
chatUI: {
  ...
  when: (args, result) => {
    ...
    return typeof output.deepLink === "string" && Boolean(output.draft);
  },
},
```

Pour les actions avec plusieurs opérations d'écriture, vérifiez les arguments et le résultat réussi. L'action de filtres Gmail de Mail sélectionne la carte de règle uniquement pour les filtres créés ou remplacés qui contiennent les champs de résumé :
...
when: (args, result) => {
  ...
  const output = result as Record<string, unknown>;
  const filter = output.filter as Record<string, unknown> | undefined;
  return output.ok === true && Boolean(filter?.criteriaSummary);
},
```
