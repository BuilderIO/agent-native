...
<Diagram id="doc-block-1jdbi0g" title="Trois frames, un runtime" summary={"Votre application et le panneau d'agent sont les mêmes dans chaque cadre ; seul l'emballage qui les entoure change."}>

```html
<div class="diagram-frames">
  <div class="diagram-card" data-rough>
    <span class="diagram-pill accent">Panneau intégré</span
    ><small class="diagram-muted">fourni dans chaque app · dev + prod</small>
  </div>
  <div class="diagram-card" data-rough>
    <span class="diagram-pill">Frame de dev local</span
    ><small class="diagram-muted"
      >app dans une iframe + panneau + terminal CLI</small
    >
  </div>
  <div class="diagram-card" data-rough>
    <span class="diagram-pill">Frame cloud Builder.io</span
    ><small class="diagram-muted"
      >hébergé : collaboration · édition visuelle · exécutions parallèles</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
  <div class="diagram-box" data-rough>
    Même runtime<br /><small class="diagram-muted"
      >votre app · actions · état applicatif</small
    >
  </div>
</div>
```
...
- Fournit la surface Chat / CLI / Ressources de l'agent complète, de sorte que l'entrée de l'agent reste activée
...
<Diagram id="doc-block-xwnfo4" title={"Contrôle de demande de code"} summary={"Un message codé nécessite une trame compatible avec le code. Avec un connecté, la requête y est acheminée ; sans cela, le panneau explique que les modifications de code nécessitent Desktop ou Builder."}>
...
[Builder.io](https://www.builder.io?utm_source=agent-native&utm_medium=docs&utm_campaign=framework&utm_content=frames) fournit un cadre géré qui héberge le
