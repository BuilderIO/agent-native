...
<DataModel id="doc-block-1pfbxbv" title={"Modèle de données Calendar"} summary={"Seules les données non événementielles sont stockées localement — les événements vivent dans Google Calendar. Les liens de réservation utilisent ownableColumns donc le système de partage s'applique."} entities={[
  {
    "id": "booking_links",
    "name": "booking_links",
