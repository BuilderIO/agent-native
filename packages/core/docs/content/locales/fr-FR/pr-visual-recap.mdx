...
Les guillemets JSON sont obligatoires. Le job léger `gate` et le job complet `recap` utilisent tous deux le runner sélectionné. Pour le configurer pendant l'installation guidée, transmettez la même valeur JSON à `--runs-on`, puis validez le workflow et la disponibilité du runner avec `doctor` :
...
Les mêmes secrets et variables décrits dans [Secrets and variables](#secrets-and-variables) s'appliquent : définissez-les dans les paramètres de votre dépôt de la même manière que pour la variante de copie. L'appelant réutilisable transmet `VISUAL_RECAP_RUNS_ON` via son entrée `runs-on` ; le workflow appelé analyse le JSON et utilise le résultat pour les deux jobs. L'omission de l'entrée conserve la valeur par défaut `ubuntu-latest`.
