...
<DataModel id="doc-block-jpv4xy" title={"Modèle de données Plan"} summary={"Une ligne de plan possède des sections ordonnées ainsi que des commentaires, des événements, des versions, des partages et des ressources en ligne."} entities={[
  {
    "id": "plans",
    "name": "plans",
