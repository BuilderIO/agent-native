...
Dans un espace de travail multi-applications, définissez
`ONBOARDING_SHARED_COMPLETION` pour réutiliser l'intégration terminée entre
les applications. Si vous ne le définissez pas, chaque application suit
l'achèvement de l'intégration séparément.
...
Les cartes de configuration affichent les choix de configuration disponibles.
Elles ne répertorient pas toutes les clés ou informations d'identification possibles.
