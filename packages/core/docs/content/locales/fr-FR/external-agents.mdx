...
Les agents externes reçoivent les vrais outils de création/mise à jour de l'application — les mêmes actions nommées que l'agent intégré appelle — donc quand un utilisateur demande un nouveau contenu (un design, une présentation, un formulaire, un document), créez-le directement et enregistrez-le avec ces outils au lieu de déléguer le travail via `ask_app`. Les actions marquées `endsTurn: true`, comme un formulaire de questions intégré qui attend la réponse de l'utilisateur dans le chat de cette application, sont masquées de MCP, WebMCP et A2A par défaut ; elles sont l'outil de l'agent intégré, pas celui d'un appelant externe, sauf si l'action choisit explicitement de les réactiver avec `mcpTool: true`. Le pont WebMCP accepte des entrées et résultats d'outils jusqu'à 500 000 caractères, afin qu'un écran, une présentation ou un document entièrement rédigé tienne en une seule écriture au lieu de devoir être découpé. Les appels WebMCP locaux à la page transmettent aussi l'identifiant de l'onglet du navigateur, afin que le contexte propre à cet onglet (navigation, sélection) se résolve vers l'onglet que l'utilisateur regarde réellement.
