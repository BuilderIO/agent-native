...
Dans l'application, ouvrez **Settings → Connections → Integrations** et recherchez le nom
...
externe ouvre **Settings → MCP server** avec le guide le plus adapté déjà sélectionné.
...
Les réglages incluent une page **MCP server** dans le groupe de
l'application. Il affiche l'URL MCP canonique de l'application, une fiche
d'agent A2A si elle est disponible et les mêmes guides que la page de connexion
complète. Utilisez **Settings → MCP server** comme point de départ ou ouvrez
