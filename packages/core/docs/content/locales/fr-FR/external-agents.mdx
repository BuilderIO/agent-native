...
L’authentification identifie le compte connecté sans élargir ses autorisations. Un document public reste lisible depuis son lien de partage. Un document privé n’est lisible via MCP que si ce compte y a déjà accès. Si `get-document` renvoie un refus d’accès après la connexion, le propriétaire doit accorder l’accès à ce compte.

La réinstallation du connecteur ou la modification du lien de partage public ne résout pas un refus d’autorisation du compte connecté.
