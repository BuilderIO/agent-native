...
- `isA2AProductionRuntime()` renvoie `true` sur Netlify, AWS Lambda, Cloudflare Workers, Vercel, Render, Fly et Cloud Run, même lorsque `NODE_ENV` n'est pas `"production"`. Certains fournisseurs sans serveur ne définissent pas `NODE_ENV` de manière cohérente. La stratégie lit donc également les indicateurs spécifiques au fournisseur.
