...
<DataModel id="doc-block-1xgbjxl" title={"Modèle de données Slides"} summary={"Un deck possède ses diapositives sous forme de JSON dans decks.data ; commentaires, versions, partages et systèmes de design s’y rattachent."} entities={[
  {
    "id": "decks",
    "name": "decks",
