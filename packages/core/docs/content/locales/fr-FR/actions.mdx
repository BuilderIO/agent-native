...
Utilisez `useFeatureFlag(FULL_APP_BUILDING.key)` depuis `@agent-native/core/client` pour masquer ou afficher la surface cliente. Core fournit `get-feature-flags`, `list-feature-flags` et `set-feature-flag` ; l’onglet partagé de réglages des indicateurs de fonctionnalités utilise ces mêmes actions pour les déploiements ciblant un utilisateur précis, une organisation, un pourcentage ou tout le monde. La gestion vérifie les autorisations et fait l’objet d’un audit. Ne faites jamais transiter les écritures d’indicateurs par des endpoints de réglages génériques ou du SQL brut, et ne placez jamais l’authentification, l’autorisation, l’audit, les secrets ou le comportement du cache SSR derrière un indicateur d’exécution. Consultez la skill `feature-flags` pour les recommandations de déploiement et de retrait.
