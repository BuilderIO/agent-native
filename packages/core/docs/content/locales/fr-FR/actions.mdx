...
Pour les opérations d'application](/docs/server-overview#actions-first).
...
- **`readOnly: true`** — ignorez explicitement l'actualisation du sondage, même pour les POST actions qui ne mutent pas. Ce n'est **pas** un contrôle d'accès : cela ne restreint pas qui peut appeler l'action et n'affecte ni `useActionQuery`, ni `useActionMutation`, ni aucun hook client. Ses seuls effets sont de supprimer l'événement de rafraîchissement après l'appel et d'exclure l'action de l'audit des écritures. Pour masquer une action au modèle, utilisez `agentTool: false` ; pour restreindre les appelants, utilisez les options d'auth/accès ci-dessus.
...
Cinq indicateurs contrôlent _qui_ peut invoquer une action. Tous sont définis par défaut sur la valeur permissive, vous n'en définissez donc qu'un seul pour resserrer une surface spécifique. Ce tableau est le résumé visible ; les sous-sections ajoutent le détail dont chacune a besoin.
...
| `authorize`     | aucun             | une fonction de garde → seuls les appelants acceptés, sur **toutes** les surfaces | Opérations réservées à un rôle (RBAC par application, opérations réservées aux administrateurs)                   |

Ceux-ci sont indépendants : `agentTool` contrôle la vue du modèle, `toolCallable` contrôle uniquement l'iframe d'extension, `publicAgent` ajoute une surface publique opt-in (les routes Web publiques n'impliquent jamais l'exposition publique de l'outil), `needsApproval` contrôle l'exécution une fois l'appel effectué - voir [Human-in-the-loop approval](#needs-approval) ci-dessous - et `authorize` décide si cet appelant peut exécuter l'action, sur chaque chemin de répartition, avant d'entrer dans `run` - voir [Authorization](#authorize).
...
`accessFilter` / `assertAccess` délimitent _quelles lignes_ un appelant autorisé peut toucher. `authorize` répond à la question qui vient avant celle-là : **cet appelant peut-il exécuter cette opération, tout simplement ?** Donnez-lui une fonction de garde et elle s'exécute avant `run()` pour chaque appelant — outil d'agent, HTTP, frontend, MCP, A2A et CLI :
...
La garde enveloppe `run` lui-même au lieu d'être accrochée à l'entrée de l'action, car `run` est la seule chose que traversent les six sites de répartition — aucun appelant ne peut atteindre le corps sans passer le contrôle. Une garde qui lève une exception refuse avec son propre message ; une garde qui renvoie `false` refuse avec un 403 générique ; tout le reste, y compris `undefined`, autorise. Comme la barrière a besoin d'une identité d'appelant, une invocation CLI ou cron sans surveillance et sans e-mail d'utilisateur est refusée plutôt que laissée passer.

Les enveloppes se composent dans cet ordre :

```text
validate input -> authorize -> run -> validate output -> audit
```

L'autorisation se situe _à l'intérieur_ de la validation des entrées : une garde qui inspecte `args` reçoit donc la valeur analysée et convertie que son type promet, et non la forme brute envoyée par l'appelant. L'audit est la couche la plus externe, si bien qu'un refus est tout de même enregistré.

`requireAny` échoue au moment de la définition, pas au moment de la requête : l'appeler sans aucun rôle, ou avec un rôle absent du vocabulaire déclaré, lève une exception au chargement du module. `authorize: coachAccess.requireAny()` est un argument oublié, et cela doit casser là où se trouve l'erreur plutôt que ressortir plus tard sous la forme d'un 403 inexpliqué.

Les trois mécanismes ne sont pas interchangeables :

| Mécanisme                     | Question à laquelle il répond                                 | S'applique à                              |
| ----------------------------- | ------------------------------------------------------------- | ----------------------------------------- |
| `authorize`                   | Cet appelant peut-il effectuer l'opération ?                  | Les six sites de répartition              |
| `needsApproval`               | Un humain doit-il valider cet appel autorisé en particulier ? | La boucle d'agent uniquement              |
| `accessFilter`/`assertAccess` | Quelles lignes cet appelant autorisé peut-il voir ou écrire ? | À l'intérieur de `run`, par requête/ligne |

Une action d'écriture restreinte a généralement besoin à la fois de `authorize` et de `assertAccess` : le premier tient les non-coachs à l'écart de l'opération, le second empêche un coach de toucher au plan d'une autre organisation. `needsApproval` est encore un autre axe : il ne restreint jamais _qui_ peut appeler, seulement si l'agent peut continuer sans surveillance.

`coachAccess` ci-dessus provient de `defineAppRoles`, la superposition de rôles par application au-dessus de l'adhésion à l'organisation. Voir [Organizations, Teams & Permissions → App roles](/docs/organizations-teams-permissions#app-roles-per-app-rbac) pour le modèle de rôles et ses garanties.
