...
- **Monté automatiquement** – chaque application obtient `/mcp` gratuitement, aucune configuration requise
...
| **Point de terminaison** | `/mcp`                                                                  | `/_agent-native/a2a`                           |
...
Les requêtes MCP hébergées utilisent une attente en ligne bornée afin qu'une longue boucle d'agent ne maintienne pas un `tools/call` ouvert jusqu'à l'expiration du délai du client ou de la passerelle. L'argument facultatif `async` démarre la tâche durable et renvoie immédiatement ; sans lui, le serveur attend brièvement une réussite rapide, puis renvoie un descripteur de tâche lorsqu'il reste du travail à faire. L'attente hébergée est plafonnée à 25 secondes.
...
L'agent exécute la même boucle que le chat interactif : il peut appeler plusieurs outils, raisonner sur le contexte et produire une réponse réfléchie. Une tâche qui n'est pas terminée renvoie un JSON contenant `taskId`, `status`, `pollAfterMs`, et :
...
| `/mcp/oauth/register`                     | Inscription dynamique du client public                     |
| `/mcp/oauth/authorize`                    | Autorisation du navigateur + consentement                  |
| `/mcp/oauth/token`                        | Accords de code d'autorisation et de jeton d'actualisation |
