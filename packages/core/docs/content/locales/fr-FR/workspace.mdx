...
<DataModel id="doc-block-c2uuu2" title={"Le modèle de ressources de l'espace de travail"} summary={"Une forme de ressource soutient chaque fichier d’espace de travail. Le runtime le classe par chemin et portée et résout la valeur effective lors de la lecture."} entities={[
  {
    "id": "resource",
