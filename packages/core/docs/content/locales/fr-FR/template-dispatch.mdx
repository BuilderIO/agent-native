...
Dispatch est le **plan de contrôle de l'espace de travail**. Là où d'autres modèles sont des applications de domaine Agent-Native (Mail, Calendar, Analytics, Brain), Dispatch est l'application que vous exécutez _à côté_ d'eux pour tout coordonner : une boîte de réception centrale, un coffre-fort secret, des tâches planifiées, une intégration Slack/Telegram et un agent orchestrateur qui délègue le travail de domaine à la bonne application spécialisée via [A2A](/docs/a2a-protocol).
...
- **Exposer un connecteur MCP.** Ajouter
  `https://dispatch.agent-native.com/mcp` dans Claude, ChatGPT,
