...
<DataModel id="doc-block-c8ms7d" title={"Schéma du coffre-fort de secrets"} summary={"Secrets are stored once; grants give a named app access; requests + reviews gate sensitive access; the audit log records who used which secret when. Defined in @agent-native/dispatch (packages/dispatch/src/db/schema.ts)."} entities={[
  {
    "id": "secrets",
    "name": "vault_secrets",
