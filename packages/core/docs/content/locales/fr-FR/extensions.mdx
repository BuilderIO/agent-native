...
- **Capacités liées à la version** — Une extension déclare des actions, chemins et méthodes HTTP, accès SQL et origines HTTPS externes exacts. Chaque personne accepte explicitement un sous-ensemble ; toute modification du manifeste invalide l’autorisation précédente.
- **Mode historique fermé par défaut** — Sans manifeste courant accepté, l’extension peut seulement appeler des actions en lecture et lire l’état de l’application et ses propres données. SQL, écritures et sorties externes restent désactivés.
- **Contrôle côté serveur** — Les routes d’actions, SQL, état, données et proxy vérifient l’autorisation indépendamment. La révocation prend effet dès la requête suivante.
