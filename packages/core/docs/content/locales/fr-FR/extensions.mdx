...
## Dois-je utiliser une extension ou modifier le code de l’application ? {#vs-app-code}

Le framework permet à l’agent de modifier directement le code source de l’application : composants, itinéraires, styles et logique métier. Utilisez cette question lorsque l’un ou l’autre choix pourrait répondre à la demande :

- **Commencez par une extension** pour un widget autonome et relativement ponctuel : une visualisation ou interaction personnalisée, une tuile de tableau de bord, un utilitaire autonome ou un ajout dans un slot existant. Elle est rapide, ne nécessite ni modification du code source, ni build, ni déploiement, et peut rester privée ou être partagée.
- **Utilisez le code de l’application** pour une fonctionnalité centrale du modèle, une modification de l’interface native, de la mise en page, des itinéraires ou de la logique métier, ou tout ce qui doit être largement réutilisé entre tableaux de bord ou utilisateurs. Si l’hôte n’a pas de slot adapté ou si la demande nécessite un nouveau slot, utilisez le code.
- **Faites évoluer les extensions répétées.** Si la même extension est recréée régulièrement, ou si la maintenance de plusieurs copies devient moins pertinente qu’une implémentation partagée, déplacez-la dans le code de l’application comme fonctionnalité native réutilisable.
...
Règle générale : **si une extension peut résoudre le problème, commencez généralement par là.** Utilisez le code lorsque la limite des extensions ne convient pas ou lorsque la réutilisation à grande échelle et la disponibilité dans tout le produit justifient une implémentation native. Par exemple, une visualisation personnalisée ponctuelle sur un tableau de bord convient à une extension ; un nouveau type de graphique circulaire, à barres multiples ou linéaire qui doit être disponible sur tous les tableaux de bord relève du code du modèle.
