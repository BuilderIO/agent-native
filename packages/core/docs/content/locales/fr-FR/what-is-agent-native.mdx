...
L'agent natif n'est pas réservé aux développeurs. L'agent intégré donne à toute l'équipe une façon commune d'exploiter l'application, tandis que les modifications de source restent dans le flux de développement normal. Il ne peut modifier la source que si l'hôte accorde intentionnellement des outils d'écriture repository/workspace :
...
## Partir d'un modèle et personnaliser {#fork-and-customize}

Les applications natives d'agent suivent un modèle où vous partez d'une app fonctionnelle. Vous partez d'un **modèle** : calendrier, contenu, diapositives, analyses, courrier, clips, conception, formulaires, répartition – et vous vous l'appropriez. Chacun est un produit SaaS complet et fonctionnel, et non un échafaudage vierge :
...
3. Créez votre propre app à partir du modèle lorsque vous souhaitez personnaliser : "connectez notre compte Stripe", "ajoutez un graphique de cohorte"
4. Personnalisez la source dans votre dépôt et votre flux de développement
5. Déployez votre app sur votre propre domaine — ou restez sur agent-native.com

Comme il s'agit de _votre_ application et non d'une infrastructure partagée, vous possédez sa source et pouvez la faire évoluer sans modifier le framework partagé. L'agent intégré agit via les actions et les données, et ne peut aider sur la source qu'avec des outils d'écriture accordés intentionnellement. Voir [Templates](/docs/cloneable-saas) pour l'histoire complète.
