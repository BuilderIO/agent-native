...
Déployez d'abord le flag d'identité du navigateur. Après le déploiement des colonnes de migration de l'organisation dans l'application et Dispatch, activez `organization.cross-app-federation` pour une organisation et vérifiez son ID canonique et son rôle. Les organisations existantes sans mappage restent inchangées jusqu'à une migration explicite.
