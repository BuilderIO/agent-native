...
Les applications hébergées canoniques reconnaissent les requêtes de Desktop SSO Canary empaqueté sans configuration du hub d’identité pour chaque application. Les navigateurs ordinaires et les applications auto-hébergées nécessitent toujours `AGENT_NATIVE_IDENTITY_HUB_URL`. Les versions stables de Desktop n’initialisent pas le courtier avant l’approbation séparée d’une version ultérieure.
