...
- **Client hébergé canonique** → l'application affiche l'option **« Se connecter avec Agent-Native »** pour l'origine enregistrée ; les applications auto-hébergées doivent toujours définir la variable. Il n'y a aucun changement de schéma ni rien à migrer.
...
Les applications hébergées canoniques reconnaissent les requêtes de Desktop SSO Canary empaqueté et les navigateurs ordinaires sans configuration du hub d’identité pour chaque application. Les applications auto-hébergées nécessitent toujours `AGENT_NATIVE_IDENTITY_HUB_URL`. Les versions stables de Desktop n’initialisent pas le courtier avant l’approbation séparée d’une version ultérieure.
