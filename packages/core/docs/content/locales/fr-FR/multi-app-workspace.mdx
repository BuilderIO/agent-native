...
Chaque application est construite avec `APP_BASE_PATH=/<name>` et `VITE_APP_BASE_PATH=/<name>` et émise via le préréglage Nitro sélectionné. **Netlify** est le préréglage par défaut pour la commande unifiée `deploy` ; il émet des fonctions d'application sous `.netlify/functions-internal/<app>-server` et génère des redirections qui laissent les assets statiques non forcés afin que le CDN serve les fichiers en premier. **Vercel** est pris en charge avec `npx @agent-native/core@latest deploy --preset vercel` ; il écrit un bundle `.vercel/output` racine à l'aide du Build Output API de Vercel.
...
Vous préférez chaque application sur son propre domaine (`mail.company.com`, `calendar.company.com`) ? Chaque application de l'espace de travail est toujours un déployable indépendant : `cd apps/mail && pnpm build` se comporte exactement comme un échafaudage autonome. Le A2A inter-applications passe ensuite par le chemin signé JWT standard avec un `A2A_SECRET` partagé. Le SSO inter-domaines entre des applications déployées séparément est géré par la fédération d'identité avec Dispatch comme hub — voir [Cross-App SSO](/docs/cross-app-sso) ; le déploiement unifié à origine unique évite d'en avoir besoin.

### Base de données partagée, identifiants partagés

Pour partager les comptes utilisateur, les organisations, les paramètres et les autres données entre applications, définissez le même `DATABASE_URL` pour chaque application. En développement local, définissez-le dans le fichier `.env` à la racine de l'espace de travail afin que toutes les applications utilisent la même base PostgreSQL. S'il n'est pas défini, chaque application utilise sa propre base PGlite locale au processus. Les utilisateurs et les organisations ne sont alors pas partagés.

La demande de connexions s'additionne entre les processus qui partagent une base de données. Si cinq processus d'applications Node.js de longue durée utilisent chacun un pool partagé pour le même `DATABASE_URL`, ils peuvent utiliser jusqu'à 100 connexions avec la limite par défaut de 20 par pool (`5 × 1 pool × 20`). Pour les espaces de travail sans serveur, estimez le nombre d'instances de fonctions actives par application à partir des limites par défaut propres à l'environnement. Consultez [Base de données du serveur](/docs/server-database) pour `AGENT_NATIVE_DB_POOL_MAX` et les conseils complets de dimensionnement.
