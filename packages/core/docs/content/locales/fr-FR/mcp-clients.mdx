...
`agentNative.mcp.listTools`, `agentNative.mcp.callTool` et
`agentNative.providerApi` via les actions de l'hôte ; elles ne reçoivent jamais
les URL de connexion ni les jetons.

### Atlassian / Jira

Le préréglage Atlassian utilise le point de terminaison OAuth 2.1 d'Atlassian Rovo MCP,
`https://mcp.atlassian.com/v1/mcp`, et donne accès aux outils Jira et Confluence. La création de tickets Jira nécessite les autorisations Atlassian permettant l'écriture dans Jira ; l'utilisateur ne voit toujours que les projets et les contenus autorisés par son compte Atlassian.
