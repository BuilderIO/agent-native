...
| **La plateforme identifie le déploiement**                                      | L'enregistrement est indexé sur l'origine publique de l'application ; un processus qui n'est pas réellement le déploiement ne doit donc pas la revendiquer.                                                          | Netlify, Vercel, AWS Lambda, Cloud Run et Render se signalent d'eux-mêmes. Cloudflare Workers ne le fait pas : définissez `AGENT_NATIVE_REALTIME_APP_URL` avec l'origine propre du déploiement. Sur un simple conteneur ou une VM, définissez cette même variable |
