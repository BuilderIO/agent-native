...
<DataModel id="doc-block-2dx4f0" title={"Modèle de données de conception"} summary={"Une conception est propriétaire de ses fichiers et de ses instantanés versionnés, et relie éventuellement un système de conception réutilisable. Les conceptions et les systèmes peuvent être possédés, chacun avec une table de partage de cadre."} entities={[
  {
    "id": "designs",
    "name": "designs",
