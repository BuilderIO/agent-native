...
> **يقوم معظم الأشخاص بتثبيت Plan كمهارة، وليس كتطبيق داعم.** يضيف أمر CLI واحد مهارات `/visual-plan` و`/visual-recap` و`/visualize-repo` بالإضافة إلى موصل Plan المستضاف إلى وكيل الترميز الخاص بك — راجع [Plan plugin & marketplace](/docs/plan-plugin) لمسارات الإضافة والسوق. يبقى تفريع قالب Plan (الموضح في [For developers](#for-developers)) هو المسار الثانوي للاستضافة الذاتية أو البناء على Plan نفسه.
...
<Diagram id="doc-block-kp1ha1" title={"أمرين، وسطح مراجعة واحد"} summary={"يتم نشر كلا الأمرين من خلال موصل Plan MCP المستضاف في نفس سطح التعليق التوضيحي والتعليق."}>
...
<div class="diagram-plan">
  <div class="diagram-col">
    <div class="diagram-node">
      ...
      ><small class="diagram-muted"
        >قبل الكود - البنية وواجهة UI وإعادة التنظيم</small
      >
    </div>
    <div class="diagram-node">
      <span class="diagram-pill">/visual-recap</span
      ><small class="diagram-muted"
        >بعد الكود - PR و commit و branch و diff</small
      >
    </div>
  </div>
  ...
  <div class="diagram-box">
    سطح المراجعة<br /><small class="diagram-muted"
      >مخططات · إطارات سلكية · كود مشروح · تعليقات</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&harr;</div>
  <div class="diagram-node">
    وكيل البرمجة<br /><small class="diagram-muted">تمت إعادة الملاحظات</small>
  </div>
</div>
```
...
<WireframeBlock id="doc-block-pdhou">
  <Screen
    surface="desktop"
    html={
...
يقوم الأمر بتثبيت ثلاثة أوامر: `/visual-plan` و`/visual-recap` و`/visualize-repo`.
...
`https://plan.agent-native.com/mcp` — راجع [MCP Clients](/docs/mcp-clients) للتعرف على الإعداد الخاص بالعميل.
...
- نموذج الدردشة الافتراضي للتطبيق هو GPT-5.6 Luna. وتحتفظ اتصالات Anthropic المباشرة بـ Claude Sonnet 5 كنموذج افتراضي. يمكنك تغيير النماذج في أي وقت من **الإعدادات**؛ ويظل Haiku خيار Claude الأرخص.
...
موصل الخطط المستضاف موجود في `https://plan.agent-native.com/mcp`.
...
## المستندات المرئية للمستودع {#visualize-repo}

استخدم `/visualize-repo` عندما تريد طبقة توثيق مرئية ودائمة لقاعدة الكود بدلاً من خطة أو ملخص لمرة واحدة. ينشئ أو يفتح مساحة عمل Plan MDX محلية، ويحدّث `agent-native.json` بقسم `apps.visualize-repo` في وضع local-files، ويعرض المستندات عبر جسر localhost نفسه:

```bash
npx @agent-native/core@latest visualize-repo --open
```

إذا لم يكن `agent-native.json` موجودًا، يبدأ الأمر ملفًا جديدًا ويوصي بمجموعة بداية صغيرة من بنية المستودع، مثل `actions/` و`app/components/` و`app/pages/` و`server/db/schema.ts` و`src/` و`packages/` و`templates/` و`docs/` أو `content/`. يمكنك استهداف نقاط ربط مصدر محددة:
...
يعيش مجلد البداية افتراضيًا في `.agent-native/visual-docs/repo-overview`. تبقى التعليقات محلية في `comments.json`، حتى يتمكن المراجعون من تحديد الأقسام لوكيل برمجة، ويمكن للوكيل تحديث الكود وملفات MDX في التغيير نفسه. شغّل `visualize-repo check` للفحص المحلي، و`visualize-repo verify` عندما تريد أن يتحقق مخطط عارض Plan من MDX.
...
- "قم بتشغيل `/visualize-repo` وابدأ بإجراءات API العامة والمخطط لدينا."
...
<DataModel
  id="doc-block-1ovvfph"
  title={"نموذج بيانات الخطة"}
  summary={
    "يحتوي صف الخطة الواحد على أقسام مرتبة بالإضافة إلى التعليقات والأحداث والإصدارات والمشاركات والأصول المضمنة."
  }
  entities={[
    {
      id: "plans",
