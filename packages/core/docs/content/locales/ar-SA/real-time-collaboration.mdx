...
| **أن تعرّف المنصّة النشر** | يرتكز التسجيل على الأصل (origin) العام للتطبيق، لذا يجب ألا تدّعي عملية ليست هي النشر فعليًا ملكيته. | تعرّف Netlify وVercel وAWS Lambda وCloud Run وRender وCloudflare Pages عن نفسها جميعًا. أما على حاوية أو جهاز افتراضي بسيط، فاضبط `URL` على الأصل الخاص بالنشر نفسه |
...
السرّ يتيح إصدار رمز اشتراك لأي مستخدم في التطبيق، مما يكشف *تدفق التغييرات* الخاص
