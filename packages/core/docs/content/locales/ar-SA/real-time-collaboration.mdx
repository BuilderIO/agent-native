...
تستخدم التطبيقات افتراضيًا نقل poll/SSE الموضّح أعلاه. ويمكن لأي تطبيق أن يختار
بدلًا من ذلك بوابة المزامنة الفورية المستضافة، التي توزّع أحداث المزامنة على عدة
نسخ بلا خادم بدل أن تحتفظ كل نسخة باتصالاتها الطويلة الأمد.

- `AGENT_NATIVE_REALTIME_TRANSPORT=hosted` هو المتغيّر الوحيد الذي تضبطه، وضبطه هو
  الموافقة الصريحة نفسها — فلا يحدث أي شيء مما هنا في نشر لم يضبطه.
  ويتجاوز `AGENT_NATIVE_REALTIME_GATEWAY_URL` نقطة نهاية البوابة لبيئة الاختبار؛
  وإن لم يُضبط فإنه يُشتق من `BUILDER_GATEWAY_BASE_URL`.
- في تطبيق مستضاف لدى Builder، يكون معرّف القناة وسر التوقيع
  (`AGENT_NATIVE_REALTIME_HMAC_SECRET`) متغيّري بيئة محجوزين يحقنهما مسار التزويد
  في Builder — فقاعدة البيانات التي تقرأها البوابة سبق أن زوّدها خط الإنتاج.
- **في أي مكان آخر، يسجّل التطبيق نفسه، وهذا يعني إرسال عنوان قاعدة بياناتك إلى
  Builder.** فمع ضبط متغيّر النقل، يرسل النشر المعتمد على Postgres والذي يملك
  `BUILDER_PRIVATE_KEY` سلسلة الاتصال والأصل العام إلى البوابة مرة واحدة، ويستعيد
  معرّف قناة وسر توقيع. ويخزّن Builder سلسلة الاتصال تلك ويتصل بقاعدة بياناتك لتتبّع
  `sync_events`؛ والاتصال ليس للقراءة فقط، لأن تخصيص الإصدارات واكتشاف التغييرات
  الخارجية كليهما يكتبان. وإن لم ترغب في ذلك، فاترك متغيّر النقل غير مضبوط — فهذا هو
  القرار كله، ولا يوجد مسار آخر يشارك بيانات الاعتماد. ويُعاد التسجيل عند تغيّر عنوان
  قاعدة البيانات أو أصل التطبيق، فتُطبَّق دورة تدوير بيانات الاعتماد دون إعادة نشر أي
  شيء آخر.
- يجب أيضًا أن تكون المؤسسة مُفعّلة للبوابة. وإلى أن يحدث ذلك، يُرفض التسجيل ويبقى
  التطبيق على الاستقصاء المحلي.
- عمليات النشر الإنتاجية وحدها هي التي تُسجّل. أما معاينة النشر فتبقى على المزامنة
  المحلية، فلا تغادر بيانات اعتماد قاعدة بياناتها المؤقتة الجهاز، ولا تُنشأ قناة
  لكل طلب سحب.
- **المؤسسة هي حدّ الثقة.** أي بيانات اعتماد تستطيع التسجيل تستطيع أيضًا إعادة
  تسجيل أصل قائم، وهو ما يُعيد سر التوقيع الخاص بتلك القناة ويوجّه قاعدة بياناتها
  إلى مكان آخر. وامتلاك ذلك السر يتيح إصدار رمز اشتراك لأي مستخدم في التطبيق، مما
  يكشف *تدفّق التغييرات* الخاص به — أي السجلات التي تغيّرت ومتى — لا السجلات نفسها،
  لأن العملاء يعيدون قراءة المحتوى من التطبيق بجلستهم الخاصة. تعامل مع بيانات
  اعتماد التسجيل على أنها على مستوى المؤسسة كلها.
- يصدر `GET /_agent-native/realtime-token` رمز اشتراك قصير الأجل (10 دقائق) ومحصورًا
  بالجلسة، يستخدمه المتصفح لفتح اتصال البوابة. وهو يقبل الأصل نفسه فقط ويعيد 401 دون
  جلسة نشطة.
- `GET /_agent-native/can-see` هو فحص الظهور لكل حدث في البوابة: يتحقق من رمز وصول
  موقّع، ثم يعيد تشغيل `resolveAccess` الخاص بالتطبيق نفسه، حتى لا تستطيع البوابة
  إطلاقًا تمرير حدث إلى مشارَك فقد صلاحيته. وأنواع الموارد غير المعروفة أو إخفاقات
  البحث تفشل بأمان (`{ allowed: false }`).
- على Postgres، تخصّص التطبيقات المستضافة إصدارات المزامنة من قاعدة البيانات
  (`dbAssignedVersions`) بدل عدّاد داخل العملية، لأن نسخ البوابة ونسخ التطبيق بلا
  خادم تتشارك قاعدة بيانات واحدة ويجب أن تتفق على ترتيب الكتابة.

كل إخفاق هنا صامت بحكم التصميم: غياب متغيّر النقل، أو غياب مفتاح Builder، أو قاعدة
بيانات ليست Postgres، أو مؤسسة لم تُفعَّل بعد، أو بوابة يتعذّر الوصول إليها — كلها
تُبقي التطبيق على الاستقصاء المحلي بدل أن تُصدر خطأ. وإن لم تُفعَّل المزامنة الفورية
المستضافة كما تتوقع، فافحص هذه النقاط بالترتيب نفسه — فعودة 404 من
`GET /_agent-native/realtime-token` تعني أنه لم يُحَل أي قناة.
