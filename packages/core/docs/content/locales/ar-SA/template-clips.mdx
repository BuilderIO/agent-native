...
<DataModel id="doc-block-lu79or" title={"نموذج البيانات الأساسية للمقاطع"} summary={"recordings is the source of truth for media; transcripts, meetings, and dictations compose with it rather than duplicating video. (Engagement and org tables omitted for clarity — see the full table below.)"} entities={[
  {
    "id": "recordings",
    "name": "recordings",
