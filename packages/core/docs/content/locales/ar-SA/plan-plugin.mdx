...
description: "قم بتثبيت مهارات Agent-Native Plan (/visual-plan، /visual-recap، /visualize-repo) بالإضافة إلى موصل Plan MCP المستضاف كمكوّن Claude Code أو Codex، أو باستخدام CLI العام. كيفية عمل التحديثات وما إذا كنت بحاجة إلى إرسال أي شيء."
---

# المكون الإضافي للخطة والسوق

يأتي تطبيق Agent-Native **Plan** كحزمة واحدة قابلة للتثبيت. يضيف التثبيت الواحد مهارات أوامر Plan المائلة **و** يوصّل موصل Plan MCP المستضاف، حتى يتمكن الوكيل من إنشاء الخطط ويمكن للمهارات نشرها مباشرة في تطبيق Plan.

## ما تحصل عليه {#what-you-get}

تمنحك عملية التثبيت الواحدة ما يلي:

- **ثلاث skills** — `/visual-plan` (نقطة الدخول الأساسية)، و`/visual-recap`، و`/visualize-repo`.
- **موصل Plan MCP** — مسجل في التطبيق المستضاف على `https://plan.agent-native.com` (نقطة نهاية MCP `https://plan.agent-native.com/mcp`، اسم الخادم `plan`).

<Diagram id="doc-block-10bw7bb" title={"ثلاث طرق، حزمة واحدة"} summary={"يقوم كل من CLI العام ومكوّن Claude Code ومكوّن Codex بتثبيت مهارات Plan نفسها بالإضافة إلى موصل Plan المستضاف."}>
...
<div class="diagram-routes">
  <div class="diagram-col">
    ...
    <div class="diagram-node">
      إضافة Claude Code<br /><small class="diagram-muted"
        >/plugin install</small
      >
    </div>
    <div class="diagram-node">
      إضافة Codex<br /><small class="diagram-muted">codex plugin add</small>
    </div>
  </div>
...
افتراضيًا، ينشر `/visual-plan` و`/visual-recap` إلى تطبيق Plan المستضاف — فهما ينشئان خطة عبر
...
<Diagram id="doc-block-1t4cmtn" title={"وضع الملفات المستضافة مقابل وضع الملفات المحلية"} summary={"بشكل افتراضي، يتم نشر المهارات من خلال الموصل؛ يقوم وضع الملفات المحلية بكتابة MDX على القرص ومعاينته عبر جسر مضيف محلي بدلاً من ذلك."}>
...
<div class="diagram-modes">
  ...
  <div class="diagram-card">
    ...
    ><strong>اكتب MDX إلى القرص</strong
    ><small class="diagram-muted"
      >plan.mdx + canvas.mdx + prototype.mdx &rarr; localhost bridge &rarr;
      hosted Plan UI reads local source. No كتابة في قاعدة البياناتs until
...
يعمل مع أي مضيف — Claude Code وCodex وCursor وCline وGoose وتطبيقات ChatGPT المخصصة MCP وClaude Cowork وأي شيء آخر متوافق مع MCP. يثبت Agent-Native CLI مهارات Plan، ويسجل موصل Plan MCP المستضاف، **ويشغل المصادقة للعميل (العملاء) المحليين المحددين في نفس الخطوة**، حتى لا يصطدم أول استدعاء للأداة بجدار OAuth:

```bash
npx @agent-native/core@latest skills add visual-plan
```

يؤدي هذا إلى تثبيت `visual-plan` بالإضافة إلى مهارتي `visual-recap` و`visualize-repo` المصاحبتين، ثم تسجيل موصل `plan`، ثم تشغيل المصادقة (مطالبة OAuth للمشاركة المستضافة/المدعومة بالحساب). إشارات مفيدة:
...
`/plugin install` يضيف مهارات Plan وتكوين MCP **خاصًا بعنوان URL فقط** (لا توجد أسرار في الحزمة)؛ `/mcp` → **المصادقة** تكمل مصافحة OAuth. استخدم مسار CLI العام بدلاً من ذلك عندما تريد الملفات المحلية أو وضع الاستضافة الذاتية.
