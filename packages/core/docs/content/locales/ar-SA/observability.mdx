...
تسجل كل من إمكانية المراقبة وبوابة تقييم CI _الجودة_ ولكن من طرفين متعاكسين - تسجيل سلبي لاحق لحركة المرور الحقيقية مقابل عمليات فحص النجاح/الفشل النشطة على المدخلات الثابتة. ذاكرة الملاحظة لا علاقة لها بالجودة؛ يتعلق الأمر بتكلفة الرمز المميز وضغط نافذة السياق. رقِّ أثر إنتاج مكتمل إلى حالة `*.eval.ts` (لوحة **Promote to eval** أو `promote-trace-eval` أو `agent-native eval promote <runId> --write evals/from-trace.eval.ts`). انظر [من أثر إنتاج](/docs/evals#from-trace).
...
| POST    | `/traces/:runId/promote`   | ترقية تشغيل مكتمل إلى تقييم CI |
