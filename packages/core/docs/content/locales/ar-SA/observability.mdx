...
تحتوي لوحة التحكم على 6 علامات تبويب:
...
| **المراجعة البشرية** | مراجعة الطلب والإجابة، وتسجيل الملاحظات، وإعداد مسودة لتحديث التعليمات                          |

### تفاصيل المحادثات {#conversations}

تعرض علامة تبويب المحادثات عمليات التشغيل المسجلة. اختر عملية لعرض النطاقات، ثم وسّع أي نطاق لرؤية مدخلات الأدوات ومخرجاتها وأخطائها وبياناتها الوصفية التي تم التقاطها. استخدم **فتح المحادثة الكاملة** للانتقال إلى سلسلة الدردشة المرتبطة.

يكون التقاط وسيطات الأدوات ونتائجها متوقفًا افتراضيًا. فعّل `captureToolArgs` و`captureToolResults` في إعدادات `observability` لحفظها. تُنقّح الحقول الحساسة ورسائل أخطاء الأدوات قبل التخزين.

### المراجعة البشرية {#review}

تُظهر علامة تبويب المراجعة البشرية طلب كل عملية تشغيل وإجابتها، وتتيح تسجيل الملاحظات وإعداد مسودة لتحديث التعليمات كي يراجعها شخص. حفظ المسودة لا يغيّر التعليمات تلقائيًا.
