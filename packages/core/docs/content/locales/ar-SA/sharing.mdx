...
<DataModel id="doc-block-1ebud6w" title={"جدول المشاركات + الموارد المرافقة"} summary={"Coarse visibility lives on the resource; each fine-grained grant is a row in the shares table."} entities={[
  {
    "id": "deck",
    "name": "decks",
    "note": "...ownableColumns()",
