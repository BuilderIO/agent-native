---
title: "الوكلاء الخارجيون: Claude، ChatGPT، Codex، المؤشر، العمل المشترك، Grok"
description: "قم بتوصيل Claude، أو ChatGPT، أو Codex، أو Cursor، أو Claude Cowork، أو Grok، أو أي مضيف متوافق مع MCP بتطبيق وكيل مستضاف - ثم إرجاع العناصر ذهابًا وإيابًا إلى UI قيد التشغيل باستخدام تطبيقات MCP والروابط العميقة."
search: "Claude Anthropic ChatGPT OpenAI Claude الكود Codex المؤشر Claude Cowork Grok xAI MCP وكيل التطبيقات الأصلي، توصيل أدوات الوكيل المحلي، الوكلاء الخارجيون"
---
...
يوفر كل تطبيق Agent-Native عنوان MCP أساسيًا واحدًا:

```text
https://<your-app>.agent-native.com/mcp
```

في التطبيق، افتح **Settings → Integrations** وابحث باسم المضيف الذي تعرفه:
Claude أو Claude Cowork أو Claude Code أو Anthropic أو ChatGPT أو OpenAI أو Codex
أو Grok أو xAI. تفتح النتيجة الوحيدة للمضيف الخارجي **Settings → MCP** مع تحديد
الدليل الأقرب. انسخ **MCP server URL** والصقه في مضيف الذكاء الاصطناعي. العنوان هو
عنوان URL الأساسي للتطبيق المنشور مع إضافة `/mcp`.
...
علامات التبويب الخاصة بالمضيف لـ Claude، وChatGPT، والمؤشر، وClaude Code، وCodex، وGrok، وغيرها.

<a id="access-tab"></a>

### إعداد MCP داخل التطبيق {#mcp-settings}

تتضمن إعدادات التطبيق القياسية علامة تبويب **MCP** بجانب **Integrations**. تعرض
عنوان MCP الأساسي للتطبيق، وبطاقة وكيل A2A عند توفرها، وتعليمات الإعداد نفسها
المستخدمة في صفحة الاتصال الكاملة. استخدم **Settings → MCP** كنقطة البداية داخل
التطبيق، أو افتح `/mcp/connect` مباشرةً للتدفق الكامل والرمز الاحتياطي.
...
ChatGPT: تتطلب تطبيقات MCP المخصصة مساحة عمل مدعومة على ChatGPT web. إذا كان
لديك وصول، افتح **Settings → Apps → Advanced settings**، وفعّل وضع المطور عند
الحاجة، واختر **Create app**، والصق عنوان MCP نفسه، واختر OAuth، وافحص الأدوات،
وسجّل الدخول باستخدام Agent-Native، ووافق على النطاقات المطلوبة، ثم فعّل التطبيق
في محادثة. تدعم مساحات Business وEnterprise وEdu بروتوكول MCP الكامل، بما في
ذلك إجراءات الكتابة؛ بينما يدعم Pro القراءة والجلب في وضع المطور. إذا لم يظهر
**Apps** أو **Create app**، فإن خطتك أو سياسة مساحة العمل لا تسمح بهذا الإعداد.
...
- [Actions](/docs/actions-overview) — تعريف actions، `publicAgent`، GET / `readOnly`.
