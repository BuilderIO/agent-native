...
في التطبيق، افتح **Settings**، وابحث عن **MCP**، وانسخ **MCP server URL**.
الصق عنوان URL هذا في مضيف الذكاء الاصطناعي الذي تريد استخدامه. العنوان هو أصل
عنوان URL الأساسي للتطبيق المنشور مع `/mcp`؛ ولا تحتاج إلى البحث عن إعداد للمطورين.
