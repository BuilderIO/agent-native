...
`agentNative.mcp.listTools` و`agentNative.mcp.callTool` وواجهات
`agentNative.providerApi` للوصول إلى البيانات المتصلة عبر إجراءات المضيف؛ ولا
تتلقى عناوين الاتصال أو رموز الوصول أو الرموز السرية.

### Atlassian / Jira

يستخدم إعداد Atlassian المسبق نقطة نهاية OAuth 2.1 الخاصة بـ Atlassian Rovo MCP،
`https://mcp.atlassian.com/v1/mcp`، ويوفّر أدوات Jira وConfluence.
