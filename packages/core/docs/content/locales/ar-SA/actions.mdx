...
لعمليات التطبيقات](/docs/server-overview#actions-first).
...
- **`readOnly: true`** — تخطي تحديث الاستقصاء بشكل صريح حتى بالنسبة إلى POST actions التي لم تتغير. إنه **ليس** تحكّمًا في الوصول: فهو لا يقيّد من يمكنه استدعاء الـ action، ولا يؤثر على `useActionQuery` أو `useActionMutation` أو أي hook في العميل. تأثيره الوحيد هو منع حدث التحديث بعد الاستدعاء واستثناء الـ action من تدقيق الكتابة. لإخفاء action عن النموذج استخدم `agentTool: false`، ولتقييد المستدعين استخدم خيارات المصادقة/الوصول أعلاه.
