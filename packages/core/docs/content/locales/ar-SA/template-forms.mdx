...
<DataModel id="doc-block-qazfqj" title={"نموذج بيانات النماذج"} summary={"ثلاث جداول. الحقول وعمليات التكامل عبارة عن أعمدة JSON في النماذج، لذا فإن تعديلات الوكيل عبارة عن تصحيحات جراحية بدلاً من تغييرات الصفوف عبر الجدول."} entities={[
  {
    "id": "forms",
    "name": "forms",
