...
> تفرض بوابات البيئة المستضافة (مثل Netlify أو Vercel أو Cloudflare Workers) حدود تنفيذ صارمة (غالبًا من 10 إلى 30 ثانية) على مسارات HTTP العامة. نظرًا لأن حلقات الوكيل يمكن أن تستغرق وقتًا طويلاً لتشغيل الاستعلامات وجلب السياق وتنفيذ الأدوات، **يجب عليك استخدام `async: true`** عند استدعاء نقاط نهاية A2A أو التعامل مع webhooks الخارجية. يؤدي هذا على الفور إلى إرجاع حالة `working` إلى بوابة API، مع إبقاء الاتصال مفتوحًا لبضعة ميلي ثانية فقط، بينما يقوم `/process-task` POST الذي يتم تشغيله ذاتيًا بتنفيذ حلقة الوكيل في الخلفية. لا تقم بحظر طلب HTTP الأساسي في انتظار انتهاء حلقة الوكيل.
...
- يُرجع `isA2AProductionRuntime()` `true` على Netlify وAWS Lambda وCloudflare Workers وVercel وRender وFly وCloud Run - حتى عندما لا يكون `NODE_ENV` `"production"`. لا يقوم بعض موفري الخدمة بدون خادم بتعيين `NODE_ENV` بشكل متسق، لذا تقرأ السياسة العلامات الخاصة بموفر الخدمة أيضًا.
