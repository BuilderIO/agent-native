...
لا يملك الـ Worker نظام ملفات، لذلك تكون قاعدة البيانات إما `DATABASE_URL`
خارجيًا أو Cloudflare D1. اضبط `CLOUDFLARE_D1_DATABASE_NAME` و
`CLOUDFLARE_D1_DATABASE_ID` معًا في بيئة البناء ليكتب البناء ربط `DB` في
`wrangler.json` المُولَّد؛ ضبط أحدهما فقط يُعدّ خطأً بدلًا من إسقاط الربط
بصمت. راجع دليل النشر بالإنجليزية للحصول على تسلسل التشغيل المحلي كاملًا.


### عرض المتصفح {#cloudflare-browser-rendering}

لا يستطيع الـ Worker تشغيل عملية Chromium، لذلك يكون المتصفح ربطًا بدلًا من ذلك،
ويُصرِّح `wrangler.json` المُولَّد بـ `{ "browser": { "binding": "BROWSER" } }`.
تعتمد الإجراءات المعتمدة على المتصفح على هذا الربط عبر `@cloudflare/playwright`.
غياب الربط يُبلَّغ عنه باسم الربط نفسه بدلًا من الادعاء بغياب ملف Chromium
التنفيذي. يعمل هذا محليًا مع `wrangler dev` بقيد واحد: يرفض وقت التشغيل المحلي
أي طلب أكبر من 1 ميغابايت.

### Cloudflare R2 {#cloudflare-r2}

لا يملك الـ Worker نظام ملفات، ولا يجوز تخزين محتويات الملفات في SQL، لذلك
تذهب الملفات المرفوعة إلى R2. اضبط `CLOUDFLARE_R2_BUCKET_NAME` في بيئة البناء
ليضيف البناء ربط `UPLOADS` إلى `wrangler.json` المُولَّد، واضبط
`CLOUDFLARE_R2_PUBLIC_BASE_URL` على العنوان العام للحاوية. المزوِّد مسجَّل
مسبقًا في سجل رفع الملفات. غياب الربط أو كونه غير صالح يؤدي إلى فشل صريح مع
إرشادات الإعداد بدلًا من تخزين الحمولة في SQL. التفاصيل في دليل النشر
الإنجليزي.
...
- `extensionTools: true` — يفعّل للتطبيق actions وإرشادات إدارة امتدادات
...
[Builder.io](https://www.builder.io?utm_source=agent-native&utm_medium=docs&utm_campaign=framework&utm_content=deployment) هو سير عمل منفصل مُدار وقادر على تعديل التعليمات البرمجية، ويمكن للوكيل فيه تعديل UI لتطبيقك في الإنتاج. صِل مستودعك بـ Builder.io واطلب تغييرات UI مباشرة - لا حاجة لإعادة النشر.
