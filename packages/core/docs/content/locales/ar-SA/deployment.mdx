...
| `A2A_SECRET`             | HMAC مشترك لـ A2A JSON-RPC بين التطبيقات. عند عدم ضبط `SECRETS_ENCRYPTION_KEY` تستخدم الخزنة المشتركة مفتاحًا مشتقًا لهذا الغرض من هذه القيمة، لذا يؤدي تدويرها أيضًا إلى تدوير مفتاح الخزنة.                                                                                     |
| `SECRETS_ENCRYPTION_KEY` | مفتاح AES-256-GCM مستقر ومفضّل للخزنة المشتركة. اضبط القيمة نفسها في كل تطبيق يقرأها. تعود القراءة إلى `A2A_SECRET` المشتق ثم مفاتيح Better Auth والمفاتيح الخاصة بالتطبيق القديمة. قبل تدوير A2A أضف هذا المفتاح في كل مكان واقرأ/رحّل كل الصفوف مع بقاء مادة A2A القديمة متاحة. |
