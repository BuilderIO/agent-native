...
### اختيار وقت ظهور واجهة الإجراء {#conditional-native-ui}

قد يتعامل الإجراء مع عمليات متعددة، بينما تحتاج بعض النتائج فقط إلى بطاقة. أضف `when(args, result)` إلى `chatUI` لتحديد هذه الاستدعاءات. يشغّل الخادم هذا الشرط بعد نجاح الإجراء. إذا أعاد `false`، تبقى النتيجة ضمن سجل الأدوات العادي القابل للطي. لا يُحفظ الشرط ولا يُرسل إلى المتصفح.

يستخدم Mail هذا النمط في `manage-draft` لعرض بطاقة عند إنشاء مسودة، مع إبقاء التحديث والحذف كاستدعاءات أدوات عادية:
...
في الإجراءات ذات عمليات الكتابة المتعددة، تحقّق من الوسيطات والنتيجة الناجحة. يختار إجراء فلاتر Gmail في Mail بطاقة القاعدة فقط للفلاتر التي أُنشئت أو استُبدلت وتحتوي على حقول الملخص:
