...
<DataModel id="doc-block-b58qel" title={"نموذج بيانات المحتوى"} summary={"Nine tables in server/db/schema.ts. documents is the page tree; the rest hang off it for versions, comments, Notion sync, inline databases, and sharing."} entities={[
  {
    "id": "documents",
    "name": "documents",
