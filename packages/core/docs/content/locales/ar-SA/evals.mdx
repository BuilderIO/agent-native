...
## من أثر إنتاج {#from-trace}

عندما يفاجئك تشغيل حقيقي — نجح في العرض ثم فشل في الإنتاج، أو استدعى الأداة الخطأ — رقِّ ذلك الأثر المكتمل إلى حالة CI بدل اختراع موجّه.

```bash
agent-native eval promote <runId> --write evals/from-trace.eval.ts
```

يحفظ الأمر صف `EvalDataset` (نفس المساعد الذي يستخدمه لوحة المراقبة والإجراء `promote-trace-eval`) ويكتب تثبيت `defineEval` تكتشفه بوابة `agent-native eval` أصلًا. المقيّمون الافتراضيون هم `usesTool(...)` لكل أداة ناجحة مع `--must-contain` اختياري. التشغيل المقطوع أو الملغى أو بلا رسالة مستخدم يفشل مغلقًا.

الإجراء المستضاف و`POST /_agent-native/observability/traces/:runId/promote` لا يكتبان شجرة التطبيق. استخدم `--write` لبوابة CI. تفاصيل التتبع في لوحة Observability فيها **Promote to eval**.
