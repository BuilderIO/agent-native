...
| Agent और UI के बीच Live sync      | किसी long-lived host पर, same-process writes तुरंत `/_agent-native/events` पर stream होते हैं; एक lightweight poll cron और cross-process writes को convergent रखती है, और प्रोडक्शन serverless पर उसी गति से ऐप का यही एकमात्र रास्ता है, जब तक ऐप ने [होस्टेड रियलटाइम सिंक गेटवे](/docs/real-time-collaboration#hosted-gateway) चालू न किया हो। Mutating actions automatically action-backed queries को invalidate कर देती हैं, इसलिए agent द्वारा बनाए गए records बिना manual refresh के दिखाई देते हैं। नीचे देखें [Live Sync](#polling-sync)। |
...
जब agent data बदलता है, तो UI को उसे बिना manual refresh के reflect करना होता है। `useDbSync()` इसे automatic बनाता है। किसी long-lived host पर, same-process writes `/_agent-native/events` पर stream होते हैं; `/_agent-native/poll` cross-process fallback बना रहता है। जब agent database (application state, settings, या domain data) में लिखता है, तो एक version counter बढ़ता है और client relevant React Query caches को invalidate कर देता है।

SSE, WebSockets के बजाय common case को cover करता है, क्योंकि यह same-process writes को plain HTTP के ज़रिए browser तक तुरंत पहुंचने का रास्ता देता है, जबकि version-counter poll एक fallback के रूप में बना रहता है जो हर deployment environment में काम करता है। किसी प्रोडक्शन serverless host (Netlify, Lambda, Vercel) पर ऐप की अपनी `/_agent-native/events` stream कोई धीमा रास्ता नहीं है — यह बंद है। उस stream को खुला रखने से एक function instance रुक जाएगा जो एक समय में केवल एक request serve करता है (इसलिए वह किसी दूसरे invocation का write वैसे भी कभी नहीं देख सकता) जब तक platform उसे kill न कर दे और container recycle न हो जाए; इसके बजाय server तुरंत 204 के साथ जवाब देता है, और पूरी sync उसी गति से `/_agent-native/poll` संभालता है, जब तक ऐप ने [होस्टेड रियलटाइम सिंक गेटवे](/docs/real-time-collaboration#hosted-gateway) चालू न किया हो, जो इसके बजाय अपनी long-lived प्रक्रिया से stream करता है।
