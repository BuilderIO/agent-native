...
- **एक MCP टूल** - Claude, ChatGPT कस्टम MCP ऐप्स, Claude डेस्कटॉप/कोड, कर्सर, Codex और किसी अन्य MCP होस्ट के संपर्क में है।
...
<Diagram id="doc-block-13exb5k" title={"एक परिभाषा, सात उपभोक्ता"} summary={"एक मान्य स्कीमा और एक रन() बॉडी के साथ एक एकल defineAction() हर सतह - एजेंट, UI, HTTP, MCP, A2A, और CLI को संचालित करता है।"}>

```html
<div class="diagram-fanout">
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">defineAction()</span
    ><small class="diagram-muted">schema + run(), एक बार परिभाषित</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-grid">
    <div class="diagram-node">
      एजेंट टूल<br /><small class="diagram-muted">संदर्भ में JSON Schema</small>
    </div>
    <div class="diagram-node">
      React हुक्स हुक<br /><small class="diagram-muted"
        >useActionQuery/Mutation</small
      >
    </div>
    <div class="diagram-node">
      callAction()<br /><small class="diagram-muted">इम्पेरेटिव क्लाइंट</small>
    </div>
    <div class="diagram-node">
    ...
    <div class="diagram-node">
      MCP टूल<br /><small class="diagram-muted">बाहरी होस्ट</small>
    </div>
    <div class="diagram-node">
      A2A टूल<br /><small class="diagram-muted">अन्य agent-native ऐप्स</small>
    </div>
    <div class="diagram-node">
...
ऐप संचालन के लिए](/docs/server-overview#actions-first).
...
<AnnotatedCode
  id="doc-block-b271zy"
  title={"किसी क्रिया की शारीरिक रचना"}
  ...
  annotations={[
    {
      lines: "5",
      label: "टूल सतह",
      note: "`description` वह पाठ है जिसे एजेंट यह तय करने के लिए पढ़ता है कि इसे कब कॉल करना है। हर फ़ील्ड की `.describe()` कॉल JSON Schema में भी जाती है।",
    },
    {
      lines: "6-9",
      label: "टाइप किया हुआ अनुबंध",
      note: "एक schema **हर** सतह से आने वाले input को validate करता है और model के लिए JSON Schema में बदलता है। अमान्य input कभी `run` तक नहीं पहुंचते।",
    },
    {
      lines: "10-13",
      label: "एक कार्यान्वयन",
      note: "`run` का मुख्य भाग ही भरोसेमंद स्रोत है — UI बटन और एजेंट टूल दोनों यही चलाते हैं।",
    },
  ]}
/>
...
<Endpoint id="doc-block-1msf0yi" title={"अपने आप mount हुआ action endpoint"} method="GET" path="/_agent-native/actions/get-lead" summary={"हर action यहां अपने आप mount होता है - filename ही action name है।"} auth={"Session cookie; frontend calls `X-Agent-Native-Frontend: 1` लेकर चलते हैं"} params={[
  {
    "name": "leadId",
    "in": "query",
    "type": "string",
  ...
  {
    "status": "200",
    "description": "Action का return value JSON के रूप में।"
  },
  {
    "status": "400",
    "description": "run() चलने से पहले input schema validation में fail हुआ।"
  }
]}>

Default POST है; `http: { method: "GET" }` इसे GET बनाता है। React हुक्स हुक और `callAction` हमेशा इस path को name से call करते हैं, चाहे कोई भी `http.path` override हो।
...
- **`readOnly: true`** - स्पष्ट रूप से उन POST actions के लिए भी पोल-रीफ्रेश को छोड़ दें जो उत्परिवर्तन नहीं करते हैं। यह **एक्सेस कंट्रोल नहीं है**: यह इस बात को सीमित नहीं करता कि action को कौन कॉल कर सकता है, और यह `useActionQuery`, `useActionMutation` या किसी भी क्लाइंट hook को प्रभावित नहीं करता। इसका एकमात्र प्रभाव कॉल के बाद के रिफ़्रेश इवेंट को रोकना और action को राइट-ऑडिटिंग से बाहर रखना है। मॉडल से action छिपाने के लिए `agentTool: false` का उपयोग करें; कॉल करने वालों को सीमित करने के लिए ऊपर दिए गए auth/access विकल्पों का उपयोग करें।
...
CLI और गैर-UI MCP होस्ट के लिए `link` को फ़ॉलबैक के रूप में रखें; यह एंबेड का लॉन्च लक्ष्य भी है। एंबेड ब्रिज - हस्ताक्षरित एंबेड-स्टार्ट सत्र, ट्रांसप्लांट बनाम नियंत्रित-फ़्रेम रेंडरिंग, `ui/*` होस्ट ब्रिज, CSP, और ऊंचाई क्लैंपिंग - [External Agents](/docs/external-agents#mcp-app-bridge) के स्वामित्व में है।
