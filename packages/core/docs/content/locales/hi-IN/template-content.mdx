...
<DataModel id="doc-block-18ohlgg" title={"सामग्री डेटा मॉडल"} summary={"Nine tables in server/db/schema.ts. documents is the page tree; the rest hang off it for versions, comments, Notion sync, inline databases, and sharing."} entities={[
  {
    "id": "documents",
    "name": "documents",
