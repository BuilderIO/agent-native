...
इसके बजाय होस्टेड रियलटाइम सिंक गेटवे चुन सकता है, जो सिंक इवेंट्स को कई सर्वरलेस
इंस्टेंस तक पहुँचाता है, बजाय इसके कि हर इंस्टेंस अपने लंबे समय तक चलने वाले कनेक्शन
बनाए रखे।

- `AGENT_NATIVE_REALTIME_TRANSPORT=hosted` ही एकमात्र वेरिएबल है जिसे आप सेट करते
  हैं, और उसे सेट करना ही स्पष्ट सहमति है — जिस डिप्लॉयमेंट ने इसे सेट नहीं किया,
  वहाँ इसमें से कुछ भी नहीं होता। `AGENT_NATIVE_REALTIME_GATEWAY_URL` स्टेजिंग के
  लिए गेटवे एंडपॉइंट को ओवरराइड करता है; सेट न होने पर यह
  `BUILDER_GATEWAY_BASE_URL` से निकाला जाता है।
- Builder द्वारा होस्ट किए गए ऐप में, चैनल id और साइनिंग सीक्रेट
  (`AGENT_NATIVE_REALTIME_HMAC_SECRET`) आरक्षित एनवायरनमेंट वेरिएबल हैं जिन्हें
  Builder का प्रोविज़निंग पथ इंजेक्ट करता है — गेटवे जिस डेटाबेस को पढ़ता है, उसे
  पाइपलाइन पहले ही प्रोविज़न कर चुकी है।
- **कहीं और, ऐप खुद को पंजीकृत करता है, और इसका अर्थ है Builder को अपना डेटाबेस URL
  भेजना।** ट्रांसपोर्ट वेरिएबल सेट होने पर, `BUILDER_PRIVATE_KEY` वाला
  Postgres-आधारित डिप्लॉयमेंट अपनी कनेक्शन स्ट्रिंग और सार्वजनिक ऑरिजिन एक बार गेटवे
  को भेजता है और बदले में चैनल id तथा साइनिंग सीक्रेट पाता है। Builder उस कनेक्शन
  स्ट्रिंग को संग्रहीत करता है और `sync_events` को ट्रैक करने के लिए आपके डेटाबेस से
  जुड़ता है; यह कनेक्शन केवल-पढ़ने वाला नहीं है, क्योंकि वर्ज़न आवंटन और बाहरी बदलाव
  की पहचान दोनों लिखते हैं। यदि आप यह नहीं चाहते, तो ट्रांसपोर्ट वेरिएबल सेट न करें —
  पूरा निर्णय बस इतना ही है, और क्रेडेंशियल साझा करने वाला कोई दूसरा रास्ता नहीं है।
  डेटाबेस URL या ऐप ऑरिजिन बदलने पर पंजीकरण दोबारा चलता है, इसलिए क्रेडेंशियल रोटेशन
  बिना कुछ और दोबारा डिप्लॉय किए लागू हो जाता है।
- संगठन के लिए भी गेटवे सक्षम होना चाहिए। जब तक ऐसा नहीं होता, पंजीकरण अस्वीकार होता
  है और ऐप स्थानीय पोलिंग पर बना रहता है।
- केवल प्रोडक्शन डिप्लॉयमेंट पंजीकृत होते हैं। डिप्लॉय प्रीव्यू स्थानीय सिंक पर
  बना रहता है, इसलिए उसका अस्थायी डेटाबेस क्रेडेंशियल मशीन से बाहर नहीं जाता और हर
  पुल रिक्वेस्ट के लिए एक चैनल नहीं बनता।
- **संगठन ही विश्वास की सीमा है।** जो भी क्रेडेंशियल पंजीकरण कर सकता है, वह किसी
  मौजूदा ऑरिजिन को दोबारा पंजीकृत भी कर सकता है, जिससे उस चैनल का साइनिंग सीक्रेट
  वापस मिलता है और उसका डेटाबेस दूसरी ओर मुड़ जाता है। वह सीक्रेट होने पर ऐप के किसी
  भी उपयोगकर्ता के लिए सब्सक्राइब टोकन बनाया जा सकता है, जिससे उनकी *चेंज स्ट्रीम*
  उजागर होती है — कौन-से रिकॉर्ड कब बदले — पर स्वयं रिकॉर्ड नहीं, क्योंकि क्लाइंट
  सामग्री अपनी सेशन से ऐप से दोबारा पढ़ते हैं। पंजीकरण क्रेडेंशियल को पूरे संगठन पर
  लागू मानें।
- `GET /_agent-native/realtime-token` एक अल्पकालिक (10 मिनट), सेशन-स्कोप्ड सब्सक्राइब
  टोकन जारी करता है जिसका उपयोग ब्राउज़र गेटवे कनेक्शन खोलने के लिए करता है। यह केवल
  समान-ऑरिजिन स्वीकार करता है और सक्रिय सेशन के बिना 401 देता है।
- `GET /_agent-native/can-see` गेटवे की प्रति-इवेंट विज़िबिलिटी जाँच है: यह हस्ताक्षरित
  गेटवे एक्सेस टोकन सत्यापित करता है, फिर ऐप का अपना `resolveAccess` दोबारा चलाता है,
  ताकि गेटवे किसी ऐसे साझेदार को इवेंट कभी न भेजे जिसकी पहुँच हट चुकी है। अज्ञात
  रिसोर्स प्रकार या लुकअप विफलताएँ सुरक्षित रूप से विफल होती हैं
  (`{ allowed: false }`)।
- Postgres पर, होस्टेड ऐप्स सिंक वर्ज़न इन-प्रोसेस काउंटर के बजाय डेटाबेस
  ...
  सर्वरलेस इंस्टेंस एक ही डेटाबेस साझा करते हैं और उन्हें लेखन क्रम पर सहमत होना
  आवश्यक है।

यहाँ हर विफलता जानबूझकर मौन है: ट्रांसपोर्ट वेरिएबल न होना, Builder कुंजी न होना,
Postgres के अलावा कोई डेटाबेस, अभी तक सक्षम न किया गया संगठन, या पहुँच से बाहर गेटवे —
इनमें से कोई भी स्थिति त्रुटि देने के बजाय ऐप को स्थानीय पोलिंग पर छोड़ देती है। यदि
होस्टेड रियलटाइम अपेक्षा के अनुसार सक्रिय नहीं है, तो इसी क्रम में जाँचें —
`GET /_agent-native/realtime-token` का 404 लौटाना यह बताता है कि कोई चैनल हल नहीं हुआ।
