...
Las comillas JSON son obligatorias. Tanto el trabajo ligero `gate` como el trabajo completo `recap` utilizan el ejecutor seleccionado. Para configurarlo durante la instalación guiada, pase el mismo valor JSON a `--runs-on` y después valide el flujo de trabajo y la disponibilidad del ejecutor con `doctor`:
...
- **`VISUAL_RECAP_MODEL`** fija el modelo pasado al CLI (`--model`), por ejemplo, `gpt-5.6-sol` para Codex o una identificación de modelo Claude. Déjelo sin configurar para usar el modelo predeterminado del CLI.
...
| `VISUAL_RECAP_MODEL`     | el valor predeterminado de cada CLI | Variable. Fija el modelo, p.e. `gpt-5.6-sol` para Codex, o una identificación de modelo Claude. Unset utiliza el valor predeterminado del CLI.        |
...
Se aplican los mismos secretos y variables descritos en [Secrets and variables](#secrets-and-variables); configúrelos en la configuración de su repositorio de la misma manera que para la variante de copia. El llamador reutilizable reenvía `VISUAL_RECAP_RUNS_ON` mediante su entrada `runs-on`; el flujo de trabajo llamado analiza el JSON y utiliza el resultado para ambos trabajos. Si omite la entrada, se conserva el valor predeterminado `ubuntu-latest`.
