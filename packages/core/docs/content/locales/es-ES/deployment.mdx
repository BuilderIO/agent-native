...
| `A2A_SECRET`             | HMAC compartido para A2A JSON-RPC entre apps. Si `SECRETS_ENCRYPTION_KEY` no está definido, la bóveda compartida usa una clave derivada para este propósito; rotar este valor también rota la clave de la bóveda.                                                                                                                                        |
| `SECRETS_ENCRYPTION_KEY` | Clave AES-256-GCM estable y preferida para la bóveda compartida. Usa el mismo valor en toda app que la lea. Las lecturas vuelven a `A2A_SECRET` derivado y después a claves heredadas de Better Auth y específicas de la app. Antes de rotar A2A, añádela en todas partes y lee/migra todas las filas mientras el material A2A anterior siga disponible. |
