...
| `AGENT_NATIVE_BUILDER_RELAY_TARGET_ORIGINS` | Lista de orígenes de vista previa de confianza exactos, separados por comas y solo para la devolución de llamada corporativa (por ejemplo, `https://deploy-preview-42--content.netlify.app`). Configúrela únicamente en el despliegue de devolución de llamada corporativo. No se admiten comodines ni sufijos de dominio.                                     |
