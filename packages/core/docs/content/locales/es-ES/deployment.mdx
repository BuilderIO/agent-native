...
Un Worker no tiene sistema de archivos, así que la base de datos es una
`DATABASE_URL` externa o Cloudflare D1. Define
`CLOUDFLARE_D1_DATABASE_NAME` y `CLOUDFLARE_D1_DATABASE_ID` juntas en el
entorno de compilación para que la compilación escriba el binding `DB` en el
`wrangler.json` generado; definir solo una de las dos es un error, no un
binding omitido en silencio. La secuencia completa de arranque local está en la
guía de despliegue en inglés.


### Browser Rendering {#cloudflare-browser-rendering}

Un Worker no puede lanzar un proceso de Chromium: el navegador es un binding, y
el `wrangler.json` generado declara
`{ "browser": { "binding": "BROWSER" } }`. Las acciones que dependen del
navegador resuelven ese binding a través de `@cloudflare/playwright`. Si el
binding falta, se informa nombrándolo en lugar de afirmar que falta un binario
de Chromium. En local funciona con `wrangler dev` con una limitación: el
runtime local rechaza peticiones de más de 1 MB.

### Cloudflare R2 {#cloudflare-r2}

Un Worker no tiene sistema de archivos y el contenido de los archivos no puede
vivir en SQL, así que las subidas van a R2. Defina `CLOUDFLARE_R2_BUCKET_NAME`
en el entorno de compilación para que el build escriba el enlace `UPLOADS` en
el `wrangler.json` generado, y `CLOUDFLARE_R2_PUBLIC_BASE_URL` con la URL
pública del bucket. El proveedor ya está registrado en el registro de subidas.
Un enlace ausente o inservible falla con instrucciones de configuración en
lugar de guardar la carga en SQL. Detalles en la guía de despliegue en inglés.
...
- `extensionTools: true`: habilita para la aplicación las actions y la guía de
