...
- `extensionTools: true`: habilita para la aplicación las actions y la guía de
  gestión de extensiones (`create-extension`, `update-extension`, etc.). Las
  extensiones están desactivadas de forma predeterminada.
...
[Builder.io](https://www.builder.io?utm_source=agent-native&utm_medium=docs&utm_campaign=framework&utm_content=deployment) resuelve este problema proporcionando un entorno de nube administrado donde el agente conserva la capacidad de modificar el UI de su aplicación en producción. Conecte su repositorio a Builder.io y solicite los cambios de UI directamente, sin necesidad de volver a implementarlo.
