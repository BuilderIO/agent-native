...
Usa `useFeatureFlag(FULL_APP_BUILDING.key)` de `@agent-native/core/client` para ocultar o mostrar la superficie del cliente. Core proporciona `get-feature-flags`, `list-feature-flags` y `set-feature-flag`; la pestaña compartida de configuración de indicadores de funciones usa esas mismas actions para despliegues por usuario exacto, organización, porcentaje y globales. La administración comprueba los permisos y queda auditada. Nunca canalices escrituras de indicadores mediante endpoints genéricos de configuración o SQL sin procesar, ni coloques la autenticación, autorización, auditoría, secretos o el comportamiento de caché SSR detrás de un indicador en tiempo de ejecución. Lee la skill `feature-flags` para obtener orientación sobre el despliegue y la retirada.
