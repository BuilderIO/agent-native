...
Para operaciones de aplicaciones](/docs/server-overview#actions-first).
...
- **`readOnly: true`**: omite explícitamente la actualización de la encuesta incluso para POST actions que no mutan. **No** es control de acceso: no restringe quién puede llamar a la action y no afecta a `useActionQuery`, `useActionMutation` ni a ningún hook de cliente. Sus únicos efectos son suprimir el evento de refresco posterior a la llamada y excluir la action de la auditoría de escrituras. Para ocultar una action al modelo usa `agentTool: false`; para restringir a quienes la llaman usa las opciones de auth/acceso anteriores.
...
Cinco banderas controlan quién puede invocar una acción. Todos tienen por defecto el valor permisivo, por lo que solo configura uno para apretar una superficie específica. Esta tabla es el resumen visible; las subsecciones añaden el detalle que cada una necesita.
...
| `authorize`     | ninguno           | una función de guarda → solo los llamadores que acepta, en **todas** las superficies | Operaciones restringidas a un rol (RBAC por aplicación, operaciones solo para administradores)          |

Estos son independientes: `agentTool` controla la vista del modelo, `toolCallable` controla solo el iframe de extensión, `publicAgent` agrega una superficie pública de participación voluntaria (las rutas web públicas nunca implican exposición de herramientas públicas), `needsApproval` controla la ejecución después de realizar la llamada (consulte [Human-in-the-loop approval](#needs-approval) a continuación) y `authorize` decide si quien llama puede ejecutar la acción, en cada ruta de despacho, antes de entrar en `run`; consulte [Authorization](#authorize).
...
`accessFilter` / `assertAccess` delimitan _qué filas_ puede tocar quien llama con permiso. `authorize` responde a la pregunta anterior: **¿puede quien llama ejecutar esta operación siquiera?** Dele una función de guarda y se ejecuta antes de `run()` para todos los llamadores: herramienta de agente, HTTP, frontend, MCP, A2A y CLI:
...
La guarda envuelve el propio `run` en lugar de colgar de la entrada de la acción, porque `run` es lo único por lo que pasan los seis sitios de despacho: no hay ninguna persona que llame que pueda alcanzar el cuerpo sin superar la comprobación. Una guarda que lanza deniega con su propio mensaje; una que devuelve `false` deniega con un 403 genérico; cualquier otra cosa, incluido `undefined`, permite. Como la barrera necesita una identidad de quien llama, una invocación desatendida desde CLI o cron sin correo electrónico de usuario se deniega en lugar de dejarse pasar.

Las envolturas se componen en este orden:

```text
validate input -> authorize -> run -> validate output -> audit
```

La autorización se sitúa _dentro_ de la validación de entrada: así, una guarda que inspecciona `args` recibe el valor ya analizado y convertido que promete su tipo, y no la forma que envió quien llama. La auditoría es la capa más externa, de modo que una denegación también queda registrada.

`requireAny` falla en tiempo de definición, no en tiempo de solicitud: llamarlo sin roles, o con un rol ajeno al vocabulario declarado, lanza al cargar el módulo. `authorize: coachAccess.requireAny()` es un argumento olvidado, y debe romperse donde está el error en lugar de aparecer después como un 403 inexplicable.

Los tres mecanismos no son intercambiables:

| Mecanismo                     | Pregunta que responde                                    | Se aplica a                        |
| ----------------------------- | -------------------------------------------------------- | ---------------------------------- |
| `authorize`                   | ¿Puede quien llama realizar la operación?                | Los seis sitios de despacho        |
| `needsApproval`               | ¿Debe un humano aprobar esta llamada permitida concreta? | Solo el bucle del agente           |
| `accessFilter`/`assertAccess` | ¿Qué filas puede ver o escribir quien llama con permiso? | Dentro de `run`, por consulta/fila |

Una action de escritura restringida suele necesitar tanto `authorize` como `assertAccess`: el primero mantiene fuera de la operación a quienes no son coaches, el segundo impide que un coach toque el plan de otra organización. `needsApproval` es otro eje distinto: nunca restringe _quién_ puede llamar, solo si el agente puede continuar sin supervisión.

`coachAccess` arriba proviene de `defineAppRoles`, la capa de roles por aplicación sobre la membresía de la organización. Consulte [Organizations, Teams & Permissions → App roles](/docs/organizations-teams-permissions#app-roles-per-app-rbac) para conocer el modelo de roles y sus garantías.
