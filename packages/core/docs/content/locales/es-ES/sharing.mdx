...
<DataModel id="doc-block-tb24uz" title="Tabla de recursos compartidos + complementos" summary={"Coarse visibility lives on the resource; each fine-grained grant is a row in the shares table."} entities={[
  {
    "id": "deck",
    "name": "decks",
    "note": "...ownableColumns()",
