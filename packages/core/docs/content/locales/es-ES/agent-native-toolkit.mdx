...
## Catálogo equilibrado {#balanced-catalog}

El catálogo también cubre fundamentos, editores y lienzos, contexto y conocimiento del agente, colaboración y gobernanza, marco de espacio de trabajo, operaciones y paquetes de capacidad. El editor enriquecido pertenece a Toolkit; Core mantiene reexportaciones de compatibilidad. Los bloques siguen en Core. Design es un patrón de plantilla con `design-tweaks`, no un editor completo instalable. El motor Context X-Ray es de Core y su presentación reutilizable es de Toolkit. Scheduling y Creative Context son paquetes de capacidad separados; consulte [Ciclo de vida de paquetes](/docs/package-lifecycle).
