...
El agente nativo no es solo para desarrolladores. El agente integrado ofrece a todo el equipo una forma compartida de operar la aplicación, mientras los cambios de código fuente permanecen en el flujo de desarrollo normal. Solo puede editar el código fuente si el host concede intencionalmente herramientas de escritura de repository/workspace:
...
## Empezar con una plantilla y personalizar {#fork-and-customize}

Las aplicaciones nativas del agente siguen un modelo de empezar con una app funcional. Comienza desde una **plantilla** (calendario, contenido, diapositivas, análisis, correo, clips, diseño, formularios, envío) y la hace suya. Cada uno es un producto SaaS completo y funcional, no un andamio en blanco:
...
3. Cree su propia app desde la plantilla cuando desee personalizar: "conectar nuestra cuenta de Stripe", "agregar un gráfico de cohortes"
4. Personaliza el código fuente en tu repositorio y flujo de desarrollo
5. Implemente su app en su propio dominio o permanezca en agent-native.com

Debido a que es _su_ aplicación, no infraestructura compartida, eres dueño del código fuente y puedes evolucionarlo sin cambiar el framework compartido. El agente integrado trabaja mediante actions y datos, y solo puede ayudar con el código fuente si se le conceden intencionalmente herramientas de escritura. Consulte [Templates](/docs/cloneable-saas) para conocer la historia completa.

¿No estás listo para empezar con una plantilla completa? También puede probar el agente nativo agregando una **habilidad** a un agente de codificación que ya usa: instale la habilidad Planes con `npx @agent-native/core@latest skills add visual-plan`. Ver el [Skills Guide](/docs/skills-guide#app-backed-skills).
