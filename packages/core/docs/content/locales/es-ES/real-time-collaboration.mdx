...
**La reescritura simultánea en la misma región es la última escritura gana.** Si el agente reescribe un pasaje mientras un humano tiene ediciones no guardadas en la *exactamente la misma región*, la instantánea del cliente principal puede afectar la edición humana en vuelo. Las ediciones en diferentes regiones siempre se fusionan limpiamente a través de CRDT. Para documentos estructurados, utilice la combinación granular del lado del servidor para evitar esto por completo.
