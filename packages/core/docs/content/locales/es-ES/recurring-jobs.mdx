...
<AnnotatedCode
  ...
  annotations={[
    ...
    },
    {
      lines: "7-12",
      label: "el aviso",
      note: "El cuerpo es solo un mensaje: el agente lo ejecuta en cada disparo con todas sus herramientas normales y el contexto del espacio de trabajo.",
    },
  ]}
/>
...
Los trabajos recurrentes pueden usar herramientas MCP remotas o locales que ya estén conectadas a la aplicación. Pide al agente que descubra los nombres exactos con `tool-search` y vincula solo las herramientas necesarias mediante `mcpTools`:

```md
---
schedule: "0 * * * *"
enabled: true
mcpTools: ["mcp__meeting-notes__list_recordings", "mcp__meeting-notes__get_transcript"]
---

Busca grabaciones nuevas desde la última ejecución correcta. Extrae solo los
seguimientos accionables y llama una vez a la acción de importación limitada de
la aplicación con identificadores de origen y externos estables, una cita
breve y la URL de la grabación. No importes la transcripción completa.
```

El trabajo guarda solo nombres de herramientas, no URL de MCP, tokens de acceso ni tokens de actualización OAuth. En tiempo de ejecución, el marco resuelve la concesión conectada en el contexto de solicitud del creador del trabajo o de la organización y expone únicamente las herramientas permitidas. Mantén la lista pequeña y usa una acción de aplicación idempotente para la importación final, de modo que las ejecuciones superpuestas no creen duplicados. Consulta [MCP Clients](/docs/mcp-clients) para la configuración de conexión y OAuth.
