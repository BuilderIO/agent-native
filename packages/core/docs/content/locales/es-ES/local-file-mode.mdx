...
archivos. Esto es apropiado para el desarrollo local y Agent Native Desktop,
pero no es el modelo de seguridad de producción predeterminado.

El marco rechaza el modo `local-files` en entornos conocidos de alojamiento,
serverless o con bases de datos remotas. Esta comprobación no se puede omitir
con `AGENT_NATIVE_ALLOW_LOCAL_FILES_IN_PRODUCTION`; una variable de entorno no
demuestra el aislamiento de un solo inquilino. El modo de producción se limita
a procesos secundarios de Agent Native Desktop. El desarrollo local, incluido
SQLite local, sigue funcionando sin una variable. Para aplicaciones alojadas o
multiusuario, use el modo de base de datos y el uso compartido respaldado por SQL.
