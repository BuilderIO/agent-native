...
Sí. Si desea una app agentic básica, ejecute `npx @agent-native/core@latest create my-app --template base`; obtiene actions, autenticación, estado de ejecución respaldado por SQL y espacio para agregar sus propias pantallas. Si crea una automatización sin navegador, ejecute `npx @agent-native/core@latest create my-agent --headless`. Consulte [Getting Started](/docs/getting-started) y [Automation-First Apps](/docs/pure-agent-apps). Use `--template chat` solo cuando quiera el producto Chat.
