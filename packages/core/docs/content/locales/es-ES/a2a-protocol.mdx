...
Foundry Agent Consumer. Use la tarjeta y el punto de conexión v0.3 del proveedor
con `cardUrl` y `protocolVersion` cuando el punto de conexión hable JSON-RPC de
Agent-Native. El proveedor de tokens debe obtener y renovar un token de acceso
de Entra. `A2A_SECRET` y una clave API estática no satisfacen la autenticación de
Foundry.
...
y Agent Registry puede publicar un punto de conexión proxy. Su enlace HTTP+JSON
usa rutas específicas del proveedor: el descubrimiento usa `GET {url}/v1/card`,
la invocación usa `POST {url}/v1/message:send` y la transmisión usa
`POST {url}/v1/message:stream`. Las solicitudes usan tokens de portador de
Google OAuth o ADC y permisos de Google Cloud IAM. Consulte [Llamar a un agente mediante su punto de conexión A2A de registro](https://docs.cloud.google.com/gemini/enterprise/docs/invoke-agent-a2a)
para conocer los requisitos del punto de conexión y los permisos.

Estas rutas son distintas de las rutas JSON-RPC de Agent-Native
(`/.well-known/agent-card.json` y `/_agent-native/a2a`). Registre el punto de
conexión solo después de añadir un adaptador que traduzca el formato del cable y
obtenga y renueve las credenciales de Google. Si el destino expone por separado
un punto de conexión A2A estándar, úselo con la guía de portador genérico. El
tráfico de registro de Gemini Enterprise no pasa por las políticas de Agent
Gateway, así que aplique la política de autenticación y autorización propia del
destino.
