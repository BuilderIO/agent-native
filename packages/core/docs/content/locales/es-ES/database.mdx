...
<Diagram id="doc-block-efvglx" title="Un esquema, muchos backends" summary={"El código de la aplicación utiliza los ayudantes independientes del dialecto del marco. El dialecto se detecta automáticamente desde DATABASE_URL en tiempo de ejecución; unset significa un archivo SQLite local."}>
...
<DataModel
  id="doc-block-pe44ma"
  title="la tabla de tareas"
  summary={
    "Definida una sola vez con los helpers del framework; el dialecto se elige en tiempo de ejecución a partir de DATABASE_URL."
  }
  entities={[
    {
      id: "tasks",
...
<AnnotatedCode
  id="doc-block-3q7984"
  title={"Un complemento de migración aditiva"}
  ...
  annotations={[
    {
      lines: "6-7",
      label: "Sólo aditivo",
      note: "Es seguro volver a ejecutar `ADD COLUMN IF NOT EXISTS` y nunca pierde datos. Los cambios de nombre se ven como soltar+crear a Drizzle, así que agregue y luego migre.",
    },
    {
      lines: "13-16",
      label: "Puerta de dialecto",
      note: "Pase un objeto indexado por dialecto para ejecutar SQL diferente por backend. Haga que la otra clave sea un no-op (`SELECT 1`) para funciones exclusivas de Postgres o de SQLite.",
    },
    {
      lines: "19",
      label: "Tabla de versiones por aplicación",
      note: "Cada aplicación rastrea sus propias versiones aplicadas, por lo que las migraciones son idempotentes entre reinicios e instancias.",
    },
  ]}
/>
