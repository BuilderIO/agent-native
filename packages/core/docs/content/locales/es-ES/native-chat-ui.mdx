...
### Elegir cuándo aparece la interfaz de una acción {#conditional-native-ui}

Una acción puede admitir varias operaciones, aunque solo algunos resultados necesiten una tarjeta. Añade `when(args, result)` a `chatUI` para seleccionar esas llamadas. El servidor ejecuta este predicado después de una acción correcta. Si devuelve `false`, el resultado permanece en el historial normal y plegable de herramientas. El predicado no se guarda ni se envía al navegador.

Mail usa este patrón en `manage-draft` para mostrar una tarjeta al crear un borrador y dejar las actualizaciones y eliminaciones como llamadas normales:
...
En acciones con varias operaciones de escritura, comprueba los argumentos y el resultado correcto. La acción de filtros de Gmail de Mail solo selecciona la tarjeta de regla para filtros creados o sustituidos que tengan los campos de resumen:
