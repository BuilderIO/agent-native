...
| **Revisión humana**     | Revisar la solicitud y la respuesta, registrar comentarios y redactar una actualización de instrucciones              |

### Detalles de las conversaciones {#conversations}

La pestaña Conversaciones muestra las ejecuciones registradas. Selecciona una para inspeccionar sus tramos y expande cualquiera para ver las entradas, salidas, errores y metadatos de herramientas capturados. **Abrir conversación completa** te lleva al hilo de chat correspondiente.

Los argumentos y resultados de herramientas no se capturan de forma predeterminada. Activa `captureToolArgs` y `captureToolResults` en la configuración `observability` para almacenarlos. Los campos sensibles y los mensajes de error de herramientas se redactan antes de guardarlos.

### Revisión humana {#review}

La pestaña Revisión humana muestra la solicitud y la respuesta de cada ejecución, permite registrar comentarios y guardar un borrador de actualización de instrucciones para que lo revise una persona. Guardar el borrador no cambia las instrucciones automáticamente.
