...
Los agentes externos obtienen las herramientas reales de creación/actualización de la aplicación —las mismas acciones con nombre que llama el agente interno—, así que cuando un usuario pide contenido nuevo (un diseño, una presentación, un formulario, un documento), redáctalo directamente y guárdalo con esas herramientas en lugar de delegar el trabajo mediante `ask_app`. Las acciones marcadas con `endsTurn: true`, como un formulario de preguntas dentro de la app que espera la respuesta del usuario en el chat de esa misma app, se ocultan de MCP, WebMCP y A2A de forma predeterminada; son la herramienta del agente interno, no la de un llamador externo, a menos que la acción vuelva a activarse explícitamente con `mcpTool: true`. El puente WebMCP acepta entradas y resultados de herramientas de hasta 500.000 caracteres, de modo que una pantalla, presentación o documento redactado completo cabe en una sola escritura sin necesidad de dividirlo. Las llamadas WebMCP locales a la página también incluyen el id de la pestaña del navegador, de modo que el contexto propio de cada pestaña (navegación, selección) se resuelve a la pestaña que el usuario está viendo realmente.
