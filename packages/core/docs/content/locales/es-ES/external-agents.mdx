...
En la aplicación, abra **Settings → Connections → Integrations** y busque el nombre del host
que ya conoce: Claude, Claude Cowork, Claude Code, Anthropic, ChatGPT, OpenAI,
Codex, Grok o xAI. El único resultado de host externo abre **Settings → MCP server**
...
La configuración incluye una página **MCP server** en el grupo de la
aplicación. Muestra la URL MCP canónica de la aplicación, una tarjeta de
agente A2A si está disponible y las mismas guías de configuración que la página
de conexión completa. Use **Settings → MCP server** como punto de partida o abra
