...
La autenticación identifica la cuenta conectada; no amplía sus permisos sobre documentos. Un documento público sigue disponible desde su enlace. Un documento privado solo se puede leer mediante MCP si esa cuenta ya tiene acceso. Si `get-document` devuelve una denegación después de conectar, el propietario debe dar acceso a esa cuenta.

Reinstalar el conector o cambiar el enlace público compartido no resuelve una denegación de permisos de la cuenta conectada.
