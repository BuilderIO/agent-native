...
- **Capacidades vinculadas a la versión** — Una extensión declara acciones, rutas y métodos HTTP, acceso SQL y orígenes HTTPS externos exactos. Cada persona acepta explícitamente un subconjunto; cualquier cambio del manifiesto invalida la concesión anterior.
- **Modo heredado cerrado por defecto** — Sin un manifiesto actual aceptado, la extensión solo puede llamar acciones de lectura y leer el estado de la aplicación y sus propios datos. SQL, escrituras y salida externa permanecen desactivados.
- **Aplicación en el servidor** — Las rutas de acciones, SQL, estado, datos y proxy comprueban la concesión de forma independiente. La revocación se aplica en la siguiente solicitud.
