...
> Para una personalización más profunda, pide al agente que edite el código
> fuente de la plantilla. La personalización completa del código es una
> capacidad central de Agent Native; las extensiones son la capa rápida y sin
> despliegues.
...
## ¿Debo usar una extensión o editar el código de la aplicación? {#vs-app-code}

El marco permite al agente editar directamente el código fuente de la aplicación —componentes, rutas, estilos y lógica de negocio—. Usa esta pregunta cuando cualquiera de las dos opciones podría resolver la solicitud:

- **Empieza con una extensión** para un widget autónomo y relativamente puntual: una visualización o interacción personalizada, un bloque de dashboard, una utilidad independiente o un complemento en un slot existente. Es rápida, no requiere cambios en el código fuente, compilación ni despliegue, y puede ser privada o compartida.
- **Usa el código de la aplicación** para una función principal de la plantilla, cambios en la UI nativa, el layout, las rutas o la lógica de negocio, o cualquier cosa que deba reutilizarse ampliamente en dashboards o usuarios. Si el host no tiene un slot adecuado o la solicitud necesita uno nuevo, usa código.
- **Promueve las extensiones repetidas.** Si se vuelve a crear la misma extensión una y otra vez, o mantener muchas copias sería peor que una implementación compartida, pásala al código de la aplicación como una función nativa reutilizable.
...
Regla general: **si una extensión puede resolver el problema, normalmente empieza por ahí.** Usa código cuando el límite de las extensiones no encaje o cuando la reutilización amplia y la disponibilidad para todo el producto justifiquen una implementación nativa. Por ejemplo, una visualización personalizada puntual en un dashboard es una buena extensión; un nuevo tipo de gráfico circular, de barras múltiples o de líneas que deba estar disponible en todos los dashboards pertenece al código de la plantilla.
