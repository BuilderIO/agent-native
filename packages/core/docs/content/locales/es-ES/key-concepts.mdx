  ...
  <div class="diagram-row">
    <div class="diagram-card">
      ...
      ><small class="diagram-muted"
        >lee y escribe datos, ejecuta acciones, modifica código</small
      >
    </div>
    <div class="diagram-card">
...
(`databaseTools: "read"`), por lo que los agentes inspeccionan datos de la app con `db-schema` /
`db-query` y escriben mediante actions tipadas de la app. Establece
`databaseTools: "write"` (o `true`) solo para superficies de mantenimiento que
también deban exponer `db-exec` / `db-patch` con alcance; usa
`databaseTools: "off"` / `false` para requerir actions tipadas para todos los datos
acceso.
...
1. Empezar con una plantilla (por ejemplo, la plantilla de análisis)
