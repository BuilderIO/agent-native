...
Cada aplicación se crea con `APP_BASE_PATH=/<name>` y `VITE_APP_BASE_PATH=/<name>` y se emite a través del ajuste preestablecido de Nitro seleccionado. **Netlify** es el ajuste preestablecido predeterminado para el comando unificado `deploy`; emite funciones de aplicación bajo `.netlify/functions-internal/<app>-server` y genera redirecciones que dejan los activos estáticos sin forzar para que CDN entregue los archivos primero. **Vercel** es compatible con `npx @agent-native/core@latest deploy --preset vercel`; escribe un paquete raíz `.vercel/output` utilizando la Build Output API de Vercel.
...
¿Prefieres cada aplicación en su propio dominio (`mail.company.com`, `calendar.company.com`)? Cada aplicación en el espacio de trabajo sigue siendo implementable de forma independiente: `cd apps/mail && pnpm build` se comporta exactamente como un andamio independiente. Luego, el A2A entre aplicaciones pasa por la ruta estándar firmada por JWT con un `A2A_SECRET` compartido. El SSO de dominio cruzado entre aplicaciones implementadas por separado se maneja mediante la federación de identidades con Dispatch como centro; consulte [Cross-App SSO](/docs/cross-app-sso); la implementación unificada de origen único evita su necesidad.

### Base de datos compartida, credenciales compartidas

Para compartir cuentas de usuario, organizaciones, configuraciones y otros datos entre aplicaciones, apunta todas las aplicaciones al mismo `DATABASE_URL`. Durante el desarrollo local, configúralo en el `.env` raíz del espacio de trabajo para que todas las aplicaciones usen la misma base de datos PostgreSQL. Si no está definido, cada aplicación usa su propia base de datos PGlite local al proceso, por lo que los usuarios y las organizaciones no se comparten.

La demanda de conexiones se suma entre los procesos que comparten una base de datos. Si cinco procesos de aplicaciones Node.js de larga duración usan cada uno un pool compartido para el mismo `DATABASE_URL`, pueden mantener hasta 100 conexiones con el límite predeterminado de 20 por pool (`5 × 1 pool × 20`). En espacios de trabajo sin servidor, calcule las instancias de función activas por aplicación usando los límites predeterminados específicos del entorno. Consulte [Base de datos del servidor](/docs/server-database) para ver `AGENT_NATIVE_DB_POOL_MAX` y las pautas completas de cálculo.
