...
变化：**它是 SQL 行，而不是文件系统文件。**每个用户都将自己的一套代理资源存储在数据库中。没有要启动的开发盒，没有每个用户的容器，没有要安装的文件。多租户 SaaS 可以为每个用户提供基本上免费的完全可定制的代理，因为所有这些都是行——个人内存、个人 MCP 服务器、个人 skills、个人子代理——并且共享代码库同时托管所有这些。

<Diagram id="doc-block-1729nmr" title={"Claude-Code 设置，但作为代理资源存储在 SQL 中"} summary={"相同的定制层——指令、技能、内存、代理、作业、MCP——除了每个文件都是共享多租户数据库中的一行。"}>
