...
在应用中打开 **Settings → Connections → Integrations**，然后搜索您熟悉的主机名称：
Claude、Claude Cowork、Claude Code、Anthropic、ChatGPT、OpenAI、Codex、Grok
或 xAI。唯一的外部主机结果会打开 **Settings → MCP server**，并选中最匹配的指南。
...
Settings 在应用分组中提供 **MCP server** 页面。它显示应用的规范
MCP URL、可用时的 A2A 代理卡片，以及与完整连接页面相同的设置指南。使用
**Settings → MCP server** 作为应用内起点，或直接打开 `/mcp/connect` 查看完整流程和令牌回退。
