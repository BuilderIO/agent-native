...
外部代理可以使用应用真正的创建/更新工具——与应用内代理调用的是同一批命名操作——因此当用户要求新内容(设计、幻灯片、表单、文档)时,应直接撰写并用这些工具保存,而不是把工作委托给 `ask_app`。标记为 `endsTurn: true` 的操作(例如在该应用自身聊天中等待用户回答的应用内提问表单)默认会对 MCP、WebMCP 和 A2A 隐藏;它们是应用内代理的工具,不是外部调用者的工具,除非该操作显式地用 `mcpTool: true` 重新启用。WebMCP 桥接接受最多 500,000 个字符的工具输入和结果,因此整份手写的界面、幻灯片或文档可以一次写入,无需拆分。页面本地的 WebMCP 调用也会携带浏览器标签页 ID,因此按标签页划分的上下文(导航、选择)会解析到用户实际正在查看的那个标签页。
