...
<DataModel id="doc-block-1fre735" title={"工作区资源模型"} summary={"一种资源形状支持每个工作区文件。运行时按路径和范围对其进行键控，并解析读取时的有效值。"} entities={[
  {
    "id": "resource",
...
<AnnotatedCode id="doc-block-z5fs16" title={"自定义代理配置文件"} filename="agents/design.md" language="markdown" code={"---\nname: Design\ndescription: >-\n  Reviews layouts, interaction patterns, and product UX decisions.\nmodel: inherit\ntools: inherit\ndelegate-default: false\n---\n\n# 角色\n\n您是一名专注的设计代理。\n\n##职责\n\n-审查布局和交互流程\n-提出更强的视觉方向\n-简洁且有主见"} annotations={[
  {
    "lines": "2",
