...
在将应用推广到生产环境之前，在部署提供程序中设置 `DATABASE_URL`。 Agent-native 使用 Drizzle 进行架构和查询，因此数据层可跨 Drizzle 兼容的 SQL 后端移植，并且框架会自动检测 URL 的方言。有关适配器列表和方言详细信息，请参阅 [Database](/docs/server-database#production)。
...
数据库连接变量（`DATABASE_URL`、`DATABASE_AUTH_TOKEN`、每个应用程序 `<APP_NAME>_DATABASE_URL`）位于 [Database](/docs/server-database#production) 中。
...
- [**Database**](/docs/server-database#production) — 在首次生产部署前选择持久化 SQL 后端
