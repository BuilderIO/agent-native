...
| `A2A_SECRET`             | 应用间 A2A JSON-RPC 的共享 HMAC。未设置 `SECRETS_ENCRYPTION_KEY` 时，共享保管库使用由此值按用途派生的密钥，因此轮换它也会轮换保管库密钥。                                                                                              |
| `SECRETS_ENCRYPTION_KEY` | 共享保管库首选的稳定 AES-256-GCM 密钥。每个读取保管库的应用都应设置相同值。读取时会回退到派生的 `A2A_SECRET`，再回退到旧版 Better Auth 和应用范围密钥。轮换 A2A 前，请先在各处添加此密钥，并在旧 A2A 材料仍可用时读取/迁移所有现有行。 |
