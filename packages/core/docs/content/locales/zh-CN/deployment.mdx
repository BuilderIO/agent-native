...
Worker 没有文件系统，因此数据库要么是外部的 `DATABASE_URL`，要么是
Cloudflare D1。在构建环境中同时设置 `CLOUDFLARE_D1_DATABASE_NAME` 和
`CLOUDFLARE_D1_DATABASE_ID`，构建就会把 `DB` 绑定写入生成的
`wrangler.json`；只设置其中一个会报错，而不是静默省略绑定。完整的本地启动
步骤见英文部署指南。


### Browser Rendering {#cloudflare-browser-rendering}

Worker 无法启动 Chromium 进程，因此浏览器以绑定的形式提供，生成的
`wrangler.json` 会声明 `{ "browser": { "binding": "BROWSER" } }`。依赖浏览器
的 action 通过 `@cloudflare/playwright` 解析该绑定。绑定缺失时会报出绑定名称，
而不是声称缺少 Chromium 可执行文件。本地使用 `wrangler dev` 同样可用，但有一个
限制：本地运行时会拒绝大于 1 MB 的请求。

### Cloudflare R2 {#cloudflare-r2}

Worker 没有文件系统，文件内容也不能存进 SQL，因此上传的文件会写入 R2。在构建
环境中设置 `CLOUDFLARE_R2_BUCKET_NAME`，构建就会把 `UPLOADS` 绑定写入生成的
`wrangler.json`；再把 `CLOUDFLARE_R2_PUBLIC_BASE_URL` 设为存储桶的公开地址。
该提供方已注册到上传注册表中。绑定缺失或不可用时会带着配置说明直接失败，而不是
把负载写进 SQL。详情见英文部署指南。
...
- `extensionTools: true` — 为应用启用框架扩展管理 actions 和提示指导
...
[Builder.io](https://www.builder.io?utm_source=agent-native&utm_medium=docs&utm_campaign=framework&utm_content=deployment) 是一个独立的托管代码工作流，代理可在其中修改生产应用的 UI。连接仓库后即可直接请求 UI 更改，无需重新部署。
