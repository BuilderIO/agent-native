---
title: "FAQ"
description: "有关 Agent-Native 的常见问题：它是什么、适合谁、可以构建什么以及如何工作。"
---

# FAQ

有关 Agent-Native 的常见问题，从“我只是在了解”到“我正在配置身份验证”。

## 基础知识 {#general}

### 什么是 Agent-Native？ {#what-is-agent-native}

Agent-Native 是一个用于构建应用的框架，其中 AI 智能体及其周围的产品界面是平等的伙伴。通常的路径是从聊天开始，添加类型化动作，在聊天中渲染结构化结果，然后围绕同一份 SQL 状态扩展成持久页面。不变的是智能体和人类共享相同的动作、数据库和状态。完整解释请参阅[什么是 Agent-Native？](/docs/what-is-agent-native)。

### 这是给谁的？ {#who-is-this-for}

Agent-Native 适合希望真正的应用和 AI 智能体使用相同数据和动作工作的人。常见路径是：
...
- **连接另一个智能体或编码工具**：如果你希望 Claude、ChatGPT、Codex、Cursor 或 GitHub Copilot / VS Code 使用 Agent-Native 应用，请参阅 [External Agents](/docs/external-agents) 和 [Skills Guide](/docs/skills-guide)。

### 这与向现有应用添加 AI 有何不同？ {#how-is-this-different}

大多数应用会把 AI 作为事后补充，它实际上无法在应用中*做*事情。在 Agent-Native 应用中，智能体是一等公民，与 UI 共享相同的动作、数据库和状态，因此它可以执行按钮能执行的任何操作。参阅[为什么要以这种方式构建应用](/docs/what-is-agent-native#why-build-apps-this-way)。

<Diagram id="doc-block-149omhf" title={"后加的 AI 与 Agent-Native"} summary={"后加的聊天侧边栏有自己的世界。Agent-Native 智能体与 UI 共享相同的动作、数据库和状态。"}>
...
是的。选择运行 Node 的任何主机 — Netlify、Vercel、Cloudflare、AWS、Deno Deploy、您自己的服务器 — 以及任何 SQL 数据库（Postgres、SQLite、Turso、D1）。该框架是为了可移植而构建的。先从[本地 Docker 快速入门](https://www.agent-native.com/docs/deployment#self-host-quickstart)开始，然后参阅[Deployment](/docs/deployment)进行生产部署。
...
这就是重点。从模板开始，在自己的代码库和开发流程中进行定制。嵌入式代理随后通过 actions 和数据操作生成的应用程序。
...
### 嵌入式代理会编辑应用程序源代码吗？ {#can-the-agent-modify-code}

默认不会。只有当主机有意授予 repository/workspace 写入工具时，它才能编辑源代码。否则，嵌入式代理通过 actions、SQL 支持的状态和已配置的集成工作。要修改组件、路由、样式或 actions，请使用代码库的正常开发流程，或使用单独的代码能力开发/Builder frame。
