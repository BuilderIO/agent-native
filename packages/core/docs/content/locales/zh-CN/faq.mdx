...
是的。选择运行 Node 的任何主机 — Netlify、Vercel、Cloudflare、AWS、Deno Deploy、您自己的服务器 — 以及任何 SQL 数据库（Postgres、SQLite、Turso、D1）。该框架是为了可移植而构建的。先从[本地 Docker 快速入门](/docs/deployment#self-host-quickstart)开始，然后参阅[Deployment](/docs/deployment)进行生产部署。
