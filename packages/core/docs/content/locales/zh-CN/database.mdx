...
<DataModel id="doc-block-1m0kgyx" title={"任务表"} summary={"Defined once with the framework helpers; the dialect is chosen at runtime from DATABASE_URL."} entities={[
  {
    "id": "tasks",
    "name": "tasks",
