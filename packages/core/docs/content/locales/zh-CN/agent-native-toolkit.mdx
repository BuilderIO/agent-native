...
## 均衡目录 {#balanced-catalog}

目录还覆盖基础、编辑器与画布、智能体上下文与知识、协作与治理、工作区外壳、运营以及能力包。富文本编辑器归 Toolkit；Core 保留兼容性重导出。Blocks 在本阶段仍归 Core。Design 是带有 `design-tweaks` 的模板模式，而不是完整的即插即用编辑器。Context X-Ray 引擎归 Core，可复用呈现归 Toolkit。Scheduling 与 Creative Context 是独立能力包；请参阅[包生命周期](/docs/package-lifecycle)。
