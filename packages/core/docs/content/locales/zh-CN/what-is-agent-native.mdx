...
Agent-native 不仅仅适合开发人员。嵌入式代理为整个团队提供操作应用程序的共同方式，而源代码更改仍属于正常开发流程。只有当主机有意授予 repository/workspace 写入工具时，它才能编辑源代码：
...
## 从模板开始并进行定制 {#fork-and-customize}

代理本机应用程序遵循从可运行 app 开始的模型。您从**模板**开始 - 日历、内容、幻灯片、分析、邮件、剪辑、设计、表单、调度 - 并将其变成您的。每一个都是一个完整的、可工作的 SaaS 产品，而不是一个空白的脚手架：

1. 在[agent-native.com/templates](/templates)上选择模板
2. 立即将其用作托管应用程序（例如 mail.agent-native.com）
3. 当您想要自定义时，从模板创建您自己的 app - “连接我们的 Stripe 帐户”、“添加同类图表”
4. 在自己的代码库和开发流程中定制源代码
5. 将您的 app 部署到您自己的域 - 或留在agent-native.com

因为它是*您的*应用程序，而不是共享基础设施，所以您拥有源代码，并可在不更改共享 framework 的情况下演进它。嵌入式代理通过 actions 和数据工作，只有在有意授予写入工具时才能协助源代码更改。完整故事请参见 [Templates](/docs/cloneable-saas)。

尚未准备好从整个模板开始？您还可以通过向已使用的编码代理添加**技能**来尝试代理原生 - 使用 `npx @agent-native/core@latest skills add visual-plan` 安装计划技能。请参阅 [Skills Guide](/docs/skills-guide#app-backed-skills)。
