...
<Endpoint id="doc-block-1mrk9b2" title={"MCP端点"} summary={"每个代理本机应用程序都会公开自动安装的 Streamable HTTP 端点。"} method="POST" path="/mcp" summary={"MCP 可流式 HTTP 端点"} auth="Standard remote MCP OAuth (Bearer access token), connect-minted JWT, or static ACCESS_TOKEN/ACCESS_TOKENS" params={[
  {
    "name": "Authorization",
    "in": "header",
    "type": "string",
...
托管 MCP 请求使用有限时长的内联等待，这样一次较长的代理循环就不会让某个 `tools/call` 一直保持打开状态，直到客户端或网关超时。可选参数 `async` 会启动持久任务并立即返回；如果不传该参数，服务器会短暂等待以争取快速完成，若仍有工作未完成，则返回一个任务句柄。托管等待的上限为 25 秒。
...
代理运行与交互式聊天相同的循环 - 它可以调用多个工具、推理上下文并生成深思熟虑的响应。尚未完成的任务会返回包含 `taskId`、`status`、`pollAfterMs` 的 JSON，以及：
...
| `/mcp/oauth/register`                     | 动态公共客户端注册        |
| `/mcp/oauth/authorize`                    | 浏览器授权+同意           |
| `/mcp/oauth/token`                        | 授权代码和刷新令牌授予    |
