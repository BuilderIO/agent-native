...
每个应用都使用 `APP_BASE_PATH=/<name>` 和 `VITE_APP_BASE_PATH=/<name>` 构建，并通过选定的 Nitro 预设输出。统一 `deploy` 命令的默认预设是 **Netlify**；它会在 `.netlify/functions-internal/<app>-server` 下输出应用函数，并生成重定向，使静态资源不被强制处理，从而让 CDN 优先提供文件。**Vercel** 可通过 `npx @agent-native/core@latest deploy --preset vercel` 使用；它使用 Vercel 的 Build Output API 写入根级 `.vercel/output` 包。
...
更喜欢每个应用程序在自己的域中（`mail.company.com`、`calendar.company.com`）？工作区中的每个应用程序仍然是独立可部署的 - `cd apps/mail && pnpm build` 的行为与独立脚手架完全相同。然后，跨应用程序 A2A 通过具有共享 `A2A_SECRET` 的标准 JWT 签名路径。单独部署的应用程序之间的跨域 SSO 由以 Dispatch 作为中心的身份联合处理 - 请参阅 [Cross-App SSO](/docs/cross-app-sso)；统一的单源部署避免了需要它。

### 共享数据库、共享凭证

如需共享用户帐户、组织、设置及其他跨应用数据，请让每个应用使用相同的 `DATABASE_URL`。本地开发时，请在工作区根目录的 `.env` 中设置该值，让所有应用使用同一个 PostgreSQL 数据库。如果未设置，每个应用都会使用各自的进程本地 PGlite 数据库，因此用户和组织不会共享。

共享同一数据库的进程会累加连接需求。例如，五个长期运行的 Node.js 应用进程各自为同一个 `DATABASE_URL` 使用一个共享运行时连接池时，按每个池默认 20 个连接计算，最多可占用 100 个连接（`5 × 1 个池 × 20`）。对于无服务器工作区，请根据运行时对应的连接池默认值估算每个应用的预热函数实例数。`AGENT_NATIVE_DB_POOL_MAX` 和完整的预算说明请参阅[服务器数据库](/docs/server-database)。
