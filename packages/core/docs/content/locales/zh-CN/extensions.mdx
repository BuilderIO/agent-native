...
## 我应该使用扩展还是编辑应用代码？ {#vs-app-code}

该框架允许代理直接编辑应用程序的源代码——组件、路由、样式和业务逻辑。当两种方式都可能解决请求时，请使用以下问题来判断：

- **从扩展开始**：适用于独立且相对一次性的组件，例如自定义可视化或交互、仪表板卡片、独立工具，或添加到现有插槽中的功能。它创建快速，不需要修改源代码、构建或部署，并且可以设为私有或共享。
- **使用应用代码**：适用于模板的核心功能，或对原生 UI、布局、路由、业务逻辑的修改，也适用于需要在多个仪表板或用户之间广泛复用的功能。如果宿主没有合适的插槽，或请求需要新增插槽，请使用代码。
- **将重复的扩展升级为代码**：如果同一个扩展不断被重新创建，或维护多个副本不如维护一个共享实现，请将其移入应用代码，作为可复用的原生功能。
...
经验法则：**如果扩展可以解决问题，通常先从扩展开始。**当扩展的边界不适用，或广泛复用和全产品可用性需要原生实现时，再使用代码。例如，仪表板上的一次性自定义可视化适合做成扩展；而所有仪表板都应可用的新饼图、多条形图或折线图类型，则应放在模板代码中。
