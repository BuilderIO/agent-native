...
- **与版本绑定的能力** — 扩展必须声明准确的 action、框架路径和 HTTP 方法、SQL 访问以及外部 HTTPS 来源。每位用户显式接受其中一部分；manifest 一旦变化，旧授权立即失效。
- **旧扩展默认关闭高权限** — 未接受当前 manifest 时，扩展只能调用只读 action，以及读取应用状态和自身数据。SQL、写入和外部访问保持关闭。
- **服务端强制执行** — action、SQL、应用状态、数据和代理路由会独立检查授权。撤销从下一次请求起生效。
