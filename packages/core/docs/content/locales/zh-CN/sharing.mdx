...
<DataModel id="doc-block-1bixamr" title={"资源+同伴分享表"} summary={"Coarse visibility lives on the resource; each fine-grained grant is a row in the shares table."} entities={[
  {
    "id": "deck",
    "name": "decks",
    "note": "...ownableColumns()",
