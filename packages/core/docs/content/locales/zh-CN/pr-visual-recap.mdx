...
JSON 引号是必需的。轻量级 `gate` 作业和完整的 `recap` 作业都使用所选运行器。若要在引导设置期间进行配置，请将相同的 JSON 值传给 `--runs-on`，然后使用 `doctor` 验证工作流程和运行器可用性：
...
适用 [Secrets and variables](#secrets-and-variables) 中描述的相同秘密和变量 - 在存储库设置中以与复制变体相同的方式设置它们。可复用调用方通过 `runs-on` 输入转发 `VISUAL_RECAP_RUNS_ON`；被调用的工作流程会解析 JSON，并将结果用于两个作业。省略该输入时会保留 `ubuntu-latest` 默认值。
