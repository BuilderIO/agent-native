---
title: "关键概念"
description: "代理本机应用如何在三层架构中运作：Core 框架、可选的 Toolkit 构建块，以及可选的 Templates，加上共享的 actions、SQL 数据库、应用-代理循环，以及可移植性规则。"
---

# 关键概念

代理本机应用在幕后如何运作：原则与架构。此页是契约：一个应用要被视为代理本机应用，必须遵循的固定规则。关于这种构建方式背后的愿景与理由，请参阅 [What Is Agent-Native?](/docs/what-is-agent-native)。
...
| 层级                                       | 内容                                                                                                                                                     |
| ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Core：框架**                             | 基础运行时契约：actions、SQL 和 Drizzle 辅助函数、身份验证、应用状态、代理执行、访问检查、路由和实时同步。每个应用都可以直接使用 Core。                   |
...
1. **数据存在于 SQL 中：** 所有应用状态都通过 Drizzle ORM 存在于数据库中。
...
5. **应用状态存储在 SQL 中：** 临时 UI 状态存在于数据库中，代理和 UI 都可读取。
...
| 功能                                   | 你获得了什么                                                                                                                                                                                                                                                                                                     |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 一个 action = 每个表面                 | 使用 `defineAction()` 定义的每个 action 同时是一个代理工具、一个类型安全的前端 hook（`useActionQuery` / `useActionMutation`）、一个框架自有的 HTTP 传输、一个 CLI 命令、一个供外部客户端使用的 MCP 工具，以及一个供其他代理本机应用使用的 A2A 工具。可选的 `link` 和 `mcpApp` 元数据无需第二次实现，即可添加深层链接和 MCP Apps UI。 |
...
| BYO 代理聊天运行时                     | 同一个聊天 UI 可以架在 OpenAI Agents、OpenAI Responses、Claude Agent SDK、Vercel AI SDK、AG-UI，或者你自己的规范化 HTTP 流之上。参见 [Native Chat UI](/docs/native-chat-ui#byo-agent-runtimes)。                                                                                                                |
...
| 上下文感知                             | 代理始终通过 `navigation` app-state 键知道用户正在查看什么。参见 [Context Awareness](/docs/context-awareness)。                                                                                                                                                                                                 |
...
## SQL 中的数据 {#data-in-sql}

所有应用状态都通过 Drizzle ORM 存在于一个 SQL 数据库中。Schema 与提供商无关；支持的数据库、`DATABASE_URL` 配置和可移植性规则见 [Database](/docs/database)。
...
生产环境的代理聊天插件默认将原始 SQL 工具设为只读（`databaseTools: "read"`），因此代理使用 `db-schema` / `db-query` 检查应用自有的数据，并通过类型化的应用 actions 执行写入。只有在刻意设计、也需要暴露范围受限的 `db-exec` / `db-patch` 的维护类表面上，才设置 `databaseTools: "write"`（或 `true`）；设置 `databaseTools: "off"` / `false` 可要求所有数据访问都必须通过类型化的应用 actions。
...
每一个消费者调用的都是同一个底层函数，因此只需要编写和维护一份实现。完整参考请见 [Actions](/docs/actions)。
...
<div class="diagram-sync">
  ...
  <div class="diagram-node">
    变更事件<br /><small class="diagram-muted"
...
Agent-native 应用都包含一个嵌入式代理面板，在应用 UI 旁边提供 AI 代理。这正是这一架构得以运作的原因：代理需要一台计算机（数据库、浏览器、代码执行），而应用需要代理来完成 AI 工作。

- **嵌入式代理面板：** 内置于每个应用中的聊天和可选 CLI 终端。支持 Claude Code、Codex、Gemini、OpenCode 和 Builder.io。在本地运行，免费且开源。
- **云：** 部署到任意云端，具备实时协作、可视化编辑、角色和权限管理。最适合团队使用。
...
完整模式请见 [Context Awareness](/docs/context-awareness)：导航状态、view-screen、navigate 命令，以及防抖动机制。
...
完整的协议/表面矩阵（MCP 服务器与 OAuth、MCP Apps、A2A、深层链接、原生聊天小部件、Generative UI、AgentChatRuntime 连接器、Agent Web，以及面向 ACP 和 A2UI 的适配器展望），以及如何选择产品形态（聊天、内联 UI、完整应用页面、嵌入式 sidecar、自动化，或外部代理访问），请见 [Agent Surfaces](/docs/agent-surfaces)。
...
## 默认可移植 {#hosting-agnostic}

两条架构规则让应用可以跨数据库和主机移植：

- **数据库无关。** 使用 `@agent-native/core/db/schema` 编写 schema，并使用 Drizzle 的可移植查询 DSL 进行读写，这样相同的代码可以在任何受支持的提供商上运行。仅将原始 SQL 用于增量迁移或一次性维护，且要保持参数化并与具体方言无关。参见 [Database](/docs/database)。
...
- **[A2A Protocol](/docs/a2a-protocol)：** 同一工作区内的应用如何通过 JSON-RPC 相互发现并调用。
