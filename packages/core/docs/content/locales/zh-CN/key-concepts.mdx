...
| 代理和 UI 之间的实时同步  | 在长期运行的宿主上，同进程写入会立即通过 `/_agent-native/events` 流式传输；轻量级轮询让 cron 和跨进程写入保持收敛，除非应用已接入[托管实时同步网关](/docs/real-time-collaboration#hosted-gateway)，否则在生产环境无服务器环境中，这是应用以相同节奏运行的唯一路径。变更类 actions 会自动使 action 支持的查询失效，因此代理创建的记录无需手动刷新即可出现。参见下方的 [Live Sync](#polling-sync)。 |
...
当代理更改数据时，UI 需要在无需手动刷新的情况下反映这一变化。`useDbSync()` 让这一切自动发生。在长期运行的宿主上，同进程写入通过 `/_agent-native/events` 流式传输；`/_agent-native/poll` 仍然是跨进程场景的回退方案。当代理写入数据库（应用状态、设置或领域数据）时，一个版本计数器会递增，客户端会使相关的 React Query 缓存失效。

SSE 覆盖了常见场景而不是使用 WebSockets，因为它能以普通 HTTP 的方式让同进程写入立即到达浏览器；而版本计数器轮询则作为回退方案，在所有部署环境中都能正常工作。在生产环境无服务器宿主（Netlify、Lambda、Vercel）上，应用自身的 `/_agent-native/events` 并不是一条较慢的路径——它是关闭的。让该流保持打开会占用一个一次只服务一个请求的函数实例（反正它也永远看不到另一个调用的写入），直到平台将其杀死、回收容器；服务器转而立即返回 204，其余的同步以相同节奏全部由 `/_agent-native/poll` 承担，除非应用已接入[托管实时同步网关](/docs/real-time-collaboration#hosted-gateway)，该网关会从自己的长期运行进程中流式传输。
