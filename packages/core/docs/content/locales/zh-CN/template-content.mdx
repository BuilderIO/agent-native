...
迁移请参阅[本地文件夹源](/docs/template-content-local-files)。
...
- **`server/db/schema.ts`** — 数据模型。此处添加列或表。内容模板没有 `db:push` 脚本；它依赖于在启动时运行的严格附加迁移。编辑 `server/db/schema.ts`，编写匹配的附加迁移，并在下次应用启动时应用更改 - 架构更新绝不能删除、重命名或破坏性地更改现有表或列（有关准则，请参阅 [Database](/docs/server-database#migrations)）。
