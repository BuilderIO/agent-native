...
<Diagram id="doc-block-18d663e" title={"浏览器数据循环"} summary={"钩子通过动作进行读写； useDbSync 监视数据库，以便代理和后台写入自动重新获取相同的缓存。"}>
...
<div class="diagram-client">
  ...
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&harr;</div>
  <div class="diagram-panel" data-rough><strong>SQL 数据库</strong></div>
...
<Diagram id="doc-block-1nhkvht" title={"重新获取的两条路径"} summary={"本地突变会立即使自己的缓存失效；远程写入通过 SSE 到达此选项卡，或作为后备的轮询标记。"}>
