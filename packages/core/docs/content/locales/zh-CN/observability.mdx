...
| **反馈**     | 赞成/反对、类别细分、挫败感分数                              |
| **人工审核** | 审阅请求和答复、记录反馈并拟定指令更新                       |

### 对话详情 {#conversations}

“对话”选项卡显示已记录的运行。选择一个运行可查看其跨度，展开任意跨度即可查看已捕获的工具输入、输出、错误和元数据。选择**打开完整对话**可跳转到关联的聊天线程。

默认不捕获工具参数和结果。若要存储这些内容，请在 `observability` 配置中启用 `captureToolArgs` 和 `captureToolResults`。敏感字段和工具错误消息会在存储前脱敏。

### 人工审核 {#review}

“人工审核”选项卡会显示每次运行的请求和答复，允许记录反馈并起草指令更新，供人工审核。保存草稿不会自动更改指令。
