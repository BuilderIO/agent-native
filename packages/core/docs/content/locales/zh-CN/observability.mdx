...
可观察性和 CI 评估门都对质量进行评分，但两端不同——对实际流量进行被动事后评分，与对固定输入进行主动通过/失败检查。观察记忆与质量无关；这与代币成本和上下文窗口压力有关。 将已完成的生产 trace 提升为 `*.eval.ts` 用例（仪表盘 **Promote to eval**、`promote-trace-eval` 或 `agent-native eval promote <runId> --write evals/from-trace.eval.ts`）。见 [从生产 Trace 提升](/docs/evals#from-trace)。
...
| POST | `/traces/:runId/promote`   | 将已完成运行提升为 CI eval |
| POST | `/feedback`                | 提交反馈                  |
| GET  | `/feedback`                | 列出反馈                  |
