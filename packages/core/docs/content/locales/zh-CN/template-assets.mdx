...
<DataModel id="doc-block-1d8jb34" title={"资产数据模型"} summary={"Libraries are the ownable container; collections, folders, and presets organize them. Sessions drive generate-and-choose; assets and runs hold output and the audit log. Table names keep the legacy image_* prefix but cover all media."} entities={[
  {
    "id": "library",
