...
通过 ngrok 测试 Claude 时，请使用生产版本（`pnpm build` 然后 `npx @agent-native/core@latest start`）或已部署的预览/生产 URL。 Claude的单帧移植路径适用于生产资产块；原始 Vite 开发模块（例如 `/app/root.tsx`）可以受到应用程序身份验证的保护，并且无法从 Claude 资源源进行动态导入。
