...
<DataModel id="doc-block-u9i9hp" title={"邮件 SQL 表"} summary={"电子邮件本身位于 Gmail 中。 SQL 表包含 Gmail 不包含的内容：排队草稿、发送跟踪事件和 OAuth 令牌。设置和临时状态位于 settings 和 application_state 存储中。"} entities={[
  {
