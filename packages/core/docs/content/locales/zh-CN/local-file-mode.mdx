...
文件。这适合本地开发和 Agent Native Desktop，但不是默认的生产安全模型。

框架会在已知的托管、无服务器或远程数据库支持的运行时中拒绝
`local-files` 模式。此检查无法通过
`AGENT_NATIVE_ALLOW_LOCAL_FILES_IN_PRODUCTION` 绕过；环境变量不能证明单租户隔离。
生产模式仅限 Agent Native Desktop 子进程。本地开发（包括本地 SQLite）仍可在
没有标志的情况下运行。对于托管或多用户应用，请使用数据库模式和 SQL 支持的共享。
