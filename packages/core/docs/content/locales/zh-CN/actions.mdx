...
对于应用程序操作](/docs/server-overview#actions-first)。
...
- **`readOnly: true`** — 即使对于不变异的 POST actions 也显式跳过轮询刷新。 它**不是**访问控制：它不限制谁可以调用该 action，也不影响 `useActionQuery`、`useActionMutation` 或任何客户端 hook。它的唯一作用是抑制调用后的刷新事件，并让该 action 不参与写入审计。要对模型隐藏某个 action，请使用 `agentTool: false`；要限制调用方，请使用上面的认证/访问选项。
