...
| **平台能标识该部署**                                | 注册以应用的公开来源（origin）为键，因此并非该部署的进程不得声称拥有它。                                                            | Netlify、Vercel、AWS Lambda、Cloud Run 和 Render 都会标记自身。Cloudflare Workers 不会，因此请把 `AGENT_NATIVE_REALTIME_APP_URL` 设为该部署自己的来源。在纯容器或虚拟机上，请设置同一个变量 |
