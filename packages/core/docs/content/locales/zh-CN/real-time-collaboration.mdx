...
- **代理作为对等编辑器** — 代理通过相同的 Yjs 进行编辑流程
  ...
  位置、选择或撤消堆栈。
- **随处可用** — Drizzle 支持的任何 SQL 数据库（PGlite、Postgres）。
...
| 要求                                                | 原因                                                                                                                             | 如何检查                                                                                                                                                       |
| --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **位于公网可达主机上的 Postgres**                   | 网关从自己的网络拨号连接数据库。PGlite、`localhost`、`*.internal` 以及裸 IP 地址在连接字符串离开你的机器之前就会被拒绝。 | `DATABASE_URL` 指向 Neon 之类的托管 Postgres                                                                                                                   |
