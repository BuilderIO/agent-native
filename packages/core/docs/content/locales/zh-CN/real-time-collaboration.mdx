...
| **平台能标识该部署**                                | 注册以应用的公开来源（origin）为键，因此并非该部署的进程不得声称拥有它。                                                            | Netlify、Vercel、AWS Lambda、Cloud Run、Render 和 Cloudflare Workers 都会标记自身。在纯容器或虚拟机上，请把 `AGENT_NATIVE_REALTIME_APP_URL` 设为该部署自己的来源 |
