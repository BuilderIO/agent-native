...
应用默认使用上文描述的 poll/SSE 传输。任何应用都可以改为选用托管实时同步网关：它把
同步事件扇出到多个无服务器实例，而不是让每个实例各自维持长连接。

- `AGENT_NATIVE_REALTIME_TRANSPORT=hosted` 是你唯一需要设置的变量，设置它本身就是
  明确的选择加入——没有设置它的部署不会发生本节描述的任何事情。
  `AGENT_NATIVE_REALTIME_GATEWAY_URL` 用于在预发布环境覆盖网关端点；未设置时会从
  `BUILDER_GATEWAY_BASE_URL` 推导。
- 在由 Builder 托管的应用中，通道 id 和签名密钥
  (`AGENT_NATIVE_REALTIME_HMAC_SECRET`) 是由 Builder 预配流程注入的保留环境变量——
  网关读取的数据库已经由流水线预配完成。
- **在其他任何地方，应用会自行注册，这意味着把你的数据库 URL 发送给 Builder。**
  在设置了传输变量后，拥有 `BUILDER_PRIVATE_KEY` 的 Postgres 部署会把连接字符串和
  公开来源一次性发送给网关，并取回通道 id 和签名密钥。Builder 会保存该连接字符串，
  并连接到你的数据库以跟踪 `sync_events`；由于版本分配和带外变更检测都会写入，该
  连接并非只读。如果你不希望这样，就不要设置传输变量——整个决定就只有这一个，没有
  其他共享凭据的路径。当数据库 URL 或应用来源变化时会重新注册，因此轮换凭据无需
  重新部署其他任何东西。
- 组织也必须已启用该网关。在启用之前，注册会被拒绝，应用保持本地轮询。
- 只有生产部署会注册。部署预览保持本地同步，因此其一次性数据库凭据不会离开本机，
  也不会为每个拉取请求创建一个通道。
- **组织就是信任边界。** 任何能够注册的凭据也能重新注册一个已有来源，这会返回该通道
  的签名密钥并把它的数据库改指到别处。持有该密钥即可为应用中的任意用户签发订阅
  令牌，从而暴露该用户的*变更流*（哪些记录在何时发生了变化），但不会暴露记录本身，
  因为客户端仍使用自己的会话从应用重新读取内容。请把注册凭据视为覆盖整个组织。
- `GET /_agent-native/realtime-token` 会签发一个短期（10 分钟）、会话范围的订阅
  令牌，浏览器用它来打开网关连接。它仅接受同源请求，没有活动会话时返回 401。
- `GET /_agent-native/can-see` 是网关的逐事件可见性检查：它先校验已签名的网关访问
  令牌，然后重新执行应用自己的 `resolveAccess`，因此网关绝不会把事件转发给已经失去
  访问权限的共享对象。未知资源类型或查询失败会安全失败 (`{ allowed: false }`)。
- 在 Postgres 上，托管应用从数据库分配同步版本 (`dbAssignedVersions`)，而不是使用
  进程内计数器，因为网关实例和应用自身的无服务器实例共享同一个数据库，必须就写入
  顺序达成一致。

这里的所有失败都是有意静默的：没有传输变量、没有 Builder 密钥、数据库不是
Postgres、组织尚未启用，或网关不可达，都会让应用停留在本地轮询而不是报错。如果托管
实时同步没有按预期启用，请按此顺序排查——`GET /_agent-native/realtime-token` 返回
404 表示没有解析到通道。
