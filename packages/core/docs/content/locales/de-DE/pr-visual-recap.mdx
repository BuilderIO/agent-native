...
Die JSON-Anführungszeichen sind erforderlich. Sowohl der einfache `gate`-Job als auch der vollständige `recap`-Job verwenden den ausgewählten Runner. Übergeben Sie zur Konfiguration während der geführten Einrichtung denselben JSON-Wert an `--runs-on` und prüfen Sie anschließend den Workflow und die Verfügbarkeit des Runners mit `doctor`:
...
Es gelten die gleichen Geheimnisse und Variablen wie in [Secrets and variables](#secrets-and-variables) beschrieben – legen Sie sie in Ihren Repo-Einstellungen auf die gleiche Weise fest wie für die Kopiervariante. Der wiederverwendbare Aufrufer leitet `VISUAL_RECAP_RUNS_ON` über seine `runs-on`-Eingabe weiter; der aufgerufene Workflow analysiert das JSON und verwendet das Ergebnis für beide Jobs. Ohne diese Eingabe bleibt `ubuntu-latest` die Standardeinstellung.
