...
<FileTree id="doc-block-yblsyc" title="Anleitung und Verhalten" entries={[
  {
    "path": "AGENTS.md",
...
<DataModel id="doc-block-1bgy8qd" title={"Kerngeschäfte SQL"} summary={"Wird in jeder Vorlage automatisch erstellt – der Agent und die Benutzeroberfläche lesen und schreiben diese."} entities={[
  {
