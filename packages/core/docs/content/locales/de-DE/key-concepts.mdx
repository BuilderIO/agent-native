---
title: "Kernkonzepte"
description: "Wie agentennative Apps über drei Ebenen hinweg funktionieren: das Core-Framework, optionale Toolkit-Bausteine und optionale Templates, sowie die gemeinsamen Aktionen, die SQL-Datenbank, die App-Agent-Schleife und die Portabilitätsregeln."
---

# Kernkonzepte

Wie agentennative Apps im Detail funktionieren: die Prinzipien und die Architektur. Diese Seite ist der Vertrag: die festen Regeln, denen eine App folgen muss, um als agentennativ zu gelten. Für die Vision und die Begründung, warum man so baut, siehe [What Is Agent-Native?](/docs/what-is-agent-native).
...
| Ebene                                       | Was es ist                                                                                                                                                                                              |
| -------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Core: das Framework**                      | Der grundlegende Laufzeitvertrag: Aktionen, SQL- und Drizzle-Hilfsfunktionen, Auth, Anwendungsstatus, Agentenausführung, Zugriffsprüfungen, Routing und Live-Synchronisierung. Jede App kann Core direkt verwenden. |
...
Zur Laufzeit ist jede agentennative App drei Dinge, die zusammenarbeiten:
  ...
  <div class="diagram-row">
    <div class="diagram-card">
      ...
      ><small class="diagram-muted"
        >liest + schreibt Daten, führt Aktionen aus, nutzt konfigurierte Tools</small
      >
    </div>
    <div class="diagram-card">
      <span class="diagram-pill">Anwendung</span
      ><small class="diagram-muted"
        >Chat, Inline-Ergebnisse, Steuerzentrale oder vollständige React-UI</small
      >
    </div>
  </div>
...
Jede agentennative App hat dieselben Bausteine des Agenten, unabhängig davon, ob
...
1. **Daten leben in SQL:** Der gesamte App-Zustand lebt in der Datenbank via Drizzle ORM.
...
5. **Anwendungsstatus in SQL:** flüchtiger UI-Zustand lebt in der Datenbank, lesbar sowohl von Agent als auch UI.

## Die Vier-Bereiche-Checkliste {#four-area-checklist}

Jedes benutzerseitige Feature sollte alle zutreffenden Bereiche aktualisieren. Einen zutreffenden Bereich auszulassen bricht den agentennativen Vertrag; einer Automatisierung, die kein Mensch durchsuchen muss, einen Bildschirm aufzuzwingen, ist ebenfalls ein Smell.
...
| BYO Agent-Chat-Runtimes              | Dieselbe Chat-UI kann auf OpenAI Agents, OpenAI Responses, Claude Agent SDK, Vercel AI SDK, AG-UI oder Ihrem eigenen normalisierten HTTP-Stream aufsetzen. Siehe [Native Chat UI](/docs/native-chat-ui#byo-agent-runtimes).                                                                                                                                                          |
...
| Context Awareness                    | Der Agent weiß über den App-State-Key `navigation` immer, was der Benutzer gerade betrachtet. Siehe [Context Awareness](/docs/context-awareness).                                                                                                                                                                                                                                    |
...
Der gesamte Anwendungsstatus lebt in einer SQL-Datenbank via Drizzle ORM. Schemas sind anbieteragnostisch; die unterstützten Datenbanken, die `DATABASE_URL`-Konfiguration und die Portabilitätsregeln finden Sie unter [Database](/docs/database).

Core-SQL-Stores werden automatisch erstellt und sind in jedem Template verfügbar:
...
Das Produktions-Agent-Chat-Plugin stellt rohe SQL-Tools standardmäßig auf schreibgeschützt (`databaseTools: "read"`), sodass Agenten app-eigene Daten mit `db-schema` / `db-query` untersuchen und Schreibvorgänge über typisierte App-Aktionen durchführen. Setzen Sie `databaseTools: "write"` (oder `true`) nur für bewusste Wartungsoberflächen, die zusätzlich die eingeschränkten `db-exec` / `db-patch` bereitstellen sollen; setzen Sie `databaseTools: "off"` / `false`, um für jeden Datenzugriff typisierte App-Aktionen zu verlangen.
...
Jeder Verbraucher ruft dieselbe zugrunde liegende Funktion auf, sodass es nur eine Implementierung zu schreiben und zu warten gibt. Die vollständige Referenz finden Sie unter [Actions](/docs/actions).
...
<div class="diagram-sync">
  ...
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node">
    Query-Neuabruf<br /><small class="diagram-muted">Render, kein Neuladen</small>
  </div>
</div>
```
...
Ein _Frame_ ist die Umgebung, die den Agenten neben Ihrer App hostet. Lokal ist das embedded Panel; in der Cloud ist es die verwaltete Oberfläche von Builder.io. Siehe [Frames](/docs/frames).

Agentennative Apps enthalten ein eingebettetes Agent-Panel, das den KI-Agenten neben der App-UI bereitstellt. Das ist es, was die Architektur funktionieren lässt: Der Agent braucht einen Computer (Datenbank, Browser, Codeausführung), und die App braucht den Agenten für KI-Arbeit.

- **Embedded Agent Panel:** Chat und optionales CLI-Terminal, in jede App eingebaut. Unterstützt Claude Code, Codex, Gemini, OpenCode und Builder.io. Läuft lokal, kostenlos und Open Source.
- **Cloud:** In jeder Cloud bereitstellen, mit Echtzeit-Zusammenarbeit, visuellem Bearbeiten, Rollen und Berechtigungen. Am besten für Teams.
...
Das vollständige Muster – Navigationsstatus, view-screen, navigate-Befehle und Jitter-Vermeidung – finden Sie unter [Context Awareness](/docs/context-awareness).
...
Zwei architektonische Regeln halten Apps über Datenbanken und Hosts hinweg portabel:

- **Datenbankagnostisch.** Schreiben Sie Schemas mit `@agent-native/core/db/schema` und Lese-/Schreibvorgänge mit Drizzles portabler Query-DSL, sodass derselbe Code auf jedem unterstützten Anbieter läuft. Verwenden Sie rohes SQL nur für additive Migrationen oder einmalige Wartung, parametrisiert und dialektagnostisch gehalten. Siehe [Database](/docs/database).
...
- **[A2A Protocol](/docs/a2a-protocol):** wie Apps im selben Workspace einander über JSON-RPC entdecken und aufrufen.
