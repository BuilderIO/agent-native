...
| Live Sync zwischen Agent und UI            | Auf einem langlebigen Host werden Schreibvorgänge im selben Prozess sofort über `/_agent-native/events` gestreamt; ein leichtgewichtiges Polling hält Cron- und prozessübergreifende Schreibvorgänge konvergent und ist auf serverlosen Produktions-Hosts (im gleichen Takt) der einzige Weg der App, sofern sie nicht das [gehostete Realtime-Sync-Gateway](/docs/real-time-collaboration#hosted-gateway) aktiviert hat. Mutierende Aktionen invalidieren aktionsbasierte Queries automatisch, sodass vom Agenten erstellte Datensätze ohne manuelles Neuladen erscheinen. Siehe [Live Sync](#polling-sync) weiter unten. |
...
Wenn der Agent Daten ändert, muss die UI das ohne manuelles Neuladen widerspiegeln. `useDbSync()` macht das automatisch. Auf einem langlebigen Host werden Schreibvorgänge im selben Prozess über `/_agent-native/events` gestreamt; `/_agent-native/poll` bleibt der prozessübergreifende Fallback. Wenn der Agent in die Datenbank schreibt (Anwendungsstatus, Einstellungen oder Domänendaten), erhöht sich ein Versionszähler und der Client invalidiert die relevanten React-Query-Caches.
...
SSE deckt den häufigsten Fall ab statt WebSockets, weil es Schreibvorgängen im selben Prozess einen unmittelbaren Weg zum Browser über einfaches HTTP verschafft, während der Versionszähler-Poll als Fallback erhalten bleibt, der in jeder Deployment-Umgebung funktioniert. Auf einem serverlosen Produktions-Host (Netlify, Lambda, Vercel) ist der eigene `/_agent-native/events`-Stream der App kein langsamerer Weg – er ist deaktiviert. Den Stream offen zu halten würde eine Funktionsinstanz blockieren, die jeweils nur eine Anfrage bedient (und daher ohnehin nie den Schreibvorgang einer anderen Invocation sehen könnte), bis die Plattform sie beendet und den Container zurücksetzt; der Server antwortet stattdessen sofort mit 204, und `/_agent-native/poll` übernimmt die gesamte Synchronisierung, im gleichen Takt, sofern die App nicht das [gehostete Realtime-Sync-Gateway](/docs/real-time-collaboration#hosted-gateway) aktiviert hat, das stattdessen aus seinem eigenen langlebigen Prozess streamt.
