...
Jede App wird mit `APP_BASE_PATH=/<name>` und `VITE_APP_BASE_PATH=/<name>` erstellt und über die ausgewählte Nitro-Voreinstellung ausgegeben. **Netlify** ist die Standardvoreinstellung für den einheitlichen `deploy`-Befehl; sie gibt App-Funktionen unter `.netlify/functions-internal/<app>-server` aus und erzeugt Weiterleitungen, die statische Assets nicht erzwingen, sodass das CDN Dateien zuerst ausliefert. **Vercel** wird mit `npx @agent-native/core@latest deploy --preset vercel` unterstützt; sie schreibt ein Root-`.vercel/output`-Bundle mit Vercels Build Output API.
...
Bevorzugen Sie jede App in einer eigenen Domain (`mail.company.com`, `calendar.company.com`)? Jede App im Arbeitsbereich ist immer noch ein unabhängiges Deployable – `cd apps/mail && pnpm build` verhält sich genau wie ein eigenständiges Gerüst. Der App-übergreifende A2A durchläuft dann den standardmäßigen JWT-signierten Pfad mit einem gemeinsam genutzten `A2A_SECRET`. Der domänenübergreifende SSO zwischen separat bereitgestellten Apps wird von der Identitätsföderation mit Dispatch als Hub abgewickelt – siehe [Cross-App SSO](/docs/cross-app-sso); Durch die einheitliche Single-Origin-Bereitstellung ist dies nicht erforderlich.

### Gemeinsame Datenbank, gemeinsame Anmeldeinformationen

Damit Benutzerkonten, Organisationen, Einstellungen und andere App-übergreifende Daten geteilt werden, muss jede App dieselbe `DATABASE_URL` verwenden. Legen Sie sie für die lokale Entwicklung in der `.env`-Datei des Workspace-Stammverzeichnisses fest, damit alle Apps dieselbe PostgreSQL-Datenbank nutzen. Ist sie nicht gesetzt, verwendet jede App eine eigene prozesslokale PGlite-Datenbank. Benutzer und Organisationen werden dann nicht geteilt.

Der Verbindungsbedarf summiert sich über alle Prozesse, die dieselbe Datenbank verwenden. Wenn fünf dauerhaft laufende Node.js-App-Prozesse jeweils einen gemeinsamen Pool für dieselbe `DATABASE_URL` nutzen, können sie beim Standardwert von 20 Verbindungen pro Pool bis zu 100 Verbindungen belegen (`5 × 1 Pool × 20`). Bei serverlosen Workspaces schätzen Sie die Zahl warmer Funktionsinstanzen pro App anhand der laufzeitspezifischen Pool-Standardwerte. Unter [Serverdatenbank](/docs/server-database) finden Sie `AGENT_NATIVE_DB_POOL_MAX` und die vollständigen Hinweise zur Verbindungsplanung.
