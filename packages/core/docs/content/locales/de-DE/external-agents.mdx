...
| `ask_app({ app, message, async?, maxWaitMs? })`    | Agentenschleife | leitet eine Aufgabe in natürlicher Sprache an den In-App-Agenten dieser App weiter; bei länger laufender Arbeit wird eine dauerhafte `taskId` für das Polling mit `ask_app_status` zurückgegeben |
