...
Öffnen Sie in der App **Settings**, suchen Sie nach **MCP** und kopieren Sie die
**MCP server URL**. Fügen Sie diese URL in den gewünschten KI-Host ein. Die URL
ist die Basis-URL der bereitgestellten App plus `/mcp`; Sie müssen keine
Entwicklereinstellung suchen.
