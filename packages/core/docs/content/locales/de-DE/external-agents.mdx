...
Öffnen Sie in der App **Settings → Connections → Integrations** und suchen Sie nach dem Ihnen
...
**Settings → MCP server** und wählt die passendste Anleitung aus. Kopieren Sie die
...
Die Einstellungen enthalten in der Gruppe der App eine Seite **MCP server**.
...
Verwenden Sie **Settings → MCP server** als Startpunkt in der App oder öffnen Sie
