...
Die Authentifizierung identifiziert das verbundene Konto und erweitert dessen Dokumentberechtigungen nicht. Öffentliche Dokumente bleiben über ihren Freigabelink lesbar. Private Dokumente sind über MCP nur lesbar, wenn das verbundene Konto bereits Zugriff hat. Gibt `get-document` nach der Verbindung eine Zugriffsverweigerung zurück, muss der Eigentümer diesem Konto Zugriff gewähren.

Eine Neuinstallation der Verbindung oder eine Änderung des öffentlichen Freigabelinks behebt keine Berechtigungsverweigerung für das verbundene Konto.
