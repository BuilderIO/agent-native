...
Externe Agenten erhalten die echten Erstellungs-/Aktualisierungswerkzeuge der App – dieselben benannten Aktionen, die auch der In-App-Agent aufruft – sodass sie, wenn ein Nutzer neuen Inhalt anfordert (ein Design, eine Präsentation, ein Formular, ein Dokument), diesen direkt verfassen und mit diesen Werkzeugen speichern sollten, statt die Arbeit über `ask_app` zu delegieren. Aktionen, die mit `endsTurn: true` markiert sind – etwa ein In-App-Fragenformular, das auf die Antwort des Nutzers im Chat dieser App wartet – sind standardmäßig vor MCP, WebMCP und A2A verborgen; sie gehören dem In-App-Agenten, nicht einem externen Aufrufer, es sei denn, die Aktion aktiviert dies mit `mcpTool: true` ausdrücklich wieder. Die WebMCP-Brücke akzeptiert Tool-Eingaben und -Ergebnisse bis zu 500.000 Zeichen, sodass ein komplett verfasster Bildschirm, eine Präsentation oder ein Dokument in einem einzigen Schreibvorgang Platz findet, statt aufgeteilt werden zu müssen. Seiten-lokale WebMCP-Aufrufe übermitteln außerdem die Browser-Tab-ID, sodass sich der tab-bezogene Kontext (Navigation, Auswahl) auf genau den Tab bezieht, den der Nutzer gerade betrachtet.
