...
<DataModel id="doc-block-1e6o7vt" title="Das Verbindungsmodell" summary={"A connection records safe provider metadata and credentialRefs (pointers, not secrets). Each grant unlocks one app — one connection, many grants."} entities={[
  {
    "id": "conn",
