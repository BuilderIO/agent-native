...
`runDrizzleMigrations()` im Release-Schritt. Behalten Sie `runMigrations()` für
handgeschriebene Historie und Sonderfälle. Verwenden Sie niemals destruktives SQL,
`drizzle-kit push`, Tabellenumbenennungen oder Spaltenlöschungen.
