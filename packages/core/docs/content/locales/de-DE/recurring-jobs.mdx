...
Wiederkehrende Jobs können bereits mit der App verbundene Remote- oder lokale MCP-Tools verwenden. Lassen Sie den Agenten die exakten Namen mit `tool-search` finden und binden Sie dann nur die benötigten Tools über `mcpTools`:
...
Finde neue Aufnahmen seit dem letzten erfolgreichen Lauf. Extrahiere nur
umsetzbare Follow-ups und rufe dann einmal die begrenzte Import-Aktion der App
mit stabilen Quell- und externen IDs, einer kurzen Quellenangabe und der
Aufnahme-URL auf. Importiere nicht das vollständige Transkript.
```

Der Job speichert nur Toolnamen, keine MCP-URLs, Zugriffstoken oder OAuth-Refresh-Tokens. Zur Laufzeit löst das Framework die verbundene Berechtigung im Anfragekontext des Job-Erstellers oder der Organisation auf und stellt nur die allowlisteten Tools bereit. Halten Sie die Liste klein und verwenden Sie für den finalen Import eine idempotente App-Aktion, damit überlappende Läufe keine Duplikate erzeugen. Siehe [MCP Clients](/docs/mcp-clients) für Verbindungs- und OAuth-Einrichtung.
