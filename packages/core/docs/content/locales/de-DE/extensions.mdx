...
- **Versionsgebundene Berechtigungen** — Eine Extension deklariert exakte Actions, Framework-Pfade und HTTP-Methoden, SQL-Zugriff sowie externe HTTPS-Ursprünge. Jede Person akzeptiert ausdrücklich eine Teilmenge; jede Manifeständerung macht die alte Freigabe ungültig.
- **Sicherer Legacy-Modus** — Ohne akzeptiertes aktuelles Manifest darf eine Extension nur schreibgeschützte Actions sowie Application State und eigene Extension-Daten lesen. SQL, Schreibzugriffe und externe Verbindungen bleiben deaktiviert.
- **Serverseitige Durchsetzung** — Action-, SQL-, Application-State-, Daten- und Proxy-Routen prüfen die Freigabe unabhängig. Ein Widerruf gilt ab der nächsten Anfrage.
