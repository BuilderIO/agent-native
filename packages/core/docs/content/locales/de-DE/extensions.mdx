...
## Sollte ich eine Erweiterung verwenden oder den App-Code bearbeiten? {#vs-app-code}

Mit dem Framework kann der Agent den Quellcode der App direkt bearbeiten – Komponenten, Routen, Stile und Geschäftslogik. Verwenden Sie diese Frage, wenn beide Wege die Anfrage lösen könnten:

- **Beginnen Sie mit einer Erweiterung** für ein eigenständiges, relativ einmaliges Widget: eine benutzerdefinierte Visualisierung oder Interaktion, eine Dashboard-Kachel, ein eigenständiges Dienstprogramm oder ein Zusatz in einem vorhandenen Slot. Sie ist schnell, benötigt keine Quellcodeänderung, keinen Build und keine Bereitstellung und kann privat oder geteilt sein.
- **Verwenden Sie App-Code** für eine Kernfunktion der Vorlage, Änderungen an nativer UI, Layout, Routen oder Geschäftslogik oder für etwas, das über viele Dashboards oder Benutzer hinweg breit wiederverwendet werden soll. Wenn kein geeigneter Slot vorhanden ist oder ein neuer Slot benötigt wird, verwenden Sie Code.
- **Überführen Sie wiederholte Erweiterungen.** Wenn dieselbe Erweiterung immer wieder erstellt wird oder die Pflege vieler Kopien schlechter wäre als eine gemeinsame Implementierung, verschieben Sie sie als wiederverwendbare native Funktion in den App-Code.
...
Faustregel: **Wenn eine Erweiterung das Problem lösen kann, beginnen Sie im Allgemeinen damit.** Verwenden Sie Code, wenn die Erweiterungsgrenze nicht passt oder breite Wiederverwendung und produktweite Verfügbarkeit eine native Implementierung rechtfertigen. Eine einmalige benutzerdefinierte Visualisierung in einem Dashboard ist beispielsweise eine gute Erweiterung; ein neuer Kreis-, Mehrfachbalken- oder Liniendiagrammtyp, der in allen Dashboards verfügbar sein soll, gehört in den Vorlagencode.
