...
`extensionTools: true` in `createAgentChatPlugin()`. Dadurch werden die
agentenseitigen actions und Anweisungen ergänzt; Sandbox-Laufzeit und bestehende
Links bleiben aus Kompatibilitätsgründen erhalten.
