...
Bei den meisten Apps wird die KI nachträglich eingebaut, sodass sie in der App eigentlich keine Aufgaben erledigen kann. In einer agentennativen App ist der Agent ein erstklassiger Bürger, der dasselbe actions, dieselbe Datenbank und denselben Status wie UI verwendet, sodass er alles tun kann, was die Schaltflächen können. Siehe [What Is Agent-Native?](/docs/what-is-agent-native#the-ladder).
...
Ja. Wenn Sie eine einfache agentic app wünschen, führen Sie `npx @agent-native/core@latest create my-app --template base` aus. Sie erhalten actions, Authentifizierung, SQL-gestützten Laufzeitstatus und Platz zum Hinzufügen eigener Bildschirme. Wenn Sie eine Automation ohne Browser bauen, führen Sie `npx @agent-native/core@latest create my-agent --headless` aus. Siehe [Getting Started](/docs/getting-started) und [Automation-First Apps](/docs/pure-agent-apps). Verwenden Sie `--template chat` nur, wenn Sie das Chat-Produkt wollen.
