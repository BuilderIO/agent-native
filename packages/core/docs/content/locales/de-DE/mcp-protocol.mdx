...
- **Automatisch gemountet** – jede App erhält `/mcp` kostenlos, keine Einrichtung erforderlich
...
| **Endpunkt**        | `/mcp`                                                                   | `/_agent-native/a2a`                            |
...
<Endpoint id="doc-block-arbi17" title="MCP Endpunkt" summary={"Der automatisch gemountete Streamable HTTP-Endpunkt, den jede agentennative App verfügbar macht."} method="POST" path="/mcp" summary="MCP Streambarer HTTP Endpunkt" auth="Standard remote MCP OAuth (Bearer access token), connect-minted JWT, or static ACCESS_TOKEN/ACCESS_TOKENS" params={[
  {
    "name": "Authorization",
    "in": "header",
    "type": "string",
...
Gehostete MCP-Anfragen verwenden eine begrenzte Inline-Wartezeit, damit eine lange Agentenschleife nicht einen einzelnen `tools/call` bis zu einem Client- oder Gateway-Timeout offen hält. Das optionale Argument `async` startet die dauerhafte Aufgabe und kehrt sofort zurück; ohne dieses Argument wartet der Server kurz auf einen schnellen Abschluss und gibt dann ein Aufgaben-Handle zurück, wenn noch weitere Arbeit aussteht. Die gehostete Wartezeit ist auf 25 Sekunden begrenzt.
...
Der Agent führt die gleiche Schleife aus wie der interaktive Chat – er kann mehrere Tools aufrufen, über den Kontext nachdenken und eine durchdachte Antwort produzieren. Eine nicht abgeschlossene Aufgabe gibt JSON mit `taskId`, `status`, `pollAfterMs` und Folgendem zurück:
...
| `/mcp/oauth/register`                     | Dynamische öffentliche Kundenregistrierung             |
| `/mcp/oauth/authorize`                    | Browserautorisierung + Einwilligung                    |
| `/mcp/oauth/token`                        | Autorisierungscode- und Aktualisierungstoken-Gewährung |
