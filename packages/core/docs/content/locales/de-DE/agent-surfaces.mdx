...
- **Automations** — persönliche und Organisations-Automatisierungen für Scheduled und Event mit Pausieren/Fortsetzen, Details und Löschen. Die Kompatibilitäts-URL bleibt `/agent#jobs`.
...
Die Seite verwendet derzeit den persönlichen Bereich und hat keinen
seitenweiten Umschalter **Personal / Organization**. Ein Tab kann eigene
Organisationsbereiche anzeigen, wenn die zugrunde liegenden Actions sie
unterstützen: **Automations** zeigt persönliche und Organisationsbereiche für
Scheduled und Event. Organisations-Event-Automatisierungen laufen immer als ihr Ersteller. Die Seite
ist eine dünne Hülle um vorhandene Komponenten, Actions und
Zugriffskontrollen, keine neue Admin-Konsole.
