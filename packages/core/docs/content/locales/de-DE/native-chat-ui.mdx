...
### Auswählen, wann Aktionsoberflächen erscheinen {#conditional-native-ui}

Eine Aktion kann mehrere Vorgänge behandeln, während nur einige Ergebnisse eine Karte benötigen. Fügen Sie `when(args, result)` zu `chatUI` hinzu, um diese Aufrufe auszuwählen. Der Server ruft das Prädikat nach einer erfolgreichen Aktion auf. Gibt es `false` zurück, bleibt das Ergebnis im normalen einklappbaren Tool-Verlauf. Das Prädikat wird weder gespeichert noch an den Browser gesendet.

Mail verwendet dies für `manage-draft`. Ergebnisse beim Erstellen erhalten eine Karte. Aktualisierungen und Löschungen bleiben normale Tool-Aufrufe:
...
Prüfen Sie bei Aktionen mit mehreren Schreibvorgängen die Argumente und das erfolgreiche Ergebnis. Mail wählt die Gmail-Filterkarte nur für erstellte oder ersetzte Filter mit Zusammenfassungsfeldern aus:
