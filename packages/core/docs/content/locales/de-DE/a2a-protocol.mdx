...
Berechtigung. Verwenden Sie die v0.3-Karte und den v0.3-Endpunkt des Anbieters
mit `cardUrl` und `protocolVersion`, wenn der Endpunkt Agent-Native JSON-RPC
spricht. Der Token-Provider muss ein Entra-Zugriffstoken beziehen und
...
und die Agent Registry kann einen Proxy-Endpunkt veröffentlichen. Die
HTTP+JSON-Bindung hat anbieterspezifische Pfade: Die Erkennung verwendet
`GET {url}/v1/card`, der Aufruf `POST {url}/v1/message:send` und Streaming
`POST {url}/v1/message:stream`. Die Anfragen verwenden Google-OAuth- oder
ADC-Bearer-Tokens und Google-Cloud-IAM. Siehe [Agent über einen Registry-A2A-Endpunkt aufrufen](https://docs.cloud.google.com/gemini/enterprise/docs/invoke-agent-a2a)
für Endpunkt- und Berechtigungsanforderungen.

Diese Pfade unterscheiden sich von den Agent-Native-JSON-RPC-Pfaden
(`/.well-known/agent-card.json` und `/_agent-native/a2a`). Registrieren Sie den
Endpunkt erst, wenn ein Adapter das Wire-Format übersetzt und Google-
Anmeldedaten bezieht und erneuert. Falls das Ziel separat einen standardmäßigen
A2A-Endpunkt bereitstellt, verwenden Sie diesen mit der generischen
Bearer-Anleitung. Der Registrierungsverkehr von Gemini Enterprise läuft nicht
durch Agent-Gateway-Richtlinien. Wenden Sie daher die Authentifizierungs- und
Autorisierungsrichtlinien des Ziels an.
