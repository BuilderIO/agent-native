...
## Ausgewogener Katalog {#balanced-catalog}

Der Katalog umfasst außerdem Grundlagen, Editor und Arbeitsflächen, Agent-Kontext und Wissen, Zusammenarbeit und Governance, Workspace-Chrome, Betrieb und Capability-Pakete. Der Rich Editor gehört zu Toolkit; Core bietet Kompatibilitäts-Re-Exports. Blöcke bleiben in Core. Design ist ein Template-Muster mit `design-tweaks`, kein vollständiger Drop-in-Editor. Die Context-X-Ray-Engine gehört zu Core, ihre wiederverwendbare Darstellung zu Toolkit. Scheduling und Creative Context sind getrennte Capability-Pakete; siehe [Package Lifecycle](/docs/package-lifecycle).
