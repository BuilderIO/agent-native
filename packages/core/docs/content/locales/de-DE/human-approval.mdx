...
<AnnotatedCode
  id="doc-block-6g3d1l"
  title={"Die eine Folgemaßnahme blockieren"}
  ...
  annotations={[
    {
      lines: "10",
      label: "Das ganze Tor",
      note: "Eine Flagge. Wenn es wahr ist und der Aufruf nicht genehmigt ist, stoppt die Schleife vor `run` – das Modell erreicht den Nebeneffekt nie von selbst.",
    },
    {
      lines: "11-13",
      label: "run() is untouched",
      note: "Der Handler bleibt derselbe. Die Genehmigung wird durch die Schleife erzwungen, die sie umgibt, nicht durch irgendetwas in `run`.",
    },
  ]}
/>
...
<Diagram id="doc-block-mq9lv" title="Die Genehmigungsunterbrechung" summary={"Ein Gated-Aufruf unterbricht die Runde, bevor run() ausgelöst wird. Die Genehmigung erteilt demjenigen, der den Schlüssel des Anrufs trägt, erneut eine Genehmigung. Erst dann tritt die Nebenwirkung ein."}>
...
<div class="diagram-approve">
  ...
  <div class="diagram-panel warn" data-rough>
    <strong>Gate ist truthy, Aufruf noch nicht genehmigt</strong
  ...
  <div class="diagram-box" data-rough>
    Mensch klickt im Chat auf Genehmigen<br /><small class="diagram-muted"
      >Client sendet den Turn erneut mit approvedToolCalls: [approvalKey]</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
...
- [**Actions**](/docs/actions-access-control#needs-approval) – die vollständige `defineAction`-Oberfläche, einschließlich `outputSchema` zur Validierung von Rückgabewerten.
