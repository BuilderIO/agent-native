...
<Diagram id="doc-block-g9ou1e" title="Das Leiterprinzip" summary={"Die meisten Teams bleiben bei Stufe 1 oder 2 stehen. Agent-nativ ist Stufe 3 – eine echte App und ein echter Agent über eine gemeinsame Aktionsoberfläche."}>

```html
<div class="diagram-ladder">
  <div class="diagram-card rung rung-3">
    <span class="diagram-pill accent">Stufe 3 · agent-native</span
  ...
  <div class="diagram-card rung rung-2">
    <span class="diagram-pill">Stufe 2</span><strong>Ein Chat mit Tools</strong
    ><small class="diagram-muted"
      >Der Agent kann handeln, ist aber weiter nur ein Chatfenster. Keine
      Dashboards, Listen oder Shortcuts.</small
    >
  </div>
  <div class="diagram-card rung rung-1">
    <span class="diagram-pill warn">Stufe 1</span
...
<Diagram id="doc-block-1wrsf1e" title="Ein System, zwei Wege hinein" summary={"Der Agent und die Benutzeroberfläche schreiben in dieselben Aktionen und dieselbe Datenbank. Was der eine tut, der andere sieht es."}>

```html
<div class="diagram-parity">
  <div class="diagram-col">
    <div class="diagram-node">
      Human<br /><small class="diagram-muted"
        >Klicks, Formulare, Shortcuts</small
      >
    </div>
    <div class="diagram-node">
      Agent<br /><small class="diagram-muted"
        >natürliche Sprache · A2A · Slack</small
      >
    </div>
  </div>
  ...
  <div class="diagram-panel center">
    ...
    ><small class="diagram-muted">defined once</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box">SQL-Datenbank</div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&#8635;</div>
  <div class="diagram-box">UI aktualisiert live</div>
</div>
```
...
.diagram-parity .diagram-col {
  display: flex;
  flex-direction: column;
  gap: 10px;
}
.diagram-parity .diagram-arrow {
  font-size: 22px;
  line-height: 1;
}
.diagram-parity .center {
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 4px;
...
Agent-native bietet jedem Benutzer die gleiche Anpassungsebene – ohne die App jemals zu verlassen. Jede App verfügt über persönliche **Agent-Ressourcen**, mit denen Sie (oder jeder in Ihrem Team) Folgendes tun können:
...
Der Clou: Es wird alles in der Datenbank gespeichert, nicht im Dateisystem. Es muss keine Entwicklungsumgebung eingerichtet werden, kein Container pro Benutzer. Jeder Benutzer erhält seinen eigenen vollständigen Satz von Agent-Ressourcen – persönlichen Speicher, persönliche Verbindungen, persönliche skills – praktisch kostenlos, da es sich um alle Zeilen einer Tabelle handelt. Das macht die Flexibilität auf Claude-Codeebene in einem echten mandantenfähigen SaaS-Produkt realisierbar.

Das vollständige Konzept finden Sie unter [Agent-Ressourcen](/docs/agent-resources).
...
Agent-native ist nicht nur für Entwickler. Der eingebettete Agent gibt dem ganzen Team eine gemeinsame Möglichkeit, die App zu bedienen, während Quelländerungen im normalen Entwicklungsablauf bleiben. Quellcode wird nur bearbeitet, wenn der Host bewusst Repository-/Workspace-Schreibwerkzeuge bereitstellt:
...
## Mit einer Vorlage starten und anpassen {#fork-and-customize}

Agent-native Apps folgen einem Modell, bei dem Sie mit einer funktionierenden App starten. Sie beginnen mit einer **Vorlage** – Kalender, Inhalt, Folien, Analysen, E-Mail, Clips, Design, Formulare, Versand – und machen sie zu Ihrer eigenen. Bei jedem handelt es sich um ein vollständiges, funktionierendes SaaS-Produkt und nicht um ein leeres Gerüst:
...
3. Erstellen Sie Ihre eigene App aus der Vorlage, wenn Sie sie anpassen möchten – „Verbinden Sie unser Stripe-Konto“, „fügen Sie ein Kohortendiagramm hinzu“
4. Passen Sie den Quellcode in Ihrem Repository und Entwicklungsablauf an
5. Stellen Sie Ihre App in Ihrer eigenen Domain bereit – oder bleiben Sie auf agent-native.com

Da es sich um _Ihre_ App und nicht um gemeinsam genutzte Infrastruktur handelt, besitzen Sie den Quellcode und können ihn weiterentwickeln, ohne das gemeinsame Framework zu ändern. Der eingebettete Agent arbeitet über Actions und Daten und kann Quelländerungen nur mit bewusst bereitgestellten Schreibwerkzeugen unterstützen. Die ganze Geschichte finden Sie unter [Templates](/docs/cloneable-saas).
  ...
  annotations={[
    {
      ...
      note: "`description` ist das, was der Agent liest, um zu entscheiden, wann er dies als Tool aufrufen soll.",
    },
    {
      lines: "6",
    ...
    },
    {
      ...
      note: "Der `run`-Körper ist die einzige Quelle der Wahrheit. Sowohl die UI-Schaltfläche als auch das Agententool führen genau dies aus.",
    },
  ]}
/>
...
- [**Agent-Ressourcen**](/docs/agent-resources) – die benutzerspezifische Anpassungsschicht (skills, Speicher, Anweisungen, MCP), unterstützt durch SQL, nicht durch Dateien
