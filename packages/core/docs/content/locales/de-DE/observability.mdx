...
Das Dashboard verfügt über 6 Registerkarten:
...
| **Menschliche Überprüfung** | Anfrage und Antwort prüfen, Feedback erfassen und eine Anweisungsänderung entwerfen                      |

### Gesprächsdetails {#conversations}

Im Tab „Gespräche“ sehen Sie aufgezeichnete Agentenläufe. Wählen Sie einen Lauf aus, um seine Spans aufzurufen, und klappen Sie einen Span auf, um erfasste Tool-Eingaben, Ausgaben, Fehler und Metadaten zu sehen. „Vollständige Unterhaltung öffnen“ springt zum zugehörigen Chat.

Die Erfassung von Tool-Argumenten und -Ergebnissen ist standardmäßig deaktiviert. Aktivieren Sie `captureToolArgs` und `captureToolResults` in der `observability`-Konfiguration, um diese Inhalte zu speichern. Sensible Felder und Tool-Fehlermeldungen werden vor dem Speichern bereinigt.

### Menschliche Überprüfung {#review}

Der Tab „Menschliche Überprüfung“ zeigt die Anfrage und Antwort jedes Laufs. Sie können Feedback erfassen und einen Entwurf für eine Anweisungsänderung zur menschlichen Prüfung speichern. Der Entwurf ändert Anweisungen nicht automatisch.
