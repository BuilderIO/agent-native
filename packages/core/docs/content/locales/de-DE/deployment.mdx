...
Legen Sie `DATABASE_URL` in Ihrem Bereitstellungsanbieter fest, bevor Sie eine App in die Produktion hochstufen. Agent-native verwendet Drizzle für Schemata und Abfragen, sodass die Datenschicht über Drizzle-kompatible SQL-Backends portierbar ist und das Framework den Dialekt von URL automatisch erkennt. Die Adapterliste und Dialektdetails finden Sie unter [Database](/docs/server-database#production).
...
Wenn aktiviert, erzeugt der Netlify-Build eine geplante Funktion, die **jede Minute** `/_agent-native/health` aufruft. Das hält die Server-Funktion und ihre Datenbank warm, damit der nächste Besucher nach einer Leerlaufphase keinen Kaltstart bezahlt (1–5 s bei Neon).

Keep-warm ist **standardmäßig deaktiviert**, weil ein Aufwecken pro Minute Autosuspend bei kostenpflichtigen Scale-to-Zero-Datenbanken verhindert. Setze `AGENT_NATIVE_ENABLE_KEEP_WARM=1`, wenn die geringere Latenz für den nächsten Besucher die zusätzliche Datenbank-Laufzeit wert ist.

| Variable                                    | Wirkung                                                                                                                              |
| ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `AGENT_NATIVE_ENABLE_KEEP_WARM`             | Aktiviert das geplante Warmhalten von Server und Datenbank. Standardmäßig nicht gesetzt.                                             |
| `AGENT_NATIVE_DISABLE_KEEP_WARM`            | Kompatibles Abschalt-Flag, das bei gesetztem Wert die Opt-in-Einstellung überstimmt.                                                 |
| `AGENT_NATIVE_KEEP_WARM_SCHEDULE`           | Überschreibt die Kadenz bei aktiviertem Keep-warm; Standard ist `* * * * *`, ein nicht parsebarer Wert lässt den Build fehlschlagen. |
| `AGENT_NATIVE_DISABLE_KEEP_WARM_BACKGROUND` | Verwirft nur das Aufwärmen der Hintergrundfunktion, wenn Keep-warm aktiviert ist, und hält den Server warm.                          |

Wenn du einen kostenlosen oder anderweitig kontingentierten Datenbank-Tarif nutzt, lasse `AGENT_NATIVE_ENABLE_KEEP_WARM` unset. Wenn du es aktivierst, wähle eine langsamere Kadenz oder setze `AGENT_NATIVE_DISABLE_KEEP_WARM_BACKGROUND=1`.
...
| `AGENT_NATIVE_ENABLE_KEEP_WARM`             | Aktiviert die geplante Netlify-Keep-warm-Funktion; standardmäßig nicht gesetzt. Siehe [Keep-warm](#netlify-keep-warm).                                                                                                                                                                                                                                                                                                                                                                                      |
| `AGENT_NATIVE_DISABLE_KEEP_WARM`            | Kompatibles Abschalt-Flag für die geplante Netlify-Keep-warm-Funktion. Siehe [Keep-warm](#netlify-keep-warm).                                                                                                                                                                                                                                                                                                                                                                                               |
| `AGENT_NATIVE_KEEP_WARM_SCHEDULE`           | Überschreibt die Keep-warm-Kadenz bei Aktivierung (Standard `* * * * *`). Ein nicht parsebarer Wert lässt den Build fehlschlagen. Siehe [Keep-warm](#netlify-keep-warm).                                                                                                                                                                                                                                                                                                                                    |
| `AGENT_NATIVE_DISABLE_KEEP_WARM_BACKGROUND` | Verwirft nur das Aufwärmen der Hintergrundfunktion, wenn Keep-warm aktiviert ist. Siehe [Keep-warm](#netlify-keep-warm).                                                                                                                                                                                                                                                                                                                                                                                    |
...
Datenbankverbindungsvariablen (`DATABASE_URL`, `DATABASE_AUTH_TOKEN`, pro App `<APP_NAME>_DATABASE_URL`) leben in [Database](/docs/server-database#production).
...
- [**Database**](/docs/server-database#production) — Wählen Sie vor der ersten Produktionsbereitstellung ein persistentes SQL-Backend
