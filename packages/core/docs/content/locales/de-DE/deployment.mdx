...
| `AGENT_NATIVE_BUILDER_RELAY_TARGET_ORIGINS` | Nur für den Unternehmens-Callback bestimmte, kommagetrennte Zulassungsliste exakt übereinstimmender, vertrauenswürdiger Preview-Origins (zum Beispiel `https://deploy-preview-42--content.netlify.app`). Legen Sie sie nur in der Unternehmens-Callback-Bereitstellung fest. Platzhalter und Domain-Suffixe werden nicht unterstützt.                    |
