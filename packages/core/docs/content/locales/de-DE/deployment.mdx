...
Ein Worker hat kein Dateisystem, die Datenbank ist also entweder eine externe
`DATABASE_URL` oder Cloudflare D1. Setzen Sie `CLOUDFLARE_D1_DATABASE_NAME`
und `CLOUDFLARE_D1_DATABASE_ID` gemeinsam in der Build-Umgebung, damit der
Build die `DB`-Bindung in die generierte `wrangler.json` schreibt. Nur eine
der beiden zu setzen ist ein Fehler und keine stillschweigend weggelassene
Bindung. Die vollständige lokale Startsequenz steht im englischen
Deployment-Leitfaden.


### Browser Rendering {#cloudflare-browser-rendering}

Ein Worker kann keinen Chromium-Prozess starten; der Browser ist stattdessen
eine Bindung, und die generierte `wrangler.json` deklariert
`{ "browser": { "binding": "BROWSER" } }`. Browserabhängige Aktionen lösen
diese Bindung über `@cloudflare/playwright` auf. Fehlt die Bindung, wird sie
namentlich gemeldet, statt eine fehlende Chromium-Binärdatei zu behaupten.
Lokal funktioniert das in `wrangler dev` mit einer Einschränkung: Die lokale
Laufzeit lehnt Anfragen über 1 MB ab.

### Cloudflare R2 {#cloudflare-r2}

Ein Worker hat kein Dateisystem, und Dateiinhalte gehören nicht in SQL —
hochgeladene Dateien landen daher in R2. Setzen Sie `CLOUDFLARE_R2_BUCKET_NAME`
in der Build-Umgebung, damit der Build die `UPLOADS`-Bindung in die generierte
`wrangler.json` schreibt, und `CLOUDFLARE_R2_PUBLIC_BASE_URL` auf die
öffentliche Basis-URL des Buckets. Der Provider ist in der Upload-Registry
bereits registriert. Eine fehlende oder unbrauchbare Bindung schlägt mit einer
Einrichtungsanleitung fehl, statt die Nutzdaten in SQL abzulegen. Details im
englischen Deployment-Leitfaden.
...
- `extensionTools: true` – aktiviert für die App die actions und Anweisungen
...
[Builder.io](https://www.builder.io?utm_source=agent-native&utm_medium=docs&utm_campaign=framework&utm_content=deployment) ist ein separater verwalteter codefähiger Workflow, in dem ein Agent die UI Ihrer App in Produktion ändern kann. Verbinden Sie Ihr Repository mit Builder.io und fordern Sie UI-Änderungen direkt an – keine erneute Bereitstellung erforderlich.
