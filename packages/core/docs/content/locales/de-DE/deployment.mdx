...
| `A2A_SECRET`             | Gemeinsames HMAC für App-übergreifendes A2A JSON-RPC. Ist `SECRETS_ENCRYPTION_KEY` nicht gesetzt, nutzt der gemeinsame Vault einen zweckgebunden abgeleiteten Schlüssel aus diesem Wert; eine Rotation rotiert daher auch den Vault-Schlüssel.                                                                                                  |
| `SECRETS_ENCRYPTION_KEY` | Bevorzugter stabiler AES-256-GCM-Schlüssel für den gemeinsamen Vault. Setzen Sie denselben Wert in jeder lesenden App. Lese-Fallbacks sind abgeleitetes `A2A_SECRET`, dann alte Better-Auth- und app-spezifische Schlüssel. Vor einer A2A-Rotation überall setzen und alle Zeilen lesen/migrieren, solange das alte A2A-Material verfügbar ist. |
