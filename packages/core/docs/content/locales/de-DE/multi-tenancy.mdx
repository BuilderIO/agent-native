...
## Das Kontomenü {#org-switcher}

Das Kontomenü (`OrgSwitcher`, unten in der Seitenleiste) und die Organisationsseiten in den Einstellungen werden in jeder Vorlage ohne zusätzlichen Code gerendert. Sie steuern die REST-Routen der Kernorganisation unter `/_agent-native/org/*` (Organisation erstellen, Organisation wechseln, Mitglieder auflisten/einladen/entfernen, Rollen ändern, zulässige E-Mail-Domäne festlegen). Benutzer wählen die aktive Organisation im Kontomenü aus; **Settings › Organization › Members** verwaltet Einladungen und Rollenänderungen.
