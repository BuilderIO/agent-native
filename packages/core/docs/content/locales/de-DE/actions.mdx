...
Für App Operations](/docs/server-overview#actions-first).
...
- **`readOnly: true`** – Überspringen Sie die Polling-Aktualisierung explizit, auch für POST actions, die nicht mutieren. Es ist **keine** Zugriffskontrolle: Es schränkt nicht ein, wer die Action aufrufen darf, und es wirkt sich nicht auf `useActionQuery`, `useActionMutation` oder andere Client-Hooks aus. Die einzigen Effekte sind das Unterdrücken des Refresh-Events nach dem Aufruf und das Ausnehmen der Action vom Schreib-Auditing. Um eine Action vor dem Modell zu verbergen, verwenden Sie `agentTool: false`; um Aufrufer einzuschränken, verwenden Sie die obigen Auth-/Zugriffsoptionen.
...
| `authorize`     | keine        | eine Guard-Funktion → nur Aufrufer, die sie akzeptiert, auf **jeder** Oberfläche        | Vorgänge, die auf eine Rolle beschränkt sind (RBAC pro App, Admin-only-Vorgänge)              |

Diese sind unabhängig: `agentTool` steuert die Modellansicht, `toolCallable` steuert nur den Erweiterungs-Iframe, `publicAgent` fügt eine öffentliche Opt-in-Oberfläche hinzu (öffentliche Webrouten bedeuten niemals die Offenlegung öffentlicher Tools), `needsApproval` steuert die Ausführung, nachdem der Aufruf erfolgt ist – siehe [Human-in-the-loop approval](#needs-approval) unten – und `authorize` entscheidet, ob dieser Aufrufer die Aktion überhaupt ausführen darf, auf jedem Dispatch-Pfad, bevor `run` betreten wird – siehe [Authorization](#authorize).
...
`accessFilter` / `assertAccess` grenzen ein, _welche Zeilen_ ein zugelassener Aufrufer anfassen darf. `authorize` beantwortet die Frage davor: **Darf dieser Aufrufer diesen Vorgang überhaupt ausführen?** Geben Sie ihm eine Guard-Funktion, und sie läuft vor `run()` bei jedem Aufrufer – Agent-Tool, HTTP, Frontend, MCP, A2A und CLI:
...
Der Guard umschließt `run` selbst, statt am Eintrag der Aktion zu hängen, denn `run` ist das Einzige, was alle sechs Dispatch-Stellen durchlaufen – es gibt keinen Aufrufer, der den Rumpf erreichen kann, ohne die Prüfung zu bestehen. Ein Guard, der wirft, verweigert mit seiner eigenen Meldung; einer, der `false` zurückgibt, verweigert mit einem generischen 403; alles andere, einschließlich `undefined`, erlaubt. Da die Sperre eine Aufrufer-Identität benötigt, wird ein unbeaufsichtigter CLI- oder Cron-Aufruf ohne Benutzer-E-Mail abgelehnt statt durchgewunken.

Die Wrapper werden in dieser Reihenfolge zusammengesetzt:

```text
validate input -> authorize -> run -> validate output -> audit
```

Die Autorisierung sitzt _innerhalb_ der Eingabevalidierung: Ein Guard, der `args` prüft, erhält damit den geparsten, konvertierten Wert, den sein Typ verspricht, und nicht die Form, die der Aufrufer gesendet hat. Das Auditing liegt ganz außen, sodass auch eine Ablehnung protokolliert wird.

`requireAny` schlägt zur Definitionszeit fehl, nicht zur Anfragezeit: Ein Aufruf ohne Rollen oder mit einer Rolle außerhalb des deklarierten Vokabulars wirft beim Laden des Moduls. `authorize: coachAccess.requireAny()` ist ein vergessenes Argument, und das soll dort brechen, wo der Fehler liegt, statt später als unerklärter 403 aufzutauchen.

Die drei Mechanismen sind nicht austauschbar:

| Mechanismus                   | Frage, die er beantwortet                                            | Gilt für                               |
| ----------------------------- | -------------------------------------------------------------------- | -------------------------------------- |
| `authorize`                   | Darf dieser Aufrufer den Vorgang überhaupt ausführen?                | Alle sechs Dispatch-Stellen            |
| `needsApproval`               | Soll ein Mensch diesen konkreten zugelassenen Aufruf absegnen?       | Nur die Agentenschleife                |
| `accessFilter`/`assertAccess` | Welche Zeilen darf dieser zugelassene Aufrufer sehen oder schreiben? | Innerhalb von `run`, pro Abfrage/Zeile |

Eine eingeschränkte Schreib-Aktion benötigt typischerweise sowohl `authorize` als auch `assertAccess`: Ersteres hält Nicht-Coaches aus dem Vorgang heraus, Letzteres hält einen Coach vom Plan einer anderen Organisation fern. `needsApproval` ist wiederum eine eigene Achse – es schränkt nie ein, _wer_ aufrufen darf, sondern nur, ob der Agent unbeaufsichtigt fortfahren darf.

`coachAccess` oben stammt von `defineAppRoles`, der App-spezifischen Rollenüberlagerung der Organisationsmitgliedschaft. Das Rollenmodell und seine Garantien finden Sie unter [Organizations, Teams & Permissions → App roles](/docs/organizations-teams-permissions#app-roles-per-app-rbac).
...
Wenn MCP aktiviert ist, wird Ihr actions auf dem MCP-Server des Frameworks unter `/mcp` angezeigt. Jeder Anrufer erhält standardmäßig einen kompakten Katalog – App-orientierte integrierte Apps sowie die in der Vorlage deklarierte App actions – und `tool-search` ist immer vorhanden, sodass jedes andere Tool bei Bedarf erreichbar bleibt. Die vollständige Aktionsoberfläche wird nur bei explizitem Opt-in bereitgestellt (`--full-catalog`-Token oder `AGENT_NATIVE_MCP_FULL_CATALOG=1`), und `publicAgent.expose` wählt ein sicheres Lese-/Aufnahmetool für die öffentliche Oberfläche aus. Siehe [MCP Protocol](/docs/mcp-protocol) für Katalogstufen, Authentifizierung und die `mcpApp`-Ressourcendetails.
