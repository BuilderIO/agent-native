...
Dateien. Dies eignet sich für die lokale Entwicklung und Agent Native Desktop,
ist aber nicht das Standard-Produktionssicherheitsmodell.

Das Framework verweigert den `local-files`-Modus in bekannten Hosting-,
Serverless- oder entfernten datenbankgestützten Laufzeiten. Diese Prüfung kann
nicht mit `AGENT_NATIVE_ALLOW_LOCAL_FILES_IN_PRODUCTION` umgangen werden; eine
Umgebungsvariable kann keine Einzelmandanten-Isolation beweisen. Der
Produktionsmodus ist auf Agent-Native-Desktop-Kindprozesse beschränkt. Lokale
Entwicklung einschließlich lokalem SQLite funktioniert weiterhin ohne Flag.
Für gehostete oder Mehrbenutzer-Apps verwenden Sie den Datenbankmodus und die
SQL-gestützte Freigabe.
