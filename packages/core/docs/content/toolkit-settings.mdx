...
For apps using the shared framework settings surface, the Agent settings page
includes an experimental way to use Codex through a user's ChatGPT
subscription. It is a lab, not a general OpenAI API provider, and it exposes a
fixed set of supported Codex models.
...
Open **Settings → Agent → Overview** and select **Connect ChatGPT** in the
**ChatGPT subscription** row. Finish sign-in in the popup. After it connects,
owners and admins can select **Use in chat** to make it the organization's
default model. Anyone who connected can pick its models in the chat's model
picker.
