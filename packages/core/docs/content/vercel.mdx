...
Building several apps? Set `deployment.workspace.buildConcurrency: "auto"` in `agent-native.config.ts` to build them in parallel; on Vercel's standard build machine (4 cores, 8 GB) that runs two at a time. See [Workspace Deployment](/docs/workspace-deployment).
