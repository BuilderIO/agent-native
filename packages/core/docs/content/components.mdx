...
## Prompt Home And Template Libraries {#prompt-home}

`PromptHome`, `PromptHomeLibrary`, `TemplateLibraryGrid`, and `TemplateLibraryCard` are exported from `@agent-native/toolkit/app-shell`. They share home spacing, a centered 700px composer, and a responsive four-column library. The app shell keeps its main landmark. Apps supply real data, localized labels, previews, actions, and routing.

`PromptHome` accepts `title`, `composer`, optional `mobileToolbar`, `connection`, `quickActions`, and library `children`. `PromptHomeLibrary` takes controlled `value` (`templates` or `recent`), `onValueChange`, `labels`, `showRecent`, `templates`, and optional `recent`, `browseAll`, and `recentActions`. With no recents it displays templates without a fake tab. `browseAll` is a real link supplied by the app. The semantic `Tabs.headerActions` slot keeps active actions beside the tabs.
...
`TemplateLibraryGrid` accepts typed `items` with `id`, `title`, optional stored `description` and `disabled`, plus `renderPreview`. It supports `selectedId`, `pendingId`, `disabled`, `loading`, `error`, `onRetry`, optional `empty`, and injected `labels` for loading, empty, and retry. Choose `onSelect(item, element)` or `renderLink(item, children)`, never both. Image and caption share one activation target. `renderActions` stays outside the activation target, inline beside the title and optional description. Cards share fixed preview and caption dimensions; direct activation uses or copies the template, while Preview belongs only in the caption menu. Loaded items remain visible after refresh errors. `TemplateLibraryCard` exposes the same behavior for one `item`, with `preview`, optional `actions`, `selected`, `pending`, and `disabled`.
...
`TemplatePreviewDialog`, exported from the same entry point, uses the semantic `Dialog` with `size="viewport"` by default or `size="large"`. It takes controlled `open`, `onOpenChange`, `title`, and app-rendered `children`; the header has no use action. Optional `thumbnails` contain `{ id, title, preview, disabled? }` and require `selectedId` plus `onSelectedIdChange`. The rail becomes horizontal on narrow screens and supports arrow keys, Home, and End.

The `viewport` size is an inset, centered modal: 90dvh tall, capped at 96rem wide, with 1rem mobile side margins and 1.5rem on wider screens. It preserves the standard dialog border, radius, shadow, and backdrop. Caption menus appear on card hover or keyboard focus on hover-capable devices and remain visible on touch devices; their reserved space never changes.

Pass localized `labels` (`close`, `loading`, `empty`, `retry`, `thumbnails`), optional `loading`, `error`, `empty`, and `onRetry` for exclusive readiness states. `initialFocusRef` and `restoreFocusRef` control focus. Open the viewer from the caption menu after its close handoff; keep reads, cancellation, authorization, and safe isolated rendering in the app.
...
Toolkit styles include the shared geometry. `@agent-native/toolkit/app-shell/prompt-home.css` is the focused stylesheet export. The existing `toolkit/app-shell` ejection unit includes the layout and stylesheet. Data loading, authorization, import workflows, and composer state stay app-owned.
...
### Staged composer context {#composer-context}

`PromptComposer` and `TiptapComposer` accept `submissionDisabled` to block sending while leaving draft, file, and context staging enabled. Missing provider setup blocks submission, not staging. `disabled` still disables the full composer.

`PromptComposer`, `TiptapComposer`, and `AgentKitComposer` accept `contextItems` and `contextMenuItems`. Apps own source reads and localized menu labels.

Each `AgentChatContextItem` has `key`, `title`, and `context`. Optional `status` is `ready`, `pending`, or `error`. `statusMessage` explains failures. `onRemoveContextItem`, `onInspectContextItem`, and `onRetryContextItem` receive the item key. `removable: false` hides removal. Pending or failed items block submission unless `blocksSubmission: false` explicitly keeps them informational.

A host can stage already-authorized context and receive the frozen snapshot on submission:
...
Context sources register `picker` on a `ComposerContextMenuItem`. `ComposerContextMenu` owns connected shadcn/Radix cascading submenus, not replacement panels: the root offers file upload, then Add context → host categories → source pickers. Add context is omitted when there are no host entries. Arrow keys and Escape navigate the menu hierarchy. Back within a link picker only returns from results to link entry in the same flyout. The shared picker owns search, list rows, loading, empty and error states, retry, pagination, and link entry. Apps provide authorized data and selection callbacks, not their own picker JSX.

The picker configuration accepts:

- `searchPlaceholder` and optional `emptyMessage` as localized app labels. `selectedIds` marks selected rows. Items have `id`, `title`, optional `disabled`, and optional `url`.
- Either local `items` with optional `loading`, `error`, and `onRetry()`, or remote `load(request)`. These modes cannot be combined.
- `load(request)` receives `search`, one-based `page`, optional `cursor` and `url`, and an `AbortSignal`. It returns `{ items, hasMore?, nextCursor? }`. Failures must reject, not return an empty result.
- `onSelect(item, request)` stages the source. It may be asynchronous. Returning `false` keeps the picker open. Throwing shows an error with retry. Optional `clearSelection` supplies a localized `label` and an `onSelect()` callback.
- Optional `link` supplies localized `placeholder` and `submitLabel` strings. `validate(url)` may return an error string. The shared form owns link input and navigation.
- `scopeKey` resets all picker state when identity or access scope changes. `refreshKey` invalidates the current query when a source/version token changes without resetting the URL. Search changes return to page one. Apps pass refreshed tokens when agent writes change their source data.

An app can register already-authorized choices. Here, `labels` contains localized app copy and `stageReference` updates the host's staged context:
...
For a searchable or linked source, the app supplies its authenticated loader. `loadAuthorizedReferences` implements the request/result contract above. It preserves access checks and forwards the abort signal to its client helper:
...
`presentation` defaults to `"submenu"`. With `{ type: "dialog", mode: "multiple", onAttach }`, an ordinary menu item opens a shared dialog titled by the entry label. `link.label` labels its URL input. Continue validates an HTTP(S) URL without credentials before loading results. Blank or invalid submission never calls the provider. `link.validate()` can enforce provider-specific rules. Dialog buttons use shared localized labels, not `link.submitLabel`.

`presentation.onAttach(items, request)` receives one frozen batch of newly checked items. This mode excludes `onSelect`. Choices survive search and pagination, and `selectedIds` are checked, disabled, and omitted from the batch. IDs must distinguish different source files. Returning `false` preserves the dialog. A rejection displays a retryable error. Cancellation or a `scopeKey` change aborts pending work and ignores late completion. Reopening starts with a fresh URL. Hosts must honor `request.signal` before asynchronous callbacks commit changes. `footerAction` and `clearSelection` remain submenu controls.

`{ type: "dialog", mode: "url" }` uses Attach directly, without a results loader. Its `onSelect(item, request)` receives `{ id: url, title: url, url }` and `request.url`:
...
`readComposerWebsiteSource(url, extractCallback)` from `@agent-native/core/server` builds website references through the host's existing SSRF-safe extractor. `composerSourceRequestSchema` from `@agent-native/core/shared` accepts `source: "website"` only with `operation: "read"` and a credential-free HTTP(S) `url` of at most 2,048 characters. The helper returns `{ id, title, url, context }` with a stable URL-hash ID and at most 20,000 context characters. Extraction status, warnings, rendering provenance, and any `[truncated]` marker remain in the context text. Partial extraction stays explicit. Failed extraction or missing usable `designMd` throws a typed failure. Authorization still belongs in the host's [action](/docs/actions-overview).

Picker entries do not also define `render` or entry-level `onSelect`. Categories use `children`. The root and every category keep a searchable header. Categories can supply a localized `searchPlaceholder`; without it they use the shared context-search placeholder. `picker.searchPlaceholder` labels the source list's search input. Registration does not grant source access. The shared picker ignores superseded load responses.

Optional `footerAction` stays below the results even during loading, errors, or an empty list. It accepts a localized `label`, optional `icon` and `disabled`, and either `onSelect()` or `renderLink(children)`. Use a callback for a modal or another stateful workflow: `false` keeps the menu open, `true` or `void` closes it, and rejection displays an error with retry. For pure navigation, return a real React Router `Link` or native anchor from `renderLink`; the shared menu supplies the row through `asChild`, preserving its `href` and browser navigation semantics. Wire existing authorized workflows, not placeholder actions.

Passing `addAttachment` to `ComposerContextMenu` adds the standard upload action first and preserves the host's file callback.

`PromptComposer` and `AgentKitComposer` accept `attachmentAdapter`, using the public `AttachmentAdapter` type from `@assistant-ui/react`. Supply the host's file-acceptance, size-validation, and staging policy through this adapter; omit it to retain the default adapters. The shared composer still owns the upload control, file picker, context picker, and attachment chips. Do not replace those controls to customize file staging.

`inlineTextAttachments` defaults to `true` on `PromptComposer` and `AgentKitComposer`. Set it to `false` when the host already extracts ordinary text files: the composer passes the original files to the host without reading and appending their contents to the prompt. Synthetic pasted-text attachments still become prompt text. This opt-out does not change attachment acceptance or the shared controls.
...
The snapshot preserves `undefined` versus an explicit empty array. It rejects more than 32 items or 64 KiB of serialized UTF-8 data without truncation. `AgentKitComposer.beforeSend(submission)` is awaited before send or queue and receives frozen context, references, uploaded attachments, and run options. Rejection preserves the draft. Context is included in message text and metadata.

`composerRef.current.submitWithText(text)` uses the same submission path with current files and model settings without replacing the editor first. It resolves to `true` after acceptance or `false` when blocked or rejected. Context selection is not an access grant. Apps must resolve and recheck source access through their authenticated actions.
