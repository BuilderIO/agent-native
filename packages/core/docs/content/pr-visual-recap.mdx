...
JSON before GitHub can schedule the gate.
...
**Security:** Prefer dedicated, ephemeral self-hosted runners. The gate runner
does not check out or execute PR code; it only evaluates base-branch workflow
logic and GitHub PR metadata. A persistent machine still retains process state
across jobs, so do not share it with unrelated workloads. The workflow fails
closed before dispatching the full self-hosted recap job for untrusted public or
fork PR authors. Do not weaken that gate or route untrusted contributions to a
persistent runner.
