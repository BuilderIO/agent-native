...
The JSON quotes are required. Both the lightweight `gate` job and the full
`recap` job use the selected runner. To configure it during guided setup, pass
the same JSON value to `--runs-on`, then validate the workflow and runner
...
**Security:** Prefer ephemeral self-hosted runners. Persistent self-hosted
runners should only serve private repositories whose contributors are trusted.
The workflow fails closed before dispatching self-hosted work for untrusted
public or fork PR authors, but a persistent machine still retains workspace and
process state across jobs. Do not weaken that gate or route untrusted
contributions to a persistent runner.
...
The same secrets and variables described in [Secrets and variables](#secrets-and-variables) apply — set them in your repo settings the same way as for the copy variant. The reusable caller forwards `VISUAL_RECAP_RUNS_ON` through its `runs-on` input; the called workflow parses the JSON and uses the result for both jobs. Omitting the input preserves the `ubuntu-latest` default.
