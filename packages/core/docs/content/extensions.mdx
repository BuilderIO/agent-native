...
`extensionTools: true` on `createAgentChatPlugin()`. That adds the agent-facing
extension actions and prompt guidance while leaving the sandbox runtime and
existing deep links available for compatibility either way.
