...
Tool arguments and results are off by default. Enable `captureToolArgs` and `captureToolResults` in the `observability` config to store them. Sensitive fields and tool error messages are redacted before storage.
