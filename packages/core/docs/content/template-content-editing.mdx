...
Type `/` on an empty line to open the slash menu and insert headings, bulleted or numbered lists, tables, images, dividers, and callouts. Enable advanced code, layout, visual, or developer-documentation blocks from **Settings → Labs** before inserting them. Select any text to pop up a formatting toolbar for bold, italic, links, and inline code.
...
- Ask the agent instead of formatting by hand: "add a callout warning about the deprecated endpoint," "turn this list into a table," or "make this heading a level 2." After you save an edit, Content keeps that version. A late recovery draft does not replace the saved change or show it as an external edit.

## Organizing your pages

Choose **Page** or **Collection** from the top-level **+** menu in either sidebar state. Press Escape to cancel. If you create an item from a row in another workspace, wait for that workspace to load first. Every page lives in a sidebar tree. Drag a page to reorder it or nest it under another page as a sub-page. Star a page to pin it to **Favorites** at the top of the sidebar.

Use the search box (or ask the agent to "find my meeting notes from last week") to search titles and content across every page you can see, with a preview snippet for each match. Results are ranked by relevance, so a strong title match appears before a recent document that only mentions the search terms in its body.
