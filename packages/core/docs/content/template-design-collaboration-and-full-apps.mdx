...
Once Design is open, `Cmd+\` on Apple devices or `Ctrl+\` elsewhere toggles all editing chrome so only the canvas remains. Claude Code Desktop also reserves `Cmd+\` for closing its focused pane; if the host intercepts the shortcut, right-click empty canvas space and choose **Hide UI** or **Show UI** instead.
