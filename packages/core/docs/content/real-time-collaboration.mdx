...
| **The platform identifies the deployment**                     | Registration is keyed on the app's public origin, so a process that is not actually the deployment must not claim it.                                                           | Netlify, Vercel, AWS Lambda, Cloud Run, and Render mark themselves. Cloudflare Workers do not, so set `AGENT_NATIVE_REALTIME_APP_URL` to the deployment's own origin. On a bare container or VM, set that same variable |
