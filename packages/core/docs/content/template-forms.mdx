...
<DataModel id="doc-block-nhh66t" title="Forms data model" summary={"Three tables. Fields and integrations are JSON columns on forms, so the agent's edits are surgical patches rather than cross-table row changes."} entities={[
  {
    "id": "forms",
    "name": "forms",
