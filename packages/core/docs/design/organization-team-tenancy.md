...
Only the recorded owner can explicitly grant **viewer** access to a conversation, one conversation at a time. The target must be a marked team in the conversation's organization, and the owner must currently belong to that organization and team. For a bound conversation, only its bound team is an allowed target; an unbound conversation may be shared with one allowed team without acquiring a binding or changing its personal ownership and organization/personal context. This grants read access to all current team members, including leads, but never continuation, management, or access to other conversations. Only the recorded owner can revoke the team grant. Team membership and active-team selection alone never share a conversation.
...
Add a chat-thread-specific policy at the action boundary for team grants and revocations; do not enable the generic group-share action unchanged. The generic action accepts `commenter`, `editor`, and `admin` as well as `viewer`, and permits resource admins to manage shares. Chat team grants must reject every role except `viewer` and require recorded-owner authority, not merely resource-admin access; revocation must enforce the same owner boundary. Validate the marked team, same organization, and owner's current team membership at grant time, then check the viewer's current organization and team membership on every direct read and list. An organization or resource admin gets no exception to these chat team-share rules. Offer one team-wide list of explicitly shared conversations and their linked runs to all current members, including leads. This is a discovery surface over authorized shares, not a new visibility grant. A private conversation does not appear in it merely because it is bound to the team. Enabling generic group principals alone is insufficient: chat group grants are not currently enabled, and chat list filtering does not yet admit group shares.
...
Before offering this V1, prove: existing groups and older conversations are unchanged; converted groups retain grants and connection permissions; only the selected team's context loads; personal/team/organization instruction and skill order is deterministic; a former member (including the owner) cannot read or continue a bound conversation; unshared work stays private; only the recorded owner can grant or revoke a chat team share, only for an allowed team and only as `viewer`, with `commenter`/`editor`/`admin` and resource-admin callers rejected; team-shared work and linked runs are visible only to current members across direct, list, and background paths; switching teams never changes a conversation's bound context; and deletion makes bound context and conversations inaccessible without deleting unrelated resources. Test changes to group membership and org membership against cached/listed as well as direct access. A failed or incomplete team-context lookup must not masquerade as an empty context or a successful authorization.
