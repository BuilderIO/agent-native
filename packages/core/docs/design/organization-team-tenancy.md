...
| Storage                 | V1 contract                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `workspace_user_groups` | Add `is_team BOOLEAN NOT NULL DEFAULT false` and `lead_emails_json TEXT NOT NULL DEFAULT '[]'`, using the same JSON email encoding as `member_emails_json`. The existing group ID, organization ID, and member list remain authoritative. All group write paths preserve `lead_emails_json ⊆ member_emails_json`; ordinary groups have no leads.                                                                                                             |
| `chat_threads`          | Add `team_group_id TEXT NULL` and an index for bound-team lookups. Record the group ID at creation only; do not derive it from the current selection on later reads. Do not add a foreign key that cascades or blocks group deletion: a deleted group's ID remains on its bound threads, which become inaccessible. Existing thread owner and visibility remain unchanged.                                                                                   |
| `chat_thread_shares`    | Reuse the existing share rows: one explicit team grant per thread has `principal_type = 'group'`, `principal_id = team_group_id of the recipient group`, and `role = 'viewer'`. For an unbound thread, the recipient group is the share target, not a thread binding. Enforce at most one team grant per thread on the chat write boundary; do not add a separate team-share or run-share table. Existing non-team shares retain their current behavior.     |
| Agent `resources`       | Reuse the existing `(owner, path)` key and content fields. Reserve `__team__:<group-id>` as the owner namespace keyed by the stable group ID; store team instructions, `skills/…`, and `memory/…` under their existing resource paths in that namespace. No team-resource table or copied organization resources. Reads and writes resolve the group and current membership before accessing that namespace.                                                 |
| Application state       | Persist the active team group ID per user and current organization across sessions and devices. Extend the existing session-based application-state behavior so each current session reflects that user/organization selection; a session-only key is not the durable source of truth. Clear or reject a selection when its group or membership is no longer valid. A selected ID is never an access grant or a substitute for `chat_threads.team_group_id`. |
...
| Action                                                                    | Input and result                                                                                                                                                                                                                       | Authorization and write rule                                                                                                                                                                                             |
| ------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Existing `list-workspace-user-groups`                                     | Return each group's ID, `isTeam`, members, and lead emails/derived roles.                                                                                                                                                              | Keep existing organization visibility; do not turn ordinary groups into teams on read.                                                                                                                                   |
| Existing `upsert-workspace-user-group`                                    | Accept optional `isTeam` on creation or explicit conversion of an existing group ID; omitted means false on creation and unchanged on update. Return the updated group. A supplied missing ID fails instead of creating a replacement. | Only organization owners/admins can create or convert a team; conversion retains its ID, grants, and connection allow-lists. Do not permit an implicit conversion back to an ordinary group.                             |
| Existing `bulk-update-workspace-user-groups` and group membership updates | Add/remove existing organization-member emails; return the updated membership and leads. Removing a member also removes their lead email in the same transaction.                                                                      | Apply the same team role checks to every bulk item: owners/admins manage membership, while leads can add existing organization members and remove ordinary members of their own team. A lead cannot remove another lead. |
| New `set-workspace-team-leads`                                            | Accept `teamGroupId` and the complete desired list of lead emails; return the updated membership and leads.                                                                                                                            | Owners/admins only; reject an ordinary group or a lead who is not a current member. Update roles atomically and audit changes.                                                                                           |
| Existing `delete-workspace-user-group`                                    | Accept the team group ID; return the deletion result.                                                                                                                                                                                  | Owners/admins only for teams. Bound conversations do not block deletion; existing connection-reference checks still require removing the team from connection allow-lists first. Never reuse the deleted ID.             |
| Existing resource list/read/write/delete operations                       | Accept a team resource owner scope derived from `teamGroupId` and the normal resource path; return the same resource shapes as other scopes.                                                                                           | Current team members read and edit team context. Resolve and authorize the group on every operation, including effective prompt loading; failure is not empty context.                                                   |
| New `set-active-workspace-team`                                           | Accept `teamGroupId` or `null` for the current organization; return the stored user/organization selection and reflect it in the current session.                                                                                      | Current organization and team membership required for a non-null ID. Selection follows the user across sessions and devices, and affects new conversations only.                                                         |
| Existing conversation creation path                                       | Accept the current active team ID or no team; persist `chat_threads.team_group_id` and return the new thread.                                                                                                                          | Resolve the marked group and creator's current membership server-side before insertion; never accept a client-supplied binding as proof of access.                                                                       |
| New `share-chat-thread-with-team` / `unshare-chat-thread-from-team`       | Accept `threadId` and `teamGroupId`; create/remove that thread's single group-`viewer` share and return the resulting share state.                                                                                                     | Recorded owner only, with current organization and target-team membership. A bound thread can target only its bound team. Reject non-viewer roles, a second team grant, and generic resource-admin authority.            |
| New `list-team-shared-chat-threads`                                       | Accept `teamGroupId` and pagination; return only explicitly shared threads. Linked runs remain available through the existing run read/list paths.                                                                                     | Current organization and team membership required; apply the same access checks to every returned thread/run. A merely bound, private thread never appears.                                                              |
