...
| Application state       | Persist the active team group ID per user and current organization using the existing application-state surface, not a second membership record. Clear or reject a selection when its group or membership is no longer valid. A selected ID is never an access grant or a substitute for `chat_threads.team_group_id`.                                                                                                                                   |
...
| New `set-active-workspace-team`                                           | Accept `teamGroupId` or `null` for the current organization; return the stored active selection.                                                                                                                                       | Current organization and team membership required for a non-null ID. Selection affects new conversations only.                                                                                                           |
